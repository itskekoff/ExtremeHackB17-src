...
public final class GLUtils {
    ...
    public static void glScissor(float x2, float y2, float x1, float y1) {
        int scaleFactor = GLUtils.getScaleFactor();
        GL11.glScissor((int)(x2 * (float)scaleFactor), (int)((float)Minecraft.getMinecraft().displayHeight - y1 * (float)scaleFactor), (int)((x1 - x2) * (float)scaleFactor), (int)((y1 - y2) * (float)scaleFactor));
    }
    ...
    public static boolean isHovered(int x2, int y2, int width, int height, int mouseX, int mouseY) {
        return mouseX >= x2 && mouseX <= x2 + width && mouseY >= y2 && mouseY < y2 + height;
    }

    public static int genVBO() {
        int id2 = GL15.glGenBuffers();
        vbos.add(id2);
        GL15.glBindBuffer(34962, id2);
        return id2;
    }
