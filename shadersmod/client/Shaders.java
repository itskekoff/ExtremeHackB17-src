...
public class Shaders {
    ...
    private static CustomTexture[] loadCustomTextures(Properties props, int stage) {
        ...
        for (Object s10 : set) {
            ...
            CustomTexture customtexture = Shaders.loadCustomTexture(i2, s3);
    ...
    private static CustomTexture loadCustomTexture(int index, String path) {
        ...
        block5: {
            ...
            try {
                s2 = "shaders/" + StrUtils.removePrefix(path, "/");
                inputstream = shaderPack.getResourceAsStream(s2);
        ...
        SimpleShaderTexture simpleshadertexture = new SimpleShaderTexture(s2);
        simpleshadertexture.loadTexture(mc.getResourceManager());
        CustomTexture customtexture = new CustomTexture(index, s2, simpleshadertexture);
    ...
    public static ShaderOption[] getShaderPackOptions(String screenName) {
        ...
        ScreenShaderOptions screenshaderoptions = shaderPackGuiScreens.get(s2);
    ...
    public static int getShaderPackColumns(String screenName, int def) {
        String s2;
        String string = s2 = screenName != null ? "screen." + screenName : "screen";
        if (shaderPackGuiScreens == null) {
            return def;
        }
        ScreenShaderOptions screenshaderoptions = shaderPackGuiScreens.get(s2);
    ...
    private static ShaderOption[] getShaderOptionsRest(Map<String, ScreenShaderOptions> mapScreens, ShaderOption[] ops) {
        ...
        for (String s2 : mapScreens.keySet()) {
            ScreenShaderOptions screenshaderoptions = mapScreens.get(s2);
    ...
    private static void saveOptionProperties(IShaderPack sp2, Properties props) throws IOException {
        String s2 = String.valueOf(shaderpacksdirname) + "/" + sp2.getName() + ".txt";
    ...
    static String versiontostring(int vv2) {
        ...
        return String.valueOf(Integer.toString(Integer.parseInt(s2.substring(1, 3)))) + "." + Integer.toString(Integer.parseInt(s2.substring(3, 5))) + "." + Integer.toString(Integer.parseInt(s2.substring(5)));
    }
    ...
    public static void printIntBuffer(String title, IntBuffer buf2) {
        StringBuilder stringbuilder = new StringBuilder(128);
        stringbuilder.append(title).append(" [pos ").append(buf2.position()).append(" lim ").append(buf2.limit()).append(" cap ").append(buf2.capacity()).append(" :");
        int i2 = buf2.limit();
        for (int j2 = 0; j2 < i2; ++j2) {
            stringbuilder.append(" ").append(buf2.get(j2));
        }
    ...
    public static void init() {
        ...
        if (!isShaderPackInitialized) {
            ...
            String s2 = "";
            if (currentWorld != null && shaderPackDimensions.contains(i2 = Shaders.currentWorld.provider.getDimensionType().getId())) {
                s2 = "world" + i2 + "/";
            }
            if (saveFinalShaders) {
            ...
            for (int k1 = 0; k1 < 33; ++k1) {
                ...
                newColorAtmSetting = null;
                newCompositeMipmapSetting = 0;
                String s22 = String.valueOf(s2) + s1;
                if (shaderprofile != null && shaderprofile.isProgramDisabled(s22)) {
                    SMCLog.info("Program disabled: " + s22);
                    s1 = "<disabled>";
                    s22 = String.valueOf(s2) + s1;
                }
                if ((j2 = Shaders.setupProgram(k1, String.valueOf(s3 = "/shaders/" + s22) + ".vsh", String.valueOf(s3) + ".fsh")) > 0) {
    ...
    public static void resetDisplayListModelRenderer(ModelRenderer mrr) {
        ...
        if (mrr.childModels != null) {
            ...
            for (int i2 = 0; i2 < j2; ++i2) {
                Shaders.resetDisplayListModelRenderer(mrr.childModels.get(i2));
            }
        }
    }
    ...
    private static int setupProgram(int program, String vShaderPath, String fShaderPath) {
        ...
        if (i2 != 0) {
            ...
            if (j2 == 0 && k2 == 0) {
                ...
                Shaders.printLogInfo(i2, String.valueOf(vShaderPath) + ", " + fShaderPath);
                if (j2 != 0) {
        ...
        if (bufferedreader != null) {
            try {
                ...
                while (true) {
                    ...
                    s2 = Shaders.applyOptions(s2, ashaderoption);
                    stringbuilder.append(s2).append('\n');
                    ShaderLine shaderline = ShaderParser.parseLine(s2);
                                ...
                                if (!shaderline.isConstFloat("wetnessHalflife") && !shaderline.isComment("WETNESSHL")) {
                                    if (!shaderline.isConstFloat("drynessHalflife") && !shaderline.isComment("DRYNESSHL")) {
                                        ...
                                        if (shaderline.isConstIntSuffix("Format")) {
                                            ...
                                            int k2 = Shaders.getBufferIndexFromString(s4);
                                            int l2 = Shaders.getTextureFormatFromString(s6);
                                        ...
                                        if (shaderline.isConstBoolSuffix("MipmapEnabled", true)) {
                                            String s22;
                                            int j2;
                                            if (!ShaderParser.isComposite(filename) && !ShaderParser.isFinal(filename) || (j2 = Shaders.getBufferIndexFromString(s22 = StrUtils.removeSuffix(shaderline.getName(), "MipmapEnabled"))) < 0) continue;
                                            newCompositeMipmapSetting |= 1 << j2;
                                            SMCLog.info("%s mipmap enabled", s22);
                                            continue;
                                        }
    ...
    public static void useProgram(int program) {
        ...
        if (activeProgram != program) {
            ...
            if (programsID[program] == 0) {
                ...
                Block block = null;
                if (item != null) {
                    i2 = Item.REGISTRY.getIDForObject(item);
                    block = Block.REGISTRY.getObjectById(i2);
                }
                int j2 = block != null ? block.getLightValue(block.getDefaultState()) : 0;
                ...
                Block block1 = null;
                if (item1 != null) {
                    k2 = Item.REGISTRY.getIDForObject(item1);
                    block1 = Block.REGISTRY.getObjectById(k2);
                }
                int n2 = l2 = block1 != null ? block1.getLightValue(block1.getDefaultState()) : 0;
                if (Shaders.isOldHandLight() && l2 > j2) {
                    i2 = k2;
                    j2 = l2;
                }
                Shaders.setProgramUniform1i("heldItemId", i2);
                Shaders.setProgramUniform1i("heldBlockLightValue", j2);
                Shaders.setProgramUniform1i("heldItemId2", k2);
                Shaders.setProgramUniform1i("heldBlockLightValue2", l2);
    ...
    private static void loadEntityDataMap() {
        ...
        if (mapBlockToEntityData.isEmpty()) {
            for (ResourceLocation resourcelocation : Block.REGISTRY.getKeys()) {
                ...
                int i2 = Block.REGISTRY.getIDForObject(block);
    ...
    private static IntBuffer fillIntBufferZero(IntBuffer buf2) {
        int i2 = buf2.limit();
        for (int j2 = buf2.position(); j2 < i2; ++j2) {
            buf2.put(j2, 0);
        }
        return buf2;
    }
    ...
    private static void setupFrameBuffer() {
        ...
        for (int k2 = 0; k2 < usedColorBuffers; ++k2) {
            ...
            GL11.glTexImage2D(3553, 0, gbuffersFormat[k2], renderWidth, renderHeight, 0, 32993, 33639, null);
        ...
        for (int l2 = 0; l2 < usedColorBuffers; ++l2) {
            ...
            GL11.glTexParameteri(3553, 10241, 9729);
            GL11.glTexParameteri(3553, 10240, 9729);
            GL11.glTexImage2D(3553, 0, gbuffersFormat[l2], renderWidth, renderHeight, 0, 32993, 33639, null);
        ...
        if (entity != null) {
            ...
            if (Shaders.mc.player != null) {
                ...
                if (Shaders.mc.player.isPotionActive(MobEffects.BLINDNESS)) {
                    int i2 = Shaders.mc.player.getActivePotionEffect(MobEffects.BLINDNESS).getDuration();
    ...
    private static void checkWorldChanged(World worldd) {
        if (currentWorld != worldd) {
            ...
            if (world != null && worldd != null) {
                int i2 = world.provider.getDimensionType().getId();
                int j2 = worldd.provider.getDimensionType().getId();
    ...
    public static void setViewport(int vx2, int vy, int vw2, int vh) {
    ...
    public static void setEntityId(Entity entity) {
        if (isRenderingWorld && !isShadowPass && uniformEntityId.isDefined()) {
            int i2 = EntityUtils.getEntityIdByClass(entity);
    ...
    public static void pushEntity(Block block) {
        int i2 = block.getRenderType(block.getDefaultState()).ordinal();
        Shaders.entityData[++Shaders.entityDataIndex * 2] = Block.REGISTRY.getIDForObject(block) & 0xFFFF | i2 << 16;
        Shaders.entityData[Shaders.entityDataIndex * 2 + 1] = 0;
    }
