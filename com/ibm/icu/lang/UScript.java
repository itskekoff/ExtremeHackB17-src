...
public final class UScript {
    ...
    private static int[] findCodeFromLocale(ULocale locale) {
        ...
        try {
            rb2 = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt51b", locale);
        }
        catch (MissingResourceException e2) {
            return null;
        }
        rb2 = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt51b", locale);
        if (rb2.getLoadingStatus() == 3 && !locale.equals(ULocale.getDefault())) {
            return null;
        }
        UResourceBundle sub = rb2.get(kLocaleScript);
        int[] result = new int[sub.getSize()];
        int w2 = 0;
        for (int i2 = 0; i2 < result.length; ++i2) {
            int code = UCharacter.getPropertyValueEnum(4106, sub.getString(i2));
            result[w2++] = code;
        }
        if (w2 < result.length) {
            throw new IllegalStateException("bad locale data, listed " + result.length + " scripts but found only " + w2);
        }
        return result;
    }
    ...
    public static final boolean hasScript(int c2, int sc2) {
        ...
        while (sc2 > scriptExtensions[scx]) {
            ++scx;
        }
        return sc2 == (scriptExtensions[scx] & 0x7FFF);
    }
