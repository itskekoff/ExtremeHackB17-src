...
public final class UScriptRun {
    ...
    public final boolean next() {
        ...
        while (this.textIndex < this.textLimit) {
            int ch2 = UTF16.charAt(this.text, this.textStart, this.textLimit, this.textIndex - this.textStart);
            int codePointCount = UTF16.getCharCount(ch2);
            int sc2 = UScript.getScript(ch2);
            int pairIndex = UScriptRun.getPairIndex(ch2);
    ...
    private static int getPairIndex(int ch2) {
        int probe = pairedCharPower;
        int index = 0;
        if (ch2 >= pairedChars[pairedCharExtra]) {
            index = pairedCharExtra;
        }
        while (probe > 1) {
            if (ch2 < pairedChars[index + (probe >>= 1)]) continue;
            index += probe;
        }
        if (pairedChars[index] != ch2) {
            index = -1;
        }
        return index;
    }
