...
Freezable<UnicodeSet> {
    ...
    private static void _appendToPat(StringBuffer buf2, String s2, boolean escapeUnprintable) {
        int cp2;
        for (int i2 = 0; i2 < s2.length(); i2 += Character.charCount(cp2)) {
            cp2 = s2.codePointAt(i2);
            UnicodeSet._appendToPat(buf2, cp2, escapeUnprintable);
        }
    }

    private static void _appendToPat(StringBuffer buf2, int c2, boolean escapeUnprintable) {
        if (escapeUnprintable && Utility.isUnprintable(c2) && Utility.escapeUnprintable(buf2, c2)) {
            return;
        }
        switch (c2) {
            case 36: 
    ...
    public final UnicodeSet add(CharSequence s2) {
        ...
        if (cp2 < 0) {
            this.strings.add(((Object)s2).toString());
            this.pat = null;
        } else {
...
lbl51:
    ...
    private UnicodeSet applyFilter(Filter filter, int src) {
        ...
        for (int j2 = 0; j2 < limitRange; ++j2) {
            ...
            for (int ch2 = start; ch2 <= end; ++ch2) {
                if (filter.contains(ch2)) {
                    if (startHasProperty >= 0) continue;
                    startHasProperty = ch2;
                    continue;
                }
                if (startHasProperty < 0) continue;
                this.add_unchecked(startHasProperty, ch2 - 1);
                startHasProperty = -1;
            }
        }
    ...
    private static String mungeCharName(String source) {
        ...
        for (int i2 = 0; i2 < source.length(); ++i2) {
            int ch2 = source.charAt(i2);
            if (PatternProps.isWhiteSpace(ch2)) {
                if (buf2 == null) {
                    buf2 = new StringBuilder().append(source, 0, i2);
                } else if (buf2.charAt(buf2.length() - 1) == ' ') continue;
                ch2 = 32;
            }
            if (buf2 == null) continue;
            buf2.append((char)ch2);
        }
        return buf2 == null ? source : buf2.toString();
    }
    ...
    public UnicodeSet applyPropertyAlias(String propertyAlias, String valueAlias, SymbolTable symbols) {
        block23: {
            block21: {
                block22: {
                    ...
                    try {
                        ...
                        if ((v = Integer.parseInt(PatternProps.trimWhiteSpace(valueAlias))) < 0) throw e;
                        if (v <= 255) ** GOTO lbl75
                        throw e;
                    }
                }
            ...
            if (v == -1 && (v = pnames.getPropertyValueEnum(p = 4106, propertyAlias)) == -1) {
    ...
    implements Filter {
        ...
        public boolean contains(int ch2) {
            VersionInfo v2 = UCharacter.getAge(ch2);
    ...
    implements Filter {
        ...
        public boolean contains(int ch2) {
            return UCharacter.getIntPropertyValue(ch2, this.prop) == this.value;
        }
    }
    ...
    implements Filter {
        ...
        public boolean contains(int ch2) {
            return (1 << UCharacter.getType(ch2) & this.mask) != 0;
        }
    }
    ...
    implements Filter {
        ...
        public boolean contains(int ch2) {
            return UCharacter.getUnicodeNumericValue(ch2) == this.value;
        }
    }
