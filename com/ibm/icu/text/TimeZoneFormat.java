...
Serializable {
    ...
    public String getGMTOffsetDigits() {
        StringBuilder buf2 = new StringBuilder(this._gmtOffsetDigits.length);
    ...
    private String formatOffsetLocalizedGMT(int offset, boolean isShort) {
        ...
        buf2.append(this._gmtPatternPrefix);
        block5: for (Object item : offsetPatternItems) {
            if (item instanceof String) {
                buf2.append((String)item);
                continue;
            }
            ...
            switch (field.getType()) {
                case 'H': {
                    this.appendOffsetDigits(buf2, offsetH, isShort ? 1 : 2);
                    continue block5;
                }
                case 'm': {
                    this.appendOffsetDigits(buf2, offsetM, 2);
                    continue block5;
                }
                case 's': {
                    this.appendOffsetDigits(buf2, offsetS, 2);
                }
            }
        }
        buf2.append(this._gmtPatternSuffix);
        return buf2.toString();
    }
    ...
    private static Object[] parseOffsetPattern(String pattern, String letters) {
        ...
        for (int i2 = 0; i2 < pattern.length(); ++i2) {
            ...
            int patFieldIdx = letters.indexOf(ch2);
            if (patFieldIdx >= 0) {
                if (ch2 == itemType) {
                    ++itemLength;
                    continue;
    ...
    private void appendOffsetDigits(StringBuilder buf2, int n2, int minDigits) {
        assert (n2 >= 0 && n2 < 60);
        int numDigits = n2 >= 10 ? 2 : 1;
        for (int i2 = 0; i2 < minDigits - numDigits; ++i2) {
            buf2.append(this._gmtOffsetDigits[0]);
        }
        if (numDigits == 2) {
            buf2.append(this._gmtOffsetDigits[n2 / 10]);
        }
        buf2.append(this._gmtOffsetDigits[n2 % 10]);
    }
    ...
    private static String parseZoneID(String text, ParsePosition pos) {
        ...
        if (ZONE_ID_TRIE == null) {
            Class<TimeZoneFormat> class_ = TimeZoneFormat.class;
