...
public final class CanonicalIterator {
    ...
    private Set<String> extract(int comp, String segment, int segmentPos, StringBuffer buf2) {
        int cp2;
        String decomp;
        if (PROGRESS) {
        ...
        for (int i2 = segmentPos; i2 < segment.length(); i2 += UTF16.getCharCount(cp2)) {
            ...
            if (cp2 == decompCp) {
                ...
                if (decompPos == decomp.length()) {
                    buf2.append(segment.substring(i2 + UTF16.getCharCount(cp2)));
                    ok2 = true;
                    break;
                }
        ...
        String remainder = buf2.toString();
