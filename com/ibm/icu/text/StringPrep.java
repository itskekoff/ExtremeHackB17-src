...
public final class StringPrep {
    ...
    private char getCodePointValue(int ch2) {
        return this.sprepTrie.getCodePointValue(ch2);
    }
    ...
    public StringPrep(InputStream inputStream) throws IOException {
        BufferedInputStream b2 = new BufferedInputStream(inputStream, 25000);
        StringPrepDataReader reader = new StringPrepDataReader(b2);
    ...
    private StringBuffer map(UCharacterIterator iter, int options) throws StringPrepParseException {
        ...
        boolean bl2 = allowUnassigned = (options & 1) > 0;
        while ((ch2 = iter.nextCodePoint()) != -1) {
            result = this.getCodePointValue(ch2);
    ...
    public StringBuffer prepare(UCharacterIterator src, int options) throws StringPrepParseException {
        ...
        while ((ch2 = iter.nextCodePoint()) != -1) {
            char result = this.getCodePointValue(ch2);
            StringPrep.getValues(result, val);
            if (val.type == 2) {
            ...
            direction = this.bdp.getClass(ch2);
            if (firstCharDir == 19) {
