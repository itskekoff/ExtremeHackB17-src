...
extends NumberFormat {
    ...
    private final boolean subparse(String text, ParsePosition parsePosition, DigitList digits, boolean[] status, Currency[] currency, String negPrefix, String negSuffix, String posPrefix, String posSuffix, int type) {
        ...
        if (text.regionMatches(position, this.symbols.getInfinity(), 0, this.symbols.getInfinity().length())) {
            ...
            while (position < text.length()) {
                int ch2 = UTF16.charAt(text, position);
                digit = ch2 - digitSymbols[0];
                if (digit < 0 || digit > 9) {
                    digit = UCharacter.digit(ch2, 10);
                }
                if (digit < 0 || digit > 9) {
                    for (digit = 0; digit < 10 && ch2 != digitSymbols[digit]; ++digit) {
                    }
                }
                if (digit == 0) {
                    ...
                    sawDecimal = true;
                } else if (this.isGroupingUsed() && ch2 == grouping) {
                    if (sawDecimal) break;
                    ...
                    backup = position;
                    sawGrouping = true;
                } else if (!sawDecimal && decimalEquiv.contains(ch2)) {
                    if (strictParse && (backup != -1 || lastGroup != -1 && this.countCodePoints(text, lastGroup, position) != this.groupingSize + 1)) {
                        strictFail = true;
                        break;
                    }
                    ...
                    decimal = (char)ch2;
                    sawDecimal = true;
                } else if (this.isGroupingUsed() && !sawGrouping && groupEquiv.contains(ch2)) {
                    if (sawDecimal) break;
                    if (strictParse && (!sawDigit || backup != -1)) {
                        strictFail = true;
                        break;
                ...
                position += UTF16.getCharCount(ch2);
            }
            if (backup != -1) {
                position = backup;
            }
    ...
    static final int match(String text, int pos, int ch2) {
        if (pos >= text.length()) {
            return -1;
        }
        if (PatternProps.isWhiteSpace(ch2)) {
            int s2 = pos;
        ...
        return pos >= 0 && UTF16.charAt(text, pos) == ch2 ? pos + UTF16.getCharCount(ch2) : -1;
    }

    static final int match(String text, int pos, String str) {
        int ch2;
        for (int i2 = 0; i2 < str.length() && pos >= 0; i2 += UTF16.getCharCount(ch2)) {
            ch2 = UTF16.charAt(str, i2);
            pos = DecimalFormat.match(text, pos, ch2);
            if (!PatternProps.isWhiteSpace(ch2)) continue;
    ...
    private String unquote(String pat) {
        StringBuilder buf2 = new StringBuilder(pat.length());
        int i2 = 0;
        while (i2 < pat.length()) {
            char ch2;
            if ((ch2 = pat.charAt(i2++)) == '\'') continue;
            buf2.append(ch2);
        }
    ...
    private int appendAffix(StringBuffer buf2, boolean isNegative, boolean isPrefix, boolean parseAttr) {
        ...
        if (parseAttr) {
            ...
            this.formatAffix2Attribute(affix, buf2.length() + offset, buf2.length() + affix.length());
        }
        buf2.append(affix);
        return affix.length();
    }
        ...
        while (i3 < affixPat.length()) {
            block17: {
                ...
                switch (ch3) {
                    ...
                    case '\u2030': {
                        ch3 = this.symbols.getPerMill();
                        break;
                    }
                    case '%': {
                        ch3 = this.symbols.getPercent();
                        break;
                    }
                    case '-': {
                        ch3 = this.symbols.getMinusSign();
                    }
                }
                if (ch3 == this.symbols.getDecimalSeparator() || ch3 == this.symbols.getGroupingSeparator()) {
                    buffer.append('\'');
                    buffer.append(ch3);
                    buffer.append('\'');
                } else {
    ...
    private void applyPatternWithoutExpandAffix(String pattern, boolean localized) {
        ...
        while (true) {
                ...
                switch (subpart) {
                    case 0: {
                        if (ch2 == digit) {
                            ...
                            if (groupingCount < 0 || decimalPos >= 0) break;
                            groupingCount = (byte)(groupingCount + 1);
                            break;
                        }
                        if (ch2 >= zeroDigit && ch2 <= nineDigit || ch2 == sigDigit) {
                            if (digitRightCount > 0) {
                                this.patternError("Unexpected '" + ch2 + '\'', pattern);
                            }
                            if (ch2 == sigDigit) {
                                ++sigDigitCount;
                            } else {
                                ++zeroDigitCount;
                                if (ch2 != zeroDigit) {
                                    int p2 = digitLeftCount + zeroDigitCount + digitRightCount;
                                    if (incrementPos >= 0) {
                                        while (incrementPos < p2) {
                                            incrementVal *= 10L;
                                            ++incrementPos;
                                        }
                                    } else {
                                    ...
                                    incrementVal += (long)(ch2 - zeroDigit);
                                }
                            }
                            if (groupingCount < 0 || decimalPos >= 0) break;
                            groupingCount = (byte)(groupingCount + 1);
                        ...
                        if (ch2 == groupingSeparator) {
                            char after;
                            if (ch2 == '\'' && pos + 1 < pattern.length() && (after = pattern.charAt(pos + 1)) != digit && (after < zeroDigit || after > nineDigit)) {
                                if (after == '\'') {
                                    ++pos;
                                } else {
                                    if (groupingCount < 0) {
                    ...
                    case 1: 
                    case 2: {
                        if (ch2 == digit || ch2 == groupingSeparator || ch2 == decimalSeparator || ch2 >= zeroDigit && ch2 <= nineDigit || ch2 == sigDigit) {
                            if (subpart == 1) {
                                subpart = 0;
                                sub0Start = pos--;
                                break;
                            ...
                            this.patternError("Unquoted special character '" + ch2 + '\'', pattern);
                        } else if (ch2 == '\u00a4') {
                            boolean doubled;
                            boolean bl2 = doubled = pos + 1 < pattern.length() && pattern.charAt(pos + 1) == '\u00a4';
                            if (doubled) {
                                affix.append(ch2);
                                if (++pos + 1 < pattern.length() && pattern.charAt(pos + 1) == '\u00a4') {
                                    ++pos;
                            ...
                            if (ch2 == percent || ch2 == perMill) {
                                if (multpl != 1) {
                                    this.patternError("Too many percent/permille characters", pattern);
                                }
                                multpl = ch2 == percent ? 100 : 1000;
                                ch2 = ch2 == percent ? (char)'%' : '\u2030';
                            } else if (ch2 == minus) {
                                ch2 = '-';
                            } else if (ch2 == padEscape) {
                                if (padPos >= 0) {
            ...
            if (zeroDigitCount == 0 && sigDigitCount == 0 && digitLeftCount > 0 && decimalPos >= 0) {
                ...
                digitRightCount = digitLeftCount - n2;
                digitLeftCount = n2 - 1;
                zeroDigitCount = 1;
            }
            ...
            if (part == 0) {
                ...
                boolean bl3 = this.useExponentialNotation = expDigits >= 0;
                if (this.useExponentialNotation) {
