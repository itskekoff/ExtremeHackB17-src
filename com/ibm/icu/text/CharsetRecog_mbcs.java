...
extends CharsetRecognizer {
        ...
        block13: {
            ...
            while (this.nextChar(iter, det)) {
                ...
                if (iter.error) {
                    ...
                    long cv2 = (long)iter.charValue & 0xFFFFFFFFL;
                    if (cv2 <= 255L) {
                        ++singleByteCharCount;
                    } else {
                        ++doubleByteCharCount;
                        if (commonChars != null && Arrays.binarySearch(commonChars, (int)cv2) >= 0) {
                            ++commonCharCount;
                        }
                    }
                }
    ...
    extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar it2, CharsetDetector det) {
            it2.index = it2.nextIndex;
            it2.error = false;
            int firstByte = 0;
            int secondByte = 0;
            int thirdByte = 0;
            int fourthByte = 0;
            it2.charValue = it2.nextByte(det);
            firstByte = it2.charValue;
            if (firstByte < 0) {
                it2.done = true;
            } else if (firstByte > 128) {
                secondByte = it2.nextByte(det);
                it2.charValue = it2.charValue << 8 | secondByte;
                if (!(firstByte < 129 || firstByte > 254 || secondByte >= 64 && secondByte <= 126 || secondByte >= 80 && secondByte <= 254)) {
                    if (secondByte >= 48 && secondByte <= 57 && (thirdByte = it2.nextByte(det)) >= 129 && thirdByte <= 254 && (fourthByte = it2.nextByte(det)) >= 48 && fourthByte <= 57) {
                        it2.charValue = it2.charValue << 16 | thirdByte << 8 | fourthByte;
                    } else {
                        it2.error = true;
                    }
                }
        ...
        boolean nextChar(iteratedChar it2, CharsetDetector det) {
            ...
            if (firstByte < 0) {
                ...
                if (firstByte >= 161 && firstByte <= 254) {
                    ...
                    thirdByte = it2.nextByte(det);
                    it2.charValue = it2.charValue << 8 | thirdByte;
                    if (thirdByte < 161) {
                        it2.error = true;
                    }
                }
    ...
    extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar it2, CharsetDetector det) {
            ...
            int secondByte = it2.nextByte(det);
            if (secondByte < 0) {
                return false;
            }
            it2.charValue = it2.charValue << 8 | secondByte;
    ...
    extends CharsetRecog_mbcs {
        ...
        boolean nextChar(iteratedChar it2, CharsetDetector det) {
            ...
            it2.charValue = firstByte << 8 | secondByte;
