...
implements RBNFPostProcessor {
    ...
    public void process(StringBuffer buf2, NFRuleSet ruleSet) {
        int n2;
        int i2;
        ...
        for (int i3 = 0; i3 < m2.length - 1; ++i3) {
            n2 = buf2.indexOf(m2[i3]);
            if (n2 == -1) continue;
            buf2.insert(n2 + m2[i3].length(), '|');
        }
        int x2 = buf2.indexOf("\u9ede");
        if (x2 == -1) {
            x2 = buf2.length();
        }
        int s2 = 0;
        ...
        block14: while (x2 >= 0) {
            int m3 = buf2.lastIndexOf("|", x2);
            int nn = buf2.lastIndexOf(ling, x2);
            int ns2 = 0;
            if (nn > m3) {
                ns2 = nn > 0 && buf2.charAt(nn - 1) != '*' ? 2 : 1;
            }
            x2 = m3 - 1;
            switch (s2 * 3 + ns2) {
                case 0: {
                ...
                case 4: {
                    buf2.delete(nn - 1, nn + ling.length());
                    s2 = 0;
                    n2 = -1;
                    continue block14;
                }
                case 5: {
                    buf2.delete(n2 - 1, n2 + ling.length());
                    s2 = ns2;
                    n2 = -1;
                    continue block14;
                }
