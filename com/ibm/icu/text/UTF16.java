...
public final class UTF16 {
    ...
    public static int bounds(String source, int offset16) {
        char ch2 = source.charAt(offset16);
    ...
    public static int findOffsetFromCodePoint(String source, int offset32) {
        ...
        for (count = offset32; result < size && count > 0; --count, ++result) {
            char ch2 = source.charAt(result);
            if (!UTF16.isLeadSurrogate(ch2) || result + 1 >= size || !UTF16.isTrailSurrogate(source.charAt(result + 1))) continue;
            ++result;
        }
        if (count != 0) {
            throw new StringIndexOutOfBoundsException(offset32);
    ...
    public static int findOffsetFromCodePoint(char[] source, int start, int limit, int offset32) {
        ...
        for (count = offset32; result < limit && count > 0; --count, ++result) {
            char ch2 = source[result];
            if (!UTF16.isLeadSurrogate(ch2) || result + 1 >= limit || !UTF16.isTrailSurrogate(source[result + 1])) continue;
            ++result;
        }
        if (count != 0) {
            throw new ArrayIndexOutOfBoundsException(offset32);
    ...
    public static int findCodePointOffset(String source, int offset16) {
        ...
        for (int i2 = 0; i2 < offset16; ++i2) {
            char ch2 = source.charAt(i2);
            if (hadLeadSurrogate && UTF16.isTrailSurrogate(ch2)) {
                hadLeadSurrogate = false;
                continue;
            }
            hadLeadSurrogate = UTF16.isLeadSurrogate(ch2);
            ++result;
        }
    ...
    public static int moveCodePointOffset(String source, int offset16, int shift32) {
        ...
        if (shift32 > 0) {
            ...
            for (count = -shift32; count > 0 && --result >= 0; --count) {
                char ch3 = source.charAt(result);
                if (!UTF16.isTrailSurrogate(ch3) || result <= 0 || !UTF16.isLeadSurrogate(source.charAt(result - 1))) continue;
                --result;
            }
        }
        if (count != 0) {
    ...
    public static int moveCodePointOffset(char[] source, int start, int limit, int offset16, int shift32) {
        ...
        if (shift32 > 0) {
            ...
            for (count = -shift32; count > 0 && --result >= start; --count) {
                char ch3 = source[result];
                if (!UTF16.isTrailSurrogate(ch3) || result <= start || !UTF16.isLeadSurrogate(source[result - 1])) continue;
                --result;
            }
        }
        if (count != 0) {
    ...
    public static StringBuffer reverse(StringBuffer source) {
        ...
        while (i2-- > 0) {
            char ch2;
            char ch3 = source.charAt(i2);
            if (UTF16.isTrailSurrogate(ch3) && i2 > 0 && UTF16.isLeadSurrogate(ch2 = source.charAt(i2 - 1))) {
                result.append(ch2);
                result.append(ch3);
                --i2;
                continue;
    ...
    private static String toString(int ch2) {
        if (ch2 < 65536) {
