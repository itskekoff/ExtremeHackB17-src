...
class RBBIRuleScanner {
    ...
    static String stripRules(String rules) {
        ...
        while (idx < rulesLength) {
            char ch2;
            if ((ch2 = rules.charAt(idx++)) == '#') {
                while (idx < rulesLength && ch2 != '\r' && ch2 != '\n' && ch2 != '\u0085') {
                    ch2 = rules.charAt(idx++);
                }
            }
            if (UCharacter.isISOControl(ch2)) continue;
            strippedRules.append(ch2);
        }
    ...
    int nextCharLL() {
        ...
        int ch2 = UTF16.charAt(this.fRB.fRules, this.fNextIndex);
        this.fNextIndex = UTF16.moveCodePointOffset(this.fRB.fRules, this.fNextIndex, 1);
        if (ch2 == 13 || ch2 == 133 || ch2 == 8232 || ch2 == 10 && this.fLastChar != 13) {
            ++this.fLineNum;
            this.fCharNum = 0;
            if (this.fQuoteMode) {
                this.error(66057);
