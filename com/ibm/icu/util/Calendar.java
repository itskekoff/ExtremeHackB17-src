...
Comparable<Calendar> {
    ...
    private static Calendar getInstanceInternal(TimeZone tz2, ULocale locale) {
        ...
        Calendar cal2 = Calendar.getShim().createInstance(locale);
    ...
    static Calendar createInstance(ULocale locale) {
        ...
        switch (calType) {
            case 0: {
                cal2 = new GregorianCalendar(zone, locale);
                break;
            }
            case 1: {
                cal2 = new JapaneseCalendar(zone, locale);
                break;
            }
            case 2: {
                cal2 = new BuddhistCalendar(zone, locale);
                break;
            }
            case 3: {
                cal2 = new TaiwanCalendar(zone, locale);
                break;
            }
            case 4: {
                cal2 = new PersianCalendar(zone, locale);
                break;
            }
            case 5: {
                cal2 = new IslamicCalendar(zone, locale);
                break;
            }
            case 6: {
                cal2 = new IslamicCalendar(zone, locale);
                ((IslamicCalendar)cal2).setCivil(false);
                break;
            }
            case 7: {
                cal2 = new HebrewCalendar(zone, locale);
                break;
            }
            case 8: {
                cal2 = new ChineseCalendar(zone, locale);
                break;
            }
            case 9: {
                cal2 = new IndianCalendar(zone, locale);
                break;
            }
            case 10: {
                cal2 = new CopticCalendar(zone, locale);
                break;
            }
            case 11: {
                cal2 = new EthiopicCalendar(zone, locale);
                break;
            }
            case 12: {
                cal2 = new EthiopicCalendar(zone, locale);
            ...
            case 14: {
                cal2 = new DangiCalendar(zone, locale);
                break;
            }
            case 13: {
                cal2 = new GregorianCalendar(zone, locale);
    ...
    public int getActualMaximum(int field) {
        ...
        switch (field) {
            case 5: {
                Calendar cal2 = (Calendar)this.clone();
                cal2.setLenient(true);
                cal2.prepareGetActual(field, false);
                result = this.handleGetMonthLength(cal2.get(19), cal2.get(2));
                break;
            }
            case 6: {
                Calendar cal3 = (Calendar)this.clone();
                cal3.setLenient(true);
                cal3.prepareGetActual(field, false);
                result = this.handleGetYearLength(cal3.get(19));
                break;
            }
            case 0: 
            case 7: 
    ...
    private static DateFormat formatHelper(Calendar cal2, ULocale loc, int dateStyle, int timeStyle) {
        PatternData patternData = PatternData.make(cal2, loc);
        ...
        DateFormat result = cal2.handleGetDateFormat(pattern, override, loc);
        result.setCalendar(cal2);
        return result;
    }

    public static String getDateTimePattern(Calendar cal2, ULocale uLocale, int dateStyle) {
        PatternData patternData = PatternData.make(cal2, uLocale);
    ...
    private void setWeekData(ULocale locale) {
        ...
        if (data == null) {
            ...
            if (min.getCountry().length() > 0) {
                ...
                buf2.append(min.getLanguage());
                if (min.getScript().length() > 0) {
                    buf2.append("_" + min.getScript());
                }
                if (max.getCountry().length() > 0) {
                    buf2.append("_" + max.getCountry());
                }
                if (min.getVariant().length() > 0) {
                    buf2.append("_" + min.getVariant());
                }
                useLocale = new ULocale(buf2.toString());
            }
            UResourceBundle rb2 = UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt51b", "supplementalData", ICUResourceBundle.ICU_DATA_CLASS_LOADER);
    ...
    static class PatternData {
        ...
        private static PatternData make(Calendar cal2, ULocale loc) {
            String calType = cal2.getType();
