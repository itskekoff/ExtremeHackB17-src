...
implements Serializable {
    ...
    public static ULocale getDefault() {
        Class<ULocale> class_ = ULocale.class;
        synchronized (ULocale.class) {
            ...
            if (!defaultLocale.equals(currentDefault)) {
                ...
                if (!JDKLocaleHelper.isJava7orNewer()) {
                    for (Category cat2 : Category.values()) {
                        int idx = cat2.ordinal();
    ...
    public static synchronized void setDefault(ULocale newLocale) {
        ...
        for (Category cat2 : Category.values()) {
            ULocale.setDefault(cat2, newLocale);
        }
    }
    ...
    public String toLanguageTag() {
        ...
        if (subtag.length() > 0) {
            buf2.append(LanguageTag.canonicalizeLanguage(subtag));
        }
        if ((subtag = tag.getScript()).length() > 0) {
            buf2.append("-");
            buf2.append(LanguageTag.canonicalizeScript(subtag));
        }
        if ((subtag = tag.getRegion()).length() > 0) {
            buf2.append("-");
            buf2.append(LanguageTag.canonicalizeRegion(subtag));
        }
        List<String> subtags = tag.getVariants();
        for (String s2 : subtags) {
            buf2.append("-");
            buf2.append(LanguageTag.canonicalizeVariant(s2));
        }
        subtags = tag.getExtensions();
        for (String s2 : subtags) {
            buf2.append("-");
            buf2.append(LanguageTag.canonicalizeExtension(s2));
        }
        ...
        if (subtag.length() > 0) {
            ...
            buf2.append("x").append("-");
            buf2.append(LanguageTag.canonicalizePrivateuse(subtag));
        }
        return buf2.toString();
    }
    ...
    private static ULocale getInstance(BaseLocale base, LocaleExtensions exts) {
        ...
        if (!extKeys.isEmpty()) {
            ...
            if (!kwds.isEmpty()) {
                StringBuilder buf2 = new StringBuilder(id2);
                buf2.append("@");
                ...
                for (Map.Entry kwd : kset) {
                    ...
                    buf2.append((String)kwd.getKey());
                    buf2.append("=");
                    buf2.append((String)kwd.getValue());
                }
                id2 = buf2.toString();
            }
        }
    ...
    static {
        ...
        if (JDKLocaleHelper.isJava7orNewer()) {
            for (Category cat2 : Category.values()) {
                int idx = cat2.ordinal();
                ULocale.defaultCategoryLocales[idx] = JDKLocaleHelper.getDefault(cat2);
            ...
            for (Category cat3 : Category.values()) {
                int idx = cat3.ordinal();
    ...
    private static final class JDKLocaleHelper {
        ...
        private static ULocale toULocale7(Locale loc) {
            ...
            StringBuilder buf2 = new StringBuilder(language);
            if (script.length() > 0) {
                buf2.append('_');
                buf2.append(script);
            }
            ...
            return new ULocale(ULocale.getName(buf2.toString()), loc);
        }
        ...
        public static Locale getDefault(Category category) {
            ...
            if (isJava7orNewer) {
                ...
                if (cat2 != null) {
                    try {
                        loc = (Locale)mGetDefault.invoke(null, cat2);
                    }
        ...
        public static void setDefault(Category category, Locale newLocale) {
            if (isJava7orNewer) {
                ...
                if (cat2 != null) {
                    try {
                        mSetDefault.invoke(null, cat2, newLocale);
                    }
                    catch (InvocationTargetException e2) {
                    }
                    catch (IllegalArgumentException e3) {
