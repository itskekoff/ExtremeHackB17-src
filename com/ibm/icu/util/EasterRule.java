...
implements DateRule {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private Date computeInYear(Date date, GregorianCalendar cal2) {
        if (cal2 == null) {
            cal2 = this.calendar;
        }
        GregorianCalendar gregorianCalendar = cal2;
        synchronized (gregorianCalendar) {
            cal2.setTime(date);
            int year = cal2.get(1);
            int g2 = year % 19;
            int i2 = 0;
            int j2 = 0;
            if (cal2.getTime().after(cal2.getGregorianChange())) {
                int c2 = year / 100;
            ...
            cal2.set(5, d2);
            cal2.getTime();
            cal2.add(5, this.daysAfterEaster);
            return cal2.getTime();
        }
    }
}
