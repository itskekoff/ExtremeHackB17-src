...
public final class Utility {
    ...
    public static final String escape(String s2) {
        ...
        for (int i2 = 0; i2 < s2.length(); i2 += UTF16.getCharCount(c2)) {
            ...
            buf2.append(four ? "\\u" : "\\U");
            buf2.append(Utility.hex(c2, four ? 4 : 8));
        }
    ...
    public static String hex(long ch2) {
        return Utility.hex(ch2, 4);
    }
    ...
    public static boolean parseChar(String id2, int[] pos, char ch2) {
        int start = pos[0];
        pos[0] = PatternProps.skipWhiteSpace(id2, pos[0]);
        if (pos[0] == id2.length() || id2.charAt(pos[0]) != ch2) {
            pos[0] = start;
            return false;
        }
        pos[0] = pos[0] + 1;
    ...
    public static String parseUnicodeIdentifier(String str, int[] pos) {
        ...
        int ch2;
        StringBuilder buf2 = new StringBuilder();
        for (p2 = pos[0]; p2 < str.length(); p2 += UTF16.getCharCount(ch2)) {
            ch2 = Character.codePointAt(str, p2);
            if (buf2.length() == 0) {
                if (UCharacter.isUnicodeIdentifierStart(ch2)) {
                    buf2.appendCodePoint(ch2);
                    continue;
                }
                return null;
            }
            if (!UCharacter.isUnicodeIdentifierPart(ch2)) break;
            buf2.appendCodePoint(ch2);
        }
        pos[0] = p2;
        return buf2.toString();
    ...
    public static int parseNumber(String text, int[] pos, int radix) {
        ...
        int p2;
        int n2 = 0;
        for (p2 = pos[0]; p2 < text.length() && (d2 = UCharacter.digit(ch2 = Character.codePointAt(text, p2), radix)) >= 0; ++p2) {
