...
public class TextTrieMap<V> {
    ...
    private class Node {
        ...
        public void add(CharIterator chitr, V value) {
            ...
            while (chitr.hasNext()) {
                buf2.append(chitr.next());
            }
            this.add(TextTrieMap.toCharArray(buf2), 0, value);
        }
        ...
        public Node findMatch(CharIterator chitr) {
            ...
            Node match = null;
            Character ch2 = chitr.next();
            for (Node child : this._children) {
                if (ch2.charValue() < child._text[0]) break;
                if (ch2.charValue() != child._text[0]) continue;
        ...
        private boolean matchFollowing(CharIterator chitr) {
            ...
            for (int idx = 1; idx < this._text.length; ++idx) {
                ...
                Character ch2 = chitr.next();
                if (ch2.charValue() == this._text[idx]) continue;
                matched = false;
                break;
            }
            return matched;
