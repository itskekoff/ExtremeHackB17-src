...
public final class IDNA2003 {
    ...
    private static char toASCIILower(char ch2) {
        if ('A' <= ch2 && ch2 <= 'Z') {
            return (char)(ch2 + 32);
        }
        return ch2;
    ...
    private static boolean isLDHChar(int ch2) {
        if (ch2 > 122) {
            return false;
        }
        return ch2 == 45 || 48 <= ch2 && ch2 <= 57 || 65 <= ch2 && ch2 <= 90 || 97 <= ch2 && ch2 <= 122;
    }

    private static boolean isLabelSeparator(int ch2) {
        switch (ch2) {
            case 46: 
            case 12290: 
            case 65294: 
    ...
    public static StringBuffer convertToASCII(UCharacterIterator src, int options) throws StringPrepParseException {
        ...
        for (int j2 = 0; j2 < poLen; ++j2) {
            ch2 = processOut.charAt(j2);
            if (ch2 > 127) {
                srcIsASCII = false;
                continue;
            }
            if (IDNA2003.isLDHChar(ch2)) continue;
            srcIsLDH = false;
            failPos = j2;
        }
