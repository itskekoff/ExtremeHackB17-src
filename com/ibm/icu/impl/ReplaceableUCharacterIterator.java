...
extends UCharacterIterator {
    ...
    public ReplaceableUCharacterIterator(StringBuffer buf2) {
        if (buf2 == null) {
            throw new IllegalArgumentException();
        }
        this.replaceable = new ReplaceableString(buf2);
        this.currentIndex = 0;
    }
    ...
    public int currentCodePoint() {
        ...
        if (UTF16.isLeadSurrogate((char)ch2)) {
            this.next();
            int ch22 = this.current();
            this.previous();
            if (UTF16.isTrailSurrogate((char)ch22)) {
                return UCharacterProperty.getRawSupplementary((char)ch2, (char)ch22);
            }
        }
        return ch2;
    }
