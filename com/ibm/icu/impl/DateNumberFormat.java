...
extends NumberFormat {
    ...
    public DateNumberFormat(ULocale loc, char zeroDigit, String nsName) {
        ...
        for (int i2 = 0; i2 < 10; ++i2) {
            buf2.append((char)(zeroDigit + i2));
        }
        this.initialize(loc, buf2.toString(), nsName);
    }
    ...
    private void initialize(ULocale loc, String digitString, String nsName) {
        ...
        if (elems == null) {
            String minusString;
            ICUResourceBundle rb2 = (ICUResourceBundle)UResourceBundle.getBundleInstance("com/ibm/icu/impl/data/icudt51b", loc);
            try {
                minusString = rb2.getStringWithFallback("NumberElements/" + nsName + "/symbols/minusSign");
            }
            catch (MissingResourceException ex2) {
                if (!nsName.equals("latn")) {
                    try {
                        minusString = rb2.getStringWithFallback("NumberElements/latn/symbols/minusSign");
                    }
    ...
    public Number parse(String text, ParsePosition parsePosition) {
        ...
        while (base + offset < text.length()) {
            char ch2 = text.charAt(base + offset);
            if (offset == 0 && ch2 == this.minusSign) {
                if (this.positiveOnly) break;
                negative = true;
            } else {
                int digit = ch2 - this.digits[0];
                if (digit < 0 || 9 < digit) {
                    digit = UCharacter.digit(ch2);
                }
                if (digit < 0 || 9 < digit) {
                    for (digit = 0; digit < 10 && ch2 != this.digits[digit]; ++digit) {
                    }
                }
