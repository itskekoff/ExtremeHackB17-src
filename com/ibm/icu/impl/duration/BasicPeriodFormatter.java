...
implements PeriodFormatter {
    ...
    private String format(int tl2, boolean inFuture, int[] counts) {
        int i2;
        int first;
        int i3;
        int mask = 0;
        ...
        if (this.data.useMilliseconds() != 0 && (mask & 1 << TimeUnit.MILLISECOND.ordinal) != 0) {
            ...
            switch (this.data.useMilliseconds()) {
                case 2: {
                    if ((mask & sf2) == 0) break;
                    byte by2 = sx2;
                    counts[by2] = counts[by2] + (counts[mx] - 1) / 1000;
                    mask &= ~mf;
                    forceD3Seconds = true;
                    break;
                }
                case 1: {
                    ...
                    byte by3 = sx2;
                    counts[by3] = counts[by3] + (counts[mx] - 1) / 1000;
                    mask &= ~mf;
                    forceD3Seconds = true;
                }
            }
        ...
        int td2 = !this.customs.displayDirection || isZero ? 0 : (inFuture ? 2 : 1);
        boolean useDigitPrefix = this.data.appendPrefix(tl2, td2, sb2);
        ...
        this.data.appendSuffix(tl2, td2, sb2);
        return sb2.toString();
    }
}
