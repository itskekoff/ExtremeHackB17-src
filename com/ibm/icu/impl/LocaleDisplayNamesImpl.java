...
extends LocaleDisplayNames {
    ...
    private String adjustForUsageAndContext(CapitalizationContextUsage usage, String name) {
        ...
        if (titlecase) {
            ...
            for (stopPos = 0; !(stopPos >= stopPosLimit || (ch2 = name.codePointAt(stopPos)) < 65 || ch2 > 90 && ch2 < 97 || ch2 > 122 && ch2 < 192); ++stopPos) {
                if (ch2 < 65536) continue;
                ++stopPos;
            }
    ...
    private String localeDisplayNameInternal(ULocale locale) {
        ...
        if (hasScript) {
            buf2.append(this.scriptDisplayNameInContext(script));
        }
        if (hasCountry) {
            this.appender.append(this.regionDisplayName(country), buf2);
        }
        if (hasVariant) {
            this.appender.append(this.variantDisplayName(variant), buf2);
        }
        if ((keys = locale.getKeywords()) != null) {
            while (keys.hasNext()) {
                ...
                if (!valueDisplayName.equals(value)) {
                    this.appender.append(valueDisplayName, buf2);
                    continue;
                }
                if (!key.equals(keyDisplayName)) {
                    String keyValue = this.keyTypeFormat.format(new String[]{keyDisplayName, valueDisplayName});
                    this.appender.append(keyValue, buf2);
                    continue;
                }
                this.appender.append(keyDisplayName, buf2).append("=").append(valueDisplayName);
            }
        }
        String resultRemainder = null;
        if (buf2.length() > 0) {
            resultRemainder = buf2.toString();
        }
