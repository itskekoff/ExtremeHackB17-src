...
extends TrieBuilder {
    ...
    public int getValue(int ch2) {
        if (this.m_isCompacted_ || ch2 > 0x10FFFF || ch2 < 0) {
            return 0;
        }
        int block = this.m_index_[ch2 >> 5];
        return this.m_data_[Math.abs(block) + (ch2 & 0x1F)];
    }

    public int getValue(int ch2, boolean[] inBlockZero) {
        if (this.m_isCompacted_ || ch2 > 0x10FFFF || ch2 < 0) {
            if (inBlockZero != null) {
                inBlockZero[0] = true;
            }
    ...
    public boolean setValue(int ch2, int value) {
        if (this.m_isCompacted_ || ch2 > 0x10FFFF || ch2 < 0) {
            return false;
        }
        int block = this.getDataBlock(ch2);
        if (block < 0) {
            return false;
        }
        this.m_data_[block + (ch2 & 31)] = value;
        return true;
    }
    ...
    private int getDataBlock(int ch2) {
        int indexValue = this.m_index_[ch2 >>= 5];
        if (indexValue > 0) {
            return indexValue;
        }
