...
public abstract class Trie {
    ...
    protected final int getRawOffset(int offset, char ch2) {
        return (this.m_index_[offset + (ch2 >> 5)] << 2) + (ch2 & 0x1F);
    }

    protected final int getBMPOffset(char ch2) {
        return ch2 >= '\ud800' && ch2 <= '\udbff' ? this.getRawOffset(320, ch2) : this.getRawOffset(0, ch2);
    }

    protected final int getLeadOffset(char ch2) {
        return this.getRawOffset(0, ch2);
    }

    protected final int getCodePointOffset(int ch2) {
        if (ch2 < 0) {
            return -1;
        }
        if (ch2 < 55296) {
            return this.getRawOffset(0, (char)ch2);
        }
        if (ch2 < 65536) {
            return this.getBMPOffset((char)ch2);
        }
        if (ch2 <= 0x10FFFF) {
            return this.getSurrogateOffset(UTF16.getLeadSurrogate(ch2), (char)(ch2 & 0x3FF));
        }
        return -1;
    }
    ...
    protected void unserialize(InputStream inputStream) throws IOException {
        ...
        for (int i2 = 0; i2 < this.m_dataOffset_; ++i2) {
            this.m_index_[i2] = input.readChar();
        }
    }
