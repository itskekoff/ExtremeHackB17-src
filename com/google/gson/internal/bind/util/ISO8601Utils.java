...
public class ISO8601Utils {
    ...
    public static String format(Date date, boolean millis, TimeZone tz2) {
        int offset;
        GregorianCalendar calendar = new GregorianCalendar(tz2, Locale.US);
        calendar.setTime(date);
        int capacity = "yyyy-MM-ddThh:mm:ss".length();
        capacity += millis ? ".sss".length() : 0;
        StringBuilder formatted = new StringBuilder(capacity += tz2.getRawOffset() == 0 ? "Z".length() : "+hh:mm".length());
