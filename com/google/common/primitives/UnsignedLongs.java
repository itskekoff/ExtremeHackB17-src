...
public final class UnsignedLongs {
    ...
    public static String toString(long x2, int radix) {
        ...
        if ((radix & radix - 1) == 0) {
            ...
            int mask = radix - 1;
            do {
                buf2[--i2] = Character.forDigit((int)x2 & mask, radix);
            } while ((x2 >>>= shift) != 0L);
        } else {
            long quotient = (radix & 1) == 0 ? (x2 >>> 1) / (long)(radix >>> 1) : UnsignedLongs.divide(x2, radix);
            long rem = x2 - quotient * (long)radix;
            buf2[--i2] = Character.forDigit((int)rem, radix);
            for (x2 = quotient; x2 > 0L; x2 /= (long)radix) {
                buf2[--i2] = Character.forDigit((int)(x2 % (long)radix), radix);
            }
        }
        return new String(buf2, i2, buf2.length - i2);
    }
