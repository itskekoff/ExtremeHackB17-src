...
implements ConcurrentMap<K, V> {
    ...
    long longSize() {
        ...
        for (int i2 = 0; i2 < segments.length; ++i2) {
            sum += (long)Math.max(0, segments[i2].count);
        }
        return sum;
    }
    ...
    public V computeIfAbsent(K key, java.util.function.Function<? super K, ? extends V> function) {
        Preconditions.checkNotNull(key);
        Preconditions.checkNotNull(function);
        return (V)this.compute(key, (k2, oldValue) -> oldValue == null ? function.apply((Object)key) : oldValue);
    }
    ...
    public V computeIfPresent(K key, BiFunction<? super K, ? super V, ? extends V> function) {
        Preconditions.checkNotNull(key);
        Preconditions.checkNotNull(function);
        return (V)this.compute(key, (k2, oldValue) -> oldValue == null ? null : function.apply((Object)k2, (Object)oldValue));
    }
    ...
    public V merge(K key, V newValue, BiFunction<? super V, ? super V, ? extends V> function) {
        ...
        Preconditions.checkNotNull(function);
        return (V)this.compute(key, (k2, oldValue) -> oldValue == null ? newValue : function.apply((Object)oldValue, (Object)newValue));
    }
    ...
    public Set<K> keySet() {
        ...
        return ks2 != null ? ks2 : (this.keySet = new KeySet(this));
    }
    ...
    private static <E> ArrayList<E> toArrayList(Collection<E> c2) {
        ArrayList result = new ArrayList(c2.size());
        Iterators.addAll(result, c2.iterator());
        return result;
    }
    ...
    extends AbstractCacheSet<Map.Entry<K, V>> {
        ...
        public boolean removeIf(Predicate<? super Map.Entry<K, V>> filter) {
            Preconditions.checkNotNull(filter);
            return LocalCache.this.removeIf((? super K k2, ? super V v2) -> filter.test(Maps.immutableEntry(k2, v2)));
        }
    ...
    extends AbstractCollection<V> {
        ...
        public boolean removeIf(Predicate<? super V> filter) {
            Preconditions.checkNotNull(filter);
            return LocalCache.this.removeIf((? super K k2, ? super V v2) -> filter.test((Object)v2));
        }
        ...
        public <E> E[] toArray(E[] a2) {
            return LocalCache.toArrayList(this).toArray(a2);
        }
    }
    ...
    implements ValueReference<K, V> {
        ...
        public ListenableFuture<V> loadFuture(K key, CacheLoader<? super K, V> loader) {
            try {
                ...
                ListenableFuture<V> listenableFuture = result = this.setException(t2) ? this.futureValue : this.fullyFailedFuture(t2);
    ...
    extends ReentrantLock {
        ...
        V compute(K key, int hash, BiFunction<? super K, ? super V, ? extends V> function) {
            ...
            synchronized (referenceEntry) {
                ...
                this.lock();
                try {
                    this.removeEntry(e2, hash, RemovalCause.EXPLICIT);
                }
                finally {
                    this.unlock();
                }
                ...
                for (ReferenceEntry<K, V> e2 = first = table.get(index); e2 != null; e2 = e2.getNext()) {
                    ...
                    if (entryValue == null) {
                        if (valueReference.isActive()) {
                            int newCount = this.count - 1;
                            ++this.modCount;
                            ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e2, entryKey, hash, entryValue, valueReference, RemovalCause.COLLECTED);
                            newCount = this.count - 1;
        ...
        V remove(Object key, int hash) {
            ...
            try {
                ...
                for (ReferenceEntry<K, V> e2 = first = table.get(index); e2 != null; e2 = e2.getNext()) {
                    ...
                    ++this.modCount;
                    ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e2, entryKey, hash, entryValue, valueReference, cause);
                    newCount = this.count - 1;
                    table.set(index, newFirst);
                    this.count = newCount;
                    V v3 = entryValue;
                ...
                try {
                    ...
                    for (i2 = 0; i2 < table.length(); ++i2) {
                        for (ReferenceEntry<K, V> e2 = table.get(i2); e2 != null; e2 = e2.getNext()) {
                            ...
                            this.enqueueNotification(key, e2.getHash(), value, e2.getValueReference().getWeight(), cause);
                        }
                    }
                    for (i2 = 0; i2 < table.length(); ++i2) {
                        table.set(i2, null);
        ...
        boolean reclaimKey(ReferenceEntry<K, V> entry, int hash) {
            ...
            try {
                ...
                for (ReferenceEntry<K, V> e2 = first = table.get(index); e2 != null; e2 = e2.getNext()) {
                    ...
                    ++this.modCount;
                    ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e2, e2.getKey(), hash, e2.getValueReference().get(), e2.getValueReference(), RemovalCause.COLLECTED);
                    newCount = this.count - 1;
                    table.set(index, newFirst);
                    this.count = newCount;
                    boolean bl2 = true;
            ...
            try {
                ...
                for (ReferenceEntry<K, V> e2 = first = table.get(index); e2 != null; e2 = e2.getNext()) {
                    ...
                    if (v2 == valueReference) {
                        ++this.modCount;
                        ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e2, entryKey, hash, valueReference.get(), valueReference, RemovalCause.COLLECTED);
                        newCount = this.count - 1;
                        table.set(index, newFirst);
                        this.count = newCount;
                        boolean bl2 = true;
        ...
        boolean removeEntry(ReferenceEntry<K, V> entry, int hash, RemovalCause cause) {
            ...
            for (ReferenceEntry<K, V> e2 = first = table.get(index); e2 != null; e2 = e2.getNext()) {
                if (e2 != entry) continue;
                ++this.modCount;
                ReferenceEntry<K, V> newFirst = this.removeValueFromChain(first, e2, e2.getKey(), hash, e2.getValueReference().get(), e2.getValueReference(), cause);
                newCount = this.count - 1;
                table.set(index, newFirst);
                this.count = newCount;
                return true;
