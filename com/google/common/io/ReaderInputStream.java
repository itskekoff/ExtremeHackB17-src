...
extends InputStream {
    ...
    public int read(byte[] b2, int off, int len) throws IOException {
        ...
        block0: while (true) {
            if (this.draining) {
                if ((totalBytesRead += this.drain(b2, off + totalBytesRead, len - totalBytesRead)) == len || this.doneFlushing) {
    ...
    private static CharBuffer grow(CharBuffer buf2) {
        char[] copy = Arrays.copyOf(buf2.array(), buf2.capacity() * 2);
        CharBuffer bigger = CharBuffer.wrap(copy);
        bigger.position(buf2.position());
        bigger.limit(buf2.limit());
        return bigger;
    }
    ...
    private int drain(byte[] b2, int off, int len) {
        ...
        this.byteBuffer.get(b2, off, remaining);
        return remaining;
    }
}
