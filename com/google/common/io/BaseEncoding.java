...
public abstract class BaseEncoding {
    ...
    static Writer separatingWriter(final Writer delegate, String separator, int afterEveryChars) {
        ...
        return new Writer(){
            ...
            public void write(int c2) throws IOException {
                seperatingAppendable.append((char)c2);
            }
    ...
    extends StandardBaseEncoding {
        ...
        void encodeTo(Appendable target, byte[] bytes, int off, int len) throws IOException {
            ...
            if (i2 < off + len) {
                this.encodeChunkTo(target, bytes, i2, off + len - i2);
            }
        }
    ...
    extends StandardBaseEncoding {
        ...
        private Base16Encoding(Alphabet alphabet) {
            ...
            for (int i2 = 0; i2 < 256; ++i2) {
                ...
                this.encoding[i2 | 256] = alphabet.encode(i2 & 0xF);
            }
        }
    ...
    extends BaseEncoding {
        ...
        void encodeTo(Appendable target, byte[] bytes, int off, int len) throws IOException {
            Preconditions.checkNotNull(target);
            Preconditions.checkPositionIndexes(off, off + len, bytes.length);
            for (int i2 = 0; i2 < len; i2 += this.alphabet.bytesPerChunk) {
                this.encodeChunkTo(target, bytes, off + i2, Math.min(this.alphabet.bytesPerChunk, len - i2));
            }
        }
            ...
            return new InputStream(){
                ...
                public int read() throws IOException {
                    while (true) {
                        ...
                        ++this.readChars;
                        char ch2 = (char)readChar;
                        if (this.paddingMatcher.matches(ch2)) {
                        ...
                        if (this.hitPadding) {
                            throw new DecodingException("Expected padding character but found '" + ch2 + "' at index " + this.readChars);
                        }
                        this.bitBuffer <<= alphabet.bitsPerChar;
                        this.bitBuffer |= alphabet.decode(ch2);
                        this.bitBufferLength += alphabet.bitsPerChar;
    ...
    extends CharMatcher {
        ...
        Alphabet(String name, char[] chars) {
            ...
            try {
                ...
                throw new IllegalArgumentException("Illegal alphabet " + new String(chars), e3);
            }
        ...
        boolean canDecode(char ch2) {
            return ch2 <= '\u007f' && this.decodabet[ch2] != -1;
        }

        int decode(char ch2) throws DecodingException {
            if (ch2 > '\u007f' || this.decodabet[ch2] == -1) {
                throw new DecodingException("Unrecognized character: " + (CharMatcher.invisible().matches(ch2) ? "0x" + Integer.toHexString(ch2) : Character.valueOf(ch2)));
            }
