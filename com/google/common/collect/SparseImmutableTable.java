...
extends RegularImmutableTable<R, C, V> {
    ...
    SparseImmutableTable(ImmutableList<Table.Cell<R, C, V>> cellList, ImmutableSet<R> rowSpace, ImmutableSet<C> columnSpace) {
        ...
        for (int i2 = 0; i2 < cellList.size(); ++i2) {
            ...
            cellRowIndices[i2] = (Integer)rowIndex.get(r2);
            Map thisRow = (Map)rows.get(r2);
            cellColumnInRowIndices[i2] = thisRow.size();
            Object oldValue = thisRow.put(columnKey, value);
            if (oldValue != null) {
                throw new IllegalArgumentException("Duplicate value for row=" + r2 + ", column=" + columnKey + ": " + value + ", " + oldValue);
            }
            ((Map)columns.get(columnKey)).put(r2, value);
        }
    ...
    ImmutableTable.SerializedForm createSerializedForm() {
        ...
        for (Table.Cell cell : this.cellSet()) {
            cellColumnIndices[i2++] = (Integer)columnKeyToIndex.get(cell.getColumnKey());
        }
