...
implements FilteredMultimap<K, V> {
    ...
    extends Multimaps.Keys<K, V> {
        ...
        public int remove(@Nullable Object key, int occurrences) {
            ...
            while (itr.hasNext()) {
                Object v2 = itr.next();
                if (!FilteredEntryMultimap.this.satisfies(k2, v2) || ++oldCount > occurrences) continue;
                itr.remove();
            }
            return oldCount;
        }
        ...
        public Set<Multiset.Entry<K>> entrySet() {
            return new Multisets.EntrySet<K>(){
                ...
                public boolean removeAll(Collection<?> c2) {
                    return this.removeEntriesIf(Predicates.in(c2));
                }

                @Override
                public boolean retainAll(Collection<?> c2) {
                    return this.removeEntriesIf(Predicates.not(Predicates.in(c2)));
                }
            };
        }
    }
    ...
    extends Maps.ViewCachingAbstractMap<K, Collection<V>> {
        ...
        public Collection<V> get(@Nullable Object key) {
            ...
            Object k2 = key;
            return (result = FilteredEntryMultimap.filterCollection(result, new ValuePredicate(k2))).isEmpty() ? null : result;
        }
        ...
        public Collection<V> remove(@Nullable Object key) {
            ...
            while (itr.hasNext()) {
                Object v2 = itr.next();
                if (!FilteredEntryMultimap.this.satisfies(k2, v2)) continue;
                itr.remove();
                result.add(v2);
            }
            if (result.isEmpty()) {
                ...
                public boolean remove(@Nullable Object o2) {
                    if (o2 instanceof Collection) {
                        ...
                        while (entryIterator.hasNext()) {
                            ...
                            Object key = entry.getKey();
                            Collection collection = FilteredEntryMultimap.filterCollection(entry.getValue(), new ValuePredicate(key));
                            if (collection.isEmpty() || !c2.equals(collection)) continue;
                            if (collection.size() == entry.getValue().size()) {
                                entryIterator.remove();
                            } else {
                                collection.clear();
