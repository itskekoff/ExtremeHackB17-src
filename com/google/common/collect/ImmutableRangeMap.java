    ...
    public static final class Builder<K extends Comparable<?>, V> {
        ...
        public ImmutableRangeMap<K, V> build() {
            ...
            for (int i2 = 0; i2 < this.entries.size(); ++i2) {
                Range<K> prevRange;
                Range<K> range = this.entries.get(i2).getKey();
                if (i2 > 0 && range.isConnected(prevRange = this.entries.get(i2 - 1).getKey()) && !range.intersection(prevRange).isEmpty()) {
                    throw new IllegalArgumentException("Overlapping ranges: range " + prevRange + " overlaps with entry " + range);
                }
                rangesBuilder.add(range);
                valuesBuilder.add(this.entries.get(i2).getValue());
            }
