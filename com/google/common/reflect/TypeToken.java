...
implements Serializable {
    ...
    private boolean someRawTypeIsSubclassOf(Class<?> superclass) {
        for (Class class_ : this.getRawTypes()) {
            if (!superclass.isAssignableFrom(class_)) continue;
            return true;
        }
        return false;
    }
    ...
    private boolean isSubtypeOfParameterizedType(ParameterizedType supertype) {
        ...
        for (int i2 = 0; i2 < typeParams.length; ++i2) {
            if (super.is(toTypeArgs[i2])) continue;
            return false;
        }
    ...
    private ImmutableSet<Class<? super T>> getRawTypes() {
        ...
        new TypeVisitor(){
            ...
            void visitGenericArrayType(GenericArrayType t2) {
                builder.add(Types.getArrayClass(TypeToken.of(t2.getGenericComponentType()).getRawType()));
            }
        }.visit(this.runtimeType);
