...
extends AbstractFuture.TrustedFuture<OutputT> {
    ...
    private static boolean addCausalChain(Set<Throwable> seen, Throwable t2) {
        while (t2 != null) {
            boolean firstTimeSeen = seen.add(t2);
            if (!firstTimeSeen) {
                return false;
            }
            t2 = t2.getCause();
    ...
    implements Runnable {
        ...
        final void addInitialException(Set<Throwable> seen) {
            if (!AggregateFuture.this.isCancelled()) {
                boolean bl2 = AggregateFuture.addCausalChain(seen, AggregateFuture.this.trustedGetException());
            }
        }
        ...
        private void handleOneInputDone(int index, Future<? extends InputT> future) {
            ...
            try {
                ...
                this.handleException(e2.getCause());
            }
            catch (Throwable t2) {
        ...
        private void processCompleted() {
            if (this.collectsValues & !this.allMustSucceed) {
                int i2 = 0;
                for (ListenableFuture listenableFuture : this.futures) {
                    this.handleOneInputDone(i2++, listenableFuture);
                }
            }
            this.handleAllCompleted();
        }
