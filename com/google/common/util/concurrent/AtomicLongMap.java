...
implements Serializable {
    ...
    public long updateAndGet(K key, LongUnaryOperator updaterFunction) {
        ...
        return this.map.compute(key, (k2, value) -> updaterFunction.applyAsLong(value == null ? 0L : value));
    }
    ...
    @CanIgnoreReturnValue
    public long accumulateAndGet(K key, long x2, LongBinaryOperator accumulatorFunction) {
        Preconditions.checkNotNull(accumulatorFunction);
        return this.updateAndGet(key, oldValue -> accumulatorFunction.applyAsLong(oldValue, x2));
    }

    @CanIgnoreReturnValue
    public long getAndAccumulate(K key, long x2, LongBinaryOperator accumulatorFunction) {
        Preconditions.checkNotNull(accumulatorFunction);
        return this.getAndUpdate(key, oldValue -> accumulatorFunction.applyAsLong(oldValue, x2));
    }

    @CanIgnoreReturnValue
    public long put(K key, long newValue) {
        return this.getAndUpdate(key, x2 -> newValue);
    }

    public void putAll(Map<? extends K, ? extends Long> m2) {
        m2.forEach((arg_0, arg_1) -> this.put(arg_0, arg_1));
    }
    ...
    public void removeAllZeros() {
        this.map.values().removeIf(x2 -> x2 == 0L);
    }
    ...
    long putIfAbsent(K key, long newValue) {
        ...
        Long result = this.map.compute(key, (k2, oldValue) -> {
