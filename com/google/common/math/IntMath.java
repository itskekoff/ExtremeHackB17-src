...
public final class IntMath {
    ...
    public static int saturatedPow(int b2, int k2) {
        ...
        switch (b2) {
            ...
            case -2: {
                if (k2 >= 32) {
                    return Integer.MAX_VALUE + (k2 & 1);
                }
                return (k2 & 1) == 0 ? 1 << k2 : -1 << k2;
            }
        }
        int accum = 1;
        int limit = Integer.MAX_VALUE + (b2 >>> 31 & (k2 & 1));
        while (true) {
            switch (k2) {
                case 0: {
                    return accum;
                ...
                case 1: {
                    return IntMath.saturatedMultiply(accum, b2);
                }
            }
            if ((k2 & 1) != 0) {
                accum = IntMath.saturatedMultiply(accum, b2);
            }
            if ((k2 >>= 1) <= 0) continue;
            if (-46340 > b2 | b2 > 46340) {
                return limit;
            }
            b2 *= b2;
        }
