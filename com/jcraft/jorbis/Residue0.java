...
extends FuncResidue {
    ...
    static synchronized int _01inverse(Block vb2, Object vl2, float[][] in2, int ch2, int decodepart) {
        int j2;
        ...
        if (_01inverse_partword.length < ch2) {
            _01inverse_partword = new int[ch2][][];
        }
        for (j2 = 0; j2 < ch2; ++j2) {
    ...
    static synchronized int _2inverse(Block vb2, Object vl2, float[][] in2, int ch2) {
        LookResidue0 look = (LookResidue0)vl2;
        InfoResidue0 info = look.info;
        int samples_per_partition = info.grouping;
        int partitions_per_word = look.phrasebook.dim;
        ...
        for (int s2 = 0; s2 < look.stages; ++s2) {
            ...
            while (i2 < partvals) {
                ...
                for (int k2 = 0; k2 < partitions_per_word && i2 < partvals; ++k2, ++i2) {
                    ...
                    if ((info.secondstages[index] & 1 << s2) == 0 || (stagebook = look.fullbooks[look.partbooks[index][s2]]) == null || stagebook.decodevv_add(in2, offset, ch2, vb2.opb, samples_per_partition) != -1) continue;
                    return 0;
                }
                ++l2;
            }
