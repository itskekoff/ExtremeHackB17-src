...
public class Comment {
    ...
    private void add(byte[] comment) {
        ...
        byte[] bar2 = new byte[comment.length + 1];
        System.arraycopy(comment, 0, bar2, 0, comment.length);
        this.user_comments[this.comments] = bar2;
    ...
    public int header_out(Packet op2) {
        Buffer opb = new Buffer();
        opb.writeinit();
        if (this.pack(opb) != 0) {
            return -130;
        }
        op2.packet_base = new byte[opb.bytes()];
        op2.packet = 0;
        op2.bytes = opb.bytes();
        System.arraycopy(opb.buffer(), 0, op2.packet_base, 0, op2.bytes);
        op2.b_o_s = 0;
        op2.e_o_s = 0;
        op2.granulepos = 0L;
        return 0;
