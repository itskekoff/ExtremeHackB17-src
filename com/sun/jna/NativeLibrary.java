        ...
        if (Platform.isWindows() && "kernel32".equals(this.libraryName.toLowerCase())) {
            ...
            synchronized (map) {
                Function f2 = new Function(this, "GetLastError", 63, this.encoding){
                    ...
                    @Override
                    Object invoke(Object[] args, Class<?> returnType, boolean b2, int fixedArgs) {
            ...
            try {
                ...
                if (handle == 0L) {
                    try {
                        ...
                        e3 = new UnsatisfiedLinkError(e2.getMessage());
                    }
                }
    ...
    static String matchFramework(String libraryName) {
        ...
        if (framework.isAbsolute()) {
            ...
            for (int i2 = 0; i2 < PREFIXES.length; ++i2) {
                String libraryPath = PREFIXES[i2] + "/Library/Frameworks/" + suffix;
    ...
    private static boolean isVersionedName(String name) {
        ...
        if (name.startsWith("lib") && (so2 = name.lastIndexOf(".so.")) != -1 && so2 + 4 < name.length()) {
            for (int i2 = so2 + 4; i2 < name.length(); ++i2) {
                ...
                if (Character.isDigit(ch2) || ch2 == '.') continue;
                return false;
            }
            return true;
        }
