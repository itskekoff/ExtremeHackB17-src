...
extends Pointer {
    ...
    public Pointer share(long offset, long sz2) {
        this.boundsCheck(offset, sz2);
        return new SharedMemory(offset, sz2);
    }
    ...
    protected void boundsCheck(long off, long sz2) {
        if (off < 0L) {
            throw new IndexOutOfBoundsException("Invalid offset: " + off);
        }
        if (off + sz2 > this.size) {
            String msg = "Bounds exceeds available space : size=" + this.size + ", offset=" + (off + sz2);
    ...
    public void read(long bOff, byte[] buf2, int index, int length) {
        ...
        super.read(bOff, buf2, index, length);
    }
    ...
    public void write(long bOff, byte[] buf2, int index, int length) {
        this.boundsCheck(bOff, (long)length * 1L);
        super.write(bOff, buf2, index, length);
    }
    ...
    extends Memory {
        ...
        @Override
        protected void boundsCheck(long off, long sz2) {
            Memory.this.boundsCheck(this.peer - Memory.this.peer + off, sz2);
        }
