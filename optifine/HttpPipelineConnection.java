...
public class HttpPipelineConnection {
    ...
    public synchronized void onResponseReceived(HttpPipelineRequest p_onResponseReceived_1_, HttpResponse p_onResponseReceived_2_) {
        if (!this.terminated) {
            ...
            if (this.listRequests.size() > 0 && this.listRequests.get(0) == p_onResponseReceived_1_) {
                ...
                String s2 = p_onResponseReceived_2_.getHeader("Location");
                if (p_onResponseReceived_2_.getStatus() / 100 == 3 && s2 != null && p_onResponseReceived_1_.getHttpRequest().getRedirects() < 5) {
                    try {
                        s2 = this.normalizeUrl(s2, p_onResponseReceived_1_.getHttpRequest());
                        HttpRequest httprequest = HttpPipeline.makeRequest(s2, p_onResponseReceived_1_.getHttpRequest().getProxy());
    ...
    private String normalizeUrl(String p_normalizeUrl_1_, HttpRequest p_normalizeUrl_2_) {
        ...
        String s2 = p_normalizeUrl_2_.getHost();
        if (p_normalizeUrl_2_.getPort() != 80) {
            s2 = String.valueOf(s2) + ":" + p_normalizeUrl_2_.getPort();
        }
        if (p_normalizeUrl_1_.startsWith("/")) {
            return "http://" + s2 + p_normalizeUrl_1_;
        }
        String s1 = p_normalizeUrl_2_.getFile();
        int i2 = s1.lastIndexOf("/");
        return i2 >= 0 ? "http://" + s2 + s1.substring(0, i2 + 1) + p_normalizeUrl_1_ : "http://" + s2 + "/" + p_normalizeUrl_1_;
    }

    private void checkResponseHeader(HttpResponse p_checkResponseHeader_1_) {
        String s1;
        String s2 = p_checkResponseHeader_1_.getHeader("Connection");
        ...
        if ((s1 = p_checkResponseHeader_1_.getHeader("Keep-Alive")) != null) {
            ...
            for (int i2 = 0; i2 < astring.length; ++i2) {
                ...
                if (!astring1[0].equals("max") || (k2 = Config.parseInt(astring1[1], -1)) <= 0) continue;
    ...
    private String[] split(String p_split_1_, char p_split_2_) {
        int i2 = p_split_1_.indexOf(p_split_2_);
        if (i2 < 0) {
            return new String[]{p_split_1_};
        }
        String s2 = p_split_1_.substring(0, i2);
        String s1 = p_split_1_.substring(i2 + 1);
