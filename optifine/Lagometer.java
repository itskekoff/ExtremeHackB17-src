    ...
    public static void showLagometer(ScaledResolution p_showLagometer_0_) {
        if (gameSettings != null && (Lagometer.gameSettings.ofLagometer || Lagometer.gameSettings.showLagometer)) {
            ...
            for (int j2 = 0; j2 < timesFrame.length; ++j2) {
                ...
                long l2 = 0L;
                if (gcs[j2]) {
                    Lagometer.renderTime(j2, timesFrame[j2], k2, k2 / 2, 0, f2, bufferbuilder);
                    continue;
                }
                Lagometer.renderTime(j2, timesFrame[j2], k2, k2, k2, f2, bufferbuilder);
                f2 -= (float)Lagometer.renderTime(j2, timesServer[j2], k2 / 2, k2 / 2, k2 / 2, f2, bufferbuilder);
                f2 -= (float)Lagometer.renderTime(j2, timesTerrain[j2], 0, k2, 0, f2, bufferbuilder);
                f2 -= (float)Lagometer.renderTime(j2, timesVisibility[j2], k2, k2, 0, f2, bufferbuilder);
                f2 -= (float)Lagometer.renderTime(j2, timesChunkUpdate[j2], k2, 0, 0, f2, bufferbuilder);
                f2 -= (float)Lagometer.renderTime(j2, timesChunkUpload[j2], k2, 0, k2, f2, bufferbuilder);
                f2 -= (float)Lagometer.renderTime(j2, timesScheduledExecutables[j2], 0, 0, k2, f2, bufferbuilder);
                float f3 = f2 - (float)Lagometer.renderTime(j2, timesTick[j2], 0, k2, k2, f2, bufferbuilder);
            }
