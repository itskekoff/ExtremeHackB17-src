    ...
    public void operationComplete(F future) throws Exception {
        ...
        if (future.isSuccess()) {
            ...
            for (Promise<? super V> p2 : this.promises) {
                PromiseNotificationUtil.trySuccess(p2, result, internalLogger);
            }
        } else if (future.isCancelled()) {
            for (Promise<? super V> p3 : this.promises) {
                PromiseNotificationUtil.tryCancel(p3, internalLogger);
            }
        } else {
            Throwable cause = future.cause();
            for (Promise<? super V> p4 : this.promises) {
                PromiseNotificationUtil.tryFailure(p4, cause, internalLogger);
            }
        }
    }
}
