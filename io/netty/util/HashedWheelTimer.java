...
implements Timer {
    ...
    private static HashedWheelBucket[] createWheel(int ticksPerWheel) {
        ...
        for (int i2 = 0; i2 < wheel.length; ++i2) {
            wheel[i2] = new HashedWheelBucket();
        }
        return wheel;
    }
    ...
    implements Timeout {
        ...
        public String toString() {
            ...
            StringBuilder buf2 = new StringBuilder(192).append(StringUtil.simpleClassName(this)).append('(').append("deadline: ");
            if (remaining > 0L) {
                buf2.append(remaining).append(" ns later");
            } else if (remaining < 0L) {
                buf2.append(-remaining).append(" ns ago");
            } else {
                buf2.append("now");
            }
            if (this.isCancelled()) {
                buf2.append(", cancelled");
            }
            return buf2.append(", task: ").append(this.task()).append(')').toString();
        }
    }
    ...
    implements Runnable {
        ...
        private void transferTimeoutsToBuckets() {
            HashedWheelTimeout timeout;
            for (int i2 = 0; i2 < 100000 && (timeout = (HashedWheelTimeout)HashedWheelTimer.this.timeouts.poll()) != null; ++i2) {
        ...
        private void processCancelledTasks() {
            ...
            while ((timeout = (HashedWheelTimeout)HashedWheelTimer.this.cancelledTimeouts.poll()) != null) {
                try {
                    ...
                    logger.warn("An exception was thrown while process a cancellation task", t2);
                }
            }
        }
