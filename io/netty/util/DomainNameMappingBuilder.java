...
public final class DomainNameMappingBuilder<V> {
    ...
    extends DomainNameMapping<V> {
        ...
        public String toString() {
            ...
            StringBuilder sb2 = new StringBuilder(estimatedBufferSize).append(REPR_HEADER).append(defaultValueStr).append(REPR_MAP_OPENING);
            ImmutableDomainNameMapping.appendMapping(sb2, pattern0, value0);
            for (int index = 1; index < numberOfMappings; ++index) {
                sb2.append(", ");
                this.appendMapping(sb2, index);
            }
            return sb2.append(REPR_MAP_CLOSING).toString();
        }
        ...
        private StringBuilder appendMapping(StringBuilder sb2, int mappingIndex) {
            return ImmutableDomainNameMapping.appendMapping(sb2, this.domainNamePatterns[mappingIndex], this.values[mappingIndex].toString());
        }

        private static StringBuilder appendMapping(StringBuilder sb2, String domainNamePattern, String value) {
            return sb2.append(domainNamePattern).append('=').append(value);
        }
    }
}
