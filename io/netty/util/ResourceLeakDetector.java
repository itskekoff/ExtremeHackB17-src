...
public class ResourceLeakDetector<T> {
    ...
    static String newRecord(Object hint, int recordsToSkip) {
        ...
        if (hint != null) {
            ...
            if (hint instanceof ResourceLeakHint) {
                buf2.append(((ResourceLeakHint)hint).toHintString());
            } else {
                buf2.append(hint);
            }
            buf2.append(StringUtil.NEWLINE);
        }
        for (StackTraceElement e2 : array = new Throwable().getStackTrace()) {
            if (recordsToSkip > 0) {
                --recordsToSkip;
                continue;
            }
    ...
    ResourceLeak {
        ...
        public String toString() {
            ...
            StringBuilder buf2 = new StringBuilder(16384).append(StringUtil.NEWLINE);
            if (removedRecords > 0) {
                buf2.append("WARNING: ").append(removedRecords).append(" leak records were discarded because the leak record count is limited to ").append(MAX_RECORDS).append(". Use system property ").append(ResourceLeakDetector.PROP_MAX_RECORDS).append(" to increase the limit.").append(StringUtil.NEWLINE);
            }
            buf2.append("Recent access records: ").append(array.length).append(StringUtil.NEWLINE);
            if (array.length > 0) {
                for (int i2 = array.length - 1; i2 >= 0; --i2) {
                    buf2.append('#').append(i2 + 1).append(':').append(StringUtil.NEWLINE).append(array[i2]);
                }
            }
            buf2.append("Created at:").append(StringUtil.NEWLINE).append(this.creationRecord);
    ...
    public static enum Level {
        ...
        static Level parseLevel(String levelStr) {
            ...
            for (Level l2 : Level.values()) {
                if (!trimmedLevelStr.equalsIgnoreCase(l2.name()) && !trimmedLevelStr.equals(String.valueOf(l2.ordinal()))) continue;
                return l2;
            }
