...
extends FileDescriptor {
    ...
    public int sendTo(ByteBuffer buf2, int pos, int limit, InetAddress addr, int port) throws IOException {
        int scopeId;
        byte[] address;
        ...
        int res = Socket.sendTo(this.fd, buf2, pos, limit, address, scopeId, port);
        if (res >= 0) {
            return res;
        }
    ...
    public DatagramSocketAddress recvFrom(ByteBuffer buf2, int pos, int limit) throws IOException {
        return Socket.recvFrom(this.fd, buf2, pos, limit);
    }
