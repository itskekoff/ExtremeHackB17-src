...
implements SctpChannel {
    ...
    public boolean isActive() {
        com.sun.nio.sctp.SctpChannel ch2 = this.javaChannel();
        return ch2.isOpen() && this.association() != null;
    }
    ...
    protected int doReadMessages(List<Object> buf2) throws Exception {
        ...
        try {
            ...
            MessageInfo messageInfo = ch2.receive(data, null, this.notificationHandler);
            if (messageInfo == null) {
                int n2 = 0;
                return n2;
            }
            allocHandle.lastBytesRead(data.position() - pos);
            buf2.add(new SctpMessage(messageInfo, buffer.writerIndex(buffer.writerIndex() + allocHandle.lastBytesRead())));
            free = false;
            int n3 = 1;
            return n3;
        }
    ...
    protected boolean doWriteMessage(Object msg, ChannelOutboundBuffer in2) throws Exception {
        ...
        MessageInfo mi2 = MessageInfo.createOutgoing(this.association(), null, packet.streamIdentifier());
        mi2.payloadProtocolID(packet.protocolIdentifier());
        mi2.streamNumber(packet.streamIdentifier());
        mi2.unordered(packet.isUnordered());
        int writtenBytes = this.javaChannel().send(nioData, mi2);
    ...
    protected final Object filterOutboundMessage(Object msg) throws Exception {
        if (msg instanceof SctpMessage) {
            ...
            ByteBuf buf2 = m2.content();
            if (buf2.isDirect() && buf2.nioBufferCount() == 1) {
                return m2;
            }
            return new SctpMessage(m2.protocolIdentifier(), m2.streamIdentifier(), m2.isUnordered(), this.newDirectBuffer(m2, buf2));
        }
