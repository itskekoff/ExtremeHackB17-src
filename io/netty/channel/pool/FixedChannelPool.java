...
extends SimpleChannelPool {
    ...
    private void acquire0(Promise<Channel> promise) {
        ...
        if (this.acquiredChannelCount < this.maxConnections) {
            ...
            Promise<Channel> p2 = this.executor.newPromise();
            AcquireListener l2 = new AcquireListener(promise);
            l2.acquired();
            p2.addListener(l2);
            super.acquire(p2);
        } else {
    ...
    public Future<Void> release(Channel channel, final Promise<Void> promise) {
        Promise<Void> p2 = this.executor.newPromise();
        super.release(channel, p2.addListener(new FutureListener<Void>(){
            ...
            public void run() {
                if (!FixedChannelPool.this.closed) {
                    ...
                    while ((task = (AcquireTask)FixedChannelPool.this.pendingAcquireQueue.poll()) != null) {
                        ScheduledFuture<?> f2 = task.timeoutFuture;
                        if (f2 != null) {
                            f2.cancel(false);
                        }
