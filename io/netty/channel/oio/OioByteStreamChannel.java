...
extends AbstractOioByteChannel {
    ...
    @Override
    protected int doReadBytes(ByteBuf buf2) throws Exception {
        RecvByteBufAllocator.Handle allocHandle = this.unsafe().recvBufAllocHandle();
        allocHandle.attemptedBytesRead(Math.max(1, Math.min(this.available(), buf2.maxWritableBytes())));
        return buf2.writeBytes(this.is, allocHandle.attemptedBytesRead());
    }

    @Override
    protected void doWriteBytes(ByteBuf buf2) throws Exception {
        OutputStream os2 = this.os;
        if (os2 == null) {
            throw new NotYetConnectedException();
        }
        buf2.readBytes(os2, buf2.readableBytes());
    }
