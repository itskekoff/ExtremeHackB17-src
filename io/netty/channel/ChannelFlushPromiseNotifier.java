...
public final class ChannelFlushPromiseNotifier {
    ...
    public ChannelFlushPromiseNotifier add(ChannelPromise promise, long pendingDataSize) {
        ...
        if (promise instanceof FlushCheckpoint) {
            FlushCheckpoint cp2 = (FlushCheckpoint)((Object)promise);
            cp2.flushCheckpoint(checkpoint);
            this.flushCheckpoints.add(cp2);
        } else {
    ...
    public ChannelFlushPromiseNotifier notifyPromises(Throwable cause) {
        FlushCheckpoint cp2;
        this.notifyPromises();
        while ((cp2 = this.flushCheckpoints.poll()) != null) {
            if (this.tryNotify) {
                cp2.promise().tryFailure(cause);
                continue;
            }
            cp2.promise().setFailure(cause);
        }
        return this;
    }
    ...
    public ChannelFlushPromiseNotifier notifyPromises(Throwable cause1, Throwable cause2) {
        ...
        while ((cp2 = this.flushCheckpoints.poll()) != null) {
            if (this.tryNotify) {
                cp2.promise().tryFailure(cause2);
                continue;
            }
            cp2.promise().setFailure(cause2);
        }
        return this;
    }
    ...
    private void notifyPromises0(Throwable cause) {
        ...
        while (true) {
            FlushCheckpoint cp2;
            if ((cp2 = this.flushCheckpoints.peek()) == null) {
                this.writeCounter = 0L;
                break;
            }
            if (cp2.flushCheckpoint() > writeCounter) {
                if (writeCounter <= 0L || this.flushCheckpoints.size() != 1) break;
                this.writeCounter = 0L;
                cp2.flushCheckpoint(cp2.flushCheckpoint() - writeCounter);
                break;
            }
            this.flushCheckpoints.remove();
            ChannelPromise promise = cp2.promise();
            if (cause == null) {
                if (this.tryNotify) {
                    promise.trySuccess();
                    continue;
        ...
        if (newWriteCounter >= 0x8000000000L) {
            this.writeCounter = 0L;
            for (FlushCheckpoint cp3 : this.flushCheckpoints) {
                cp3.flushCheckpoint(cp3.flushCheckpoint() - newWriteCounter);
            }
        }
    }
