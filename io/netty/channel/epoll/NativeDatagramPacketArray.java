...
implements ChannelOutboundBuffer.MessageProcessor {
    ...
    private NativeDatagramPacketArray() {
        for (int i2 = 0; i2 < this.packets.length; ++i2) {
            this.packets[i2] = new NativeDatagramPacket();
        }
    }
    ...
    boolean add(DatagramPacket packet) {
        ...
        NativeDatagramPacket p2 = this.packets[this.count];
        InetSocketAddress recipient = (InetSocketAddress)packet.recipient();
        if (!p2.init(content, recipient)) {
            return false;
        }
        ++this.count;
        return true;
    ...
    static final class NativeDatagramPacket {
        ...
        private boolean init(ByteBuf buf2, InetSocketAddress recipient) {
            this.array.clear();
            if (!this.array.add(buf2)) {
                return false;
            }
