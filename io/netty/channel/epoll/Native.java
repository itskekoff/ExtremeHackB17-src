...
public final class Native {
    ...
    public static void epollCtlAdd(int efd, int fd2, int flags) throws IOException {
        int res = Native.epollCtlAdd0(efd, fd2, flags);
        if (res < 0) {
    ...
    public static void epollCtlMod(int efd, int fd2, int flags) throws IOException {
        int res = Native.epollCtlMod0(efd, fd2, flags);
        if (res < 0) {
            throw Errors.newIOException("epoll_ctl", res);
        }
    }
    ...
    public static void epollCtlDel(int efd, int fd2) throws IOException {
        int res = Native.epollCtlDel0(efd, fd2);
        if (res < 0) {
            throw Errors.newIOException("epoll_ctl", res);
        }
    }
    ...
    public static int splice(int fd2, long offIn, int fdOut, long offOut, long len) throws IOException {
        int res = Native.splice0(fd2, offIn, fdOut, offOut, len);
        if (res >= 0) {
            return res;
        }
    ...
    public static int sendmmsg(int fd2, NativeDatagramPacketArray.NativeDatagramPacket[] msgs, int offset, int len) throws IOException {
        int res = Native.sendmmsg0(fd2, msgs, offset, len);
        if (res >= 0) {
            return res;
        }
    ...
    public static int recvFd(int fd2) throws IOException {
        int res = Native.recvFd0(fd2);
        if (res > 0) {
            return res;
        }
        if (res == 0) {
    ...
    public static int sendFd(int socketFd, int fd2) throws IOException {
        int res = Native.sendFd0(socketFd, fd2);
        if (res >= 0) {
            return res;
        }
        if (res == Errors.ERRNO_EAGAIN_NEGATIVE || res == Errors.ERRNO_EWOULDBLOCK_NEGATIVE) {
    ...
    public static void tcpInfo(int fd2, EpollTcpInfo info) throws IOException {
        Native.tcpInfo0(fd2, info.info);
    }

    private static native void tcpInfo0(int var0, int[] var1) throws IOException;

    public static void setTcpMd5Sig(int fd2, InetAddress address, byte[] key) throws IOException {
        NativeInetAddress a2 = NativeInetAddress.newInstance(address);
        Native.setTcpMd5Sig0(fd2, a2.address(), a2.scopeId(), key);
    }
