...
implements DuplexChannel {
    ...
    @Deprecated
    protected AbstractEpollStreamChannel(Channel parent, int fd2) {
        this(parent, new Socket(fd2));
    }

    @Deprecated
    protected AbstractEpollStreamChannel(int fd2) {
        this(new Socket(fd2));
    }

    @Deprecated
    protected AbstractEpollStreamChannel(FileDescriptor fd2) {
        this(new Socket(fd2.intValue()));
    }

    @Deprecated
    protected AbstractEpollStreamChannel(Socket fd2) {
        this(fd2, AbstractEpollStreamChannel.isSoErrorZero(fd2));
    }

    protected AbstractEpollStreamChannel(Channel parent, Socket fd2) {
        super(parent, fd2, Native.EPOLLIN, true);
        this.flags |= Native.EPOLLRDHUP;
    }

    protected AbstractEpollStreamChannel(Socket fd2, boolean active) {
    ...
    public final ChannelFuture spliceTo(AbstractEpollStreamChannel ch2, int len) {
        return this.spliceTo(ch2, len, this.newPromise());
    }

    public final ChannelFuture spliceTo(AbstractEpollStreamChannel ch2, int len, ChannelPromise promise) {
        if (ch2.eventLoop() != this.eventLoop()) {
            throw new IllegalArgumentException("EventLoops are not the same.");
        }
        if (len < 0) {
            throw new IllegalArgumentException("len: " + len + " (expected: >= 0)");
        }
        if (ch2.config().getEpollMode() != EpollMode.LEVEL_TRIGGERED || this.config().getEpollMode() != EpollMode.LEVEL_TRIGGERED) {
        ...
        if (!this.isOpen()) {
            ...
            this.addToSpliceQueue(new SpliceInChannelTask(ch2, len, promise));
    ...
    public final ChannelFuture spliceTo(FileDescriptor ch2, int offset, int len) {
        return this.spliceTo(ch2, offset, len, this.newPromise());
    }
    ...
    public final ChannelFuture spliceTo(FileDescriptor ch2, int offset, int len, ChannelPromise promise) {
        ...
        if (!this.isOpen()) {
            ...
            this.addToSpliceQueue(new SpliceFdTask(ch2, offset, len, promise));
            this.failSpliceIfClosed(promise);
        }
        return promise;
    }
    ...
    private boolean writeBytes(ChannelOutboundBuffer in2, ByteBuf buf2, int writeSpinCount) throws Exception {
        int readableBytes = buf2.readableBytes();
        if (readableBytes == 0) {
            in2.remove();
            return true;
        }
        if (buf2.hasMemoryAddress() || buf2.nioBufferCount() == 1) {
            int writtenBytes = this.doWriteBytes(buf2, writeSpinCount);
            in2.removeBytes(writtenBytes);
            return writtenBytes == readableBytes;
        }
        ByteBuffer[] nioBuffers = buf2.nioBuffers();
        return this.writeBytesMultiple(in2, nioBuffers, nioBuffers.length, readableBytes, writeSpinCount);
    }

    private boolean writeBytesMultiple(ChannelOutboundBuffer in2, IovArray array, int writeSpinCount) throws IOException {
        ...
        int offset = 0;
        int end = offset + cnt;
        for (int i2 = writeSpinCount - 1; i2 >= 0 && (localWrittenBytes = this.fd().writevAddresses(array.memoryAddress(offset), cnt)) != 0L; --i2) {
            long bytes;
        ...
        in2.removeBytes(initialExpectedWrittenBytes - expectedWrittenBytes);
        return done;
    }

    private boolean writeBytesMultiple(ChannelOutboundBuffer in2, ByteBuffer[] nioBuffers, int nioBufferCnt, long expectedWrittenBytes, int writeSpinCount) throws IOException {
        long localWrittenBytes;
        assert (expectedWrittenBytes != 0L);
        ...
        block0: for (int i2 = writeSpinCount - 1; i2 >= 0 && (localWrittenBytes = this.fd().writev(nioBuffers, offset, nioBufferCnt)) != 0L; --i2) {
            int bytes;
            if ((expectedWrittenBytes -= localWrittenBytes) == 0L) {
                done = true;
                break;
    ...
    private boolean writeDefaultFileRegion(ChannelOutboundBuffer in2, DefaultFileRegion region, int writeSpinCount) throws Exception {
    ...
    private boolean writeFileRegion(ChannelOutboundBuffer in2, FileRegion region, int writeSpinCount) throws Exception {
        ...
        for (int i2 = writeSpinCount - 1; i2 >= 0 && (localFlushedAmount = region.transferTo(this.byteChannel, region.transferred())) != 0L; --i2) {
            flushedAmount += localFlushedAmount;
    ...
    protected void doWrite(ChannelOutboundBuffer in2) throws Exception {
        ...
        do {
            if ((msgCount = in2.size()) != 0) continue;
            this.clearFlag(Native.EPOLLOUT);
            return;
        } while (!(msgCount > 1 && in2.current() instanceof ByteBuf ? !this.doWriteMultiple(in2, writeSpinCount) : !this.doWriteSingle(in2, writeSpinCount)));
    ...
    protected boolean doWriteSingle(ChannelOutboundBuffer in2, int writeSpinCount) throws Exception {
        ...
        if (msg instanceof ByteBuf) {
            if (!this.writeBytes(in2, (ByteBuf)msg, writeSpinCount)) {
                return false;
            }
        } else if (msg instanceof DefaultFileRegion) {
            if (!this.writeDefaultFileRegion(in2, (DefaultFileRegion)msg, writeSpinCount)) {
                return false;
            }
        } else if (msg instanceof FileRegion) {
            if (!this.writeFileRegion(in2, (FileRegion)msg, writeSpinCount)) {
                return false;
            }
    ...
    private boolean doWriteMultiple(ChannelOutboundBuffer in2, int writeSpinCount) throws Exception {
        if (PlatformDependent.hasUnsafe()) {
            IovArray array = ((EpollEventLoop)this.eventLoop()).cleanArray();
            in2.forEachFlushedMessage(array);
            int cnt = array.count();
            if (cnt >= 1) {
                if (!this.writeBytesMultiple(in2, array, writeSpinCount)) {
                    return false;
                }
            } else {
                in2.removeBytes(0L);
            ...
            ByteBuffer[] buffers = in2.nioBuffers();
            int cnt = in2.nioBufferCount();
            if (cnt >= 1) {
                if (!this.writeBytesMultiple(in2, buffers, cnt, in2.nioBufferSize(), writeSpinCount)) {
                    return false;
                }
            } else {
                in2.removeBytes(0L);
    ...
    protected Object filterOutboundMessage(Object msg) {
        if (msg instanceof ByteBuf) {
            ...
            if (!(buf2.hasMemoryAddress() || !PlatformDependent.hasUnsafe() && buf2.isDirect())) {
                if (buf2 instanceof CompositeByteBuf) {
                    CompositeByteBuf comp = (CompositeByteBuf)buf2;
                    if (!comp.isDirect() || comp.nioBufferCount() > Native.IOV_MAX) {
                        buf2 = this.newDirectBuffer(buf2);
                        assert (buf2.hasMemoryAddress());
                    }
                } else {
                    buf2 = this.newDirectBuffer(buf2);
                    assert (buf2.hasMemoryAddress());
    ...
    private static void safeClosePipe(FileDescriptor fd2) {
        block3: {
            if (fd2 != null) {
                try {
                    fd2.close();
                    ...
                    logger.warn("Error while closing a pipe", e2);
                }
            }
        }
    }
    ...
    extends SpliceInTask {
        ...
        SpliceFdTask(FileDescriptor fd2, int offset, int len, ChannelPromise promise) {
            super(len, promise);
            this.fd = fd2;
    ...
    extends AbstractEpollChannel.AbstractEpollUnsafe {
        ...
        private void finishConnect() {
            ...
            try {
                ...
                this.fulfillConnectPromise(AbstractEpollStreamChannel.this.connectPromise, this.annotateConnectException(t2, AbstractEpollStreamChannel.this.requestedRemoteAddress));
            }
            finally {
        ...
        void epollInReady() {
            ...
            try {
                ...
                this.handleReadException(pipeline, byteBuf, t2, close, allocHandle);
            }
            finally {
