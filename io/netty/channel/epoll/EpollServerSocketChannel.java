...
implements ServerSocketChannel {
    ...
    @Deprecated
    public EpollServerSocketChannel(FileDescriptor fd2) {
        this(new Socket(fd2.intValue()));
    }

    @Deprecated
    public EpollServerSocketChannel(Socket fd2) {
        super(fd2);
        this.local = fd2.localAddress();
        this.config = new EpollServerSocketChannelConfig(this);
    }

    public EpollServerSocketChannel(Socket fd2, boolean active) {
        super(fd2, active);
        this.local = fd2.localAddress();
        this.config = new EpollServerSocketChannelConfig(this);
    }
    ...
    @Override
    protected Channel newChildChannel(int fd2, byte[] address, int offset, int len) throws Exception {
        return new EpollSocketChannel(this, new Socket(fd2), NativeInetAddress.address(address, offset, len));
    }
