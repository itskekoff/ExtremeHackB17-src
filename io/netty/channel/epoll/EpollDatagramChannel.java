...
implements DatagramChannel {
    ...
    @Deprecated
    public EpollDatagramChannel(FileDescriptor fd2) {
        this(new Socket(fd2.intValue()));
    }

    public EpollDatagramChannel(Socket fd2) {
        super(null, fd2, Native.EPOLLIN, true);
        this.local = fd2.localAddress();
        this.config = new EpollDatagramChannelConfig(this);
    }
    ...
    protected void doWrite(ChannelOutboundBuffer in2) throws Exception {
        block2: while (true) {
            ...
            try {
                ...
                int cnt;
                if (Native.IS_SUPPORTING_SENDMMSG && in2.size() > 1 && (cnt = (array = NativeDatagramPacketArray.getInstance(in2)).count()) >= 1) {
                    int offset = 0;
