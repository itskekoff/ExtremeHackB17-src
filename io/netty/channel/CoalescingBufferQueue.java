...
public final class CoalescingBufferQueue {
    ...
    public void add(ByteBuf buf2) {
        this.add(buf2, (ChannelFutureListener)null);
    }

    public void add(ByteBuf buf2, ChannelPromise promise) {
        ObjectUtil.checkNotNull(promise, "promise");
        this.add(buf2, promise.isVoid() ? null : new ChannelPromiseNotifier(promise));
    }

    public void add(ByteBuf buf2, ChannelFutureListener listener) {
        ObjectUtil.checkNotNull(buf2, "buf");
        if (this.readableBytes > Integer.MAX_VALUE - buf2.readableBytes()) {
            throw new IllegalStateException("buffer queue length overflow: " + this.readableBytes + " + " + buf2.readableBytes());
        }
        this.bufAndListenerPairs.add(buf2);
        if (listener != null) {
            this.bufAndListenerPairs.add(listener);
        }
        this.readableBytes += buf2.readableBytes();
    }
