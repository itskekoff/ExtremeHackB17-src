...
final class DnsQueryContext {
    ...
    private void setFailure(String message, Throwable cause) {
        ...
        StringBuilder buf2 = new StringBuilder(message.length() + 64);
        buf2.append('[').append(nameServerAddr).append("] ").append(message).append(" (no stack trace available)");
        DnsNameResolverException e2 = cause != null ? new DnsNameResolverException(nameServerAddr, this.question(), buf2.toString(), cause) : new DnsNameResolverException(nameServerAddr, this.question(), buf2.toString());
