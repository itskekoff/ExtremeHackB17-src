...
final class DnsQueryContextManager {
    ...
    int add(DnsQueryContext qCtx) {
        ...
        int id2 = PlatformDependent.threadLocalRandom().nextInt(65535) + 1;
        int maxTries = 131070;
        int tries = 0;
        IntObjectMap<DnsQueryContext> intObjectMap = contexts;
        synchronized (intObjectMap) {
            do {
                if (!contexts.containsKey(id2)) {
                    contexts.put(id2, qCtx);
                    return id2;
                }
                id2 = id2 + 1 & 0xFFFF;
            } while (++tries < 131070);
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    DnsQueryContext remove(InetSocketAddress nameServerAddr, int id2) {
        IntObjectMap<DnsQueryContext> contexts = this.getContextMap(nameServerAddr);
        if (contexts == null) {
            return null;
        }
    ...
    private IntObjectMap<DnsQueryContext> getOrCreateContextMap(InetSocketAddress nameServerAddr) {
        ...
        synchronized (map) {
            ...
            } else if (a2 instanceof Inet6Address) {
