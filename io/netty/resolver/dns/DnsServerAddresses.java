...
public abstract class DnsServerAddresses {
    ...
    private static InetSocketAddress[] sanitize(InetSocketAddress[] addresses) {
        ...
        for (InetSocketAddress a2 : addresses) {
            if (a2 == null) break;
            if (a2.isUnresolved()) {
                throw new IllegalArgumentException("cannot use an unresolved DNS server address: " + a2);
            }
            list.add(a2);
        }
        if (list.isEmpty()) {
    ...
    static {
        ...
        try {
            ...
            for (String a2 : list) {
                if (a2 == null) continue;
                defaultNameServers.add(new InetSocketAddress(SocketUtils.addressByName(a2), 53));
            }
        }
