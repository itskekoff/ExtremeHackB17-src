...
final class PoolThreadCache {
    ...
    private static <T> MemoryRegionCache<T>[] createSubPageCaches(int cacheSize, int numCaches, PoolArena.SizeClass sizeClass) {
        if (cacheSize > 0) {
            ...
            for (int i2 = 0; i2 < cache.length; ++i2) {
                cache[i2] = new SubPageMemoryRegionCache(cacheSize, sizeClass);
            }
            return cache;
        }
        return null;
    ...
    boolean allocateTiny(PoolArena<?> area, PooledByteBuf<?> buf2, int reqCapacity, int normCapacity) {
        return this.allocate(this.cacheForTiny(area, normCapacity), buf2, reqCapacity);
    }

    boolean allocateSmall(PoolArena<?> area, PooledByteBuf<?> buf2, int reqCapacity, int normCapacity) {
        return this.allocate(this.cacheForSmall(area, normCapacity), buf2, reqCapacity);
    }

    boolean allocateNormal(PoolArena<?> area, PooledByteBuf<?> buf2, int reqCapacity, int normCapacity) {
        return this.allocate(this.cacheForNormal(area, normCapacity), buf2, reqCapacity);
    }

    private boolean allocate(MemoryRegionCache<?> cache, PooledByteBuf buf2, int reqCapacity) {
        if (cache == null) {
            return false;
        }
        boolean allocated = cache.allocate(buf2, reqCapacity);
    ...
    private static abstract class MemoryRegionCache<T> {
        ...
        public final boolean allocate(PooledByteBuf<T> buf2, int reqCapacity) {
            Entry<T> entry = this.queue.poll();
            if (entry == null) {
                return false;
            }
            this.initBuf(entry.chunk, entry.handle, buf2, reqCapacity);
            entry.recycle();
            ++this.allocations;
            return true;
        }
    ...
    extends MemoryRegionCache<T> {
        ...
        @Override
        protected void initBuf(PoolChunk<T> chunk, long handle, PooledByteBuf<T> buf2, int reqCapacity) {
            chunk.initBuf(buf2, handle, reqCapacity);
        }
    }
    ...
    extends MemoryRegionCache<T> {
        ...
        @Override
        protected void initBuf(PoolChunk<T> chunk, long handle, PooledByteBuf<T> buf2, int reqCapacity) {
            chunk.initBufWithSubpage(buf2, handle, reqCapacity);
        }
    }
}
