...
extends WrappedByteBuf {
    ...
    private static ByteBuf unwrapSwapped(ByteBuf buf2) {
        if (buf2 instanceof SwappedByteBuf) {
            while ((buf2 = buf2.unwrap()) instanceof SwappedByteBuf) {
            }
            return buf2;
        }
        return buf2;
    ...
    protected SimpleLeakAwareByteBuf newLeakAwareByteBuf(ByteBuf buf2, ByteBuf trackedByteBuf, ResourceLeakTracker<ByteBuf> leakTracker) {
        return new SimpleLeakAwareByteBuf(buf2, trackedByteBuf, leakTracker);
    }
}
