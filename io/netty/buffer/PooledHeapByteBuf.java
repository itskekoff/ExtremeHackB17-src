...
extends PooledByteBuf<byte[]> {
    ...
    static PooledHeapByteBuf newInstance(int maxCapacity) {
        PooledHeapByteBuf buf2 = RECYCLER.get();
        buf2.reuse(maxCapacity);
        return buf2;
    }
    ...
    @Override
    public final int setBytes(int index, InputStream in2, int length) throws IOException {
        this.checkIndex(index, length);
        return in2.read((byte[])this.memory, this.idx(index), length);
    }

    @Override
    public final int setBytes(int index, ScatteringByteChannel in2, int length) throws IOException {
        this.checkIndex(index, length);
        index = this.idx(index);
        try {
            return in2.read((ByteBuffer)this.internalNioBuffer().clear().position(index).limit(index + length));
        }
    ...
    @Override
    public final int setBytes(int index, FileChannel in2, long position, int length) throws IOException {
        this.checkIndex(index, length);
        index = this.idx(index);
        try {
            return in2.read((ByteBuffer)this.internalNioBuffer().clear().position(index).limit(index + length), position);
        }
        catch (ClosedChannelException ignored) {
            return -1;
        }
    ...
    public final ByteBuffer nioBuffer(int index, int length) {
        this.checkIndex(index, length);
        index = this.idx(index);
        ByteBuffer buf2 = ByteBuffer.wrap((byte[])this.memory, index, length);
        return buf2.slice();
    }
