...
implements ByteBufAllocator {
    ...
    protected static ByteBuf toLeakAwareBuffer(ByteBuf buf2) {
        switch (ResourceLeakDetector.getLevel()) {
            case SIMPLE: {
                ResourceLeakTracker<ByteBuf> leak = AbstractByteBuf.leakDetector.track(buf2);
                if (leak == null) break;
                buf2 = new SimpleLeakAwareByteBuf(buf2, leak);
                break;
            }
            case ADVANCED: 
            case PARANOID: {
                ResourceLeakTracker<ByteBuf> leak = AbstractByteBuf.leakDetector.track(buf2);
                if (leak == null) break;
                buf2 = new AdvancedLeakAwareByteBuf(buf2, leak);
                break;
            }
        }
        return buf2;
    ...
    protected static CompositeByteBuf toLeakAwareBuffer(CompositeByteBuf buf2) {
        switch (ResourceLeakDetector.getLevel()) {
            case SIMPLE: {
                ResourceLeakTracker<ByteBuf> leak = AbstractByteBuf.leakDetector.track(buf2);
                if (leak == null) break;
                buf2 = new SimpleLeakAwareCompositeByteBuf(buf2, leak);
                break;
            }
            case ADVANCED: 
            case PARANOID: {
                ResourceLeakTracker<ByteBuf> leak = AbstractByteBuf.leakDetector.track(buf2);
                if (leak == null) break;
                buf2 = new AdvancedLeakAwareCompositeByteBuf(buf2, leak);
                break;
            }
        }
        return buf2;
