...
final class UnsafeByteBufUtil {
    ...
    static short getShort(long address) {
        if (UNALIGNED) {
            short v2 = PlatformDependent.getShort(address);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? v2 : Short.reverseBytes(v2);
        }
    ...
    static short getShortLE(long address) {
        if (UNALIGNED) {
            short v2 = PlatformDependent.getShort(address);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? Short.reverseBytes(v2) : v2;
        }
    ...
    static int getInt(long address) {
        if (UNALIGNED) {
            int v2 = PlatformDependent.getInt(address);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? v2 : Integer.reverseBytes(v2);
        }
    ...
    static int getIntLE(long address) {
        if (UNALIGNED) {
            int v2 = PlatformDependent.getInt(address);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? Integer.reverseBytes(v2) : v2;
        }
    ...
    static long getLong(long address) {
        if (UNALIGNED) {
            long v2 = PlatformDependent.getLong(address);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? v2 : Long.reverseBytes(v2);
        }
    ...
    static long getLongLE(long address) {
        if (UNALIGNED) {
            long v2 = PlatformDependent.getLong(address);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? Long.reverseBytes(v2) : v2;
        }
    ...
    static short getShort(byte[] array, int index) {
        if (UNALIGNED) {
            short v2 = PlatformDependent.getShort(array, index);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? v2 : Short.reverseBytes(v2);
        }
    ...
    static int getInt(byte[] array, int index) {
        if (UNALIGNED) {
            int v2 = PlatformDependent.getInt(array, index);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? v2 : Integer.reverseBytes(v2);
        }
    ...
    static long getLong(byte[] array, int index) {
        if (UNALIGNED) {
            long v2 = PlatformDependent.getLong(array, index);
            return PlatformDependent.BIG_ENDIAN_NATIVE_ORDER ? v2 : Long.reverseBytes(v2);
        }
    ...
    static ByteBuf copy(AbstractByteBuf buf2, long addr, int index, int length) {
        buf2.checkIndex(index, length);
        ByteBuf copy = buf2.alloc().directBuffer(length, buf2.maxCapacity());
        if (length != 0) {
            if (copy.hasMemoryAddress()) {
                PlatformDependent.copyMemory(addr, copy.memoryAddress(), length);
                copy.setIndex(0, length);
            } else {
                copy.writeBytes(buf2, index, length);
            }
        }
        return copy;
    }
     ...
     */
    static int setBytes(AbstractByteBuf buf2, long addr, int index, InputStream in2, int length) throws IOException {
        buf2.checkIndex(index, length);
        ByteBuf tmpBuf = buf2.alloc().heapBuffer(length);
        try {
            byte[] tmp = tmpBuf.array();
            int offset = tmpBuf.arrayOffset();
            int readBytes = in2.read(tmp, offset, length);
            if (readBytes > 0) {
    ...
    static void getBytes(AbstractByteBuf buf2, long addr, int index, ByteBuf dst, int dstIndex, int length) {
        buf2.checkIndex(index, length);
        ...
        if (dst.hasMemoryAddress()) {
            ...
            dst.setBytes(dstIndex, buf2, index, length);
        }
    }

    static void getBytes(AbstractByteBuf buf2, long addr, int index, byte[] dst, int dstIndex, int length) {
        buf2.checkIndex(index, length);
        ObjectUtil.checkNotNull(dst, "dst");
    ...
    static void getBytes(AbstractByteBuf buf2, long addr, int index, ByteBuffer dst) {
        buf2.checkIndex(index, dst.remaining());
        if (dst.remaining() == 0) {
            return;
        }
        if (dst.isDirect()) {
            ...
            dst.put(buf2.nioBuffer());
        }
    }

    static void setBytes(AbstractByteBuf buf2, long addr, int index, ByteBuf src, int srcIndex, int length) {
        buf2.checkIndex(index, length);
        ...
        if (length != 0) {
            if (src.hasMemoryAddress()) {
                ...
                src.getBytes(srcIndex, buf2, index, length);
            }
        }
    }

    static void setBytes(AbstractByteBuf buf2, long addr, int index, byte[] src, int srcIndex, int length) {
        buf2.checkIndex(index, length);
        if (length != 0) {
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    static void setBytes(AbstractByteBuf buf2, long addr, int index, ByteBuffer src) {
        buf2.checkIndex(index, src.remaining());
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    static void getBytes(AbstractByteBuf buf2, long addr, int index, OutputStream out, int length) throws IOException {
        buf2.checkIndex(index, length);
        if (length != 0) {
            ByteBuf tmpBuf = buf2.alloc().heapBuffer(length);
            try {
    ...
    static void setZero(AbstractByteBuf buf2, long addr, int index, int length) {
        if (length == 0) {
            return;
        }
        buf2.checkIndex(index, length);
