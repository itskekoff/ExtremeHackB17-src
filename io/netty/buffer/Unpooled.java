...
public final class Unpooled {
        ...
        switch (buffers.length) {
            ...
            default: {
                for (int i2 = 0; i2 < buffers.length; ++i2) {
                    ByteBuf buf2 = buffers[i2];
                    if (buf2.isReadable()) {
                        return new CompositeByteBuf(ALLOC, false, maxNumComponents, buffers, i2, buffers.length);
                    }
                    buf2.release();
                }
            }
    ...
    public static ByteBuf copiedBuffer(CharSequence string, int offset, int length, Charset charset) {
        ...
        if (string instanceof CharBuffer) {
            CharBuffer buf2 = (CharBuffer)string;
            if (buf2.hasArray()) {
                return Unpooled.copiedBuffer(buf2.array(), buf2.arrayOffset() + buf2.position() + offset, length, charset);
            }
            buf2 = buf2.slice();
            buf2.limit(length);
            buf2.position(offset);
            return Unpooled.copiedBuffer(buf2, charset);
        }
    ...
    public static ByteBuf copyShort(int value) {
        ByteBuf buf2 = Unpooled.buffer(2);
        buf2.writeShort(value);
        return buf2;
    }
    ...
    public static ByteBuf unreleasableBuffer(ByteBuf buf2) {
        return new UnreleasableByteBuf(buf2);
    }
