...
extends AbstractReferenceCountedByteBuf {
    ...
    public ByteBuf getBytes(int index, ByteBuf dst, int dstIndex, int length) {
        ...
        if (dst.hasArray()) {
            ...
            for (ByteBuffer bb2 : dst.nioBuffers(dstIndex, length)) {
                int bbLen = bb2.remaining();
                this.getBytes(index, bb2);
                index += bbLen;
            }
        } else {
    ...
    public ByteBuf setBytes(int index, ByteBuf src, int srcIndex, int length) {
        ...
        if (src.nioBufferCount() > 0) {
            for (ByteBuffer bb2 : src.nioBuffers(srcIndex, length)) {
                int bbLen = bb2.remaining();
                this.setBytes(index, bb2);
                index += bbLen;
            }
        } else {
    ...
    public int setBytes(int index, InputStream in2, int length) throws IOException {
        this.ensureAccessible();
        if (this.buffer.hasArray()) {
            return in2.read(this.buffer.array(), this.buffer.arrayOffset() + index, length);
        }
        byte[] tmp = new byte[length];
    ...
    public int setBytes(int index, ScatteringByteChannel in2, int length) throws IOException {
        ...
        tmpBuf.clear().position(index).limit(index + length);
        try {
            return in2.read(this.tmpNioBuf);
        }
    ...
    public int setBytes(int index, FileChannel in2, long position, int length) throws IOException {
        ...
        tmpBuf.clear().position(index).limit(index + length);
        try {
            return in2.read(this.tmpNioBuf, position);
        }
        catch (ClosedChannelException ignored) {
            return -1;
        }
