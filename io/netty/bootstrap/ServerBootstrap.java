...
extends AbstractBootstrap<ServerBootstrap, ServerChannel> {
    ...
    void init(Channel channel) throws Exception {
        ...
        synchronized (map2) {
            for (Map.Entry<AttributeKey<?>, Object> e2 : attrs.entrySet()) {
                AttributeKey<?> key = e2.getKey();
                channel.attr(key).set(e2.getValue());
            }
        }
        ...
        p2.addLast(new ChannelInitializer<Channel>(){
            ...
            @Override
            public void initChannel(final Channel ch2) throws Exception {
                final ChannelPipeline pipeline = ch2.pipeline();
                ChannelHandler handler = ServerBootstrap.this.config.handler();
                if (handler != null) {
                    pipeline.addLast(handler);
                }
                ch2.eventLoop().execute(new Runnable(){
    ...
    extends ChannelInboundHandlerAdapter {
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) {
            ...
            for (Map.Entry<AttributeKey<?>, Object> e2 : this.childAttrs) {
            ...
            try {
                ...
                ServerBootstrapAcceptor.forceClose(child, t2);
            }
        }

        private static void forceClose(Channel child, Throwable t2) {
            child.unsafe().closeForcibly();
            logger.warn("Failed to register an accepted channel: " + child, t2);
        }
