...
implements Cloneable {
    ...
    final ChannelFuture initAndRegister() {
        ...
        try {
            ...
            return new DefaultChannelPromise(channel, GlobalEventExecutor.INSTANCE).setFailure(t2);
        }
    ...
    static void setChannelOptions(Channel channel, Map<ChannelOption<?>, Object> options, InternalLogger logger) {
        for (Map.Entry<ChannelOption<?>, Object> e2 : options.entrySet()) {
            AbstractBootstrap.setChannelOption(channel, e2.getKey(), e2.getValue(), logger);
        }
    }

    static void setChannelOptions(Channel channel, Map.Entry<ChannelOption<?>, Object>[] options, InternalLogger logger) {
        for (Map.Entry<ChannelOption<?>, Object> e2 : options) {
            AbstractBootstrap.setChannelOption(channel, e2.getKey(), e2.getValue(), logger);
        }
    }
    ...
    private static void setChannelOption(Channel channel, ChannelOption<?> option, Object value, InternalLogger logger) {
        try {
            ...
            logger.warn("Failed to set channel option '{}' with value '{}' for channel '{}'", option, channel, channel, t2);
        }
    }

    public String toString() {
        StringBuilder buf2 = new StringBuilder().append(StringUtil.simpleClassName(this)).append('(').append(this.config()).append(')');
