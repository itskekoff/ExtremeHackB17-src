...
extends ChannelDuplexHandler {
    ...
    protected String format(ChannelHandlerContext ctx, String eventName, Object arg2) {
        if (arg2 instanceof ByteBuf) {
            return LoggingHandler.formatByteBuf(ctx, eventName, (ByteBuf)arg2);
        }
        if (arg2 instanceof ByteBufHolder) {
            return LoggingHandler.formatByteBufHolder(ctx, eventName, (ByteBufHolder)arg2);
        }
        return LoggingHandler.formatSimple(ctx, eventName, arg2);
    }
    ...
    protected String format(ChannelHandlerContext ctx, String eventName, Object firstArg, Object secondArg) {
        ...
        StringBuilder buf2 = new StringBuilder(chStr.length() + 1 + eventName + 2 + arg1Str.length() + 2 + arg2Str.length());
    ...
    private static String formatByteBuf(ChannelHandlerContext ctx, String eventName, ByteBuf msg) {
        ...
        StringBuilder buf3 = new StringBuilder(chStr.length() + 1 + eventName.length() + 2 + 10 + 1 + 2 + rows * 80);
        buf3.append(chStr).append(' ').append(eventName).append(": ").append(length).append('B').append(StringUtil.NEWLINE);
    ...
    private static String formatByteBufHolder(ChannelHandlerContext ctx, String eventName, ByteBufHolder msg) {
        ...
        int rows = length / 16 + (length % 15 == 0 ? 0 : 1) + 4;
        StringBuilder buf3 = new StringBuilder(chStr.length() + 1 + eventName.length() + 2 + msgStr.length() + 2 + 10 + 1 + 2 + rows * 80);
        buf3.append(chStr).append(' ').append(eventName).append(": ").append(msgStr).append(", ").append(length).append('B').append(StringUtil.NEWLINE);
        ByteBufUtil.appendPrettyHexDump(buf3, content);
        return buf3.toString();
    }
