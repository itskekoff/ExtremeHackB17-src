...
extends ChannelDuplexHandler {
    ...
    private boolean safeRemoveDecoder() {
        try {
            ...
            logger.warn("Failed to remove proxy decoders:", e2);
            return false;
        }
    }
    ...
    private boolean safeRemoveEncoder() {
        try {
            ...
            logger.warn("Failed to remove proxy encoders:", e2);
            return false;
        }
    }
    ...
    protected final String exceptionMessage(String msg) {
        ...
        StringBuilder buf2 = new StringBuilder(128 + msg.length()).append(this.protocol()).append(", ").append(this.authScheme()).append(", ").append(this.proxyAddress).append(" => ").append(this.destinationAddress);
        if (!msg.isEmpty()) {
            buf2.append(", ").append(msg);
        }
