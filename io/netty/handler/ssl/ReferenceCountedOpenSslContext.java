...
implements ReferenceCounted {
    ...
    ReferenceCountedOpenSslContext(Iterable<String> ciphers, CipherSuiteFilter cipherFilter, OpenSslApplicationProtocolNegotiator apn2, long sessionCacheSize, long sessionTimeout, int mode, Certificate[] keyCertChain, ClientAuth clientAuth, String[] protocols, boolean startTls, boolean leakDetection) throws SSLException {
        ...
        Certificate[] arrcertificate = this.keyCertChain = keyCertChain == null ? null : (Certificate[])keyCertChain.clone();
        if (ciphers == null) {
        ...
        this.apn = ObjectUtil.checkNotNull(apn2, "apn");
        ...
        try {
            ...
            synchronized (ReferenceCountedOpenSslContext.class) {
                ...
                List<String> nextProtoList = apn2.protocols();
                if (!nextProtoList.isEmpty()) {
                    String[] appProtocols = nextProtoList.toArray(new String[nextProtoList.size()]);
                    int selectorBehavior = ReferenceCountedOpenSslContext.opensslSelectorFailureBehavior(apn2.selectorFailureBehavior());
                    switch (apn2.protocol()) {
                        case NPN: {
    ...
    protected static X509Certificate[] certificates(byte[][] chain) {
        ...
        for (int i2 = 0; i2 < peerCerts.length; ++i2) {
            peerCerts[i2] = new OpenSslX509Certificate(chain[i2]);
        }
        return peerCerts;
    }

    protected static X509TrustManager chooseTrustManager(TrustManager[] managers) {
        for (TrustManager m2 : managers) {
            if (!(m2 instanceof X509TrustManager)) continue;
    ...
    protected static X509KeyManager chooseX509KeyManager(KeyManager[] kms) {
        for (KeyManager km2 : kms) {
            if (!(km2 instanceof X509KeyManager)) continue;
            return (X509KeyManager)km2;
        }
    ...
    static long toBIO(PrivateKey key) throws Exception {
        ...
        try {
            long l2 = ReferenceCountedOpenSslContext.toBIO(allocator, pem.retain());
            return l2;
        }
        finally {
            pem.release();
    ...
    static long toBIO(ByteBufAllocator allocator, PemEncoded pem) throws Exception {
        try {
            ...
            if (content.isDirect()) {
                long l3 = ReferenceCountedOpenSslContext.newBIO(content.retainedSlice());
                return l3;
            }
            ByteBuf buffer = allocator.directBuffer(content.readableBytes());
            try {
                buffer.writeBytes(content, content.readerIndex(), content.readableBytes());
                l2 = ReferenceCountedOpenSslContext.newBIO(buffer.retainedSlice());
            }
    ...
    implements CertificateVerifier {
        ...
        public final int verify(long ssl, byte[][] chain, String auth) {
            ...
            try {
                ...
                SSLHandshakeException e2 = new SSLHandshakeException("General OpenSslEngine problem");
                e2.initCause(cause);
                engine.handshakeException = e2;
                ...
                if (PlatformDependent.javaVersion() >= 7) {
                    ...
                    for (Throwable wrapped = cause.getCause(); wrapped != null; wrapped = wrapped.getCause()) {
                        ...
                        CertPathValidatorException ex2 = (CertPathValidatorException)wrapped;
                        CertPathValidatorException.Reason reason = ex2.getReason();
