...
extends SslContext {
    ...
    private static void addIfSupported(Set<String> supported, List<String> enabled, String ... names) {
        for (String n2 : names) {
            if (!supported.contains(n2)) continue;
            enabled.add(n2);
        }
    }
    ...
    public JdkSslContext(SSLContext sslContext, boolean isClient, Iterable<String> ciphers, CipherSuiteFilter cipherFilter, ApplicationProtocolConfig apn2, ClientAuth clientAuth) {
        this(sslContext, isClient, ciphers, cipherFilter, JdkSslContext.toNegotiator(apn2, !isClient), clientAuth, null, false);
    }

    JdkSslContext(SSLContext sslContext, boolean isClient, Iterable<String> ciphers, CipherSuiteFilter cipherFilter, JdkApplicationProtocolNegotiator apn2, ClientAuth clientAuth, String[] protocols, boolean startTls) {
        super(startTls);
    ...
    static {
        ...
        for (i2 = 0; i2 < supportedProtocols.length; ++i2) {
            supportedProtocolsSet.add(supportedProtocols[i2]);
        }
        ...
        for (i2 = 0; i2 < supportedCiphers.length; ++i2) {
            SUPPORTED_CIPHERS.add(supportedCiphers[i2]);
        }
