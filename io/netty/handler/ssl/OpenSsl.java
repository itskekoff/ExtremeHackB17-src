...
public final class OpenSsl {
    ...
    static long memoryAddress(ByteBuf buf2) {
        assert (buf2.isDirect());
        return buf2.hasMemoryAddress() ? buf2.memoryAddress() : Buffer.address(buf2.nioBuffer());
    }
    ...
    private static void loadTcNative() throws Exception {
        String os2 = OpenSsl.normalizeOs(SystemPropertyUtil.get("os.name", ""));
        String arch = OpenSsl.normalizeArch(SystemPropertyUtil.get("os.arch", ""));
        LinkedHashSet<String> libNames = new LinkedHashSet<String>(3);
        libNames.add("netty-tcnative-" + os2 + '-' + arch);
        if (LINUX.equalsIgnoreCase(os2)) {
            libNames.add("netty-tcnative-" + os2 + '-' + arch + "-fedora");
        }
    ...
    static {
        ...
        if (cause == null) {
            try {
                ...
                cause = t3;
                logger.debug("Failed to load netty-tcnative; " + OpenSslEngine.class.getSimpleName() + " will be unavailable, unless the application has already loaded the symbols by some other means. See http://netty.io/wiki/forked-tomcat-native.html for more information.", t3);
            }
            try {
                ...
                logger.debug("Failed to initialize netty-tcnative; " + OpenSslEngine.class.getSimpleName() + " will be unavailable. See http://netty.io/wiki/forked-tomcat-native.html for more information.", t4);
            }
        }
        ...
        if (cause == null) {
            ...
            try {
                ...
                try {
                    ...
                    try {
                        for (String c2 : SSL.getCiphers(ssl)) {
                            if (c2 == null || c2.isEmpty() || availableOpenSslCipherSuites.contains(c2)) continue;
                            availableOpenSslCipherSuites.add(c2);
                        }
                        try {
                ...
                logger.warn("Failed to get the list of available OpenSSL cipher suites.", e2);
            }
