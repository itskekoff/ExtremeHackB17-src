...
lbl60:
    ...
    private StompCommand readCommand(ByteBuf in2) {
        String commandStr = StompSubframeDecoder.readLine(in2, this.maxLineLength);
    ...
    private static void skipNullCharacter(ByteBuf buffer) {
        ...
        if (b2 != 0) {
            throw new IllegalStateException("unexpected byte in buffer " + b2 + " while expecting NULL byte");
        }
    }

    private static void skipControlCharacters(ByteBuf buffer) {
        byte b2;
        while ((b2 = buffer.readByte()) == 13 || b2 == 10) {
        }
        buffer.readerIndex(buffer.readerIndex() - 1);
    }

    private static String readLine(ByteBuf buffer, int maxLineLength) {
        AppendableCharSequence buf2 = new AppendableCharSequence(128);
        int lineLength = 0;
        while (true) {
            byte nextByte;
