...
public final class HttpUtil {
    ...
    public static void setKeepAlive(HttpHeaders h2, HttpVersion httpVersion, boolean keepAlive) {
        if (httpVersion.isKeepAliveDefault()) {
            if (keepAlive) {
                h2.remove(HttpHeaderNames.CONNECTION);
            } else {
                h2.set((CharSequence)HttpHeaderNames.CONNECTION, (Object)HttpHeaderValues.CLOSE);
            }
        } else if (keepAlive) {
            h2.set((CharSequence)HttpHeaderNames.CONNECTION, (Object)HttpHeaderValues.KEEP_ALIVE);
        } else {
            h2.remove(HttpHeaderNames.CONNECTION);
        }
    }
    ...
    public static boolean isContentLengthSet(HttpMessage m2) {
        return m2.headers().contains(HttpHeaderNames.CONTENT_LENGTH);
    }
    ...
    public static void setTransferEncodingChunked(HttpMessage m2, boolean chunked) {
        if (chunked) {
            m2.headers().set((CharSequence)HttpHeaderNames.TRANSFER_ENCODING, (Object)HttpHeaderValues.CHUNKED);
            m2.headers().remove(HttpHeaderNames.CONTENT_LENGTH);
        } else {
            List<String> encodings = m2.headers().getAll(HttpHeaderNames.TRANSFER_ENCODING);
            ...
            if (values.isEmpty()) {
                m2.headers().remove(HttpHeaderNames.TRANSFER_ENCODING);
            } else {
                m2.headers().set((CharSequence)HttpHeaderNames.TRANSFER_ENCODING, values);
            }
        }
    }
    ...
    static void encodeAscii0(CharSequence seq, ByteBuf buf2) {
        int length = seq.length();
        for (int i2 = 0; i2 < length; ++i2) {
            buf2.writeByte(HttpUtil.c2b(seq.charAt(i2)));
        }
    }

    private static byte c2b(char c2) {
        return (byte)(c2 > '\u00ff' ? 63 : (byte)c2);
    }
}
