...
implements InterfaceHttpPostRequestDecoder {
    ...
    public HttpPostMultipartRequestDecoder offer(HttpContent content) {
        this.checkDestroyed();
        ByteBuf buf2 = content.content();
        if (this.undecodedChunk == null) {
            this.undecodedChunk = buf2.copy();
        } else {
            this.undecodedChunk.writeBytes(buf2);
        }
        ...
        while (!this.skipOneLine()) {
            ...
            if (HttpHeaderNames.CONTENT_DISPOSITION.contentEqualsIgnoreCase(contents[0])) {
                ...
                if (this.currentStatus == HttpPostRequestDecoder.MultiPartStatus.DISPOSITION) {
                    ...
                    boolean bl2 = checkSecondArg = HttpHeaderValues.ATTACHMENT.contentEqualsIgnoreCase(contents[1]) || HttpHeaderValues.FILE.contentEqualsIgnoreCase(contents[1]);
                }
                if (!checkSecondArg) continue;
                for (int i2 = 2; i2 < contents.length; ++i2) {
                    Attribute attribute;
                    String[] values = contents[i2].split("=", 2);
                    try {
            ...
            if (HttpHeaderNames.CONTENT_TYPE.contentEqualsIgnoreCase(contents[0])) {
                ...
                for (int i3 = 1; i3 < contents.length; ++i3) {
                    Attribute attribute;
                    if (contents[i3].toLowerCase().startsWith(HttpHeaderValues.CHARSET.toString())) {
                        Attribute attribute2;
                        String values = StringUtil.substringAfter(contents[i3], '=');
                        try {
                    ...
                    try {
                        attribute = this.factory.createAttribute(this.request, HttpPostMultipartRequestDecoder.cleanString(contents[0]), contents[i3]);
                    }
    ...
    private static String[] splitMultipartHeader(String sb2) {
        ...
        for (nameEnd = nameStart = HttpPostBodyUtil.findNonWhitespace(sb2, 0); nameEnd < sb2.length() && (ch2 = sb2.charAt(nameEnd)) != ':' && !Character.isWhitespace(ch2); ++nameEnd) {
        }
    ...
    private static String[] splitMultipartHeaderValues(String svalue) {
        ...
        for (int i2 = 0; i2 < svalue.length(); ++i2) {
            ...
            if (c2 != ';') continue;
            values.add(svalue.substring(start, i2));
            start = i2 + 1;
        }
