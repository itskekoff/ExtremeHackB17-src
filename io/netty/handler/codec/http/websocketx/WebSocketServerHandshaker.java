...
public abstract class WebSocketServerHandshaker {
    ...
    public final ChannelFuture handshake(final Channel channel, HttpRequest req, final HttpHeaders responseHeaders, final ChannelPromise promise) {
        ...
        if ((ctx = (p2 = channel.pipeline()).context(HttpRequestDecoder.class)) == null && (ctx = p2.context(HttpServerCodec.class)) == null) {
            promise.setFailure(new IllegalStateException("No HttpDecoder and no HttpServerCodec in the pipeline"));
            return promise;
        }
        String aggregatorName = "httpAggregator";
        p2.addAfter(ctx.name(), aggregatorName, new HttpObjectAggregator(8192));
        p2.addAfter(aggregatorName, "handshaker", new SimpleChannelInboundHandler<FullHttpRequest>(){
    ...
    protected String selectSubprotocol(String requestedSubprotocols) {
        ...
        for (String p2 : requestedSubprotocolArray = requestedSubprotocols.split(",")) {
