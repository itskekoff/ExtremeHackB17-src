...
extends DefaultHttpHeaders {
    ...
    extends DefaultHeaders<CharSequence, CharSequence, CombinedHttpHeadersImpl> {
        ...
        private static <T> CharSequence commaSeparate(CsvValueEscaper<T> escaper, T ... values) {
            StringBuilder sb2 = new StringBuilder(values.length * 10);
            if (values.length > 0) {
                int end = values.length - 1;
                for (int i2 = 0; i2 < end; ++i2) {
                    sb2.append(escaper.escape(values[i2])).append(',');
                }
                sb2.append(escaper.escape(values[end]));
            }
            return sb2;
        }

        private static <T> CharSequence commaSeparate(CsvValueEscaper<T> escaper, Iterable<? extends T> values) {
            StringBuilder sb2 = values instanceof Collection ? new StringBuilder(((Collection)values).size() * 10) : new StringBuilder();
            Iterator<T> iterator = values.iterator();
            if (iterator.hasNext()) {
                T next = iterator.next();
                while (iterator.hasNext()) {
                    sb2.append(escaper.escape(next)).append(',');
                    next = iterator.next();
                }
                sb2.append(escaper.escape(next));
            }
            return sb2;
        }
