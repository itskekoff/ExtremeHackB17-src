...
implements Iterable<Map.Entry<String, String>> {
    ...
    public static void removeTransferEncodingChunked(HttpMessage m2) {
        HttpUtil.setTransferEncodingChunked(m2, false);
    }

    @Deprecated
    public static void setTransferEncodingChunked(HttpMessage m2) {
        HttpUtil.setTransferEncodingChunked(m2, true);
    }

    @Deprecated
    public static boolean isContentLengthSet(HttpMessage m2) {
        return HttpUtil.isContentLengthSet(m2);
    }
    ...
    static void encode(HttpHeaders headers, ByteBuf buf2) throws Exception {
    ...
    public static void encodeAscii(CharSequence seq, ByteBuf buf2) {
        if (seq instanceof AsciiString) {
            ByteBufUtil.copy((AsciiString)seq, 0, buf2, seq.length());
        } else {
            HttpUtil.encodeAscii0(seq, buf2);
        }
    }
    ...
    public HttpHeaders add(HttpHeaders headers) {
        ...
        for (Map.Entry<String, String> e2 : headers) {
            this.add(e2.getKey(), (Object)e2.getValue());
        }
        return this;
    }
    ...
    public boolean contains(String name, String value, boolean ignoreCase) {
        ...
        for (String v2 : values) {
            if (!(ignoreCase ? v2.equalsIgnoreCase(value) : v2.equals(value))) continue;
            return true;
        }
        return false;
    }
    ...
    public boolean containsValue(CharSequence name, CharSequence value, boolean ignoreCase) {
        ...
        for (String v2 : values) {
            if (!HttpHeaders.contains(v2, value, ignoreCase)) continue;
            return true;
        }
        return false;
    }
    ...
    private static boolean contains(String value, CharSequence expected, boolean ignoreCase) {
        ...
        if (ignoreCase) {
            for (String s2 : parts) {
                if (!AsciiString.contentEqualsIgnoreCase(expected, s2.trim())) continue;
                return true;
            }
        } else {
            for (String s3 : parts) {
                if (!AsciiString.contentEquals(expected, s3.trim())) continue;
                return true;
            }
        }
        return false;
