...
public final class DateFormatter {
    ...
    public static StringBuilder append(Date date, StringBuilder sb2) {
        return DateFormatter.formatter().append0(ObjectUtil.checkNotNull(date, "date"), ObjectUtil.checkNotNull(sb2, "sb"));
    }
    ...
    private boolean tryParseTime(CharSequence txt, int tokenStart, int tokenEnd) {
        ...
        int numDigits = 0;
        for (int i2 = tokenStart; i2 < tokenEnd; ++i2) {
            char c2 = txt.charAt(i2);
            if (DateFormatter.isDigit(c2)) {
                currentPartValue = currentPartValue * 10 + DateFormatter.getNumericalValue(c2);
    ...
    private boolean parse1(CharSequence txt, int start, int end) {
        ...
        for (int i2 = start; i2 < end; ++i2) {
            ...
            if (DateFormatter.isDelim(c2)) {
                ...
                if (this.parseToken(txt, tokenStart, i2)) {
                    return true;
                }
                tokenStart = -1;
                continue;
    ...
    private StringBuilder append0(Date date, StringBuilder sb2) {
        ...
        sb2.append(DAY_OF_WEEK_TO_SHORT_NAME[this.cal.get(7) - 1]).append(", ");
        sb2.append(this.cal.get(5)).append(' ');
        sb2.append(CALENDAR_MONTH_TO_SHORT_NAME[this.cal.get(2)]).append(' ');
        sb2.append(this.cal.get(1)).append(' ');
        DateFormatter.appendZeroLeftPadded(this.cal.get(11), sb2).append(':');
        DateFormatter.appendZeroLeftPadded(this.cal.get(12), sb2).append(':');
        return DateFormatter.appendZeroLeftPadded(this.cal.get(13), sb2).append(" GMT");
    }

    private static StringBuilder appendZeroLeftPadded(int value, StringBuilder sb2) {
        if (value < 10) {
            sb2.append('0');
        }
        return sb2.append(value);
    }
    ...
    static {
        ...
        DELIMITERS.set(9);
        for (c2 = 32; c2 <= 47; c2 = (int)((char)(c2 + 1))) {
            DELIMITERS.set(c2);
        }
        for (c2 = 59; c2 <= 64; c2 = (int)((char)(c2 + 1))) {
            DELIMITERS.set(c2);
        }
        for (c2 = 91; c2 <= 96; c2 = (int)((char)(c2 + 1))) {
            DELIMITERS.set(c2);
        }
        for (c2 = 123; c2 <= 126; c2 = (int)((char)(c2 + 1))) {
            DELIMITERS.set(c2);
        }
