...
extends MessageToByteEncoder<ByteBuf> {
    ...
    public LzmaFrameEncoder(int lc2, int lp2, int pb2) {
        this(lc2, lp2, pb2, 65536);
    }
    ...
    public LzmaFrameEncoder(int lc2, int lp2, int pb2, int dictionarySize) {
        this(lc2, lp2, pb2, dictionarySize, false, 32);
    }

    public LzmaFrameEncoder(int lc2, int lp2, int pb2, int dictionarySize, boolean endMarkerMode, int numFastBytes) {
        if (lc2 < 0 || lc2 > 8) {
            throw new IllegalArgumentException("lc: " + lc2 + " (expected: 0-8)");
        }
        if (lp2 < 0 || lp2 > 4) {
            throw new IllegalArgumentException("lp: " + lp2 + " (expected: 0-4)");
        }
        if (pb2 < 0 || pb2 > 4) {
            throw new IllegalArgumentException("pb: " + pb2 + " (expected: 0-4)");
        }
        if (lc2 + lp2 > 4 && !warningLogged) {
        ...
        this.encoder.setLcLpPb(lc2, lp2, pb2);
        this.properties = (byte)((pb2 * 5 + lp2) * 9 + lc2);
    ...
    protected void encode(ChannelHandlerContext ctx, ByteBuf in2, ByteBuf out) throws Exception {
        ...
        try {
            bbIn = new ByteBufInputStream(in2);
    ...
    @Override
    protected ByteBuf allocateBuffer(ChannelHandlerContext ctx, ByteBuf in2, boolean preferDirect) throws Exception {
        int length = in2.readableBytes();
