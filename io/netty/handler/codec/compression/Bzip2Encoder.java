...
extends MessageToByteEncoder<ByteBuf> {
        ...
        block6: while (true) {
            switch (this.currentState) {
                ...
                case WRITE_DATA: {
                    ...
                    int length = Math.min(in2.readableBytes(), blockCompressor.availableSize());
                    int bytesWritten = blockCompressor.write(in2, in2.readerIndex(), length);
                    in2.skipBytes(bytesWritten);
                    if (!blockCompressor.isFull()) {
                        if (in2.isReadable()) continue block6;
                        return;
                    }
    ...
    public ChannelFuture close(final ChannelPromise promise) {
        ...
        executor.execute(new Runnable(){
            ...
            @Override
            public void run() {
                ChannelFuture f2 = Bzip2Encoder.this.finishEncode(Bzip2Encoder.this.ctx(), promise);
