...
final class Bzip2BlockDecompressor {
    ...
    boolean decodeHuffmanData(Bzip2HuffmanStageDecoder huffmanDecoder) {
        ...
        while (true) {
            ...
            if (repeatCount > 0) {
                ...
                int n2 = nextByte & 0xFF;
                bwtByteCounts[n2] = bwtByteCounts[n2] + repeatCount;
                while (--repeatCount >= 0) {
            ...
            nextByte = huffmanSymbolMap[mtfValue];
            int n3 = nextByte & 0xFF;
            bwtByteCounts[n3] = bwtByteCounts[n3] + 1;
            bwtBlock[bwtBlockLength++] = nextByte;
        }
        this.bwtBlockLength = bwtBlockLength;
    ...
    private void initialiseInverseBWT() {
        ...
        for (i2 = 2; i2 <= 255; ++i2) {
            int n2 = i2;
            characterBase[n2] = characterBase[n2] + characterBase[i2 - 1];
        }
        for (i2 = 0; i2 < this.bwtBlockLength; ++i2) {
            int value;
            int n3 = value = bwtBlock[i2] & 0xFF;
            int n4 = characterBase[n3];
            characterBase[n3] = n4 + 1;
            bwtMergedPointers[n4] = (i2 << 8) + value;
        }
