...
extends ZlibEncoder {
    ...
    protected void encode(ChannelHandlerContext ctx, ByteBuf in2, ByteBuf out) throws Exception {
        ...
        int inputLength = in2.readableBytes();
        if (inputLength == 0) {
            return;
        }
        try {
            int outputLength;
            int resultCode;
            boolean inHasArray = in2.hasArray();
            this.z.avail_in = inputLength;
            if (inHasArray) {
                this.z.next_in = in2.array();
                this.z.next_in_index = in2.arrayOffset() + in2.readerIndex();
            } else {
            ...
            try {
                ...
                in2.skipBytes(this.z.next_in_index - oldNextInIndex);
            }
            if (resultCode != 0) {
