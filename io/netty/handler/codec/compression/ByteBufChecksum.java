...
implements Checksum {
    ...
    public void update(ByteBuf b2, int off, int len) {
        if (b2.hasArray()) {
            this.update(b2.array(), b2.arrayOffset() + off, len);
        } else {
            b2.forEachByte(off, len, this.updateProcessor);
        }
    }
    ...
    extends SlowByteBufChecksum {
        ...
        public void update(ByteBuf b2, int off, int len) {
            if (b2.hasArray()) {
                ...
                try {
                    this.method.invoke(this.checksum, CompressionUtil.safeNioBuffer(b2));
                }
