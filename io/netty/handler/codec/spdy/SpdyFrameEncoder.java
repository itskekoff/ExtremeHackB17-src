...
public class SpdyFrameEncoder {
    ...
    public ByteBuf encodeSettingsFrame(ByteBufAllocator allocator, SpdySettingsFrame spdySettingsFrame) {
        ...
        for (Integer id2 : ids) {
            ...
            frame.writeByte(flags);
            frame.writeMedium(id2);
            frame.writeInt(spdySettingsFrame.getValue(id2));
        }
        return frame;
    }

    public ByteBuf encodePingFrame(ByteBufAllocator allocator, int id2) {
        byte flags = 0;
        int length = 4;
        ByteBuf frame = allocator.ioBuffer(8 + length).order(ByteOrder.BIG_ENDIAN);
