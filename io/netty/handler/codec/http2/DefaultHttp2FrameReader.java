...
Http2FrameReader.Configuration {
    ...
    private void processHeaderState(ByteBuf in2) throws Http2Exception {
        if (in2.readableBytes() < 9) {
            return;
        }
        this.payloadLength = in2.readUnsignedMedium();
        ...
        this.frameType = in2.readByte();
        this.flags = new Http2Flags(in2.readUnsignedByte());
        this.streamId = Http2CodecUtil.readUnsignedInt(in2);
    ...
    private void processPayloadState(ChannelHandlerContext ctx, ByteBuf in2, Http2FrameListener listener) throws Http2Exception {
        if (in2.readableBytes() < this.payloadLength) {
            return;
        }
        ByteBuf payload = in2.readSlice(this.payloadLength);
    ...
    private void readSettingsFrame(ChannelHandlerContext ctx, ByteBuf payload, Http2FrameListener listener) throws Http2Exception {
        if (this.flags.ack()) {
            ...
            for (int index = 0; index < numSettings; ++index) {
                char id2 = (char)payload.readUnsignedShort();
                long value = payload.readUnsignedInt();
                try {
                    settings.put(id2, value);
                    continue;
                }
    ...
    protected class HeadersBlockBuilder {
        ...
        final void addFragment(ByteBuf fragment, ByteBufAllocator alloc, boolean endOfHeaders) throws Http2Exception {
            ...
            if (this.headerBlock.isWritable(fragment.readableBytes())) {
                ...
                ByteBuf buf2 = alloc.buffer(this.headerBlock.readableBytes() + fragment.readableBytes());
                buf2.writeBytes(this.headerBlock);
                buf2.writeBytes(fragment);
                this.headerBlock.release();
                this.headerBlock = buf2;
            }
        }
