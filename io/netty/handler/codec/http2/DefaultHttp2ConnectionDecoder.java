...
implements Http2ConnectionDecoder {
    ...
    @Override
    public void decodeFrame(ChannelHandlerContext ctx, ByteBuf in2, List<Object> out) throws Http2Exception {
        this.frameReader.readFrame(ctx, in2, this.internalFrameListener);
    }
    ...
    implements Http2FrameListener {
        ...
        public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding, boolean endOfStream) throws Http2Exception {
            ...
            try {
                ...
                throw Http2Exception.connectionError(Http2Error.INTERNAL_ERROR, t2, "Unhandled error on data stream id %d", streamId);
            }
            if (shouldIgnore) {
                flowController.receiveFlowControlledFrame(stream, data, padding, endOfStream);
                flowController.consumeBytes(stream, bytesToReturn);
            ...
            try {
                ...
                int n2 = bytesToReturn = DefaultHttp2ConnectionDecoder.this.listener.onDataRead(ctx, streamId, data, padding, endOfStream);
                return n2;
            }
        ...
        public void onHeadersRead(ChannelHandlerContext ctx, int streamId, Http2Headers headers, int streamDependency, short weight, boolean exclusive, int padding, boolean endOfStream) throws Http2Exception {
            ...
            if (stream == null && !DefaultHttp2ConnectionDecoder.this.connection.streamMayHaveExisted(streamId)) {
                ...
                boolean bl2 = allowHalfClosedRemote = stream.state() == Http2Stream.State.HALF_CLOSED_REMOTE;
            }
