...
extends Http2FrameListenerDecorator {
    ...
    public int onDataRead(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding, boolean endOfStream) throws Http2Exception {
        ...
        ByteBuf buf2 = DelegatingDecompressorFrameListener.nextReadableBuf(channel);
        if (buf2 == null && endOfStream && channel.finish()) {
            buf2 = DelegatingDecompressorFrameListener.nextReadableBuf(channel);
        }
        if (buf2 == null) {
            if (endOfStream) {
        ...
        try {
            ...
            while (true) {
                ...
                boolean bl2 = decompressedEndOfStream = (nextBuf = DelegatingDecompressorFrameListener.nextReadableBuf(channel)) == null && endOfStream;
                ...
                decompressor.incrementDecompressedBytes(buf2.readableBytes());
                flowController.consumeBytes(stream, this.listener.onDataRead(ctx, streamId, buf2, padding, decompressedEndOfStream));
            ...
            try {
                ...
                throw Http2Exception.streamError(stream.id(), Http2Error.INTERNAL_ERROR, t2, "Decompressor error detected while delegating data read on streamId %d", stream.id());
            }
        }
        buf2.release();
        return n2;
    ...
    private static ByteBuf nextReadableBuf(EmbeddedChannel decompressor) {
        ByteBuf buf2;
        while (true) {
            if ((buf2 = (ByteBuf)decompressor.readInbound()) == null) {
                return null;
            }
            if (buf2.isReadable()) break;
            buf2.release();
        }
        return buf2;
    }
    ...
    implements Http2LocalFlowController {
        ...
        public boolean consumeBytes(Http2Stream stream, int numBytes) throws Http2Exception {
            ...
            try {
                ...
                throw Http2Exception.streamError(stream.id(), Http2Error.INTERNAL_ERROR, t2, "Error while returning bytes to flow control window", new Object[0]);
            }
        }
