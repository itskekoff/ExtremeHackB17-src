...
extends Exception {
    ...
    public static Http2Exception streamError(int id2, Http2Error error, String fmt, Object ... args) {
        return 0 == id2 ? Http2Exception.connectionError(error, fmt, args) : new StreamException(id2, error, String.format(fmt, args));
    }

    public static Http2Exception streamError(int id2, Http2Error error, Throwable cause, String fmt, Object ... args) {
        return 0 == id2 ? Http2Exception.connectionError(error, cause, fmt, args) : new StreamException(id2, error, String.format(fmt, args), cause);
    }

    public static Http2Exception headerListSizeError(int id2, Http2Error error, boolean onDecode, String fmt, Object ... args) {
        return 0 == id2 ? Http2Exception.connectionError(error, fmt, args) : new HeaderListSizeException(id2, error, String.format(fmt, args), onDecode);
    }

    public static boolean isStreamError(Http2Exception e2) {
        return e2 instanceof StreamException;
    }

    public static int streamId(Http2Exception e2) {
        return Http2Exception.isStreamError(e2) ? ((StreamException)e2).streamId() : 0;
    }
    ...
    implements Iterable<StreamException> {
        ...
        public void add(StreamException e2) {
