...
extends DecoratingHttp2ConnectionEncoder {
    ...
    public ChannelFuture writeData(ChannelHandlerContext ctx, int streamId, ByteBuf data, int padding, boolean endOfStream, ChannelPromise promise) {
        ...
        try {
            ...
            ByteBuf buf2 = CompressorHttp2ConnectionEncoder.nextReadableBuf(channel);
            if (buf2 == null) {
                if (endOfStream) {
                    if (channel.finish()) {
                        buf2 = CompressorHttp2ConnectionEncoder.nextReadableBuf(channel);
                    }
                    ChannelFuture channelFuture = super.writeData(ctx, streamId, buf2 == null ? Unpooled.EMPTY_BUFFER : buf2, padding, true, promise);
            ...
            while (true) {
                ...
                boolean bl2 = compressedEndOfStream = (nextBuf = CompressorHttp2ConnectionEncoder.nextReadableBuf(channel)) == null && endOfStream;
                ...
                super.writeData(ctx, streamId, buf2, padding, compressedEndOfStream, bufPromise);
    ...
    void cleanup(Http2Stream stream, EmbeddedChannel compressor) {
        if (compressor.finish()) {
            ByteBuf buf2;
            while ((buf2 = (ByteBuf)compressor.readOutbound()) != null) {
                buf2.release();
            }
        }
    ...
    private static ByteBuf nextReadableBuf(EmbeddedChannel compressor) {
        ByteBuf buf2;
        while (true) {
            if ((buf2 = (ByteBuf)compressor.readOutbound()) == null) {
                return null;
            }
