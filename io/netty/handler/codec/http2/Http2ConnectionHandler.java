...
ChannelOutboundHandler {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in2, List<Object> out) throws Exception {
        this.byteDecoder.decode(ctx, in2, out);
    }
    ...
    extends BaseDecoder {
        ...
        @Override
        public void decode(ChannelHandlerContext ctx, ByteBuf in2, List<Object> out) throws Exception {
            try {
                Http2ConnectionHandler.this.decoder.decodeFrame(ctx, in2, out);
            }
            catch (Throwable e2) {
                Http2ConnectionHandler.this.onError(ctx, e2);
            }
        }
    }
    ...
    extends BaseDecoder {
        ...
        public void decode(ChannelHandlerContext ctx, ByteBuf in2, List<Object> out) throws Exception {
            try {
                if (ctx.channel().isActive() && this.readClientPrefaceString(in2) && this.verifyFirstFrameIsSettings(in2)) {
                    Http2ConnectionHandler.this.byteDecoder = new FrameDecoder();
                    Http2ConnectionHandler.this.byteDecoder.decode(ctx, in2, out);
                }
            }
            catch (Throwable e2) {
                Http2ConnectionHandler.this.onError(ctx, e2);
        ...
        private boolean readClientPrefaceString(ByteBuf in2) throws Http2Exception {
            if (this.clientPrefaceString == null) {
                return true;
            }
            int prefaceRemaining = this.clientPrefaceString.readableBytes();
            int bytesRead = Math.min(in2.readableBytes(), prefaceRemaining);
            if (bytesRead == 0 || !ByteBufUtil.equals(in2, in2.readerIndex(), this.clientPrefaceString, this.clientPrefaceString.readerIndex(), bytesRead)) {
                int maxSearch = 1024;
                int http1Index = ByteBufUtil.indexOf(HTTP_1_X_BUF, in2.slice(in2.readerIndex(), Math.min(in2.readableBytes(), maxSearch)));
                if (http1Index != -1) {
                    String chunk = in2.toString(in2.readerIndex(), http1Index - in2.readerIndex(), CharsetUtil.US_ASCII);
                    throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "Unexpected HTTP/1.x request: %s", chunk);
                }
                String receivedBytes = ByteBufUtil.hexDump(in2, in2.readerIndex(), Math.min(in2.readableBytes(), this.clientPrefaceString.readableBytes()));
        ...
        private boolean verifyFirstFrameIsSettings(ByteBuf in2) throws Http2Exception {
            if (in2.readableBytes() < 5) {
                return false;
            }
            short frameType = in2.getUnsignedByte(in2.readerIndex() + 3);
            short flags = in2.getUnsignedByte(in2.readerIndex() + 4);
            if (frameType != 4 || (flags & 1) != 0) {
                throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, "First received frame was not SETTINGS. Hex dump for first 5 bytes: %s", ByteBufUtil.hexDump(in2, in2.readerIndex(), 5));
            }
            return true;
        }
