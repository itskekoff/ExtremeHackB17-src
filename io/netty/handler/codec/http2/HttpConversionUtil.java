...
public final class HttpConversionUtil {
    ...
    public static HttpResponseStatus parseStatus(CharSequence status) throws Http2Exception {
        ...
        try {
            ...
            throw Http2Exception.connectionError(Http2Error.PROTOCOL_ERROR, t2, "Unrecognized HTTP status code '%s' encountered in translation to HTTP/1.x", status);
        }
        return result;
    }
    ...
    public static Http2Headers toHttp2Headers(HttpMessage in2, boolean validateHeaders) {
        HttpHeaders inHeaders = in2.headers();
        DefaultHttp2Headers out = new DefaultHttp2Headers(validateHeaders, inHeaders.size());
        if (in2 instanceof HttpRequest) {
            HttpRequest request = (HttpRequest)in2;
        ...
        } else if (in2 instanceof HttpResponse) {
            HttpResponse response = (HttpResponse)in2;
    ...
    private static void setHttp2Scheme(HttpHeaders in2, URI uri, Http2Headers out) {
        String value = uri.getScheme();
        if (value != null) {
            out.scheme(new AsciiString(value));
            return;
        }
        String cValue = in2.get(ExtensionHeaderNames.SCHEME.text());
        if (cValue != null) {
