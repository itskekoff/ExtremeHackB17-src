...
extends MessageToMessageCodec<Http2StreamFrame, HttpObject> {
        ...
        if (frame instanceof Http2HeadersFrame) {
            ...
            if (headersFrame.isEndStream()) {
                if (headers.method() == null) {
                    ...
                    HttpConversionUtil.addHttp2ToHttpHeaders(id2, headers, last.trailingHeaders(), HttpVersion.HTTP_1_1, true, true);
                    out.add(last);
                } else {
                    FullHttpRequest full = HttpConversionUtil.toFullHttpRequest(id2, headers, ctx.alloc(), this.validateHeaders);
                    out.add(full);
                }
            } else {
                HttpRequest req = HttpConversionUtil.toHttpRequest(id2, headersFrame.headers(), this.validateHeaders);
    ...
    private void encodeLastContent(LastHttpContent last, List<Object> out) {
        boolean needFiller;
        boolean bl2 = needFiller = !(last instanceof FullHttpResponse) && last.trailingHeaders().isEmpty();
