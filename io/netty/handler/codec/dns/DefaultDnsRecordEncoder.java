...
implements DnsRecordEncoder {
    ...
    protected void encodeName(String name, ByteBuf buf2) throws Exception {
        String label;
        int labelLen;
        String[] labels;
        if (".".equals(name)) {
        ...
        for (int i2 = 0; i2 < n2 && (labelLen = (label = arrstring[i2]).length()) != 0; ++i2) {
            buf2.writeByte(labelLen);
            ByteBufUtil.writeAscii(buf2, (CharSequence)label);
        }
        buf2.writeByte(0);
    }

    static byte padWithZeros(byte b2, int lowOrderBitsToPreserve) {
