...
public class ExpressionParser {
    ...
    private IExpression makeInfixFunc(List<IExpression> listExpr, List<EnumFunctionType> listFunc) throws ParseException {
        ...
        for (EnumFunctionType enumfunctiontype : listFunc) {
            i2 = Math.min(enumfunctiontype.getPrecedence(), i2);
            j2 = Math.max(enumfunctiontype.getPrecedence(), j2);
        }
        if (j2 >= i2 && j2 - i2 <= 10) {
    ...
    private void mergeOperators(List<IExpression> listExpr, List<EnumFunctionType> listFuncs, int precedence) {
        for (int i2 = 0; i2 < listFuncs.size(); ++i2) {
            EnumFunctionType enumfunctiontype = listFuncs.get(i2);
        ...
        while (true) {
            try {
                while (true) {
                    ...
                    String s2;
                    if ((s2 = (bufferedreader = new BufferedReader(inputstreamreader = new InputStreamReader(System.in))).readLine()).length() <= 0) {
                        return;
                    }
                    IExpression iexpression = expressionparser.parse(s2);
                    float f2 = iexpression.eval();
                    Config.dbg(s2 + " = " + f2);
                }
            }
