...
public class CustomEntityModelParser {
    ...
    public static CustomEntityRenderer parseEntityRender(JsonObject obj, String path) {
        ...
        String s2 = connectedparser.parseName(path);
        String s1 = connectedparser.parseBasePath(path);
        String s22 = Json.getString(obj, TEXTURE);
        int[] aint = Json.parseIntArray(obj.get(ITEM_TEXTURE_SIZE), 2);
        float f2 = Json.getFloat(obj, SHADOW_SIZE, -1.0f);
        ...
        if (s22 != null) {
            resourcelocation = CustomEntityModelParser.getResourceLocation(s1, s22, ".png");
        }
        CustomEntityRenderer customentityrenderer = new CustomEntityRenderer(s2, s1, resourcelocation, acustommodelrenderer, f2);
        return customentityrenderer;
    }

    private static void processBaseId(JsonObject elem, Map mapModelJsons) {
        String s2 = Json.getString(elem, MODEL_BASE_ID);
        if (s2 != null) {
            JsonObject jsonobject = (JsonObject)mapModelJsons.get(s2);
            if (jsonobject == null) {
    ...
    private static void processExternalModel(JsonObject elem, Map mapModelJsons, String basePath) {
        String s2 = Json.getString(elem, MODEL_MODEL);
        if (s2 != null) {
            ResourceLocation resourcelocation = CustomEntityModelParser.getResourceLocation(basePath, s2, ".jpm");
            try {
    ...
    private static void processId(JsonObject elem, Map mapModelJsons) {
        ...
        if (s2 != null) {
            if (s2.length() < 1) {
                Config.warn("Empty model ID: " + s2);
            } else if (mapModelJsons.containsKey(s2)) {
                Config.warn("Duplicate model ID: " + s2);
            } else {
                mapModelJsons.put(s2, elem);
            }
        }
    }

    public static CustomModelRenderer parseCustomModelRenderer(JsonObject elem, int[] textureSize, String basePath) {
        String s2 = Json.getString(elem, MODEL_PART);
        CustomEntityModelParser.checkNull(s2, "Model part not specified, missing \"replace\" or \"attachTo\".");
        ...
        if (jsonarray != null) {
            ...
            for (int i2 = 0; i2 < jsonarray.size(); ++i2) {
                ...
                for (Map.Entry<String, JsonElement> entry : jsonobject.entrySet()) {
                    ...
                    String s22 = entry.getValue().getAsString();
                    ModelVariableUpdater modelvariableupdater = new ModelVariableUpdater(s1, s22);
        ...
        CustomModelRenderer custommodelrenderer = new CustomModelRenderer(s2, flag, modelrenderer, modelupdater);
    ...
    public static JsonObject loadJson(ResourceLocation location) throws IOException, JsonParseException {
        ...
        String s2 = Config.readInputStream(inputstream, "ASCII");
        inputstream.close();
