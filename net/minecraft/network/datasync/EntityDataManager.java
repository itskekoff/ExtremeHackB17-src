...
public class EntityDataManager {
    ...
    public static <T> DataParameter<T> createKey(Class<? extends Entity> clazz, DataSerializer<T> serializer) {
        ...
        NEXT_ID_MAP.put(clazz, j2);
        return serializer.createKey(j2);
    }
    ...
    public <T> void register(DataParameter<T> key, T value) {
        ...
        if (DataSerializers.getSerializerId(key.getSerializer()) < 0) {
            throw new IllegalArgumentException("Unregistered serializer " + key.getSerializer() + " for " + i2 + "!");
        }
    ...
    public static void writeEntries(List<DataEntry<?>> entriesIn, PacketBuffer buf2) throws IOException {
        if (entriesIn != null) {
            int j2 = entriesIn.size();
            for (int i2 = 0; i2 < j2; ++i2) {
                DataEntry<?> dataentry = entriesIn.get(i2);
                EntityDataManager.writeEntry(buf2, dataentry);
            }
        }
        buf2.writeByte(255);
    }
    ...
    public void writeEntries(PacketBuffer buf2) throws IOException {
        this.lock.readLock().lock();
        for (DataEntry<?> dataentry : this.entries.values()) {
            EntityDataManager.writeEntry(buf2, dataentry);
        }
    ...
    private static <T> void writeEntry(PacketBuffer buf2, DataEntry<T> entry) throws IOException {
        DataParameter<T> dataparameter = entry.getKey();
        int i2 = DataSerializers.getSerializerId(dataparameter.getSerializer());
        if (i2 < 0) {
            throw new EncoderException("Unknown serializer type " + dataparameter.getSerializer());
        }
        buf2.writeByte(dataparameter.getId());
        buf2.writeVarIntToBuffer(i2);
        dataparameter.getSerializer().write(buf2, entry.getValue());
    }

    @Nullable
    public static List<DataEntry<?>> readEntries(PacketBuffer buf2) throws IOException {
        short i2;
        ArrayList<DataEntry<?>> list = null;
        while ((i2 = buf2.readUnsignedByte()) != 255) {
            int j2;
            DataSerializer<?> dataserializer;
            if (list == null) {
                list = Lists.newArrayList();
            }
            if ((dataserializer = DataSerializers.getSerializer(j2 = buf2.readVarIntFromBuffer())) == null) {
                throw new DecoderException("Unknown serializer type " + j2);
            }
            list.add(new DataEntry(dataserializer.createKey(i2), dataserializer.read(buf2)));
        }
        return list;
    }
