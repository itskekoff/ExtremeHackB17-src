...
public enum EnumConnectionState {
    ...
    static {
        ...
        for (EnumConnectionState enumconnectionstate : EnumConnectionState.values()) {
            int i2 = enumconnectionstate.getId();
            if (i2 < -1 || i2 > 2) {
                throw new Error("Invalid protocol ID " + Integer.toString(i2));
            }
            EnumConnectionState.STATES_BY_ID[i2 - -1] = enumconnectionstate;
    ...
    protected EnumConnectionState registerPacket(EnumPacketDirection direction, Class<? extends Packet<?>> packetClass) {
        ...
        if (bimap.containsValue(packetClass)) {
            String s2 = (Object)((Object)direction) + " packet " + packetClass + " is already known to ID " + bimap.inverse().get(packetClass);
            LogManager.getLogger().fatal(s2);
    ...
    /* synthetic */ EnumConnectionState(String string, int n2, int n3, EnumConnectionState enumConnectionState) {
        this(n3);
    }
}
