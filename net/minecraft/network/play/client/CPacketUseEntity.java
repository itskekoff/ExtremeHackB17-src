...
implements Packet<INetHandlerPlayServer> {
    ...
    public void readPacketData(PacketBuffer buf2) throws IOException {
        ...
        if (this.action == Action.INTERACT_AT) {
            this.hitVec = new Vec3d(buf2.readFloat(), buf2.readFloat(), buf2.readFloat());
        }
    ...
    public void writePacketData(PacketBuffer buf2) throws IOException {
        ...
        if (this.action == Action.INTERACT_AT) {
            buf2.writeFloat((float)this.hitVec.xCoord);
            buf2.writeFloat((float)this.hitVec.yCoord);
            buf2.writeFloat((float)this.hitVec.zCoord);
        }
        if (this.action == Action.INTERACT || this.action == Action.INTERACT_AT) {
