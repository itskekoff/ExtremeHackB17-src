...
extends ByteBuf {
    ...
    public byte[] readByteArray(int maxLength) {
        int i2 = this.readVarIntFromBuffer();
        if (i2 > maxLength) {
            throw new DecoderException("ByteArray with size " + i2 + " is bigger than allowed " + maxLength);
        }
    ...
    public int[] readVarIntArray(int maxLength) {
        int i2 = this.readVarIntFromBuffer();
        if (i2 > maxLength) {
            throw new DecoderException("VarIntArray with size " + i2 + " is bigger than allowed " + maxLength);
        }
        int[] aint = new int[i2];
        for (int j2 = 0; j2 < aint.length; ++j2) {
            aint[j2] = this.readVarIntFromBuffer();
        }
        return aint;
    }
    ...
    public long[] readLongArray(@Nullable long[] p_189423_1_, int p_189423_2_) {
        int i2 = this.readVarIntFromBuffer();
        if (p_189423_1_ == null || p_189423_1_.length != i2) {
            if (i2 > p_189423_2_) {
                throw new DecoderException("LongArray with size " + i2 + " is bigger than allowed " + p_189423_2_);
            }
            p_189423_1_ = new long[i2];
        }
        for (int j2 = 0; j2 < p_189423_1_.length; ++j2) {
            p_189423_1_[j2] = this.readLong();
        }
        return p_189423_1_;
    }
    ...
    public ItemStack readItemStackFromBuffer() throws IOException {
        short i2 = this.readShort();
        if (i2 < 0) {
            return ItemStack.field_190927_a;
        }
        byte j2 = this.readByte();
        short k2 = this.readShort();
        ItemStack itemstack = new ItemStack(Item.getItemById(i2), (int)j2, (int)k2);
    ...
    public String readStringFromBuffer(int maxLength) {
        ...
        String s2 = this.toString(this.readerIndex(), i2, StandardCharsets.UTF_8);
        this.readerIndex(this.readerIndex() + i2);
