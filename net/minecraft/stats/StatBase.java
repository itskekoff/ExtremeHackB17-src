...
public class StatBase {
    ...
    public static IStatType timeStatType = new IStatType(){
        ...
        public String format(int number) {
            ...
            double d4 = d3 / 365.0;
            if (d4 > 0.5) {
                return String.valueOf(decimalFormat.format(d4)) + " y";
            }
            if (d3 > 0.5) {
                return String.valueOf(decimalFormat.format(d3)) + " d";
            }
            if (d2 > 0.5) {
                return String.valueOf(decimalFormat.format(d2)) + " h";
            }
            return d1 > 0.5 ? String.valueOf(decimalFormat.format(d1)) + " m" : String.valueOf(d0) + " s";
        }
    };
    public static IStatType distanceStatType = new IStatType(){
        ...
        public String format(int number) {
            ...
            double d1 = d0 / 1000.0;
            if (d1 > 0.5) {
                return String.valueOf(decimalFormat.format(d1)) + " km";
            }
            return d0 > 0.5 ? String.valueOf(decimalFormat.format(d0)) + " m" : String.valueOf(number) + " cm";
        }
    };
