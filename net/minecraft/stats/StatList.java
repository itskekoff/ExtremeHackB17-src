...
public class StatList {
    ...
    private static void initCraftableStats() {
        ...
        for (Item item : set) {
            ...
            String s2 = StatList.getItemName(item);
            if (s2 == null) continue;
            StatList.CRAFTS_STATS[i2] = new StatCrafting("stat.craftItem.", s2, new TextComponentTranslation("stat.craftItem", new ItemStack(item).getTextComponent()), item).registerStat();
        }
    ...
    private static void initMiningStats() {
        for (Block block : Block.REGISTRY) {
            ...
            String s2 = StatList.getItemName(item);
            if (s2 == null || !block.getEnableStats()) continue;
            StatList.BLOCKS_STATS[i2] = new StatCrafting("stat.mineBlock.", s2, new TextComponentTranslation("stat.mineBlock", new ItemStack(block).getTextComponent()), item).registerStat();
            MINE_BLOCK_STATS.add((StatCrafting)BLOCKS_STATS[i2]);
        }
    ...
    private static void initStats() {
        for (Item item : Item.REGISTRY) {
            ...
            String s2 = StatList.getItemName(item);
            if (s2 == null) continue;
            StatList.OBJECT_USE_STATS[i2] = new StatCrafting("stat.useItem.", s2, new TextComponentTranslation("stat.useItem", new ItemStack(item).getTextComponent()), item).registerStat();
            if (item instanceof ItemBlock) continue;
            USE_ITEM_STATS.add((StatCrafting)OBJECT_USE_STATS[i2]);
        }
    ...
    private static void initItemDepleteStats() {
        for (Item item : Item.REGISTRY) {
            ...
            int i2 = Item.getIdFromItem(item);
            String s2 = StatList.getItemName(item);
            if (s2 == null || !item.isDamageable()) continue;
            StatList.OBJECT_BREAK_STATS[i2] = new StatCrafting("stat.breakItem.", s2, new TextComponentTranslation("stat.breakItem", new ItemStack(item).getTextComponent()), item).registerStat();
        }
    ...
    private static void initPickedUpAndDroppedStats() {
        for (Item item : Item.REGISTRY) {
            ...
            String s2 = StatList.getItemName(item);
            if (s2 == null) continue;
            StatList.OBJECTS_PICKED_UP_STATS[i2] = new StatCrafting("stat.pickup.", s2, new TextComponentTranslation("stat.pickup", new ItemStack(item).getTextComponent()), item).registerStat();
            StatList.OBJECTS_DROPPED_STATS[i2] = new StatCrafting("stat.drop.", s2, new TextComponentTranslation("stat.drop", new ItemStack(item).getTextComponent()), item).registerStat();
        }
        StatList.replaceAllSimilarBlocks(OBJECT_BREAK_STATS);
    }
    ...
    private static void mergeStatBases(StatBase[] statBaseIn, Block block1, Block block2) {
        int i2 = Block.getIdFromBlock(block1);
        int j2 = Block.getIdFromBlock(block2);
        if (statBaseIn[i2] != null && statBaseIn[j2] == null) {
            statBaseIn[j2] = statBaseIn[i2];
        } else {
            ALL_STATS.remove(statBaseIn[i2]);
            MINE_BLOCK_STATS.remove(statBaseIn[i2]);
            BASIC_STATS.remove(statBaseIn[i2]);
            statBaseIn[i2] = statBaseIn[j2];
        }
    }

    public static StatBase getStatKillEntity(EntityList.EntityEggInfo eggInfo) {
        String s2 = EntityList.func_191302_a(eggInfo.spawnedID);
        return s2 == null ? null : new StatBase("stat.killEntity." + s2, new TextComponentTranslation("stat.entityKill", new TextComponentTranslation("entity." + s2 + ".name", new Object[0]))).registerStat();
    }

    public static StatBase getStatEntityKilledBy(EntityList.EntityEggInfo eggInfo) {
        String s2 = EntityList.func_191302_a(eggInfo.spawnedID);
        return s2 == null ? null : new StatBase("stat.entityKilledBy." + s2, new TextComponentTranslation("stat.entityKilledBy", new TextComponentTranslation("entity." + s2 + ".name", new Object[0]))).registerStat();
    }
