...
extends Enchantment {
    ...
    public static int getFireTimeForEntity(EntityLivingBase p_92093_0_, int p_92093_1_) {
        int i2 = EnchantmentHelper.getMaxEnchantmentLevel(Enchantments.FIRE_PROTECTION, p_92093_0_);
        if (i2 > 0) {
            p_92093_1_ -= MathHelper.floor((float)p_92093_1_ * (float)i2 * 0.15f);
        }
        return p_92093_1_;
    }

    public static double getBlastDamageReduction(EntityLivingBase entityLivingBaseIn, double damage) {
        int i2 = EnchantmentHelper.getMaxEnchantmentLevel(Enchantments.BLAST_PROTECTION, entityLivingBaseIn);
        if (i2 > 0) {
            damage -= (double)MathHelper.floor(damage * (double)((float)i2 * 0.15f));
        }
        return damage;
    }
