...
extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos, EnumFacing hand, float facing, float hitX, float hitY) {
        ...
        if (stack.canPlayerEdit(worldIn, hand, itemstack = stack.getHeldItem(pos)) && this.block.canPlaceBlockAt(playerIn, worldIn)) {
            ...
            int i2 = enumfacing.getFrontOffsetX();
            int j2 = enumfacing.getFrontOffsetZ();
            boolean flag = i2 < 0 && hitY < 0.5f || i2 > 0 && hitY > 0.5f || j2 < 0 && facing > 0.5f || j2 > 0 && facing < 0.5f;
    ...
    public static void placeDoor(World worldIn, BlockPos pos, EnumFacing facing, Block door, boolean isRightHinge) {
        ...
        int i2 = (worldIn.getBlockState(blockpos1).isNormalCube() ? 1 : 0) + (worldIn.getBlockState(blockpos1.up()).isNormalCube() ? 1 : 0);
        int j2 = (worldIn.getBlockState(blockpos).isNormalCube() ? 1 : 0) + (worldIn.getBlockState(blockpos.up()).isNormalCube() ? 1 : 0);
        boolean flag = worldIn.getBlockState(blockpos1).getBlock() == door || worldIn.getBlockState(blockpos1.up()).getBlock() == door;
        boolean bl2 = flag1 = worldIn.getBlockState(blockpos).getBlock() == door || worldIn.getBlockState(blockpos.up()).getBlock() == door;
        if ((!flag || flag1) && j2 <= i2) {
            if (flag1 && !flag || j2 < i2) {
                isRightHinge = false;
            }
