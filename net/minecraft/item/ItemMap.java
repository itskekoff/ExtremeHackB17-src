...
extends ItemMapBase {
    ...
    public static ItemStack func_190906_a(World p_190906_0_, double p_190906_1_, double p_190906_3_, byte p_190906_5_, boolean p_190906_6_, boolean p_190906_7_) {
        ...
        String s2 = "map_" + itemstack.getMetadata();
        MapData mapdata = new MapData(s2);
        p_190906_0_.setItemData(s2, mapdata);
    ...
    public static MapData loadMapData(int mapId, World worldIn) {
        ...
        return (MapData)worldIn.loadItemData(MapData.class, s2);
    }
    ...
    public void updateMapData(World worldIn, Entity viewer, MapData data) {
        if (worldIn.provider.getDimensionType().getId() == data.dimension && viewer instanceof EntityPlayer) {
            ...
            int j2 = data.xCenter;
            int k2 = data.zCenter;
            int l2 = MathHelper.floor(viewer.posX - (double)j2) / i2 + 64;
            int i1 = MathHelper.floor(viewer.posZ - (double)k2) / i2 + 64;
            int j1 = 128 / i2;
    ...
    public static void func_190905_a(World p_190905_0_, ItemStack p_190905_1_) {
        ...
        if (p_190905_1_.getItem() == Items.FILLED_MAP && (mapdata = Items.FILLED_MAP.getMapData(p_190905_1_, p_190905_0_)) != null && p_190905_0_.provider.getDimensionType().getId() == mapdata.dimension) {
            ...
            int j2 = mapdata.xCenter;
            int k2 = mapdata.zCenter;
            Biome[] abiome = p_190905_0_.getBiomeProvider().getBiomes(null, (j2 / i2 - 64) * i2, (k2 / i2 - 64) * i2, 128 * i2, 128 * i2, false);
            for (int l2 = 0; l2 < 128; ++l2) {
                for (int i1 = 0; i1 < 128; ++i1) {
                    int j1 = l2 * i2;
                    int k1 = i1 * i2;
                    Biome biome = abiome[j1 + k1 * 128 * i2];
                    MapColor mapcolor = MapColor.AIR;
                    int l1 = 3;
                    int i22 = 8;
                    if (l2 > 0 && i1 > 0 && l2 < 127 && i1 < 127) {
                        if (abiome[(l2 - 1) * i2 + (i1 - 1) * i2 * 128 * i2].getBaseHeight() >= 0.0f) {
                            --i22;
                        }
                        if (abiome[(l2 - 1) * i2 + (i1 + 1) * i2 * 128 * i2].getBaseHeight() >= 0.0f) {
                            --i22;
                        }
                        if (abiome[(l2 - 1) * i2 + i1 * i2 * 128 * i2].getBaseHeight() >= 0.0f) {
                            --i22;
                        }
                        if (abiome[(l2 + 1) * i2 + (i1 - 1) * i2 * 128 * i2].getBaseHeight() >= 0.0f) {
                            --i22;
                        ...
                        if (abiome[l2 * i2 + (i1 - 1) * i2 * 128 * i2].getBaseHeight() >= 0.0f) {
                            --i22;
                        }
                        if (abiome[l2 * i2 + (i1 + 1) * i2 * 128 * i2].getBaseHeight() >= 0.0f) {
                            --i22;
                        ...
                        if (biome.getBaseHeight() < 0.0f) {
                            ...
                            if (i22 > 7 && i1 % 2 == 0) {
                                l1 = (l2 + (int)(MathHelper.sin((float)i1 + 0.0f) * 7.0f)) / 8 % 5;
                                if (l1 == 3) {
                                    l1 = 1;
                                } else if (l1 == 4) {
                                    l1 = 0;
                    ...
                    mapdata.colors[l2 + i1 * 128] = (byte)(mapcolor.colorIndex * 4 + l1);
                    mapdata.updateMapData(l2, i1);
                }
            }
        }
    }
    ...
    public static int func_190907_h(ItemStack p_190907_0_) {
        ...
        if (nbttagcompound != null && nbttagcompound.hasKey("MapColor", 99)) {
            int i2 = nbttagcompound.getInteger("MapColor");
