...
extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos, EnumFacing hand, float facing, float hitX, float hitY) {
        ...
        if (stack.canPlayerEdit(worldIn, hand, itemstack = stack.getHeldItem(pos)) && Blocks.SKULL.canPlaceBlockAt(playerIn, worldIn)) {
            ...
            if (hand == EnumFacing.UP) {
                i2 = MathHelper.floor((double)(stack.rotationYaw * 16.0f / 360.0f) + 0.5) & 0xF;
            }
            if ((tileentity = playerIn.getTileEntity(worldIn)) instanceof TileEntitySkull) {
                ...
                tileentityskull.setSkullRotation(i2);
    ...
    public void getSubItems(CreativeTabs itemIn, NonNullList<ItemStack> tab) {
        if (this.func_194125_a(itemIn)) {
            for (int i2 = 0; i2 < SKULL_TYPES.length; ++i2) {
                tab.add(new ItemStack(this, 1, i2));
            }
        }
    }
    ...
    public String getUnlocalizedName(ItemStack stack) {
        ...
        if (i2 < 0 || i2 >= SKULL_TYPES.length) {
            i2 = 0;
        }
        return String.valueOf(super.getUnlocalizedName()) + "." + SKULL_TYPES[i2];
    }
