...
extends ItemBlock {
    ...
    public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos, EnumFacing hand, float facing, float hitX, float hitY) {
        ...
        if (!itemstack.func_190926_b() && stack.canPlayerEdit(worldIn, hand, itemstack)) {
            ...
            if (block == this.block && (i2 = iblockstate.getValue(BlockSnow.LAYERS).intValue()) < 8 && (axisalignedbb = (iblockstate1 = iblockstate.withProperty(BlockSnow.LAYERS, i2 + 1)).getCollisionBoundingBox(playerIn, blockpos)) != Block.NULL_AABB && playerIn.checkNoEntityCollision(axisalignedbb.offset(blockpos)) && playerIn.setBlockState(blockpos, iblockstate1, 10)) {
