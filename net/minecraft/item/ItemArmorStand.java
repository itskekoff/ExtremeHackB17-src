...
extends Item {
    ...
    public EnumActionResult onItemUse(EntityPlayer stack, World playerIn, BlockPos worldIn, EnumHand pos, EnumFacing hand, float facing, float hitX, float hitY) {
        ...
        if (!playerIn.isRemote) {
            ...
            float f2 = (float)MathHelper.floor((MathHelper.wrapDegrees(stack.rotationYaw - 180.0f) + 22.5f) / 45.0f) * 45.0f;
            entityarmorstand.setLocationAndAngles(d0 + 0.5, d1, d2 + 0.5, f2, 0.0f);
    ...
    private void applyRandomRotations(EntityArmorStand armorStand, Random rand) {
        ...
        float f2 = rand.nextFloat() * 5.0f;
        float f1 = rand.nextFloat() * 20.0f - 10.0f;
        Rotations rotations1 = new Rotations(rotations.getX() + f2, rotations.getY() + f1, rotations.getZ());
        armorStand.setHeadRotation(rotations1);
        rotations = armorStand.getBodyRotation();
        f2 = rand.nextFloat() * 10.0f - 5.0f;
        rotations1 = new Rotations(rotations.getX(), rotations.getY() + f2, rotations.getZ());
