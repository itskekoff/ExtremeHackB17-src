...
public final class ItemStack {
    ...
    public ItemStack splitStack(int amount) {
        int i2 = Math.min(amount, this.stackSize);
        ItemStack itemstack = this.copy();
        itemstack.func_190920_e(i2);
        this.func_190918_g(i2);
        return itemstack;
    }
    ...
    public boolean attemptDamageItem(int amount, Random rand, @Nullable EntityPlayerMP p_96631_3_) {
        ...
        if (amount > 0) {
            int i2 = EnchantmentHelper.getEnchantmentLevel(Enchantments.UNBREAKING, this);
            int j2 = 0;
            for (int k2 = 0; i2 > 0 && k2 < amount; ++k2) {
                if (!EnchantmentDurability.negateDamage(this, i2, rand)) continue;
                ++j2;
            }
            if ((amount -= j2) <= 0) {
                return false;
    ...
    public List<String> getTooltip(@Nullable EntityPlayer playerIn, ITooltipFlag advanced) {
        ...
        s2 = String.valueOf(s2) + (Object)((Object)TextFormatting.RESET);
        ...
        if (this.hasTagCompound()) {
            if ((i1 & 1) == 0) {
                ...
                for (int j2 = 0; j2 < nbttaglist.tagCount(); ++j2) {
                    ...
                    short k2 = nbttagcompound.getShort("id");
                    short l2 = nbttagcompound.getShort("lvl");
                    Enchantment enchantment = Enchantment.getEnchantmentByID(k2);
                    if (enchantment == null) continue;
                    list.add(enchantment.getTranslatedName(l2));
                }
            }
