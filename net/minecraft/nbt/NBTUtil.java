...
public final class NBTUtil {
    ...
    public static boolean areNBTEquals(NBTBase nbt1, NBTBase nbt2, boolean compareTagList) {
        ...
        if (nbt1 instanceof NBTTagCompound) {
            ...
            for (String s2 : nbttagcompound.getKeySet()) {
                ...
                if (NBTUtil.areNBTEquals(nbtbase1, nbttagcompound1.getTag(s2), compareTagList)) continue;
                return false;
            }
            return true;
        }
        if (nbt1 instanceof NBTTagList && compareTagList) {
            ...
            for (int i2 = 0; i2 < nbttaglist.tagCount(); ++i2) {
                ...
                for (int j2 = 0; j2 < nbttaglist1.tagCount(); ++j2) {
                    if (!NBTUtil.areNBTEquals(nbtbase, nbttaglist1.get(j2), compareTagList)) continue;
                    flag = true;
                    break;
                }
                if (flag) continue;
    ...
    public static IBlockState readBlockState(NBTTagCompound tag) {
        ...
        if (tag.hasKey("Properties", 10)) {
            ...
            for (String s2 : nbttagcompound.getKeySet()) {
                IProperty<?> iproperty = blockstatecontainer.getProperty(s2);
                if (iproperty == null) continue;
                iblockstate = NBTUtil.func_193590_a(iblockstate, iproperty, s2, nbttagcompound, tag);
            }
        }
        return iblockstate;
    }
