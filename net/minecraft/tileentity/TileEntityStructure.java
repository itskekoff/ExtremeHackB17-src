...
extends TileEntity {
    ...
    public void readFromNBT(NBTTagCompound compound) {
        ...
        int i2 = MathHelper.clamp(compound.getInteger("posX"), -32, 32);
        int j2 = MathHelper.clamp(compound.getInteger("posY"), -32, 32);
        int k2 = MathHelper.clamp(compound.getInteger("posZ"), -32, 32);
        this.position = new BlockPos(i2, j2, k2);
        int l2 = MathHelper.clamp(compound.getInteger("sizeX"), 0, 32);
        int i1 = MathHelper.clamp(compound.getInteger("sizeY"), 0, 32);
        int j1 = MathHelper.clamp(compound.getInteger("sizeZ"), 0, 32);
        this.size = new BlockPos(l2, i1, j1);
        try {
    ...
    public void setName(String nameIn) {
        String s2 = nameIn;
        char[] arrc = ChatAllowedCharacters.ILLEGAL_STRUCTURE_CHARACTERS;
        int n2 = ChatAllowedCharacters.ILLEGAL_STRUCTURE_CHARACTERS.length;
    ...
    public void writeCoordinates(ByteBuf buf2) {
        buf2.writeInt(this.pos.getX());
        buf2.writeInt(this.pos.getY());
        buf2.writeInt(this.pos.getZ());
    }
    ...
    public static enum Mode implements IStringSerializable
    {
        ...
        static {
            ...
            int n2 = arrmode.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                Mode tileentitystructure$mode;
                Mode.MODES[tileentitystructure$mode.getModeId()] = tileentitystructure$mode = arrmode[i2];
            }
        }
        ...
        public static Mode getById(int id2) {
            return id2 >= 0 && id2 < MODES.length ? MODES[id2] : MODES[0];
        }
    }
}
