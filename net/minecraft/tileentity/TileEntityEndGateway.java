...
implements ITickable {
    ...
    private void findExitPortal() {
        ...
        int i2 = 16;
        while (TileEntityEndGateway.getChunk(this.world, vec3d1).getTopFilledSegment() > 0 && i2-- > 0) {
            LOG.debug("Skipping backwards past nonempty chunk at {}", (Object)vec3d1);
            vec3d1 = vec3d1.add(vec3d.scale(-16.0));
        }
        int j2 = 16;
        while (TileEntityEndGateway.getChunk(this.world, vec3d1).getTopFilledSegment() == 0 && j2-- > 0) {
    ...
    private static BlockPos findHighestBlock(World p_184308_0_, BlockPos p_184308_1_, int p_184308_2_, boolean p_184308_3_) {
        Vec3i blockpos = null;
        for (int i2 = -p_184308_2_; i2 <= p_184308_2_; ++i2) {
            block1: for (int j2 = -p_184308_2_; j2 <= p_184308_2_; ++j2) {
                if (i2 == 0 && j2 == 0 && !p_184308_3_) continue;
                for (int k2 = 255; k2 > (blockpos == null ? 0 : blockpos.getY()); --k2) {
                    BlockPos blockpos1 = new BlockPos(p_184308_1_.getX() + i2, k2, p_184308_1_.getZ() + j2);
    ...
    private static BlockPos findSpawnpointInChunk(Chunk chunkIn) {
        ...
        int i2 = chunkIn.getTopFilledSegment() + 16 - 1;
        BlockPos blockpos1 = new BlockPos(chunkIn.xPosition * 16 + 16 - 1, i2, chunkIn.zPosition * 16 + 16 - 1);
    ...
    public int getParticleAmount() {
        ...
        for (EnumFacing enumfacing : EnumFacing.values()) {
            i2 += this.shouldRenderFace(enumfacing) ? 1 : 0;
        }
        return i2;
    }
