...
ITickable {
    ...
    private boolean updateHopper() {
        if (this.world != null && !this.world.isRemote) {
            if (!this.isOnTransferCooldown() && BlockHopper.isEnabled(this.getBlockMetadata())) {
                ...
                if (!this.isFull()) {
                    boolean bl2 = flag = TileEntityHopper.captureDroppedItems(this) || flag;
                }
                if (flag) {
    ...
    private boolean transferItemsOut() {
        ...
        for (int i2 = 0; i2 < this.getSizeInventory(); ++i2) {
            if (this.getStackInSlot(i2).func_190926_b()) continue;
            ItemStack itemstack = this.getStackInSlot(i2).copy();
            ItemStack itemstack1 = TileEntityHopper.putStackInInventoryAllSlots(this, iinventory, this.decrStackSize(i2, 1), enumfacing);
    ...
    private boolean isInventoryFull(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            int[] arrn = aint = isidedinventory.getSlotsForFace(side);
            int n2 = aint.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                int k2 = arrn[i2];
                ItemStack itemstack1 = isidedinventory.getStackInSlot(k2);
                if (!itemstack1.func_190926_b() && itemstack1.func_190916_E() == itemstack1.getMaxStackSize()) continue;
                return false;
            }
        } else {
            int i3 = inventoryIn.getSizeInventory();
    ...
    private static boolean isInventoryEmpty(IInventory inventoryIn, EnumFacing side) {
        if (inventoryIn instanceof ISidedInventory) {
            ...
            for (int i2 = 0; i2 < n2; ++i2) {
                int i3 = arrn[i2];
                if (isidedinventory.getStackInSlot(i3).func_190926_b()) continue;
                return false;
            }
        } else {
            int j2 = inventoryIn.getSizeInventory();
            for (int k2 = 0; k2 < j2; ++k2) {
                if (inventoryIn.getStackInSlot(k2).func_190926_b()) continue;
                return false;
            }
        }
        return true;
    ...
    public static boolean captureDroppedItems(IHopper hopper) {
        ...
        if (iinventory != null) {
            ...
            if (iinventory instanceof ISidedInventory) {
                ...
                int[] arrn = aint = isidedinventory.getSlotsForFace(enumfacing);
                int n2 = aint.length;
                for (int i2 = 0; i2 < n2; ++i2) {
                    int i3 = arrn[i2];
                    if (!TileEntityHopper.pullItemFromSlot(hopper, iinventory, i3, enumfacing)) continue;
                    return true;
                }
            } else {
                int j2 = iinventory.getSizeInventory();
                for (int k2 = 0; k2 < j2; ++k2) {
                    if (!TileEntityHopper.pullItemFromSlot(hopper, iinventory, k2, enumfacing)) continue;
                    return true;
                }
            }
        } else {
    ...
    public static ItemStack putStackInInventoryAllSlots(IInventory inventoryIn, IInventory stack, ItemStack side, @Nullable EnumFacing p_174918_3_) {
        if (stack instanceof ISidedInventory && p_174918_3_ != null) {
            ...
            for (int k2 = 0; k2 < aint.length && !side.func_190926_b(); ++k2) {
                side = TileEntityHopper.insertStack(inventoryIn, stack, side, aint[k2], p_174918_3_);
            }
        } else {
            int i2 = stack.getSizeInventory();
            for (int j2 = 0; j2 < i2 && !side.func_190926_b(); ++j2) {
                side = TileEntityHopper.insertStack(inventoryIn, stack, side, j2, p_174918_3_);
            }
        }
        return side;
    }
    ...
    private static ItemStack insertStack(IInventory inventoryIn, IInventory stack, ItemStack index, int side, EnumFacing p_174916_4_) {
        ...
        if (TileEntityHopper.canInsertItemInSlot(stack, index, side, p_174916_4_)) {
            ...
            if (itemstack.func_190926_b()) {
                ...
                int i2 = index.getMaxStackSize() - itemstack.func_190916_E();
                int j2 = Math.min(index.func_190916_E(), i2);
                index.func_190918_g(j2);
                itemstack.func_190917_f(j2);
                boolean bl2 = flag = j2 > 0;
            }
            if (flag) {
                ...
                if (flag1 && stack instanceof TileEntityHopper && !(tileentityhopper1 = (TileEntityHopper)stack).mayTransfer()) {
                    ...
                    tileentityhopper1.setTransferCooldown(8 - k2);
                }
                stack.markDirty();
            }
        }
    ...
    public static IInventory getInventoryAtPosition(World worldIn, double x2, double y2, double z2) {
        ...
        BlockPos blockpos = new BlockPos(i2, j2 = MathHelper.floor(y2), k2 = MathHelper.floor(z2));
        ...
        if (iinventory == null && !(list = worldIn.getEntitiesInAABBexcluding(null, new AxisAlignedBB(x2 - 0.5, y2 - 0.5, z2 - 0.5, x2 + 0.5, y2 + 0.5, z2 + 0.5), EntitySelectors.HAS_INVENTORY)).isEmpty()) {
