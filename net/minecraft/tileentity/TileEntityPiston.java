...
implements ITickable {
        ...
        if (!list.isEmpty() && !(list1 = this.world.getEntitiesWithinAABBExcludingEntity(null, this.func_190610_a(axisalignedbb = this.func_190607_a(this.func_191515_a(list)), enumfacing, d0).union(axisalignedbb))).isEmpty()) {
            ...
            for (int i2 = 0; i2 < list1.size(); ++i2) {
                ...
                for (int j2 = 0; j2 < list.size(); ++j2) {
                    ...
                    AxisAlignedBB axisalignedbb1 = this.func_190610_a(this.func_190607_a((AxisAlignedBB)list.get(j2)), enumfacing, d0);
                    if (axisalignedbb1.intersectsWith(axisalignedbb2 = entity.getEntityBoundingBox()) && (d1 = Math.max(d1, this.func_190612_a(axisalignedbb1, enumfacing, axisalignedbb2))) >= d0) break;
                }
                if (!(d1 > 0.0)) continue;
    ...
    public void update() {
        ...
        if (this.lastProgress >= 1.0f) {
            ...
            float f2 = this.progress + 0.5f;
            this.moveCollidedEntities(f2);
            this.progress = f2;
    ...
    public void func_190609_a(World p_190609_1_, BlockPos p_190609_2_, AxisAlignedBB p_190609_3_, List<AxisAlignedBB> p_190609_4_, @Nullable Entity p_190609_5_) {
        ...
        if ((double)this.progress >= 1.0 || enumfacing != (this.extending ? this.pistonFacing : this.pistonFacing.getOpposite())) {
            ...
            float f2 = this.getExtendedProgress(this.progress);
            double d0 = (float)this.pistonFacing.getFrontOffsetX() * f2;
            double d1 = (float)this.pistonFacing.getFrontOffsetY() * f2;
            double d2 = (float)this.pistonFacing.getFrontOffsetZ() * f2;
            iblockstate.addCollisionBoxToList(p_190609_1_, p_190609_2_, p_190609_3_.offset(-d0, -d1, -d2), p_190609_4_, p_190609_5_, true);
            for (int j2 = i2; j2 < p_190609_4_.size(); ++j2) {
                p_190609_4_.set(j2, p_190609_4_.get(j2).offset(d0, d1, d2));
            }
        }
    }
}
