...
public abstract class MobSpawnerBaseLogic {
    ...
    private ResourceLocation func_190895_g() {
        String s2 = this.randomEntity.getNbt().getString("id");
        return StringUtils.isNullOrEmpty(s2) ? null : new ResourceLocation(s2);
    }
        ...
        if (!this.isActivated()) {
            ...
            if (this.getSpawnerWorld().isRemote) {
                ...
                for (int i2 = 0; i2 < this.spawnCount; ++i2) {
                    ...
                    Entity entity = AnvilChunkLoader.readWorldEntityPos(nbttagcompound, world, d0 = (j2 = nbttaglist.tagCount()) >= 1 ? nbttaglist.getDoubleAt(0) : (double)blockpos.getX() + (world.rand.nextDouble() - world.rand.nextDouble()) * (double)this.spawnRange + 0.5, d1 = j2 >= 2 ? nbttaglist.getDoubleAt(1) : (double)(blockpos.getY() + world.rand.nextInt(3) - 1), d2 = j2 >= 3 ? nbttaglist.getDoubleAt(2) : (double)blockpos.getZ() + (world.rand.nextDouble() - world.rand.nextDouble()) * (double)this.spawnRange + 0.5, false);
                    if (entity == null) {
                        return;
                    }
                    int k2 = world.getEntitiesWithinAABB(entity.getClass(), new AxisAlignedBB(blockpos.getX(), blockpos.getY(), blockpos.getZ(), blockpos.getX() + 1, blockpos.getY() + 1, blockpos.getZ() + 1).expandXyz(this.spawnRange)).size();
                    if (k2 >= this.maxNearbyEntities) {
                        this.resetTimer();
                        return;
                    }
    ...
    private void resetTimer() {
        if (this.maxSpawnDelay <= this.minSpawnDelay) {
            ...
            this.spawnDelay = this.minSpawnDelay + this.getSpawnerWorld().rand.nextInt(i2);
        }
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if (nbt.hasKey("SpawnPotentials", 9)) {
            ...
            for (int i2 = 0; i2 < nbttaglist.tagCount(); ++i2) {
                this.potentialSpawns.add(new WeightedSpawnerEntity(nbttaglist.getCompoundTagAt(i2)));
            }
        }
