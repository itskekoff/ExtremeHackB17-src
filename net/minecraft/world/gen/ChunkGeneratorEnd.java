...
implements IChunkGenerator {
    ...
    public void setBlocksInChunk(int x2, int z2, ChunkPrimer primer) {
        ...
        int k2 = 33;
        int l2 = 3;
        this.buffer = this.getHeights(this.buffer, x2 * 2, 0, z2 * 2, 3, 33, 3);
                    ...
                    for (int l1 = 0; l1 < 4; ++l1) {
                        ...
                        for (int i22 = 0; i22 < 8; ++i22) {
                            ...
                            for (int j22 = 0; j22 < 8; ++j22) {
                                ...
                                int l22 = l1 + k1 * 4;
                                int i3 = j22 + j1 * 8;
                                primer.setBlockState(k22, l22, i3, iblockstate);
                                d15 += d16;
                            }
                            d10 += d12;
                            d11 += d13;
        ...
        for (int i2 = 0; i2 < 16; ++i2) {
            for (int j2 = 0; j2 < 16; ++j2) {
                ...
                for (int i1 = 127; i1 >= 0; --i1) {
                    IBlockState iblockstate2 = primer.getBlockState(i2, i1, j2);
                    ...
                    if (l2 == -1) {
                        l2 = 1;
                        if (i1 >= 0) {
                            primer.setBlockState(i2, i1, j2, iblockstate);
                            continue;
                        }
                        primer.setBlockState(i2, i1, j2, iblockstate1);
                        continue;
                    }
                    if (l2 <= 0) continue;
                    --l2;
    ...
    public Chunk provideChunk(int x2, int z2) {
        ...
        if (this.mapFeaturesEnabled) {
            this.endCityGen.generate(this.worldObj, x2, z2, chunkprimer);
        }
    ...
    private float getIslandHeightValue(int p_185960_1_, int p_185960_2_, int p_185960_3_, int p_185960_4_) {
        float f2 = p_185960_1_ * 2 + p_185960_3_;
        float f1 = p_185960_2_ * 2 + p_185960_4_;
        float f22 = 100.0f - MathHelper.sqrt(f2 * f2 + f1 * f1) * 8.0f;
        if (f22 > 80.0f) {
            f22 = 80.0f;
        }
        if (f22 < -100.0f) {
        ...
        for (int i2 = -12; i2 <= 12; ++i2) {
            for (int j2 = -12; j2 <= 12; ++j2) {
                long k2 = p_185960_1_ + i2;
                long l2 = p_185960_2_ + j2;
                if (k2 * k2 + l2 * l2 <= 4096L || !(this.islandNoise.getValue(k2, l2) < (double)-0.9f)) continue;
                float f3 = (MathHelper.abs(k2) * 3439.0f + MathHelper.abs(l2) * 147.0f) % 13.0f + 9.0f;
                f2 = p_185960_3_ - i2 * 2;
                f1 = p_185960_4_ - j2 * 2;
                float f4 = 100.0f - MathHelper.sqrt(f2 * f2 + f1 * f1) * f3;
                if (f4 > 80.0f) {
                    f4 = 80.0f;
                }
                if (f4 < -100.0f) {
    ...
    private double[] getHeights(double[] p_185963_1_, int p_185963_2_, int p_185963_3_, int p_185963_4_, int p_185963_5_, int p_185963_6_, int p_185963_7_) {
        ...
        int i2 = p_185963_2_ / 2;
        int j2 = p_185963_4_ / 2;
        int k2 = 0;
        for (int l2 = 0; l2 < p_185963_5_; ++l2) {
            for (int i1 = 0; i1 < p_185963_7_; ++i1) {
                float f2 = this.getIslandHeightValue(i2, j2, l2, i1);
                for (int j1 = 0; j1 < p_185963_6_; ++j1) {
                    double d2 = this.ar[k2] / 512.0;
                    double d3 = this.br[k2] / 512.0;
                    double d5 = (this.pnr[k2] / 10.0 + 1.0) / 2.0;
    ...
    public void populate(int x2, int z2) {
        ...
        if (this.mapFeaturesEnabled) {
            this.endCityGen.generateStructure(this.worldObj, this.rand, new ChunkPos(x2, z2));
        }
        this.worldObj.getBiome(blockpos.add(16, 0, 16)).decorate(this.worldObj, this.worldObj.rand, blockpos);
        long i2 = (long)x2 * (long)x2 + (long)z2 * (long)z2;
        if (i2 > 4096L) {
            float f2 = this.getIslandHeightValue(x2, z2, 1, 1);
            if (f2 < -20.0f && this.rand.nextInt(14) == 0) {
            ...
            if (this.getIslandHeightValue(x2, z2, 1, 1) > 40.0f) {
                int i22;
                int l1;
                int j2;
                int j3 = this.rand.nextInt(5);
                for (int k2 = 0; k2 < j3; ++k2) {
                    int k1;
                    int i1;
                    int l2 = this.rand.nextInt(16) + 8;
                    int j1 = this.worldObj.getHeight(blockpos.add(l2, 0, i1 = this.rand.nextInt(16) + 8)).getY();
                    if (j1 <= 0 || !this.worldObj.isAirBlock(blockpos.add(l2, (k1 = j1 - 1) + 1, i1)) || this.worldObj.getBlockState(blockpos.add(l2, k1, i1)).getBlock() != Blocks.END_STONE) continue;
                    BlockChorusFlower.generatePlant(this.worldObj, blockpos.add(l2, k1 + 1, i1), this.rand, 8);
                }
                if (this.rand.nextInt(700) == 0 && (j2 = this.worldObj.getHeight(blockpos.add(l1 = this.rand.nextInt(16) + 8, 0, i22 = this.rand.nextInt(16) + 8)).getY()) > 0) {
                    int k2 = j2 + 3 + this.rand.nextInt(7);
                    BlockPos blockpos1 = blockpos.add(l1, k2, i22);
