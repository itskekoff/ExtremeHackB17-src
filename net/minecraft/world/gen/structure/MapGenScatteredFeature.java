...
extends MapGenStructure {
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        int k2 = chunkX / this.maxDistanceBetweenScatteredFeatures;
        int l2 = chunkZ / this.maxDistanceBetweenScatteredFeatures;
        Random random = this.worldObj.setRandomSeed(k2, l2, 14357617);
        k2 *= this.maxDistanceBetweenScatteredFeatures;
        l2 *= this.maxDistanceBetweenScatteredFeatures;
        if (i2 == (k2 += random.nextInt(this.maxDistanceBetweenScatteredFeatures - 8)) && j2 == (l2 += random.nextInt(this.maxDistanceBetweenScatteredFeatures - 8))) {
            Biome biome = this.worldObj.getBiomeProvider().getBiome(new BlockPos(i2 * 16 + 8, 0, j2 * 16 + 8));
            if (biome == null) {
                return false;
            }
