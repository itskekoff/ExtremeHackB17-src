...
extends MapGenStructure {
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        int k2 = chunkX / this.distance;
        int l2 = chunkZ / this.distance;
        Random random = this.worldObj.setRandomSeed(k2, l2, 10387312);
        k2 *= this.distance;
        l2 *= this.distance;
        return i2 == (k2 += random.nextInt(this.distance - 8)) && j2 == (l2 += random.nextInt(this.distance - 8)) && (flag = this.worldObj.getBiomeProvider().areBiomesViable(i2 * 16 + 8, j2 * 16 + 8, 0, VILLAGE_SPAWN_BIOMES));
    }
    ...
    extends StructureStart {
        ...
        public Start(World worldIn, Random rand, int x2, int z2, int size) {
            super(x2, z2);
            List<StructureVillagePieces.PieceWeight> list = StructureVillagePieces.getStructureVillageWeightedPieceList(rand, size);
            StructureVillagePieces.Start structurevillagepieces$start = new StructureVillagePieces.Start(worldIn.getBiomeProvider(), 0, rand, (x2 << 4) + 2, (z2 << 4) + 2, list, size);
