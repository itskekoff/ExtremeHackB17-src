...
extends MapGenStructure {
    ...
    protected boolean canSpawnStructureAtCoords(int chunkX, int chunkZ) {
        ...
        int k2 = chunkX / this.spacing;
        int l2 = chunkZ / this.spacing;
        Random random = this.worldObj.setRandomSeed(k2, l2, 10387313);
        k2 *= this.spacing;
        l2 *= this.spacing;
        if (i2 == (k2 += (random.nextInt(this.spacing - this.separation) + random.nextInt(this.spacing - this.separation)) / 2) && j2 == (l2 += (random.nextInt(this.spacing - this.separation) + random.nextInt(this.spacing - this.separation)) / 2)) {
            if (!this.worldObj.getBiomeProvider().areBiomesViable(i2 * 16 + 8, j2 * 16 + 8, 16, SPAWN_BIOMES)) {
                return false;
            }
            boolean flag = this.worldObj.getBiomeProvider().areBiomesViable(i2 * 16 + 8, j2 * 16 + 8, 29, WATER_BIOMES);
            if (flag) {
                return true;
            }
        }
    ...
    extends StructureStart {
        ...
        private void create(World worldIn, Random random, int chunkX, int chunkZ) {
            ...
            long j2 = random.nextLong();
            long k2 = (long)chunkX * i2;
            long l2 = (long)chunkZ * j2;
            random.setSeed(k2 ^ l2 ^ worldIn.getSeed());
