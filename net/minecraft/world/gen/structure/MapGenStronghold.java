...
extends MapGenStructure {
    ...
    public BlockPos getClosestStrongholdPos(World worldIn, BlockPos pos, boolean p_180706_3_) {
        ...
        ChunkPos[] arrchunkPos = this.structureCoords;
        int n2 = this.structureCoords.length;
        for (int i2 = 0; i2 < n2; ++i2) {
            ChunkPos chunkpos = arrchunkPos[i2];
    ...
    private void generatePositions() {
        ...
        for (StructureStart structurestart : this.structureMap.values()) {
            if (i2 >= this.structureCoords.length) continue;
        ...
        int j2 = 0;
        int k2 = 0;
        int l2 = this.structureMap.size();
        if (l2 < this.structureCoords.length) {
            for (int i1 = 0; i1 < this.structureCoords.length; ++i1) {
                double d0 = 4.0 * this.distance + this.distance * (double)j2 * 6.0 + (random.nextDouble() - 0.5) * this.distance * 2.5;
                ...
                if (++k2 != this.spread) continue;
                k2 = 0;
                this.spread += 2 * this.spread / (++j2 + 1);
    ...
    extends StructureStart {
        ...
        public Start(World worldIn, Random random, int chunkX, int chunkZ) {
            ...
            while (!list.isEmpty()) {
                ...
                StructureComponent structurecomponent = list.remove(i2);
