...
extends WorldGenerator {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        int i2 = random.nextInt(FOSSILS.length);
        TemplateManager templatemanager = worldIn.getSaveHandler().getStructureTemplateManager();
        Template template = templatemanager.getTemplate(minecraftserver, FOSSILS[i2]);
        Template template1 = templatemanager.getTemplate(minecraftserver, FOSSILS_COAL[i2]);
        ...
        int j2 = random.nextInt(16 - blockpos.getX());
        int k2 = random.nextInt(16 - blockpos.getZ());
        int l2 = 256;
        for (int i1 = 0; i1 < blockpos.getX(); ++i1) {
            for (int j1 = 0; j1 < blockpos.getX(); ++j1) {
                l2 = Math.min(l2, worldIn.getHeight(position.getX() + i1 + j2, position.getZ() + j1 + k2));
            }
        }
        int k1 = Math.max(l2 - 15 - random.nextInt(10), 10);
        BlockPos blockpos1 = template.getZeroPositionWithTransform(position.add(j2, k1, k2), Mirror.NONE, rotation);
