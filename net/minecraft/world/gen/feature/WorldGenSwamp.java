    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        if (position.getY() >= 1 && position.getY() + i2 + 1 <= 256) {
            for (int j2 = position.getY(); j2 <= position.getY() + 1 + i2; ++j2) {
                ...
                for (int l2 = position.getX() - k2; l2 <= position.getX() + k2 && flag; ++l2) {
                    for (int i1 = position.getZ() - k2; i1 <= position.getZ() + k2 && flag; ++i1) {
                        if (j2 >= 0 && j2 < 256) {
                            IBlockState iblockstate = worldIn.getBlockState(blockpos$mutableblockpos.setPos(l2, j2, i1));
            ...
            if ((block1 == Blocks.GRASS || block1 == Blocks.DIRT) && position.getY() < 256 - i2 - 1) {
                ...
                for (int k1 = position.getY() - 3 + i2; k1 <= position.getY() + i2; ++k1) {
                    int j2 = k1 - (position.getY() + i2);
                    int l2 = 2 - j2 / 2;
                ...
                for (int i22 = position.getY() - 3 + i2; i22 <= position.getY() + i2; ++i22) {
                    ...
                    for (int l3 = position.getX() - i3; l3 <= position.getX() + i3; ++l3) {
                        for (int j4 = position.getZ() - i3; j4 <= position.getZ() + i3; ++j4) {
                            blockpos$mutableblockpos1.setPos(l3, i22, j4);
