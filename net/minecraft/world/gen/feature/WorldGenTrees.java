...
extends WorldGenAbstractTree {
    ...
    public boolean generate(World worldIn, Random rand, BlockPos position) {
        ...
        if (position.getY() >= 1 && position.getY() + i2 + 1 <= 256) {
            ...
            if ((block == Blocks.GRASS || block == Blocks.DIRT || block == Blocks.FARMLAND) && position.getY() < 256 - i2 - 1) {
                ...
                int k2 = 3;
                boolean l2 = false;
                for (int i3 = position.getY() - 3 + i2; i3 <= position.getY() + i2; ++i3) {
                    ...
                    for (int k1 = position.getX() - j1; k1 <= position.getX() + j1; ++k1) {
                        ...
                        for (int i22 = position.getZ() - j1; i22 <= position.getZ() + j1; ++i22) {
                            ...
                            if (Math.abs(l1) == j1 && Math.abs(j2) == j1 && (rand.nextInt(2) == 0 || i4 == 0) || (material = worldIn.getBlockState(blockpos = new BlockPos(k1, i3, i22)).getMaterial()) != Material.AIR && material != Material.LEAVES && material != Material.VINE) continue;
                    ...
                    if (rand.nextInt(5) == 0 && i2 > 5) {
                        for (int l3 = 0; l3 < 2; ++l3) {
                            for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
                                ...
                                this.placeCocoa(worldIn, rand.nextInt(3), position.add(enumfacing1.getFrontOffsetX(), i2 - 5 + l3, enumfacing1.getFrontOffsetZ()), enumfacing);
                            }
                        }
                    }
                }
    ...
    private void addHangingVine(World worldIn, BlockPos pos, PropertyBool prop) {
        ...
        for (int i2 = 4; worldIn.getBlockState(blockpos).getMaterial() == Material.AIR && i2 > 0; --i2) {
