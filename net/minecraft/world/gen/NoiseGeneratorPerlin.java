...
extends NoiseGenerator {
    ...
    public NoiseGeneratorPerlin(Random p_i45470_1_, int p_i45470_2_) {
        ...
        for (int i2 = 0; i2 < p_i45470_2_; ++i2) {
            this.noiseLevels[i2] = new NoiseGeneratorSimplex(p_i45470_1_);
        }
    }
    ...
    public double getValue(double p_151601_1_, double p_151601_3_) {
        ...
        for (int i2 = 0; i2 < this.levels; ++i2) {
            d0 += this.noiseLevels[i2].getValue(p_151601_1_ * d1, p_151601_3_ * d1) / d1;
            d1 /= 2.0;
        }
        return d0;
    }
    ...
    public double[] getRegion(double[] p_151600_1_, double p_151600_2_, double p_151600_4_, int p_151600_6_, int p_151600_7_, double p_151600_8_, double p_151600_10_, double p_151600_12_, double p_151600_14_) {
        if (p_151600_1_ != null && p_151600_1_.length >= p_151600_6_ * p_151600_7_) {
            for (int i2 = 0; i2 < p_151600_1_.length; ++i2) {
                p_151600_1_[i2] = 0.0;
            }
        } else {
        ...
        double d1 = 1.0;
        double d0 = 1.0;
        for (int j2 = 0; j2 < this.levels; ++j2) {
            this.noiseLevels[j2].add(p_151600_1_, p_151600_2_, p_151600_4_, p_151600_6_, p_151600_7_, p_151600_8_ * d0 * d1, p_151600_10_ * d0 * d1, 0.55 / d1);
            d0 *= p_151600_12_;
            d1 *= p_151600_14_;
        }
        return p_151600_1_;
