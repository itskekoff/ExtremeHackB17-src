...
public class LootTable {
    ...
    public List<ItemStack> generateLootForPools(Random rand, LootContext context) {
        ...
        if (context.addLootTable(this)) {
            LootPool[] arrlootPool = this.pools;
            int n2 = this.pools.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                LootPool lootpool = arrlootPool[i2];
    ...
    private void shuffleItems(List<ItemStack> stacks, int p_186463_2_, Random rand) {
        ...
        while ((p_186463_2_ -= stacks.size()) > 0 && !list.isEmpty()) {
            ...
            int i2 = MathHelper.getInt(rand, 1, itemstack2.func_190916_E() / 2);
            ItemStack itemstack1 = itemstack2.splitStack(i2);
    ...
    private List<Integer> getEmptySlotsRandomized(IInventory inventory, Random rand) {
        ...
        for (int i2 = 0; i2 < inventory.getSizeInventory(); ++i2) {
            if (!inventory.getStackInSlot(i2).func_190926_b()) continue;
            list.add(i2);
        }
