...
public class LootTableManager {
    ...
    extends CacheLoader<ResourceLocation, LootTable> {
        ...
        private LootTable loadLootTable(ResourceLocation resource) {
            ...
            File file1 = new File(new File(LootTableManager.this.baseFolder, resource.getResourceDomain()), String.valueOf(resource.getResourcePath()) + ".json");
            if (file1.exists()) {
                if (file1.isFile()) {
                    String s2;
                    try {
                        s2 = Files.toString(file1, StandardCharsets.UTF_8);
                    }
                    ...
                    try {
                        return JsonUtils.gsonDeserialize(GSON_INSTANCE, s2, LootTable.class);
                    }
        ...
        private LootTable loadBuiltinLootTable(ResourceLocation resource) {
            ...
            if (url != null) {
                String s2;
                try {
                    s2 = Resources.toString(url, StandardCharsets.UTF_8);
                }
                catch (IOException ioexception) {
