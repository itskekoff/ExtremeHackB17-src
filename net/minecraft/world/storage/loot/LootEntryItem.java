...
extends LootEntry {
    ...
    public void addLoot(Collection<ItemStack> stacks, Random rand, LootContext context) {
        ...
        LootFunction[] arrlootFunction = this.functions;
        int n2 = this.functions.length;
        for (int i2 = 0; i2 < n2; ++i2) {
            LootFunction lootfunction = arrlootFunction[i2];
        ...
        if (!itemstack.func_190926_b()) {
            if (itemstack.func_190916_E() < this.item.getItemStackLimit()) {
                ...
                for (int i3 = itemstack.func_190916_E(); i3 > 0; i3 -= itemstack1.func_190916_E()) {
                    itemstack1 = itemstack.copy();
                    itemstack1.func_190920_e(Math.min(itemstack.getMaxStackSize(), i3));
