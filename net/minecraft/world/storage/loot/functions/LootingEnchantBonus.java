...
extends LootFunction {
    ...
    public ItemStack apply(ItemStack stack, Random rand, LootContext context) {
        ...
        if (entity instanceof EntityLivingBase) {
            int i2 = EnchantmentHelper.getLootingModifier((EntityLivingBase)entity);
            if (i2 == 0) {
                return stack;
            }
            float f2 = (float)i2 * this.count.generateFloat(rand);
            stack.func_190917_f(Math.round(f2));
    ...
    extends LootFunction.Serializer<LootingEnchantBonus> {
        ...
        public LootingEnchantBonus deserialize(JsonObject object, JsonDeserializationContext deserializationContext, LootCondition[] conditionsIn) {
            int i2 = JsonUtils.getInt(object, "limit", 0);
            return new LootingEnchantBonus(conditionsIn, JsonUtils.deserializeClass(object, "count", deserializationContext, RandomValueRange.class), i2);
        }
    }
}
