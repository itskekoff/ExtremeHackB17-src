...
extends LootFunction {
    ...
    public ItemStack apply(ItemStack stack, Random rand, LootContext context) {
        Modifier[] arrmodifier = this.modifiers;
        int n2 = this.modifiers.length;
        for (int i2 = 0; i2 < n2; ++i2) {
            Modifier setattributes$modifier = arrmodifier[i2];
    ...
    static class Modifier {
        ...
        public JsonObject serialize(JsonSerializationContext context) {
            ...
            if (this.slots.length == 1) {
                ...
                EntityEquipmentSlot[] arrentityEquipmentSlot = this.slots;
                int n2 = this.slots.length;
                for (int i2 = 0; i2 < n2; ++i2) {
                    EntityEquipmentSlot entityequipmentslot = arrentityEquipmentSlot[i2];
        ...
        public static Modifier deserialize(JsonObject jsonObj, JsonDeserializationContext context) {
            ...
            String s2 = JsonUtils.getString(jsonObj, "name");
            String s1 = JsonUtils.getString(jsonObj, "attribute");
            int i2 = Modifier.getOperationFromInt(JsonUtils.getString(jsonObj, "operation"));
            ...
            if (JsonUtils.isString(jsonObj, "slot")) {
                ...
                for (JsonElement jsonelement : jsonarray) {
                    aentityequipmentslot[j2++] = EntityEquipmentSlot.fromString(JsonUtils.getString(jsonelement, "slot"));
                }
            ...
            if (jsonObj.has("id")) {
                String s22 = JsonUtils.getString(jsonObj, "id");
                try {
                    uuid = UUID.fromString(s22);
                }
                catch (IllegalArgumentException var12) {
                    throw new JsonSyntaxException("Invalid attribute modifier id '" + s22 + "' (must be UUID format, with dashes)");
                }
            }
            return new Modifier(s2, s1, i2, randomvaluerange, aentityequipmentslot, uuid);
        }
    ...
    extends LootFunction.Serializer<SetAttributes> {
        ...
        public SetAttributes deserialize(JsonObject object, JsonDeserializationContext deserializationContext, LootCondition[] conditionsIn) {
            ...
            int i2 = 0;
            for (JsonElement jsonelement : jsonarray) {
                asetattributes$modifier[i2++] = Modifier.deserialize(JsonUtils.getJsonObject(jsonelement, "modifier"), deserializationContext);
            }
