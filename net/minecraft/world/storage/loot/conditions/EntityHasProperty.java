...
implements LootCondition {
    ...
    public boolean testCondition(Random rand, LootContext context) {
        ...
        EntityProperty[] arrentityProperty = this.properties;
        int n2 = this.properties.length;
        for (int i2 = 0; i2 < n2; ++i2) {
            EntityProperty entityproperty = arrentityProperty[i2];
    ...
    extends LootCondition.Serializer<EntityHasProperty> {
        ...
        public EntityHasProperty deserialize(JsonObject json, JsonDeserializationContext context) {
            ...
            for (Map.Entry<String, JsonElement> entry : set) {
                aentityproperty[i2++] = EntityPropertyManager.getSerializerForName(new ResourceLocation(entry.getKey())).deserialize(entry.getValue(), context);
            }
