...
public abstract class LootEntry {
    ...
    JsonSerializer<LootEntry> {
        ...
        public LootEntry deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_, JsonDeserializationContext p_deserialize_3_) throws JsonParseException {
            ...
            String s2 = JsonUtils.getString(jsonobject, "type");
            int i2 = JsonUtils.getInt(jsonobject, "weight", 1);
            int j2 = JsonUtils.getInt(jsonobject, "quality", 0);
            ...
            if ("item".equals(s2)) {
                return LootEntryItem.deserialize(jsonobject, p_deserialize_3_, i2, j2, alootcondition);
            }
            if ("loot_table".equals(s2)) {
                return LootEntryTable.deserialize(jsonobject, p_deserialize_3_, i2, j2, alootcondition);
            }
            if ("empty".equals(s2)) {
                return LootEntryEmpty.deserialize(jsonobject, p_deserialize_3_, i2, j2, alootcondition);
            }
            throw new JsonSyntaxException("Unknown loot entry type '" + s2 + "'");
        }
