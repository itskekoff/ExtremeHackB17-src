...
public class LootPool {
    ...
    protected void createLootRoll(Collection<ItemStack> stacks, Random rand, LootContext context) {
        ...
        int i2 = 0;
        LootEntry[] arrlootEntry = this.lootEntries;
        int n2 = this.lootEntries.length;
        for (int i3 = 0; i3 < n2; ++i3) {
            int j2;
            LootEntry lootentry = arrlootEntry[i3];
            if (!LootConditionManager.testAllConditions(lootentry.conditions, rand, context) || (j2 = lootentry.getEffectiveWeight(context.getLuck())) <= 0) continue;
            list.add(lootentry);
            i2 += j2;
        }
        if (i2 != 0 && !list.isEmpty()) {
            ...
            for (LootEntry lootentry1 : list) {
                if ((k2 -= lootentry1.getEffectiveWeight(context.getLuck())) >= 0) continue;
    ...
    public void generateLoot(Collection<ItemStack> stacks, Random rand, LootContext context) {
        if (LootConditionManager.testAllConditions(this.poolConditions, rand, context)) {
            int i2 = this.rolls.generateInt(rand) + MathHelper.floor(this.bonusRolls.generateFloat(rand) * context.getLuck());
