...
public class Teleporter {
    ...
    public void placeInPortal(Entity entityIn, float rotationYaw) {
        if (this.worldServerInstance.provider.getDimensionType().getId() != 1) {
            ...
            int j2 = MathHelper.floor(entityIn.posY) - 1;
            ...
            for (int j1 = -2; j1 <= 2; ++j1) {
                for (int k1 = -2; k1 <= 2; ++k1) {
                    for (int l1 = -1; l1 < 3; ++l1) {
                        int i22 = i2 + k1 * 1 + j1 * 0;
                        int j22 = j2 + l1;
                        int k22 = k2 + k1 * 0 - j1 * 1;
                        boolean flag = l1 < 0;
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw) {
        ...
        long l2 = ChunkPos.asLong(j2, k2);
        if (this.destinationCoordinateCache.containsKey(l2)) {
            PortalPosition teleporter$portalposition = (PortalPosition)this.destinationCoordinateCache.get(l2);
            d0 = 0.0;
        ...
        if (d0 >= 0.0) {
            if (flag) {
                this.destinationCoordinateCache.put(l2, new PortalPosition(blockpos, this.worldServerInstance.getTotalWorldTime()));
            }
    ...
    public boolean makePortal(Entity entityIn) {
        ...
        double d0 = -1.0;
        int j2 = MathHelper.floor(entityIn.posX);
        int k2 = MathHelper.floor(entityIn.posY);
        int l2 = MathHelper.floor(entityIn.posZ);
        int i1 = j2;
        int j1 = k2;
        int k1 = l2;
        int l1 = 0;
        ...
        for (int j22 = j2 - 16; j22 <= j2 + 16; ++j22) {
            double d1 = (double)j22 + 0.5 - entityIn.posX;
            for (int l22 = l2 - 16; l22 <= l2 + 16; ++l22) {
                double d2 = (double)l22 + 0.5 - entityIn.posZ;
                block2: for (int j3 = this.worldServerInstance.getActualHeight() - 1; j3 >= 0; --j3) {
                    if (!this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(j22, j3, l22))) continue;
                    while (j3 > 0 && this.worldServerInstance.isAirBlock(blockpos$mutableblockpos.setPos(j22, j3 - 1, l22))) {
                        --j3;
                    }
                    for (int k3 = i22; k3 < i22 + 4; ++k3) {
                        int l3 = k3 % 2;
                        int i4 = 1 - l3;
                        if (k3 % 4 >= 2) {
                            l3 = -l3;
        ...
        if (d0 < 0.0) {
            for (int l5 = j2 - 16; l5 <= j2 + 16; ++l5) {
                double d3 = (double)l5 + 0.5 - entityIn.posX;
                for (int j6 = l2 - 16; j6 <= l2 + 16; ++j6) {
        ...
        if (d0 < 0.0) {
            k22 = j1 = MathHelper.clamp(j1, 70, this.worldServerInstance.getActualHeight() - 10);
