...
extends SaveFormatOld {
    ...
    public List<WorldSummary> getSaveList() throws AnvilConverterException {
        if (this.savesDirectory != null && this.savesDirectory.exists() && this.savesDirectory.isDirectory()) {
            ...
            File[] arrfile = afile = this.savesDirectory.listFiles();
            int n2 = afile.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                String s2;
                WorldInfo worldinfo;
                File file1 = arrfile[i2];
                if (!file1.isDirectory() || (worldinfo = this.getWorldInfo(s2 = file1.getName())) == null || worldinfo.getSaveVersion() != 19132 && worldinfo.getSaveVersion() != 19133) continue;
                ...
                long i3 = 0L;
                list.add(new WorldSummary(worldinfo, s2, s1, 0L, flag));
            }
            return list;
        }
    ...
    public boolean convertMapFormat(String filename, IProgressUpdate progressCallback) {
        ...
        LOGGER.info("Total conversion count is {}", (Object)i2);
        ...
        this.convertFile(new File(file1, "region"), list, biomeprovider, 0, i2, progressCallback);
        this.convertFile(new File(file2, "region"), list1, new BiomeProviderSingle(Biomes.HELL), list.size(), i2, progressCallback);
        this.convertFile(new File(file3, "region"), list2, new BiomeProviderSingle(Biomes.SKY), list.size() + list1.size(), i2, progressCallback);
    ...
    private void convertFile(File baseFolder, Iterable<File> regionFiles, BiomeProvider p_75813_3_, int p_75813_4_, int p_75813_5_, IProgressUpdate progress) {
        for (File file1 : regionFiles) {
            ...
            progress.setLoadingProgress(i2);
        }
    }
    ...
    private void convertChunks(File baseFolder, File p_75811_2_, BiomeProvider biomeSource, int p_75811_4_, int p_75811_5_, IProgressUpdate progressCallback) {
        try {
            ...
            RegionFile regionfile1 = new RegionFile(new File(baseFolder, String.valueOf(s2.substring(0, s2.length() - ".mcr".length())) + ".mca"));
            for (int i2 = 0; i2 < 32; ++i2) {
                for (int j2 = 0; j2 < 32; ++j2) {
                    if (!regionfile.isChunkSaved(i2, j2) || regionfile1.isChunkSaved(i2, j2)) continue;
