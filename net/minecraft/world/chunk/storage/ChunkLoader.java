...
public class ChunkLoader {
    ...
    public static void convertToAnvilFormat(AnvilConverterData converterData, NBTTagCompound compound, BiomeProvider provider) {
        ...
        for (int i2 = 0; i2 < converterData.heightmap.length; ++i2) {
            aint[i2] = converterData.heightmap[i2];
        }
        ...
        for (int j2 = 0; j2 < 8; ++j2) {
            boolean flag = true;
            for (int k2 = 0; k2 < 16 && flag; ++k2) {
                block3: for (int l2 = 0; l2 < 16 && flag; ++l2) {
            ...
            for (int j3 = 0; j3 < 16; ++j3) {
                for (int l1 = 0; l1 < 16; ++l1) {
                    for (int i2 = 0; i2 < 16; ++i2) {
                        int j22 = j3 << 11 | i2 << 7 | l1 + (j2 << 4);
                        byte k2 = converterData.blocks[j22];
                        abyte1[l1 << 8 | i2 << 4 | j3] = (byte)(k2 & 0xFF);
                        nibblearray.set(j3, l1, i2, converterData.data.get(j3, l1 + (j2 << 4), i2));
                        nibblearray1.set(j3, l1, i2, converterData.skyLight.get(j3, l1 + (j2 << 4), i2));
                        nibblearray2.set(j3, l1, i2, converterData.blockLight.get(j3, l1 + (j2 << 4), i2));
                    }
                }
            }
