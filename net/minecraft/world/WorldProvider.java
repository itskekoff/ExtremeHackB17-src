...
public abstract class WorldProvider {
    ...
    protected void generateLightBrightnessTable() {
        ...
        for (int i2 = 0; i2 <= 15; ++i2) {
            ...
            this.lightBrightnessTable[i2] = (1.0f - f1) / (f1 * 3.0f + 1.0f) * 1.0f + 0.0f;
        }
    }
    ...
    public boolean canCoordinateBeSpawn(int x2, int z2) {
        BlockPos blockpos = new BlockPos(x2, 0, z2);
    ...
    public float calculateCelestialAngle(long worldTime, float partialTicks) {
        int i2 = (int)(worldTime % 24000L);
        float f2 = ((float)i2 + partialTicks) / 24000.0f - 0.25f;
        if (f2 < 0.0f) {
            f2 += 1.0f;
        }
        if (f2 > 1.0f) {
        ...
        float f1 = 1.0f - (float)((Math.cos((double)f2 * Math.PI) + 1.0) / 2.0);
        f2 += (f1 - f2) / 3.0f;
        return f2;
    }
    ...
    public Vec3d getFogColor(float p_76562_1_, float p_76562_2_) {
        float f2 = MathHelper.cos(p_76562_1_ * ((float)Math.PI * 2)) * 2.0f + 0.5f;
        f2 = MathHelper.clamp(f2, 0.0f, 1.0f);
        float f1 = 0.7529412f;
        float f22 = 0.84705883f;
        float f3 = 1.0f;
        return new Vec3d(f1 *= f2 * 0.94f + 0.06f, f22 *= f2 * 0.94f + 0.06f, f3 *= f2 * 0.91f + 0.09f);
    }
