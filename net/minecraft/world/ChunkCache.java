...
implements IBlockAccess {
    ...
    public TileEntity getTileEntity(BlockPos p_190300_1_, Chunk.EnumCreateEntityType p_190300_2_) {
        int i2 = (p_190300_1_.getX() >> 4) - this.chunkX;
        int j2 = (p_190300_1_.getZ() >> 4) - this.chunkZ;
        return this.chunkArray[i2][j2].getTileEntity(p_190300_1_, p_190300_2_);
    }
    ...
    public Biome getBiome(BlockPos pos) {
        int i2 = (pos.getX() >> 4) - this.chunkX;
        int j2 = (pos.getZ() >> 4) - this.chunkZ;
        return this.chunkArray[i2][j2].getBiome(pos, this.worldObj.getBiomeProvider());
    }
        ...
        if (pos.getY() >= 0 && pos.getY() < 256) {
            if (this.getBlockState(pos).useNeighborBrightness()) {
                ...
                for (EnumFacing enumfacing : EnumFacing.values()) {
                    int k2 = this.getLightFor(type, pos.offset(enumfacing));
                    if (k2 > l2) {
                        l2 = k2;
                    }
                    if (l2 < 15) continue;
            ...
            int i2 = (pos.getX() >> 4) - this.chunkX;
            int j2 = (pos.getZ() >> 4) - this.chunkZ;
            return this.chunkArray[i2][j2].getLightFor(type, pos);
        }
