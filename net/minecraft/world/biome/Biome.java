...
public abstract class Biome {
    ...
    @Nullable
    public static Biome getBiomeForId(int id2) {
    ...
    public final float getFloatTemperature(BlockPos pos) {
        if (pos.getY() > 64) {
            float f2 = (float)(TEMPERATURE_NOISE.getValue((float)pos.getX() / 8.0f, (float)pos.getZ() / 8.0f) * 4.0);
            return this.getTemperature() - (f2 + (float)pos.getY() - 64.0f) * 0.05f / 30.0f;
        }
    ...
    public final void generateBiomeTerrain(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x2, int z2, double noiseVal) {
        int i2 = worldIn.getSeaLevel();
        IBlockState iblockstate = this.topBlock;
        IBlockState iblockstate1 = this.fillerBlock;
        int j2 = -1;
        int k2 = (int)(noiseVal / 3.0 + 3.0 + rand.nextDouble() * 0.25);
        int l2 = x2 & 0xF;
        int i1 = z2 & 0xF;
        BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos();
        for (int j1 = 255; j1 >= 0; --j1) {
            if (j1 <= rand.nextInt(5)) {
                chunkPrimerIn.setBlockState(i1, j1, l2, BEDROCK);
                continue;
            }
            ...
            if (j2 == -1) {
                ...
                if (j1 < i2 && (iblockstate == null || iblockstate.getMaterial() == Material.AIR)) {
                    iblockstate = this.getFloatTemperature(blockpos$mutableblockpos.setPos(x2, j1, z2)) < 0.15f ? ICE : WATER;
                }
                j2 = k2;
                if (j1 >= i2 - 1) {
                ...
                if (j1 < i2 - 7 - k2) {
                    iblockstate = AIR;
                    iblockstate1 = STONE;
                    chunkPrimerIn.setBlockState(i1, j1, l2, GRAVEL);
                    continue;
                }
            ...
            if (j2 <= 0) continue;
            chunkPrimerIn.setBlockState(i1, j1, l2, iblockstate1);
            if (--j2 != 0 || iblockstate1.getBlock() != Blocks.SAND || k2 <= 1) continue;
    ...
    @Nullable
    public static Biome getBiome(int id2) {
        return Biome.getBiome(id2, null);
    }
    ...
    private static void registerBiome(int id2, String name, Biome biome) {
        REGISTRY.register(id2, new ResourceLocation(name), biome);
        if (biome.isMutation()) {
