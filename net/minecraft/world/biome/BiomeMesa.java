...
extends Biome {
    ...
    public void genTerrainBlocks(World worldIn, Random rand, ChunkPrimer chunkPrimerIn, int x2, int z2, double noiseVal) {
        ...
        if (this.brycePillars) {
            int i2 = (x2 & 0xFFFFFFF0) + (z2 & 0xF);
            int j2 = (z2 & 0xFFFFFFF0) + (x2 & 0xF);
            double d0 = Math.min(Math.abs(noiseVal), this.pillarNoise.getValue((double)i2 * 0.25, (double)j2 * 0.25));
            if (d0 > 0.0) {
                double d1 = 0.001953125;
                d4 = d0 * d0 * 2.5;
                double d2 = Math.abs(this.pillarRoofNoise.getValue((double)i2 * 0.001953125, (double)j2 * 0.001953125));
        ...
        for (int j1 = 255; j1 >= 0; --j1) {
            ...
            if (l2 == -1) {
                ...
                l2 = k2 + Math.max(0, j1 - i2);
                if (j1 >= i2 - 1) {
                    if (this.hasForest && j1 > 86 + k2 * 2) {
                        if (flag) {
                        ...
                        IBlockState iblockstate2 = j1 >= 64 && j1 <= 127 ? (flag ? HARDENED_CLAY : this.getBand(x2, j1, z2)) : ORANGE_STAINED_HARDENED_CLAY;
                ...
                if (flag1) {
                    ...
                    chunkPrimerIn.setBlockState(l1, j1, k1, this.getBand(x2, j1, z2));
                }
            }
            ++i1;
        }
    ...
    private void generateBands(long p_150619_1_) {
        ...
        for (int i3 = 0; i3 < i2; ++i3) {
            int j2 = random.nextInt(3) + 1;
            int k2 = random.nextInt(64);
            for (int l2 = 0; k2 + l2 < 64 && l2 < j2; ++l2) {
                this.clayBands[k2 + l2] = STAINED_HARDENED_CLAY.withProperty(BlockColored.COLOR, EnumDyeColor.YELLOW);
            }
        }
    ...
    private IBlockState getBand(int p_180629_1_, int p_180629_2_, int p_180629_3_) {
        int i2 = (int)Math.round(this.clayBandsOffsetNoise.getValue((double)p_180629_1_ / 512.0, (double)p_180629_1_ / 512.0) * 2.0);
        return this.clayBands[(p_180629_2_ + i2 + 64) % 64];
    }
