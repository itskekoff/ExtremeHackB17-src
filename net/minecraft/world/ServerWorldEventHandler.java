...
implements IWorldEventListener {
    ...
    public void playSoundToAllNearExcept(@Nullable EntityPlayer player, SoundEvent soundIn, SoundCategory category, double x2, double y2, double z2, float volume, float pitch) {
        this.mcServer.getPlayerList().sendToAllNearExcept(player, x2, y2, z2, volume > 1.0f ? (double)(16.0f * volume) : 16.0, this.theWorldServer.provider.getDimensionType().getId(), new SPacketSoundEffect(soundIn, category, x2, y2, z2, volume, pitch));
    }
