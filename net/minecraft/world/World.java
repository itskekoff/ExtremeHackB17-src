...
implements IBlockAccess {
    ...
    private boolean isAreaLoaded(int xStart, int yStart, int zStart, int xEnd, int yEnd, int zEnd, boolean allowEmpty) {
        if (yEnd >= 0 && yStart < 256) {
            zStart >>= 4;
            zEnd >>= 4;
            for (int i2 = xStart >>= 4; i2 <= (xEnd >>= 4); ++i2) {
    ...
    public void notifyBlockUpdate(BlockPos pos, IBlockState oldState, IBlockState newState, int flags) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).notifyBlockUpdate(this, pos, oldState, newState, flags);
        }
    }
    ...
    public void markBlockRangeForRenderUpdate(int x1, int y1, int z1, int x2, int y2, int z2) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).markBlockRangeForRenderUpdate(x1, y1, z1, x2, y2, z2);
        }
    }
    ...
    public int getHeight(int x2, int z2) {
        int i2 = x2 >= -30000000 && z2 >= -30000000 && x2 < 30000000 && z2 < 30000000 ? (this.isChunkLoaded(x2 >> 4, z2 >> 4, true) ? this.getChunkFromChunkCoords(x2 >> 4, z2 >> 4).getHeightValue(x2 & 0xF, z2 & 0xF) : 0) : this.getSeaLevel() + 1;
        return i2;
    }
    ...
    public void notifyLightSet(BlockPos pos) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).notifyLightSet(pos);
        }
    }
    ...
    public RayTraceResult rayTraceBlocks(Vec3d vec31, Vec3d vec32, boolean stopOnLiquid, boolean ignoreBlockWithoutBoundingBox, boolean returnLastUncollidableBlock) {
        if (!(Double.isNaN(vec31.xCoord) || Double.isNaN(vec31.yCoord) || Double.isNaN(vec31.zCoord))) {
            if (!(Double.isNaN(vec32.xCoord) || Double.isNaN(vec32.yCoord) || Double.isNaN(vec32.zCoord))) {
                ...
                int i2 = MathHelper.floor(vec32.xCoord);
                int j2 = MathHelper.floor(vec32.yCoord);
                int k2 = MathHelper.floor(vec32.zCoord);
                int l2 = MathHelper.floor(vec31.xCoord);
                ...
                while (k1-- >= 0) {
                    ...
                    l2 = MathHelper.floor(vec31.xCoord) - (enumfacing == EnumFacing.EAST ? 1 : 0);
    ...
    public void playSound(@Nullable EntityPlayer player, double x2, double y2, double z2, SoundEvent soundIn, SoundCategory category, float volume, float pitch) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).playSoundToAllNearExcept(player, soundIn, category, x2, y2, z2, volume, pitch);
        }
    }
    ...
    public void playRecord(BlockPos blockPositionIn, @Nullable SoundEvent soundEventIn) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).playRecord(soundEventIn, blockPositionIn);
        }
    }
    ...
    public void func_190523_a(int p_190523_1_, double p_190523_2_, double p_190523_4_, double p_190523_6_, double p_190523_8_, double p_190523_10_, double p_190523_12_, int ... p_190523_14_) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).func_190570_a(p_190523_1_, false, true, p_190523_2_, p_190523_4_, p_190523_6_, p_190523_8_, p_190523_10_, p_190523_12_, p_190523_14_);
        }
    }
    ...
    private void spawnParticle(int particleID, boolean ignoreRange, double xCood, double yCoord, double zCoord, double xSpeed, double ySpeed, double zSpeed, int ... parameters) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).spawnParticle(particleID, ignoreRange, xCood, yCoord, zCoord, xSpeed, ySpeed, zSpeed, parameters);
        }
    }
    ...
    public boolean spawnEntityInWorld(Entity entityIn) {
        ...
        if (!flag && !this.isChunkLoaded(i2, j2, false)) {
            return false;
        }
        if (entityIn instanceof EntityPlayer) {
    ...
    protected void onEntityAdded(Entity entityIn) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).onEntityAdded(entityIn);
        }
    }

    protected void onEntityRemoved(Entity entityIn) {
        for (int i2 = 0; i2 < this.eventListeners.size(); ++i2) {
            this.eventListeners.get(i2).onEntityRemoved(entityIn);
        }
    }
    ...
    private boolean func_191504_a(@Nullable Entity p_191504_1_, AxisAlignedBB p_191504_2_, boolean p_191504_3_, @Nullable List<AxisAlignedBB> p_191504_4_) {
        int i2 = MathHelper.floor(p_191504_2_.minX) - 1;
        int j2 = MathHelper.ceil(p_191504_2_.maxX) + 1;
        int k2 = MathHelper.floor(p_191504_2_.minY) - 1;
        int l2 = MathHelper.ceil(p_191504_2_.maxY) + 1;
    ...
    public int calculateSkylightSubtracted(float partialTicks) {
        ...
        float f1 = 1.0f - (MathHelper.cos(f2 * ((float)Math.PI * 2)) * 2.0f + 0.5f);
    ...
    public float getSunBrightness(float p_72971_1_) {
        ...
        float f1 = 1.0f - (MathHelper.cos(f2 * ((float)Math.PI * 2)) * 2.0f + 0.2f);
        f1 = MathHelper.clamp(f1, 0.0f, 1.0f);
        f1 = 1.0f - f1;
    ...
    public Vec3d getSkyColor(Entity entityIn, float partialTicks) {
        float f10;
        float f2 = this.getCelestialAngle(partialTicks);
        float f1 = MathHelper.cos(f2 * ((float)Math.PI * 2)) * 2.0f + 0.5f;
        f1 = MathHelper.clamp(f1, 0.0f, 1.0f);
        int i2 = MathHelper.floor(entityIn.posX);
        int j2 = MathHelper.floor(entityIn.posY);
        int k2 = MathHelper.floor(entityIn.posZ);
        BlockPos blockpos = new BlockPos(i2, j2, k2);
        Biome biome = this.getBiome(blockpos);
        float f22 = biome.getFloatTemperature(blockpos);
        int l2 = biome.getSkyColorByTemp(f22);
    ...
    public float getStarBrightness(float partialTicks) {
        float f2 = this.getCelestialAngle(partialTicks);
        float f1 = 1.0f - (MathHelper.cos(f2 * ((float)Math.PI * 2)) * 2.0f + 0.25f);
        f1 = MathHelper.clamp(f1, 0.0f, 1.0f);
        return f1 * f1 * 0.5f;
    }
    ...
    public void updateEntities() {
        ...
        for (int k2 = 0; k2 < this.unloadedEntityList.size(); ++k2) {
            ...
            if (!entity1.addedToChunk || !this.isChunkLoaded(j2, k1, true)) continue;
            this.getChunkFromChunkCoords(j2, k1).removeEntity(entity1);
        }
    ...
    public boolean checkNoEntityCollision(AxisAlignedBB bb2, @Nullable Entity entityIn) {
    ...
    public boolean checkBlockCollision(AxisAlignedBB bb2) {
        int j2 = MathHelper.floor(bb2.minX);
        int k2 = MathHelper.ceil(bb2.maxX);
        int l2 = MathHelper.floor(bb2.minY);
        int i3 = MathHelper.ceil(bb2.maxY);
        int j3 = MathHelper.floor(bb2.minZ);
        int k3 = MathHelper.ceil(bb2.maxZ);
        BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain();
    ...
    public boolean containsAnyLiquid(AxisAlignedBB bb2) {
        int j2 = MathHelper.floor(bb2.minX);
        int k2 = MathHelper.ceil(bb2.maxX);
        int l2 = MathHelper.floor(bb2.minY);
        int i3 = MathHelper.ceil(bb2.maxY);
    ...
    public boolean isFlammableWithin(AxisAlignedBB bb2) {
        ...
        int i3 = MathHelper.ceil(bb2.maxY);
        int j3 = MathHelper.floor(bb2.minZ);
        if (this.isAreaLoaded(j2, l2, j3, k2, i3, k3 = MathHelper.ceil(bb2.maxZ), true)) {
            BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain();
            for (int l3 = j2; l3 < k2; ++l3) {
                for (int i4 = l2; i4 < i3; ++i4) {
                    for (int j4 = j3; j4 < k3; ++j4) {
    ...
    public Explosion createExplosion(@Nullable Entity entityIn, double x2, double y2, double z2, float strength, boolean isSmoking) {
    ...
    public float getBlockDensity(Vec3d vec, AxisAlignedBB bb2) {
    ...
    public boolean canBlockFreeze(BlockPos pos, boolean noWaterAdj) {
        ...
        float f2 = biome.getFloatTemperature(pos);
        if (f2 >= 0.15f) {
            return false;
        }
    ...
    public boolean checkLightFor(EnumSkyBlock lightType, BlockPos pos) {
        ...
        while (j2 < k2) {
            ...
            boolean bl2 = flag = k2 < this.lightUpdateBlockList.length - 6;
    ...
    public List<Entity> getEntitiesWithinAABBExcludingEntity(@Nullable Entity entityIn, AxisAlignedBB bb2) {
    ...
    public EntityPlayer getNearestAttackablePlayer(double posX, double posY, double posZ, double maxXZDistance, double maxYDistance, @Nullable Function<EntityPlayer, Double> playerToDouble, @Nullable Predicate<EntityPlayer> p_184150_12_) {
        ...
        for (int j2 = 0; j2 < this.playerEntities.size(); ++j2) {
            ...
            if (entityplayer1.isInvisible()) {
                float f2 = entityplayer1.getArmorVisibility();
                if (f2 < 0.1f) {
                    f2 = 0.1f;
                }
                d2 *= (double)(0.7f * f2);
    ...
    public void playBroadcastSound(int id2, BlockPos pos, int data) {
        for (int j2 = 0; j2 < this.eventListeners.size(); ++j2) {
            this.eventListeners.get(j2).broadcastSound(id2, pos, data);
        }
    }
    ...
    public DifficultyInstance getDifficultyForLocation(BlockPos pos) {
        ...
        return new DifficultyInstance(this.getDifficulty(), this.getWorldTime(), j2, f2);
    }
    ...
    public boolean isSpawnChunk(int x2, int z2) {
        ...
        int j2 = x2 * 16 + 8 - blockpos1.getX();
        int k2 = z2 * 16 + 8 - blockpos1.getZ();
        int l2 = 128;
