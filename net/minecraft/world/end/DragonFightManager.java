...
public class DragonFightManager {
    ...
    public DragonFightManager(WorldServer worldIn, NBTTagCompound compound) {
        ...
        if (compound.hasKey("Gateways", 9)) {
            ...
            for (int i2 = 0; i2 < nbttaglist.tagCount(); ++i2) {
                this.gateways.add(nbttaglist.getIntAt(i2));
            }
        } else {
    ...
    public NBTTagCompound getCompound() {
        ...
        for (int i2 : this.gateways) {
            nbttaglist.appendTag(new NBTTagInt(i2));
        }
    ...
    private boolean hasDragonBeenKilled() {
        for (int i2 = -8; i2 <= 8; ++i2) {
            for (int j2 = -8; j2 <= 8; ++j2) {
                Chunk chunk = this.world.getChunkFromChunkCoords(i2, j2);
    ...
    private BlockPattern.PatternHelper findExitPortal() {
        ...
        for (int l2 = k2 = this.world.getHeight(WorldGenEndPodium.END_PODIUM_LOCATION).getY(); l2 >= 0; --l2) {
            BlockPattern.PatternHelper blockpattern$patternhelper1 = this.portalPattern.match(this.world, new BlockPos(WorldGenEndPodium.END_PODIUM_LOCATION.getX(), l2, WorldGenEndPodium.END_PODIUM_LOCATION.getZ()));
    ...
    private void loadChunks() {
        for (int i2 = -8; i2 <= 8; ++i2) {
            for (int j2 = -8; j2 <= 8; ++j2) {
                this.world.getChunkFromChunkCoords(i2, j2);
            }
        }
    }
    ...
    private void spawnNewGateway() {
        if (!this.gateways.isEmpty()) {
            int i2 = this.gateways.remove(this.gateways.size() - 1);
            int j2 = (int)(96.0 * Math.cos(2.0 * (-Math.PI + 0.15707963267948966 * (double)i2)));
            int k2 = (int)(96.0 * Math.sin(2.0 * (-Math.PI + 0.15707963267948966 * (double)i2)));
            this.generateGateway(new BlockPos(j2, 75, k2));
        }
    }
    ...
    private void respawnDragon(List<EntityEnderCrystal> crystalsIn) {
        if (this.dragonKilled && this.respawnState == null) {
            ...
            while (blockpattern$patternhelper != null) {
                for (int i2 = 0; i2 < this.portalPattern.getPalmLength(); ++i2) {
                    for (int j2 = 0; j2 < this.portalPattern.getThumbLength(); ++j2) {
                        for (int k2 = 0; k2 < this.portalPattern.getFingerLength(); ++k2) {
                            BlockWorldState blockworldstate = blockpattern$patternhelper.translateOffset(i2, j2, k2);
