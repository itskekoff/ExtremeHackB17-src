...
public abstract class PathNavigate {
    ...
    @Nullable
    public final Path getPathToXYZ(double x2, double y2, double z2) {
        return this.getPathToPos(new BlockPos(x2, y2, z2));
    }
    ...
    public Path getPathToPos(BlockPos pos) {
        ...
        Path path = this.pathFinder.findPath((IBlockAccess)chunkcache, this.theEntity, this.targetPos, f2);
    ...
    public Path getPathToEntityLiving(Entity entityIn) {
        ...
        int i2 = (int)(f2 + 16.0f);
        ChunkCache chunkcache = new ChunkCache(this.worldObj, blockpos1.add(-i2, -i2, -i2), blockpos1.add(i2, i2, i2), 0);
        Path path = this.pathFinder.findPath((IBlockAccess)chunkcache, this.theEntity, entityIn, f2);
        this.worldObj.theProfiler.endSection();
        return path;
    }

    public boolean tryMoveToXYZ(double x2, double y2, double z2, double speedIn) {
        return this.setPath(this.getPathToXYZ(x2, y2, z2), speedIn);
    }
    ...
    protected void pathFollow() {
        ...
        for (int j2 = this.currentPath.getCurrentPathIndex(); j2 < this.currentPath.getCurrentPathLength(); ++j2) {
            if ((double)this.currentPath.getPathPointFromIndex((int)j2).yCoord == Math.floor(vec3d.yCoord)) continue;
            i2 = j2;
            break;
        }
        ...
        int k2 = MathHelper.ceil(this.theEntity.width);
        int l2 = MathHelper.ceil(this.theEntity.height);
        int i1 = k2;
        for (int j1 = i2 - 1; j1 >= this.currentPath.getCurrentPathIndex(); --j1) {
            if (!this.isDirectPathBetweenPoints(vec3d, this.currentPath.getVectorFromIndex(this.theEntity, j1), k2, l2, i1)) continue;
    ...
    protected void checkForStuck(Vec3d positionVec3) {
        ...
        if (this.currentPath != null && !this.currentPath.isFinished()) {
            ...
            if (vec3d.equals(this.timeoutCachedNode)) {
                ...
                double d2 = this.timeoutLimit = this.theEntity.getAIMoveSpeed() > 0.0f ? d0 / (double)this.theEntity.getAIMoveSpeed() * 1000.0 : 0.0;
            }
    ...
    protected void removeSunnyPath() {
        if (this.currentPath != null) {
            for (int i2 = 0; i2 < this.currentPath.getCurrentPathLength(); ++i2) {
                ...
                PathPoint pathpoint1 = i2 + 1 < this.currentPath.getCurrentPathLength() ? this.currentPath.getPathPointFromIndex(i2 + 1) : null;
                ...
                this.currentPath.setPoint(i2, pathpoint.cloneMove(pathpoint.xCoord, pathpoint.yCoord + 1, pathpoint.zCoord));
                if (pathpoint1 == null || pathpoint.yCoord < pathpoint1.yCoord) continue;
                this.currentPath.setPoint(i2 + 1, pathpoint1.cloneMove(pathpoint1.xCoord, pathpoint.yCoord + 1, pathpoint1.zCoord));
            }
        }
    }
