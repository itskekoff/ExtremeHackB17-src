...
extends NodeProcessor {
    ...
    public PathPoint getStart() {
        ...
        if (this.getCanSwim() && this.entity.isInWater()) {
            i2 = (int)this.entity.getEntityBoundingBox().minY;
            BlockPos.MutableBlockPos blockpos$mutableblockpos = new BlockPos.MutableBlockPos(MathHelper.floor(this.entity.posX), i2, MathHelper.floor(this.entity.posZ));
            Block block = this.blockaccess.getBlockState(blockpos$mutableblockpos).getBlock();
            while (block == Blocks.FLOWING_WATER || block == Blocks.WATER) {
                blockpos$mutableblockpos.setPos(MathHelper.floor(this.entity.posX), ++i2, MathHelper.floor(this.entity.posZ));
                block = this.blockaccess.getBlockState(blockpos$mutableblockpos).getBlock();
            }
        } else if (this.entity.onGround) {
            i2 = MathHelper.floor(this.entity.getEntityBoundingBox().minY + 0.5);
        } else {
        ...
        PathNodeType pathnodetype1 = this.getPathNodeType(this.entity, blockpos2.getX(), i2, blockpos2.getZ());
        if (this.entity.getPathPriority(pathnodetype1) < 0.0f) {
            HashSet<BlockPos> set = Sets.newHashSet();
            set.add(new BlockPos(this.entity.getEntityBoundingBox().minX, (double)i2, this.entity.getEntityBoundingBox().minZ));
            set.add(new BlockPos(this.entity.getEntityBoundingBox().minX, (double)i2, this.entity.getEntityBoundingBox().maxZ));
            set.add(new BlockPos(this.entity.getEntityBoundingBox().maxX, (double)i2, this.entity.getEntityBoundingBox().minZ));
            set.add(new BlockPos(this.entity.getEntityBoundingBox().maxX, (double)i2, this.entity.getEntityBoundingBox().maxZ));
        ...
        return this.openPoint(blockpos2.getX(), i2, blockpos2.getZ());
    }

    @Override
    public PathPoint getPathPointToCoords(double x2, double y2, double z2) {
        return this.openPoint(MathHelper.floor(x2), MathHelper.floor(y2), MathHelper.floor(z2));
    }
    ...
    public int findPathOptions(PathPoint[] pathOptions, PathPoint currentPoint, PathPoint targetPoint, float maxDistance) {
        ...
        if (this.entity.getPathPriority(pathnodetype) >= 0.0f) {
            j2 = MathHelper.floor(Math.max(1.0f, this.entity.stepHeight));
        }
        ...
        PathPoint pathpoint = this.getSafePoint(currentPoint.xCoord, currentPoint.yCoord, currentPoint.zCoord + 1, j2, d0, EnumFacing.SOUTH);
        PathPoint pathpoint1 = this.getSafePoint(currentPoint.xCoord - 1, currentPoint.yCoord, currentPoint.zCoord, j2, d0, EnumFacing.WEST);
        PathPoint pathpoint2 = this.getSafePoint(currentPoint.xCoord + 1, currentPoint.yCoord, currentPoint.zCoord, j2, d0, EnumFacing.EAST);
        PathPoint pathpoint3 = this.getSafePoint(currentPoint.xCoord, currentPoint.yCoord, currentPoint.zCoord - 1, j2, d0, EnumFacing.NORTH);
        ...
        boolean bl2 = flag3 = pathpoint1 == null || pathpoint1.nodeType == PathNodeType.OPEN || pathpoint1.costMalus != 0.0f;
        if (flag && flag3 && (pathpoint4 = this.getSafePoint(currentPoint.xCoord - 1, currentPoint.yCoord, currentPoint.zCoord - 1, j2, d0, EnumFacing.NORTH)) != null && !pathpoint4.visited && pathpoint4.distanceTo(targetPoint) < maxDistance) {
            pathOptions[i2++] = pathpoint4;
        }
        if (flag && flag2 && (pathpoint5 = this.getSafePoint(currentPoint.xCoord + 1, currentPoint.yCoord, currentPoint.zCoord - 1, j2, d0, EnumFacing.NORTH)) != null && !pathpoint5.visited && pathpoint5.distanceTo(targetPoint) < maxDistance) {
            pathOptions[i2++] = pathpoint5;
        }
        if (flag1 && flag3 && (pathpoint6 = this.getSafePoint(currentPoint.xCoord - 1, currentPoint.yCoord, currentPoint.zCoord + 1, j2, d0, EnumFacing.SOUTH)) != null && !pathpoint6.visited && pathpoint6.distanceTo(targetPoint) < maxDistance) {
            pathOptions[i2++] = pathpoint6;
        }
        if (flag1 && flag2 && (pathpoint7 = this.getSafePoint(currentPoint.xCoord + 1, currentPoint.yCoord, currentPoint.zCoord + 1, j2, d0, EnumFacing.SOUTH)) != null && !pathpoint7.visited && pathpoint7.distanceTo(targetPoint) < maxDistance) {
            pathOptions[i2++] = pathpoint7;
        }
        return i2;
    }

    @Nullable
    private PathPoint getSafePoint(int x2, int y2, int z2, int p_186332_4_, double p_186332_5_, EnumFacing facing) {
        PathPoint pathpoint = null;
        BlockPos blockpos = new BlockPos(x2, y2, z2);
        BlockPos blockpos1 = blockpos.down();
        double d0 = (double)y2 - (1.0 - this.blockaccess.getBlockState((BlockPos)blockpos1).getBoundingBox((IBlockAccess)this.blockaccess, (BlockPos)blockpos1).maxY);
        if (d0 - p_186332_5_ > 1.125) {
            return null;
        }
        PathNodeType pathnodetype = this.getPathNodeType(this.entity, x2, y2, z2);
        float f2 = this.entity.getPathPriority(pathnodetype);
        double d1 = (double)this.entity.width / 2.0;
        if (f2 >= 0.0f) {
            pathpoint = this.openPoint(x2, y2, z2);
            pathpoint.nodeType = pathnodetype;
            pathpoint.costMalus = Math.max(pathpoint.costMalus, f2);
        }
        if (pathnodetype == PathNodeType.WALKABLE) {
            return pathpoint;
        }
        if (pathpoint == null && p_186332_4_ > 0 && pathnodetype != PathNodeType.FENCE && pathnodetype != PathNodeType.TRAPDOOR && (pathpoint = this.getSafePoint(x2, y2 + 1, z2, p_186332_4_ - 1, p_186332_5_, facing)) != null && (pathpoint.nodeType == PathNodeType.OPEN || pathpoint.nodeType == PathNodeType.WALKABLE) && this.entity.width < 1.0f) {
            double d2 = (double)(x2 - facing.getFrontOffsetX()) + 0.5;
            double d3 = (double)(z2 - facing.getFrontOffsetZ()) + 0.5;
            AxisAlignedBB axisalignedbb = new AxisAlignedBB(d2 - d1, (double)y2 + 0.001, d3 - d1, d2 + d1, (float)y2 + this.entity.height, d3 + d1);
        ...
        if (pathnodetype == PathNodeType.OPEN) {
            ...
            AxisAlignedBB axisalignedbb3 = new AxisAlignedBB((double)x2 - d1 + 0.5, (double)y2 + 0.001, (double)z2 - d1 + 0.5, (double)x2 + d1 + 0.5, (float)y2 + this.entity.height, (double)z2 + d1 + 0.5);
            if (this.entity.world.collidesWithAnyBlock(axisalignedbb3)) {
                return null;
            }
            if (this.entity.width >= 1.0f && (pathnodetype1 = this.getPathNodeType(this.entity, x2, y2 - 1, z2)) == PathNodeType.BLOCKED) {
                pathpoint = this.openPoint(x2, y2, z2);
            ...
            int i2 = 0;
            while (y2 > 0 && pathnodetype == PathNodeType.OPEN) {
                --y2;
                if (i2++ >= this.entity.getMaxFallHeight()) {
                    return null;
                }
                pathnodetype = this.getPathNodeType(this.entity, x2, y2, z2);
                f2 = this.entity.getPathPriority(pathnodetype);
                if (pathnodetype != PathNodeType.OPEN && f2 >= 0.0f) {
                    pathpoint = this.openPoint(x2, y2, z2);
                    pathpoint.nodeType = pathnodetype;
                    pathpoint.costMalus = Math.max(pathpoint.costMalus, f2);
                    break;
    ...
    public PathNodeType getPathNodeType(IBlockAccess blockaccessIn, int x2, int y2, int z2, EntityLiving entitylivingIn, int xSize, int ySize, int zSize, boolean canBreakDoorsIn, boolean canEnterDoorsIn) {
        ...
        pathnodetype = this.func_193577_a(blockaccessIn, x2, y2, z2, xSize, ySize, zSize, canBreakDoorsIn, canEnterDoorsIn, enumset, pathnodetype, blockpos);
    ...
    public PathNodeType func_193577_a(IBlockAccess p_193577_1_, int p_193577_2_, int p_193577_3_, int p_193577_4_, int p_193577_5_, int p_193577_6_, int p_193577_7_, boolean p_193577_8_, boolean p_193577_9_, EnumSet<PathNodeType> p_193577_10_, PathNodeType p_193577_11_, BlockPos p_193577_12_) {
        for (int i2 = 0; i2 < p_193577_5_; ++i2) {
            for (int j2 = 0; j2 < p_193577_6_; ++j2) {
                for (int k2 = 0; k2 < p_193577_7_; ++k2) {
                    int l2 = i2 + p_193577_2_;
                    int i1 = j2 + p_193577_3_;
                    int j1 = k2 + p_193577_4_;
                    PathNodeType pathnodetype = this.getPathNodeType(p_193577_1_, l2, i1, j1);
    ...
    private PathNodeType getPathNodeType(EntityLiving entitylivingIn, int x2, int y2, int z2) {
        return this.getPathNodeType(this.blockaccess, x2, y2, z2, entitylivingIn, this.entitySizeX, this.entitySizeY, this.entitySizeZ, this.getCanBreakDoors(), this.getCanEnterDoors());
    }

    @Override
    public PathNodeType getPathNodeType(IBlockAccess blockaccessIn, int x2, int y2, int z2) {
        PathNodeType pathnodetype = this.getPathNodeTypeRaw(blockaccessIn, x2, y2, z2);
        if (pathnodetype == PathNodeType.OPEN && y2 >= 1) {
            Block block = blockaccessIn.getBlockState(new BlockPos(x2, y2 - 1, z2)).getBlock();
            PathNodeType pathnodetype1 = this.getPathNodeTypeRaw(blockaccessIn, x2, y2 - 1, z2);
        ...
        pathnodetype = this.func_193578_a(blockaccessIn, x2, y2, z2, pathnodetype);
        return pathnodetype;
    }
        ...
        if (p_193578_5_ == PathNodeType.WALKABLE) {
            for (int i2 = -1; i2 <= 1; ++i2) {
                for (int j2 = -1; j2 <= 1; ++j2) {
                    ...
                    Block block = p_193578_1_.getBlockState(blockpos$pooledmutableblockpos.setPos(i2 + p_193578_2_, p_193578_3_, j2 + p_193578_4_)).getBlock();
