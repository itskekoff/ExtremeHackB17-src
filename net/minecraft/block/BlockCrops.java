...
implements IGrowable {
    ...
    public IBlockState withAge(int age2) {
        return this.getDefaultState().withProperty(this.getAgeProperty(), age2);
    }
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (worldIn.getLightFromNeighbors(pos.up()) >= 9 && (i2 = this.getAge(state)) < this.getMaxAge() && rand.nextInt((int)(25.0f / (f2 = BlockCrops.getGrowthChance(this, worldIn, pos))) + 1) == 0) {
            worldIn.setBlockState(pos, this.withAge(i2 + 1), 2);
        }
    }

    public void grow(World worldIn, BlockPos pos, IBlockState state) {
        int j2;
        int i2 = this.getAge(state) + this.getBonemealAgeIncrease(worldIn);
        if (i2 > (j2 = this.getMaxAge())) {
            i2 = j2;
        }
        worldIn.setBlockState(pos, this.withAge(i2), 2);
    }
    ...
    public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        ...
        if (!worldIn.isRemote && (i2 = this.getAge(state)) >= this.getMaxAge()) {
            int j2 = 3 + fortune;
            for (int k2 = 0; k2 < j2; ++k2) {
                if (worldIn.rand.nextInt(2 * this.getMaxAge()) > i2) continue;
