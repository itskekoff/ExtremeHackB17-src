...
extends Block {
    ...
    public void dropBlockAsItemWithChance(World worldIn, BlockPos pos, IBlockState state, float chance, int fortune) {
        ...
        if (this.getItemDropped(state, worldIn.rand, fortune) != Item.getItemFromBlock(this)) {
            int i2 = 1 + worldIn.rand.nextInt(5);
    ...
    private void spawnParticles(World worldIn, BlockPos pos) {
        ...
        for (int i2 = 0; i2 < 6; ++i2) {
            ...
            if (i2 == 0 && !worldIn.getBlockState(pos.up()).isOpaqueCube()) {
                d2 = (double)pos.getY() + 0.0625 + 1.0;
            }
            if (i2 == 1 && !worldIn.getBlockState(pos.down()).isOpaqueCube()) {
                d2 = (double)pos.getY() - 0.0625;
            }
            if (i2 == 2 && !worldIn.getBlockState(pos.south()).isOpaqueCube()) {
                d3 = (double)pos.getZ() + 0.0625 + 1.0;
            }
            if (i2 == 3 && !worldIn.getBlockState(pos.north()).isOpaqueCube()) {
                d3 = (double)pos.getZ() - 0.0625;
            }
            if (i2 == 4 && !worldIn.getBlockState(pos.east()).isOpaqueCube()) {
                d1 = (double)pos.getX() + 0.0625 + 1.0;
            }
            if (i2 == 5 && !worldIn.getBlockState(pos.west()).isOpaqueCube()) {
