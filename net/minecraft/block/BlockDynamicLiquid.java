...
extends BlockLiquid {
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        ...
        if (this.canFlowInto(worldIn, pos.down(), iblockstate1)) {
            ...
            if (i2 >= 8) {
                this.tryFlowInto(worldIn, pos.down(), iblockstate1, i2);
            } else {
                this.tryFlowInto(worldIn, pos.down(), iblockstate1, i2 + 8);
            }
        } else if (i2 >= 0 && (i2 == 0 || this.isBlocked(worldIn, pos.down(), iblockstate1))) {
    ...
    private int getSlopeDistance(World worldIn, BlockPos pos, int distance, EnumFacing calculateFlowCost) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            if (distance >= this.getSlopeFindDistance(worldIn) || (j2 = this.getSlopeDistance(worldIn, blockpos, distance + 1, enumfacing.getOpposite())) >= i2) continue;
            i2 = j2;
        }
        return i2;
    }
    ...
    private Set<EnumFacing> getPossibleFlowDirections(World worldIn, BlockPos pos) {
        ...
        for (EnumFacing enumfacing : EnumFacing.Plane.HORIZONTAL) {
            ...
            int j2 = this.isBlocked(worldIn, blockpos.down(), worldIn.getBlockState(blockpos.down())) ? this.getSlopeDistance(worldIn, blockpos, 1, enumfacing.getOpposite()) : 0;
            if (j2 < i2) {
                set.clear();
            }
            if (j2 > i2) continue;
    ...
    protected int checkAdjacentBlock(World worldIn, BlockPos pos, int currentMinLevel) {
        int i2 = this.getDepth(worldIn.getBlockState(pos));
        if (i2 < 0) {
