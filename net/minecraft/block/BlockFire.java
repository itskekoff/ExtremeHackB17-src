...
extends Block {
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (worldIn.getGameRules().getBoolean("doFireTick")) {
            ...
            boolean bl2 = flag = (block = worldIn.getBlockState(pos.down()).getBlock()) == Blocks.NETHERRACK || block == Blocks.MAGMA;
            if (worldIn.provider instanceof WorldProviderEnd && block == Blocks.BEDROCK) {
                flag = true;
            }
            int i2 = state.getValue(AGE);
            if (!flag && worldIn.isRaining() && this.canDie(worldIn, pos) && rand.nextFloat() < 0.2f + (float)i2 * 0.03f) {
                worldIn.setBlockToAir(pos);
            } else {
                if (i2 < 15) {
                ...
                if (!flag) {
                    if (!this.canNeighborCatchFire(worldIn, pos)) {
                        if (!worldIn.getBlockState(pos.down()).isFullyOpaque() || i2 > 3) {
                            worldIn.setBlockToAir(pos);
                        }
                        return;
                    }
