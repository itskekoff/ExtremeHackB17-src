...
extends Block {
    ...
    private static int getAABBIndex(IBlockState state) {
        ...
        boolean bl2 = flag3 = state.getValue(WEST) != EnumAttachPosition.NONE;
        if (flag || flag2 && !flag && !flag1 && !flag3) {
            i2 |= 1 << EnumFacing.NORTH.getHorizontalIndex();
        }
        if (flag1 || flag3 && !flag && !flag1 && !flag2) {
            i2 |= 1 << EnumFacing.EAST.getHorizontalIndex();
        }
        if (flag2 || flag && !flag1 && !flag2 && !flag3) {
            i2 |= 1 << EnumFacing.SOUTH.getHorizontalIndex();
        }
        if (flag3 || flag1 && !flag && !flag2 && !flag3) {
            i2 |= 1 << EnumFacing.WEST.getHorizontalIndex();
        }
        return i2;
    }
    ...
    private EnumAttachPosition getAttachPosition(IBlockAccess worldIn, BlockPos pos, EnumFacing direction) {
        ...
        if (!(BlockRedstoneWire.canConnectTo(worldIn.getBlockState(blockpos), direction) || !iblockstate.isNormalCube() && BlockRedstoneWire.canConnectUpwardsTo(worldIn.getBlockState(blockpos.down())))) {
            ...
            if (!iblockstate1.isNormalCube()) {
                boolean flag;
                boolean bl2 = flag = worldIn.getBlockState(blockpos).isFullyOpaque() || worldIn.getBlockState(blockpos).getBlock() == Blocks.GLOWSTONE;
    ...
    public int getWeakPower(IBlockState blockState, IBlockAccess blockAccess, BlockPos pos, EnumFacing side) {
        ...
        int i2 = blockState.getValue(POWER);
        if (i2 == 0) {
            return 0;
        }
    ...
    public static int colorMultiplier(int p_176337_0_) {
        float f2 = (float)p_176337_0_ / 15.0f;
        ...
        int i2 = MathHelper.clamp((int)(f1 * 255.0f), 0, 255);
        int j2 = MathHelper.clamp((int)(f22 * 255.0f), 0, 255);
        int k2 = MathHelper.clamp((int)(f3 * 255.0f), 0, 255);
        return 0xFF000000 | i2 << 16 | j2 << 8 | k2;
    }

    @Override
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        int i2 = stateIn.getValue(POWER);
        if (i2 != 0) {
