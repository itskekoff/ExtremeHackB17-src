...
extends BlockContainer {
    ...
    protected void dispense(World worldIn, BlockPos pos) {
        ...
        if (tileentitydispenser != null) {
            int i2 = tileentitydispenser.getDispenseSlot();
            if (i2 < 0) {
                worldIn.playEvent(1001, pos, 0);
            } else {
                ItemStack itemstack = tileentitydispenser.getStackInSlot(i2);
                IBehaviorDispenseItem ibehaviordispenseitem = this.getBehavior(itemstack);
                if (ibehaviordispenseitem != IBehaviorDispenseItem.DEFAULT_BEHAVIOR) {
                    tileentitydispenser.setInventorySlotContents(i2, ibehaviordispenseitem.dispense(blocksourceimpl, itemstack));
                }
            }
        }
    }
