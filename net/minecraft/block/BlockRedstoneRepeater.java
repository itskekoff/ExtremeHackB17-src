...
extends BlockRedstoneDiode {
    ...
    public void randomDisplayTick(IBlockState stateIn, World worldIn, BlockPos pos, Random rand) {
        if (this.isRepeaterPowered) {
            ...
            float f2 = -5.0f;
            if (rand.nextBoolean()) {
                f2 = stateIn.getValue(DELAY) * 2 - 1;
            }
            double d3 = (f2 /= 16.0f) * (float)enumfacing.getFrontOffsetX();
            double d4 = f2 * (float)enumfacing.getFrontOffsetZ();
    ...
    public int getMetaFromState(IBlockState state) {
        ...
        return i2 |= state.getValue(DELAY) - 1 << 2;
    }
