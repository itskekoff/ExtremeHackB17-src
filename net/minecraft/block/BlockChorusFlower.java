...
extends Block {
    ...
    public void updateTick(World worldIn, BlockPos pos, IBlockState state, Random rand) {
        if (!this.canSurvive(worldIn, pos)) {
            ...
            if (worldIn.isAirBlock(blockpos) && blockpos.getY() < 256 && (i2 = state.getValue(AGE).intValue()) < 5 && rand.nextInt(1) == 0) {
                boolean flag = false;
                boolean flag1 = false;
                ...
                if (block == Blocks.END_STONE) {
                    ...
                    for (int k2 = 0; k2 < 4; ++k2) {
                        Block block1 = worldIn.getBlockState(pos.down(j2 + 1)).getBlock();
                ...
                if (flag && BlockChorusFlower.areAllNeighborsEmpty(worldIn, blockpos, null) && worldIn.isAirBlock(pos.up(2))) {
                    ...
                    this.placeGrownFlower(worldIn, blockpos, i2);
                } else if (i2 < 4) {
                    int l2 = rand.nextInt(4);
                    boolean flag2 = false;
                    if (flag1) {
                        ++l2;
                    }
                    for (int j1 = 0; j1 < l2; ++j1) {
                        ...
                        this.placeGrownFlower(worldIn, blockpos1, i2 + 1);
                        flag2 = true;
                    }
                    if (flag2) {
                        worldIn.setBlockState(pos, Blocks.CHORUS_PLANT.getDefaultState(), 2);
    ...
    private static void growTreeRecursive(World worldIn, BlockPos p_185601_1_, Random rand, BlockPos p_185601_3_, int p_185601_4_, int p_185601_5_) {
        int i2 = rand.nextInt(4) + 1;
        if (p_185601_5_ == 0) {
            ++i2;
        }
        for (int j2 = 0; j2 < i2; ++j2) {
            BlockPos blockpos = p_185601_1_.up(j2 + 1);
        ...
        if (p_185601_5_ < 4) {
            ...
            for (int k2 = 0; k2 < l2; ++k2) {
                EnumFacing enumfacing = EnumFacing.Plane.HORIZONTAL.random(rand);
                BlockPos blockpos1 = p_185601_1_.up(i2).offset(enumfacing);
        ...
        if (!flag) {
            worldIn.setBlockState(p_185601_1_.up(i2), Blocks.CHORUS_FLOWER.getDefaultState().withProperty(AGE, 5), 2);
        }
    }
