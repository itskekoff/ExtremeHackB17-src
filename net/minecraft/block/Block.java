...
public class Block {
    ...
    public static Block getBlockById(int id2) {
        return REGISTRY.getObjectById(id2);
    }
    ...
    public int getPackedLightmapCoords(IBlockState state, IBlockAccess source, BlockPos pos) {
        int i2 = source.getCombinedLight(pos, state.getLightValue());
        if (i2 == 0 && state.getBlock() instanceof BlockSlab) {
            pos = pos.down();
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean p_185477_7_) {
        this.addCollisionBoxToList(pos, entityBox, collidingBoxes, state.getCollisionBoundingBox(worldIn, pos));
    }

    public void addCollisionBoxToList(BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable AxisAlignedBB blockBox) {
        if (blockBox != NULL_AABB) {
            AxisAlignedBB axisalignedbb = blockBox.offset(pos);
            EventBB event = new EventBB(this, pos, axisalignedbb);
    ...
    public float getPlayerRelativeBlockHardness(IBlockState state, EntityPlayer player, World worldIn, BlockPos pos) {
        float f2 = state.getBlockHardness(worldIn, pos);
        if (f2 < 0.0f) {
            return 0.0f;
        }
        return !player.canHarvestBlock(state) ? player.getDigSpeed(state) / f2 / 100.0f : player.getDigSpeed(state) / f2 / 30.0f;
    }
    ...
    public void harvestBlock(World worldIn, EntityPlayer player, BlockPos pos, IBlockState state, @Nullable TileEntity te2, ItemStack stack) {
        ...
        if (this.canSilkHarvest() && EnchantmentHelper.getEnchantmentLevel(Enchantments.SILK_TOUCH, stack) > 0) {
            ...
            int i2 = EnchantmentHelper.getEnchantmentLevel(Enchantments.FORTUNE, stack);
    ...
    @Deprecated
    public boolean eventReceived(IBlockState state, World worldIn, BlockPos pos, int id2, int param) {
        return false;
    }
    ...
    public Vec3d func_190949_e(IBlockState p_190949_1_, IBlockAccess p_190949_2_, BlockPos p_190949_3_) {
        ...
        long i2 = MathHelper.getCoordinateRandom(p_190949_3_.getX(), 0, p_190949_3_.getZ());
        return new Vec3d(((double)((float)(i2 >> 16 & 0xFL) / 15.0f) - 0.5) * 0.5, block$enumoffsettype == EnumOffsetType.XYZ ? ((double)((float)(i2 >> 20 & 0xFL) / 15.0f) - 1.0) * 0.2 : 0.0, ((double)((float)(i2 >> 24 & 0xFL) / 15.0f) - 0.5) * 0.5);
    }
    ...
    public static void registerBlocks() {
        ...
        for (Block block15 : REGISTRY) {
            ...
            boolean bl2 = flag5 = block15.lightOpacity == 0;
        ...
        for (Block block16 : REGISTRY) {
            if (set.contains(block16)) {
                for (int i2 = 0; i2 < 15; ++i2) {
                    int j2 = REGISTRY.getIDForObject(block16) << 4 | i2;
                    BLOCK_STATE_IDS.put(block16.getStateFromMeta(i2), j2);
                }
                continue;
            }
            for (IBlockState iblockstate : block16.getBlockState().getValidStates()) {
                int k2 = REGISTRY.getIDForObject(block16) << 4 | block16.getMetaFromState(iblockstate);
                BLOCK_STATE_IDS.put(iblockstate, k2);
            }
        }
    }

    private static void registerBlock(int id2, ResourceLocation textualID, Block block_) {
        REGISTRY.register(id2, textualID, block_);
    }
