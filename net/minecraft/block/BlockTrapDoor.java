...
extends Block {
    ...
    protected void playSound(@Nullable EntityPlayer player, World worldIn, BlockPos pos, boolean p_185731_4_) {
        if (p_185731_4_) {
            int i2 = this.blockMaterial == Material.IRON ? 1037 : 1007;
            worldIn.playEvent(player, i2, pos, 0);
        } else {
            int j2 = this.blockMaterial == Material.IRON ? 1036 : 1013;
            worldIn.playEvent(player, j2, pos, 0);
        }
    }
    ...
    public int getMetaFromState(IBlockState state) {
        int i2 = 0;
        i2 |= BlockTrapDoor.getMetaForFacing(state.getValue(FACING));
        if (state.getValue(OPEN).booleanValue()) {
            i2 |= 4;
        }
