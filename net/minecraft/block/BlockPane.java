...
extends Block {
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean p_185477_7_) {
        ...
        this.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BY_INDEX[0]);
        if (state.getValue(NORTH).booleanValue()) {
            this.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BY_INDEX[BlockPane.getBoundingBoxIndex(EnumFacing.NORTH)]);
        }
        if (state.getValue(SOUTH).booleanValue()) {
            this.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BY_INDEX[BlockPane.getBoundingBoxIndex(EnumFacing.SOUTH)]);
        }
        if (state.getValue(EAST).booleanValue()) {
            this.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BY_INDEX[BlockPane.getBoundingBoxIndex(EnumFacing.EAST)]);
        }
        if (state.getValue(WEST).booleanValue()) {
            this.addCollisionBoxToList(pos, entityBox, collidingBoxes, AABB_BY_INDEX[BlockPane.getBoundingBoxIndex(EnumFacing.WEST)]);
        }
    }
    ...
    private static int getBoundingBoxIndex(IBlockState state) {
        int i2 = 0;
        if (state.getValue(NORTH).booleanValue()) {
            i2 |= BlockPane.getBoundingBoxIndex(EnumFacing.NORTH);
        }
        if (state.getValue(EAST).booleanValue()) {
            i2 |= BlockPane.getBoundingBoxIndex(EnumFacing.EAST);
        }
        if (state.getValue(SOUTH).booleanValue()) {
            i2 |= BlockPane.getBoundingBoxIndex(EnumFacing.SOUTH);
        }
        if (state.getValue(WEST).booleanValue()) {
            i2 |= BlockPane.getBoundingBoxIndex(EnumFacing.WEST);
        }
        return i2;
    }
