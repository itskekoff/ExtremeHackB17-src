...
extends Block {
    ...
    public void onEntityCollidedWithBlock(World worldIn, BlockPos pos, IBlockState state, Entity entityIn) {
        ...
        float f2 = (float)pos.getY() + (6.0f + (float)(3 * i2)) / 16.0f;
        if (!worldIn.isRemote && entityIn.isBurning() && i2 > 0 && entityIn.getEntityBoundingBox().minY <= (double)f2) {
            entityIn.extinguish();
            this.setWaterLevel(worldIn, pos, state, i2 - 1);
        }
    }
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, EnumFacing heldItem, float side, float hitX, float hitY) {
        ...
        if (i2 > 0 && item instanceof ItemArmor && (itemarmor = (ItemArmor)item).getArmorMaterial() == ItemArmor.ArmorMaterial.LEATHER && itemarmor.hasColor(itemstack) && !worldIn.isRemote) {
            itemarmor.removeColor(itemstack);
            this.setWaterLevel(worldIn, pos, state, i2 - 1);
            playerIn.addStat(StatList.ARMOR_CLEANED);
            return true;
        }
        if (i2 > 0 && item instanceof ItemBanner) {
    ...
    public void fillWithRain(World worldIn, BlockPos pos) {
        if (worldIn.rand.nextInt(20) == 1) {
            ...
            float f2 = worldIn.getBiome(pos).getFloatTemperature(pos);
            if (worldIn.getBiomeProvider().getTemperatureAtHeight(f2, pos.getY()) >= 0.15f && (iblockstate = worldIn.getBlockState(pos)).getValue(LEVEL) < 3) {
