...
extends BlockDirectional {
    ...
    public void addCollisionBoxToList(IBlockState state, World worldIn, BlockPos pos, AxisAlignedBB entityBox, List<AxisAlignedBB> collidingBoxes, @Nullable Entity entityIn, boolean p_185477_7_) {
        this.addCollisionBoxToList(pos, entityBox, collidingBoxes, state.getBoundingBox(worldIn, pos));
    }
    ...
    private boolean doMove(World worldIn, BlockPos pos, EnumFacing direction, boolean extending) {
        ...
        int k2 = list.size() + list2.size();
        IBlockState[] aiblockstate = new IBlockState[k2];
        EnumFacing enumfacing = extending ? direction : direction.getOpposite();
        for (int j2 = list2.size() - 1; j2 >= 0; --j2) {
            BlockPos blockpos1 = list2.get(j2);
            ...
            aiblockstate[--k2] = iblockstate;
        }
        for (int l2 = list.size() - 1; l2 >= 0; --l2) {
            BlockPos blockpos3 = list.get(l2);
            ...
            worldIn.setTileEntity(blockpos3, BlockPistonMoving.createTilePiston((IBlockState)list1.get(l2), direction, extending, false));
            aiblockstate[--k2] = iblockstate2;
        }
        ...
        for (int i1 = list2.size() - 1; i1 >= 0; --i1) {
            worldIn.notifyNeighborsOfStateChange(list2.get(i1), aiblockstate[k2++].getBlock(), false);
        }
        for (int j1 = list.size() - 1; j1 >= 0; --j1) {
            worldIn.notifyNeighborsOfStateChange(list.get(j1), aiblockstate[k2++].getBlock(), false);
        }
        if (extending) {
