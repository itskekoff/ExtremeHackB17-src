...
public class Advancement {
    ...
    public static class Builder {
        ...
        public void func_192057_a(PacketBuffer p_192057_1_) {
            ...
            String[][] arrstring = this.field_192066_f;
            int n2 = this.field_192066_f.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                String[] astring = arrstring[i2];
                p_192057_1_.writeVarIntToBuffer(astring.length);
                String[] arrstring2 = astring;
                int n3 = astring.length;
                for (int i3 = 0; i3 < n3; ++i3) {
                    String s2 = arrstring2[i3];
                    p_192057_1_.writeString(s2);
                }
            }
        }
        ...
        public static Builder func_192059_a(JsonObject p_192059_0_, JsonDeserializationContext p_192059_1_) {
            ...
            for (int i2 = 0; i2 < jsonarray.size(); ++i2) {
                JsonArray jsonarray1 = JsonUtils.getJsonArray(jsonarray.get(i2), "requirements[" + i2 + "]");
                astring[i2] = new String[jsonarray1.size()];
                for (int j2 = 0; j2 < jsonarray1.size(); ++j2) {
                    astring[i2][j2] = JsonUtils.getString(jsonarray1.get(j2), "requirements[" + i2 + "][" + j2 + "]");
                }
            }
            if (astring.length == 0) {
                ...
                for (String s2 : map.keySet()) {
                    astring[k2++] = new String[]{s2};
                }
            }
            String[][] arrstring = astring;
            int j2 = astring.length;
            for (int i3 = 0; i3 < j2; ++i3) {
                String[] astring1 = arrstring[i3];
                if (astring1.length == 0 && map.isEmpty()) {
                    throw new JsonSyntaxException("Requirement entry cannot be empty");
                }
                String[] arrstring2 = astring1;
                int n3 = astring1.length;
                for (n2 = 0; n2 < n3; ++n2) {
                    String s2 = arrstring2[n2];
                    if (map.containsKey(s2)) continue;
                    throw new JsonSyntaxException("Unknown required criterion '" + s2 + "'");
                }
            }
            for (String s1 : map.keySet()) {
                boolean flag = false;
                String[][] arrstring3 = astring;
                n2 = astring.length;
                for (int i4 = 0; i4 < n2; ++i4) {
                    Object[] astring2 = arrstring3[i4];
                    if (!ArrayUtils.contains(astring2, s1)) continue;
                    flag = true;
                    break;
                }
                if (flag) continue;
        ...
        public static Builder func_192060_b(PacketBuffer p_192060_0_) throws IOException {
            ...
            for (int i2 = 0; i2 < astring.length; ++i2) {
                astring[i2] = new String[p_192060_0_.readVarIntFromBuffer()];
                for (int j2 = 0; j2 < astring[i2].length; ++j2) {
                    astring[i2][j2] = p_192060_0_.readStringFromBuffer(32767);
                }
            }
