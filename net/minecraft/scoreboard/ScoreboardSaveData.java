...
extends WorldSavedData {
    ...
    protected void readTeams(NBTTagList tagList) {
        for (int i2 = 0; i2 < tagList.tagCount(); ++i2) {
            ...
            NBTTagCompound nbttagcompound = tagList.getCompoundTagAt(i2);
    ...
    protected void loadTeamPlayers(ScorePlayerTeam playerTeam, NBTTagList tagList) {
        for (int i2 = 0; i2 < tagList.tagCount(); ++i2) {
            this.theScoreboard.addPlayerToTeam(tagList.getStringTagAt(i2), playerTeam.getRegisteredName());
        }
    }

    protected void readDisplayConfig(NBTTagCompound compound) {
        for (int i2 = 0; i2 < 19; ++i2) {
            if (!compound.hasKey("slot_" + i2, 8)) continue;
            String s2 = compound.getString("slot_" + i2);
    ...
    protected void readObjectives(NBTTagList nbt) {
        for (int i2 = 0; i2 < nbt.tagCount(); ++i2) {
            ...
            ScoreObjective scoreobjective = this.theScoreboard.addScoreObjective(s2, iscorecriteria);
    ...
    protected void readScores(NBTTagList nbt) {
        for (int i2 = 0; i2 < nbt.tagCount(); ++i2) {
            ...
            Score score = this.theScoreboard.getOrCreateScore(s2, scoreobjective);
