...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if ("start".equals(args[0])) {
            ...
            int j2 = server.getTickCounter();
            long k2 = i2 - this.profileStartTime;
            int l2 = j2 - this.profileStartTick;
            this.saveProfilerResults(k2, l2, server);
            server.theProfiler.profilingEnabled = false;
            CommandDebug.notifyCommandListener(sender, (ICommand)this, "commands.debug.stop", String.format("%.2f", Float.valueOf((float)k2 / 1000.0f)), l2);
        }
    }
    ...
    private void appendProfilerResults(int p_184895_1_, String sectionName, StringBuilder builder, MinecraftServer server) {
        ...
        if (list != null && list.size() >= 3) {
            for (int i2 = 1; i2 < list.size(); ++i2) {
                ...
                for (int j2 = 0; j2 < p_184895_1_; ++j2) {
                    builder.append("|   ");
                }
