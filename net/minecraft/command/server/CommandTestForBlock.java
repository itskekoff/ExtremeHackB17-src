...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if (args.length >= 6 && block.hasTileEntity()) {
            String s2 = CommandTestForBlock.buildString(args, 5);
            try {
        ...
        if (args.length >= 5 && !CommandBase.func_190791_b(block, args[4]).apply(iblockstate)) {
            try {
                ...
                throw new CommandException("commands.testforblock.failed.data", blockpos.getX(), blockpos.getY(), blockpos.getZ(), i2, Integer.parseInt(args[4]));
            }
