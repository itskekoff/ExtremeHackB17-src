...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if ("set".equals(args[0])) {
            ...
            long l2 = i2 = args.length > 2 ? CommandWorldBorder.parseLong(args[2], 0L, 9223372036854775L) * 1000L : 0L;
            if (i2 > 0L) {
                worldborder.setTransition(d0, d2, i2);
                if (d0 > d2) {
                    CommandWorldBorder.notifyCommandListener(sender, (ICommand)this, "commands.worldborder.setSlowly.shrink.success", String.format("%.1f", d2), String.format("%.1f", d0), Long.toString(i2 / 1000L));
                } else {
                    CommandWorldBorder.notifyCommandListener(sender, (ICommand)this, "commands.worldborder.setSlowly.grow.success", String.format("%.1f", d2), String.format("%.1f", d0), Long.toString(i2 / 1000L));
                }
            } else {
            ...
            if ("time".equals(args[1])) {
                ...
                int j2 = CommandWorldBorder.parseInt(args[2], 0);
                int l3 = worldborder.getWarningTime();
                worldborder.setWarningTime(j2);
                CommandWorldBorder.notifyCommandListener(sender, (ICommand)this, "commands.worldborder.warning.time.success", j2, l3);
            } else if ("distance".equals(args[1])) {
                if (args.length != 3) {
                    throw new WrongUsageException("commands.worldborder.warning.distance.usage", new Object[0]);
                }
                int k2 = CommandWorldBorder.parseInt(args[2], 0);
                int i1 = worldborder.getWarningDistance();
                worldborder.setWarningDistance(k2);
                CommandWorldBorder.notifyCommandListener(sender, (ICommand)this, "commands.worldborder.warning.distance.success", k2, i1);
            }
        } else {
