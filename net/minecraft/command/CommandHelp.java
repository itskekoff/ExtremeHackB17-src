...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if (sender instanceof CommandBlockBaseLogic) {
            ...
            int i2 = 7;
            int j2 = (list.size() - 1) / 7;
            int k2 = 0;
            try {
                k2 = args.length == 0 ? 0 : CommandHelp.parseInt(args[0], 1, j2 + 1) - 1;
            }
            ...
            int l2 = Math.min((k2 + 1) * 7, list.size());
            TextComponentTranslation textcomponenttranslation1 = new TextComponentTranslation("commands.help.header", k2 + 1, j2 + 1);
            textcomponenttranslation1.getStyle().setColor(TextFormatting.DARK_GREEN);
            sender.addChatMessage(textcomponenttranslation1);
            for (int i1 = k2 * 7; i1 < l2; ++i1) {
