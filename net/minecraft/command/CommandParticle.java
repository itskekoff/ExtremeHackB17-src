...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        for (int j2 = 0; j2 < aint.length; ++j2) {
            if (args.length <= 11 + j2) continue;
            try {
                aint[j2] = Integer.parseInt(args[11 + j2]);
                continue;
            }
            catch (NumberFormatException var28) {
                throw new CommandException("commands.particle.invalidParam", args[11 + j2]);
            }
        }
        ...
        if (world instanceof WorldServer) {
            ...
            if (entityplayermp == null) {
                worldserver.spawnParticle(enumparticletypes, flag1, d0, d1, d2, i2, d3, d4, d5, d6, aint);
            } else {
                worldserver.spawnParticle(entityplayermp, enumparticletypes, flag1, d0, d1, d2, i2, d3, d4, d5, d6, aint);
            }
            CommandParticle.notifyCommandListener(sender, (ICommand)this, "commands.particle.success", s2, Math.max(i2, 1));
        }
    }
