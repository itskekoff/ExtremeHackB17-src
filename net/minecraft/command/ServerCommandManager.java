    ...
    public void notifyListener(ICommandSender sender, ICommand command, int flags, String translationKey, Object ... translationArgs) {
        ...
        if (flag) {
            for (EntityPlayer entityPlayer : minecraftserver.getPlayerList().getPlayerList()) {
                ...
                boolean bl3 = flag2 = sender instanceof RConConsoleSource && this.server.shouldBroadcastRconToOps();
        ...
        boolean bl4 = minecraftserver.worldServers[0].getGameRules().getBoolean("sendCommandFeedback");
        if (sender instanceof CommandBlockBaseLogic) {
            bl2 = ((CommandBlockBaseLogic)sender).shouldTrackOutput();
        }
        if ((flags & 1) != 1 && bl2 || sender instanceof MinecraftServer) {
