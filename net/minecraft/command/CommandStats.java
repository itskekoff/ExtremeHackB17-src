...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        if ((commandresultstats$type = CommandResultStats.Type.getTypeByName(args[i2++])) == null) {
        ...
        if ("set".equals(s2)) {
            String s1 = args[i2++];
            String s22 = args[i2];
            if (s1.isEmpty() || s22.isEmpty()) {
                throw new CommandException("commands.stats.failed", new Object[0]);
            }
            CommandResultStats.setScoreBoardStat(commandresultstats, commandresultstats$type, s1, s22);
            CommandStats.notifyCommandListener(sender, (ICommand)this, "commands.stats.success", commandresultstats$type.getTypeName(), s22, s1);
        } else if ("clear".equals(s2)) {
