    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        block21: {
            block16: {
                ...
                i2 = (blockpos3.getX() - blockpos2.getX() + 1) * (blockpos3.getY() - blockpos2.getY() + 1) * (blockpos3.getZ() - blockpos2.getZ() + 1);
                if (i2 > 32768) {
                    throw new CommandException("commands.fill.tooManyBlocks", i2, 32768);
                }
                if (blockpos2.getY() < 0 || blockpos3.getY() >= 256) break block16;
                World world = sender.getEntityWorld();
                for (int j2 = blockpos2.getZ(); j2 <= blockpos3.getZ(); j2 += 16) {
                    for (int k2 = blockpos2.getX(); k2 <= blockpos3.getX(); k2 += 16) {
                        if (world.isBlockLoaded(new BlockPos(k2, blockpos3.getY() - blockpos2.getY(), j2))) continue;
                ...
                if (args.length >= 10 && block.hasTileEntity()) {
                    String s2 = CommandFill.buildString(args, 9);
                    try {
                ...
                i2 = 0;
                for (int l2 = blockpos2.getZ(); l2 <= blockpos3.getZ(); ++l2) {
                        ...
                        for (int j1 = blockpos2.getX(); j1 <= blockpos3.getX(); ++j1) {
                            ...
                            block17: {
                                block18: {
                                    block20: {
                                        block19: {
                                            blockpos4 = new BlockPos(j1, i1, l2);
                                ...
                                if (j1 != blockpos2.getX() && j1 != blockpos3.getX() && i1 != blockpos2.getY() && i1 != blockpos3.getY() && l2 != blockpos2.getZ() && l2 != blockpos3.getZ()) {
        ...
        CommandFill.notifyCommandListener(sender, (ICommand)this, "commands.fill.success", i2);
    }
