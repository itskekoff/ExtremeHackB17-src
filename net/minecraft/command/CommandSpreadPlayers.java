...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        ...
        while (i2 < args.length) {
            String s2;
            if (EntitySelector.hasArguments(s2 = args[i2++])) {
                List<Entity> list1 = EntitySelector.matchEntities(sender, s2, Entity.class);
                if (list1.isEmpty()) {
            ...
            EntityPlayerMP entityplayer = server.getPlayerList().getPlayerByUsername(s2);
            if (entityplayer == null) {
                throw new PlayerNotFoundException("commands.generic.player.notFound", s2);
            }
    ...
    private void spread(ICommandSender sender, List<Entity> p_110669_2_, Position pos, double spreadDistance, double maxRange, World worldIn, boolean respectTeams) throws CommandException {
        ...
        int i2 = this.spreadPositions(pos, spreadDistance, worldIn, random, d0, d1, d2, d3, acommandspreadplayers$position, respectTeams);
        ...
        if (acommandspreadplayers$position.length > 1) {
            sender.addChatMessage(new TextComponentTranslation("commands.spreadplayers.info." + (respectTeams ? "teams" : "players"), String.format("%.2f", d4), i2));
        }
    }
    ...
    private int spreadPositions(Position p_110668_1_, double p_110668_2_, World worldIn, Random random, double minX, double minZ, double maxX, double maxZ, Position[] p_110668_14_, boolean respectTeams) throws CommandException {
        ...
        for (i2 = 0; i2 < 10000 && flag; ++i2) {
            ...
            for (int j2 = 0; j2 < p_110668_14_.length; ++j2) {
                ...
                if (k2 > 0) {
                    commandspreadplayers$position1.x /= (double)k2;
                    commandspreadplayers$position1.z /= (double)k2;
            ...
            if (flag) continue;
            Position[] arrposition = p_110668_14_;
            int n2 = p_110668_14_.length;
            for (int i3 = 0; i3 < n2; ++i3) {
                Position commandspreadplayers$position3 = arrposition[i3];
    ...
    private double setPlayerPositions(List<Entity> p_110671_1_, World worldIn, Position[] p_110671_3_, boolean p_110671_4_) {
        ...
        for (int j2 = 0; j2 < p_110671_1_.size(); ++j2) {
            ...
            Position[] arrposition = p_110671_3_;
            int n2 = p_110671_3_.length;
            for (int i3 = 0; i3 < n2; ++i3) {
                Position commandspreadplayers$position1 = arrposition[i3];
    ...
    private Position[] createInitialPositions(Random p_110670_1_, int p_110670_2_, double p_110670_3_, double p_110670_5_, double p_110670_7_, double p_110670_9_) {
        ...
        for (int i2 = 0; i2 < acommandspreadplayers$position.length; ++i2) {
