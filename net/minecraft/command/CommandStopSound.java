...
extends CommandBase {
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
        if (args.length >= 1 && args.length <= 3) {
            int i2 = 0;
            EntityPlayerMP entityplayermp = CommandStopSound.getPlayer(server, sender, args[i2++]);
            String s2 = "";
            String s1 = "";
            if (args.length >= 2) {
                String s22;
                SoundCategory soundcategory;
                if ((soundcategory = SoundCategory.getByName(s22 = args[i2++])) == null) {
                    throw new CommandException("commands.stopsound.unknownSoundSource", s22);
                }
                s2 = soundcategory.getName();
            }
            if (args.length == 3) {
                s1 = args[i2++];
            }
            ...
            if (s2.isEmpty() && s1.isEmpty()) {
                ...
                CommandStopSound.notifyCommandListener(sender, (ICommand)this, "commands.stopsound.success.soundSource", s2, entityplayermp.getName());
            } else {
                CommandStopSound.notifyCommandListener(sender, (ICommand)this, "commands.stopsound.success.individualSound", s1, s2, entityplayermp.getName());
            }
        } else {
