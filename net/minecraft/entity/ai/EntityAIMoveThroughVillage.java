...
extends EntityAIBase {
    ...
    public boolean continueExecuting() {
        ...
        float f2 = this.theEntity.width + 4.0f;
        return this.theEntity.getDistanceSq(this.doorInfo.getDoorBlockPos()) > (double)(f2 * f2);
    }
    ...
    private VillageDoorInfo findNearestDoor(Village villageIn) {
        ...
        for (VillageDoorInfo villagedoorinfo1 : villageIn.getVillageDoorInfoList()) {
            int j2 = villagedoorinfo1.getDistanceSquared(MathHelper.floor(this.theEntity.posX), MathHelper.floor(this.theEntity.posY), MathHelper.floor(this.theEntity.posZ));
            if (j2 >= i2 || this.doesDoorListContain(villagedoorinfo1)) continue;
