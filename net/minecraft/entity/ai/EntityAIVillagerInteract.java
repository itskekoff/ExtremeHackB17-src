                ...
                for (int i2 = 0; i2 < inventorybasic.getSizeInventory(); ++i2) {
                    ...
                    if (!itemstack.func_190926_b()) {
                        ...
                        if ((item == Items.BREAD || item == Items.POTATO || item == Items.CARROT || item == Items.BEETROOT) && itemstack.func_190916_E() > 3) {
                            int l2 = itemstack.func_190916_E() / 2;
                            itemstack.func_190918_g(l2);
                            itemstack1 = new ItemStack(item, l2, itemstack.getMetadata());
                        } else if (item == Items.WHEAT && itemstack.func_190916_E() > 5) {
                            int j2 = itemstack.func_190916_E() / 2 / 3 * 3;
                            int k2 = j2 / 3;
                            itemstack.func_190918_g(j2);
                            itemstack1 = new ItemStack(Items.BREAD, k2, 0);
                        }
                        if (itemstack.func_190926_b()) {
                            inventorybasic.setInventorySlotContents(i2, ItemStack.field_190927_a);
                        }
                    }
                    ...
                    entityitem.motionX = -MathHelper.sin(f1 * ((float)Math.PI / 180)) * MathHelper.cos(f22 * ((float)Math.PI / 180)) * 0.3f;
                    entityitem.motionZ = MathHelper.cos(f1 * ((float)Math.PI / 180)) * MathHelper.cos(f22 * ((float)Math.PI / 180)) * 0.3f;
                    entityitem.motionY = -MathHelper.sin(f22 * ((float)Math.PI / 180)) * 0.3f + 0.1f;
