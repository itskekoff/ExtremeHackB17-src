...
extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (path != null && !path.isFinished() && pathnavigateground.getEnterDoors()) {
            for (int i2 = 0; i2 < Math.min(path.getCurrentPathIndex() + 2, path.getCurrentPathLength()); ++i2) {
                PathPoint pathpoint = path.getPathPointFromIndex(i2);
    ...
    public void updateTask() {
        ...
        float f22 = this.entityPositionX * f2 + this.entityPositionZ * (f1 = (float)((double)((float)this.doorPosition.getZ() + 0.5f) - this.theEntity.posZ));
        if (f22 < 0.0f) {
