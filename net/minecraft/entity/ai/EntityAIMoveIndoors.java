...
extends EntityAIBase {
    ...
    public void startExecuting() {
        ...
        if (this.entityObj.getDistanceSq(blockpos) > 256.0) {
            Vec3d vec3d = RandomPositionGenerator.findRandomTargetBlockTowards(this.entityObj, 14, 3, new Vec3d((double)i2 + 0.5, j2, (double)k2 + 0.5));
            if (vec3d != null) {
                this.entityObj.getNavigator().tryMoveToXYZ(vec3d.xCoord, vec3d.yCoord, vec3d.zCoord, 1.0);
            }
        } else {
            this.entityObj.getNavigator().tryMoveToXYZ((double)i2 + 0.5, j2, (double)k2 + 0.5, 1.0);
        }
    }
