...
public class EntityTrackerEntry {
    ...
    public void updatePlayerList(List<EntityPlayer> players) {
        ...
        if (this.updateCounter % this.updateFrequency == 0 || this.trackedEntity.isAirBorne || this.trackedEntity.getDataManager().isDirty()) {
            if (this.trackedEntity.isRiding()) {
                ...
                boolean bl2 = flag3 = Math.abs(j1 - this.encodedRotationYaw) >= 1 || Math.abs(l1 - this.encodedRotationPitch) >= 1;
                if (flag3) {
                ...
                int i3 = MathHelper.floor(this.trackedEntity.rotationPitch * 256.0f / 360.0f);
                long j3 = i1 - this.encodedPosX;
                long k3 = i2 - this.encodedPosY;
                long l2 = j2 - this.encodedPosZ;
                Packet<INetHandlerPlayClient> packet1 = null;
                boolean flag = j3 * j3 + k3 * k3 + l2 * l2 >= 128L || this.updateCounter % 60 == 0;
                boolean bl3 = flag1 = Math.abs(k2 - this.encodedRotationYaw) >= 1 || Math.abs(i3 - this.encodedRotationPitch) >= 1;
                if (this.updateCounter > 0 || this.trackedEntity instanceof EntityArrow) {
                    if (j3 >= -32768L && j3 < 32768L && k3 >= -32768L && k3 < 32768L && l2 >= -32768L && l2 < 32768L && this.ticksSinceLastForcedTeleport <= 400 && !this.ridingEntity && this.onGround == this.trackedEntity.onGround) {
                        if (!(flag && flag1 || this.trackedEntity instanceof EntityArrow)) {
                            if (flag) {
                                packet1 = new SPacketEntity.S15PacketEntityRelMove(this.trackedEntity.getEntityId(), j3, k3, l2, this.trackedEntity.onGround);
                            } else if (flag1) {
                                packet1 = new SPacketEntity.S16PacketEntityLook(this.trackedEntity.getEntityId(), (byte)k2, (byte)i3, this.trackedEntity.onGround);
                            }
                        } else {
                            packet1 = new SPacketEntity.S17PacketEntityLookMove(this.trackedEntity.getEntityId(), j3, k3, l2, (byte)k2, (byte)i3, this.trackedEntity.onGround);
                        }
                    } else {
    ...
    public boolean isVisibleTo(EntityPlayerMP playerMP) {
        ...
        int i2 = Math.min(this.range, this.maxRange);
        return d0 >= (double)(-i2) && d0 <= (double)i2 && d1 >= (double)(-i2) && d1 <= (double)i2 && this.trackedEntity.isSpectatedByPlayer(playerMP);
    }
    ...
    public void updatePlayerEntities(List<EntityPlayer> players) {
        for (int i2 = 0; i2 < players.size(); ++i2) {
            this.updatePlayerEntity((EntityPlayerMP)players.get(i2));
        }
    }
    ...
    private Packet<?> createSpawnPacket() {
        ...
        if (this.trackedEntity instanceof EntityFireball) {
            ...
            spacketspawnobject = entityfireball.shootingEntity != null ? new SPacketSpawnObject(this.trackedEntity, i2, ((EntityFireball)this.trackedEntity).shootingEntity.getEntityId()) : new SPacketSpawnObject(this.trackedEntity, i2, 0);
