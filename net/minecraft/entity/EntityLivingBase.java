...
extends Entity {
    ...
    protected void updateFallState(double y2, boolean onGroundIn, IBlockState state, BlockPos pos) {
        ...
        if (!this.world.isRemote && this.fallDistance > 3.0f && onGroundIn) {
            float f2 = MathHelper.ceil(this.fallDistance - 3.0f);
            if (state.getMaterial() != Material.AIR) {
                double d0 = Math.min((double)(0.2f + f2 / 15.0f), 2.5);
                int i2 = (int)(150.0 * d0);
                ((WorldServer)this.world).spawnParticle(EnumParticleTypes.BLOCK_DUST, this.posX, this.posY, this.posZ, i2, 0.0, 0.0, 0.0, (double)0.15f, Block.getStateId(state));
            }
        }
        super.updateFallState(y2, onGroundIn, state, pos);
    }
                ...
                if (!(this.canBreatheUnderwater() || this.isPotionActive(MobEffects.WATER_BREATHING) || flag1)) {
                    ...
                    if (this.getAir() == -20) {
                        ...
                        for (int i2 = 0; i2 < 8; ++i2) {
                            ...
                            this.world.spawnParticle(EnumParticleTypes.WATER_BUBBLE, this.posX + (double)f2, this.posY + (double)f3, this.posZ + (double)f1, this.motionX, this.motionY, this.motionZ, new int[0]);
                        }
    ...
    protected void frostWalk(BlockPos pos) {
        int i2 = EnchantmentHelper.getMaxEnchantmentLevel(Enchantments.FROST_WALKER, this);
        if (i2 > 0) {
            EnchantmentFrostWalker.freezeNearby(this, this.world, pos, i2);
        }
    }
    ...
    protected int decreaseAirSupply(int air2) {
        int i2 = EnchantmentHelper.getRespirationModifier(this);
        return i2 > 0 && this.rand.nextInt(i2 + 1) > 0 ? air2 : air2 - 1;
    }
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("Team", 8)) {
            String s2 = compound.getString("Team");
            boolean flag = this.world.getScoreboard().addPlayerToTeam(this.getCachedUniqueIdString(), s2);
            if (!flag) {
                field_190632_a.warn("Unable to add mob to team \"" + s2 + "\" (that team probably doesn't exist)");
            }
        }
    ...
    protected void updatePotionEffects() {
        ...
        int i2 = this.dataManager.get(POTION_EFFECTS);
    ...
    protected void onNewPotionEffect(PotionEffect id2) {
        ...
        if (!this.world.isRemote) {
            id2.getPotion().applyAttributesModifiersToEntity(this, this.getAttributeMap(), id2.getAmplifier());
        }
    }
    ...
    protected void onChangedPotionEffect(PotionEffect id2, boolean p_70695_2_) {
        ...
        if (p_70695_2_ && !this.world.isRemote) {
            Potion potion = id2.getPotion();
            potion.removeAttributesModifiersFromEntity(this, this.getAttributeMap(), id2.getAmplifier());
            potion.applyAttributesModifiersToEntity(this, this.getAttributeMap(), id2.getAmplifier());
        }
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        if (this instanceof EntityPlayerMP) {
            CriteriaTriggers.field_192128_h.func_192200_a((EntityPlayerMP)this, source, f2, amount, flag);
        }
        if (entity1 instanceof EntityPlayerMP) {
            CriteriaTriggers.field_192127_g.func_192220_a((EntityPlayerMP)entity1, this, source, f2, amount, flag);
        }
        return flag2;
    }
    ...
    public void knockBack(Entity entityIn, float strength, double xRatio, double zRatio) {
        if (this.rand.nextDouble() >= this.getEntityAttribute(SharedMonsterAttributes.KNOCKBACK_RESISTANCE).getAttributeValue()) {
            ...
            float f2 = MathHelper.sqrt(xRatio * xRatio + zRatio * zRatio);
            this.motionX /= 2.0;
            this.motionZ /= 2.0;
            this.motionX -= xRatio / (double)f2 * (double)strength;
            this.motionZ -= zRatio / (double)f2 * (double)strength;
            if (this.onGround) {
                this.motionY /= 2.0;
    ...
    public boolean isOnLadder() {
        int i2 = MathHelper.floor(this.posX);
        int j2 = MathHelper.floor(this.getEntityBoundingBox().minY);
        int k2 = MathHelper.floor(this.posZ);
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        float f2 = potioneffect == null ? 0.0f : (float)(potioneffect.getAmplifier() + 1);
        int i2 = MathHelper.ceil((distance - 3.0f - f2) * damageMultiplier);
        if (i2 > 0) {
            this.playSound(this.getFallSound(i2), 1.0f, 1.0f);
            this.attackEntityFrom(DamageSource.fall, i2);
            int j2 = MathHelper.floor(this.posX);
            int k2 = MathHelper.floor(this.posY - (double)0.2f);
            int l2 = MathHelper.floor(this.posZ);
            IBlockState iblockstate = this.world.getBlockState(new BlockPos(j2, k2, l2));
    ...
    protected float applyPotionDamageCalculations(DamageSource source, float damage) {
        ...
        if (this.isPotionActive(MobEffects.RESISTANCE) && source != DamageSource.outOfWorld) {
            int i2 = (this.getActivePotionEffect(MobEffects.RESISTANCE).getAmplifier() + 1) * 5;
            int j2 = 25 - i2;
        ...
        int k2 = EnchantmentHelper.getEnchantmentModifierDamage(this.getArmorInventoryList(), source);
        if (k2 > 0) {
            damage = CombatRules.getDamageAfterMagicAbsorb(damage, k2);
        }
        return damage;
    }
    ...
    public void handleStatusUpdate(byte id2) {
        boolean flag2;
        boolean flag = id2 == 33;
        boolean flag1 = id2 == 36;
        boolean bl2 = flag2 = id2 == 37;
        if (!(id2 == 2 || flag || flag1 || flag2)) {
            if (id2 == 3) {
    ...
    public ItemStack getHeldItemHead() {
        return this.getItemStackFromSlot(EntityEquipmentSlot.HEAD);
    }
    ...
    public void dismountEntity(Entity entityIn) {
        if (!(entityIn instanceof EntityBoat) && !(entityIn instanceof AbstractHorse)) {
            ...
            float f2 = entityIn instanceof EntityBoat ? 0.0f : 1.5707964f * (float)(this.getPrimaryHand() == EnumHandSide.RIGHT ? -1 : 1);
            float f1 = -MathHelper.sin(-this.rotationYaw * ((float)Math.PI / 180) - (float)Math.PI + f2);
            float f22 = -MathHelper.cos(-this.rotationYaw * ((float)Math.PI / 180) - (float)Math.PI + f2);
            double d2 = Math.abs(f1) > Math.abs(f22) ? d0 / (double)Math.abs(f1) : d0 / (double)Math.abs(f22);
            double d3 = this.posX + (double)f1 * d2;
            double d4 = this.posZ + (double)f22 * d2;
            ...
            if (!this.isInWater() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
                if (!this.isInLava() || this instanceof EntityPlayer && ((EntityPlayer)this).capabilities.isFlying) {
                    if (this.isElytraFlying()) {
                        ...
                        float f2 = this.rotationPitch * ((float)Math.PI / 180);
                        ...
                        if (f2 < 0.0f) {
                            double d10 = d8 * (double)(-MathHelper.sin(f2)) * 0.04;
    ...
    public void onUpdate() {
        ...
        if (f3 > 0.0025000002f) {
            ...
            float f22 = MathHelper.abs(MathHelper.wrapDegrees(this.rotationYaw) - f1);
            f4 = 95.0f < f22 && f22 < 265.0f ? f1 - 180.0f : f1;
        }
        ...
        this.onGroundSpeedFactor += (f2 - this.onGroundSpeedFactor) * 0.3f;
    ...
    protected float updateDistance(float p_110146_1_, float p_110146_2_) {
        boolean flag;
        float f2 = MathHelper.wrapDegrees(p_110146_1_ - this.renderYawOffset);
    ...
    protected void collideWithNearbyEntities() {
        ...
        if (!list.isEmpty()) {
            int i2 = this.world.getGameRules().getInt("maxEntityCramming");
            if (i2 > 0 && list.size() > i2 - 1 && this.rand.nextInt(4) == 0) {
                int j2 = 0;
                for (int k2 = 0; k2 < list.size(); ++k2) {
                    if (list.get(k2).isRiding()) continue;
                    ++j2;
                }
                if (j2 > i2 - 1) {
    ...
    public void setPositionAndRotationDirect(double x2, double y2, double z2, float yaw, float pitch, int posRotationIncrements, boolean teleport) {
        this.interpTargetX = x2;
        this.interpTargetY = y2;
        this.interpTargetZ = z2;
    ...
    public void setActiveHand(EnumHand hand) {
        ...
        if (!itemstack.func_190926_b() && !this.isHandActive()) {
            ...
            if (!this.world.isRemote) {
                ...
                this.dataManager.set(HAND_STATES, (byte)i2);
            }
        }
    }
    ...
    protected void updateItemUse(ItemStack stack, int eatingParticleCount) {
        if (!stack.func_190926_b() && this.isHandActive()) {
            ...
            if (stack.getItemUseAction() == EnumAction.EAT) {
                for (int i2 = 0; i2 < eatingParticleCount; ++i2) {
                    Vec3d vec3d = new Vec3d(((double)this.rand.nextFloat() - 0.5) * 0.1, Math.random() * 0.1 + 0.1, 0.0);
                    vec3d = vec3d.rotatePitch(-this.rotationPitch * ((float)Math.PI / 180));
                    vec3d = vec3d.rotateYaw(-this.rotationYaw * ((float)Math.PI / 180));
                    double d0 = (double)(-this.rand.nextFloat()) * 0.6 - 0.3;
    ...
    public boolean attemptTeleport(double x2, double y2, double z2) {
        double d0 = this.posX;
        double d1 = this.posY;
        double d2 = this.posZ;
        this.posX = x2;
        ...
        for (int j2 = 0; j2 < 128; ++j2) {
            double d6 = (double)j2 / 127.0;
            float f2 = (random.nextFloat() - 0.5f) * 0.2f;
            float f1 = (random.nextFloat() - 0.5f) * 0.2f;
            float f22 = (random.nextFloat() - 0.5f) * 0.2f;
            ...
            world.spawnParticle(EnumParticleTypes.PORTAL, d3, d4, d5, (double)f2, (double)f1, (double)f22, new int[0]);
        }
