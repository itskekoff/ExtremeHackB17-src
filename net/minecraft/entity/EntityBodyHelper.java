        ...
        if (d0 * d0 + d1 * d1 > 2.500000277905201E-7) {
            ...
            if (Math.abs(this.theLiving.rotationYawHead - this.prevRenderYawHead) > 15.0f) {
                ...
                if (this.rotationTickCounter > 10) {
                    f2 = Math.max(1.0f - (float)(this.rotationTickCounter - 10) / 10.0f, 0.0f) * 75.0f;
                }
            }
            this.theLiving.renderYawOffset = this.computeAngleWithBound(this.theLiving.rotationYawHead, this.theLiving.renderYawOffset, f2);
        }
    }

    private float computeAngleWithBound(float p_75665_1_, float p_75665_2_, float p_75665_3_) {
        float f2 = MathHelper.wrapDegrees(p_75665_1_ - p_75665_2_);
        if (f2 < -p_75665_3_) {
            f2 = -p_75665_3_;
        }
        if (f2 >= p_75665_3_) {
