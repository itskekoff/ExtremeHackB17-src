...
extends Entity {
    ...
    public boolean onValidSurface() {
        ...
        int i2 = Math.max(1, this.getWidthPixels() / 16);
        int j2 = Math.max(1, this.getHeightPixels() / 16);
        ...
        for (int k2 = 0; k2 < i2; ++k2) {
            for (int l2 = 0; l2 < j2; ++l2) {
                int i1 = (i2 - 1) / -2;
                int j1 = (j2 - 1) / -2;
                blockpos$mutableblockpos.setPos(blockpos).move(enumfacing, k2 + i1).move(EnumFacing.UP, l2 + j1);
    ...
    public void addVelocity(double x2, double y2, double z2) {
        if (!this.world.isRemote && !this.isDead && x2 * x2 + y2 * y2 + z2 * z2 > 0.0) {
            this.setDead();
            this.onBroken(null);
        }
    }
    ...
    public void setPosition(double x2, double y2, double z2) {
        this.hangingPosition = new BlockPos(x2, y2, z2);
        this.updateBoundingBox();
