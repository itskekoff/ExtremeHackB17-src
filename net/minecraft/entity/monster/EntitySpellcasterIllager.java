...
extends AbstractIllager {
    ...
    public void onUpdate() {
        ...
        if (this.world.isRemote && this.func_193082_dl()) {
            ...
            float f2 = this.renderYawOffset * ((float)Math.PI / 180) + MathHelper.cos((float)this.ticksExisted * 0.6662f) * 0.25f;
            float f1 = MathHelper.cos(f2);
            float f22 = MathHelper.sin(f2);
            this.world.spawnParticle(EnumParticleTypes.SPELL_MOB, this.posX + (double)f1 * 0.6, this.posY + 1.8, this.posZ + (double)f22 * 0.6, d0, d1, d2, new int[0]);
            this.world.spawnParticle(EnumParticleTypes.SPELL_MOB, this.posX - (double)f1 * 0.6, this.posY + 1.8, this.posZ - (double)f22 * 0.6, d0, d1, d2, new int[0]);
        }
    }
