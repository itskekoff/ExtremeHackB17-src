...
implements IMob {
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        int i2 = compound.getInteger("Size");
        if (i2 < 0) {
            i2 = 0;
        }
        this.setSlimeSize(i2 + 1, false);
    ...
    public void onUpdate() {
        ...
        if (this.onGround && !this.wasOnGround) {
            ...
            for (int j2 = 0; j2 < i2 * 8; ++j2) {
                ...
                float f22 = MathHelper.sin(f2) * (float)i2 * 0.5f * f1;
                float f3 = MathHelper.cos(f2) * (float)i2 * 0.5f * f1;
                World world = this.world;
                EnumParticleTypes enumparticletypes = this.getParticleType();
                double d0 = this.posX + (double)f22;
    ...
    public void notifyDataManagerChange(DataParameter<?> key) {
        if (SLIME_SIZE.equals(key)) {
            int i2 = this.getSlimeSize();
            this.setSize(0.51000005f * (float)i2, 0.51000005f * (float)i2);
    ...
    public void setDead() {
        int i2 = this.getSlimeSize();
        if (!this.world.isRemote && i2 > 1 && this.getHealth() <= 0.0f) {
            int j2 = 2 + this.rand.nextInt(3);
            for (int k2 = 0; k2 < j2; ++k2) {
                ...
                entityslime.setSlimeSize(i2 / 2, true);
    ...
    protected void dealDamage(EntityLivingBase entityIn) {
        int i2 = this.getSlimeSize();
        if (this.canEntityBeSeen(entityIn) && this.getDistanceSqToEntity(entityIn) < 0.6 * (double)i2 * 0.6 * (double)i2 && entityIn.attackEntityFrom(DamageSource.causeMobDamage(this), this.getAttackStrength())) {
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
        ...
        if (i2 < 2 && this.rand.nextFloat() < 0.5f * difficulty.getClampedAdditionalDifficulty()) {
            ++i2;
        }
        int j2 = 1 << i2;
        this.setSlimeSize(j2, true);
