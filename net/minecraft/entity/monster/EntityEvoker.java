...
extends EntitySpellcasterIllager {
    ...
    extends EntitySpellcasterIllager.AIUseSpell {
        ...
        protected void func_190868_j() {
            ...
            float f2 = (float)MathHelper.atan2(entitylivingbase.posZ - EntityEvoker.this.posZ, entitylivingbase.posX - EntityEvoker.this.posX);
            if (EntityEvoker.this.getDistanceSqToEntity(entitylivingbase) < 9.0) {
                ...
                for (int k2 = 0; k2 < 8; ++k2) {
                    float f22 = f2 + (float)k2 * (float)Math.PI * 2.0f / 8.0f + 1.2566371f;
                    this.func_190876_a(EntityEvoker.this.posX + (double)MathHelper.cos(f22) * 2.5, EntityEvoker.this.posZ + (double)MathHelper.sin(f22) * 2.5, d0, d1, f22, 3);
                }
            } else {
                for (int l2 = 0; l2 < 16; ++l2) {
                    double d2 = 1.25 * (double)(l2 + 1);
                    int j2 = 1 * l2;
                    this.func_190876_a(EntityEvoker.this.posX + (double)MathHelper.cos(f2) * d2, EntityEvoker.this.posZ + (double)MathHelper.sin(f2) * d2, d0, d1, f2, j2);
                }
            }
        }
    ...
    extends EntitySpellcasterIllager.AIUseSpell {
        ...
        public boolean shouldExecute() {
            ...
            int i2 = EntityEvoker.this.world.getEntitiesWithinAABB(EntityVex.class, EntityEvoker.this.getEntityBoundingBox().expandXyz(16.0)).size();
            return EntityEvoker.this.rand.nextInt(8) + 1 > i2;
        }
