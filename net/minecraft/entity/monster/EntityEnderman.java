...
extends EntityMob {
    ...
    protected void updateAITasks() {
        ...
        if (this.world.isDaytime() && this.ticksExisted >= this.targetChangeTime + 600 && (f2 = this.getBrightness()) > 0.5f && this.world.canSeeSky(new BlockPos(this)) && this.rand.nextFloat() * 30.0f < (f2 - 0.4f) * 2.0f) {
    ...
    private boolean teleportTo(double x2, double y2, double z2) {
        boolean flag = this.attemptTeleport(x2, y2, z2);
        if (flag) {
    ...
    protected void dropEquipment(boolean wasRecentlyHit, int lootingModifier) {
        ...
        if (iblockstate != null) {
            ...
            int i2 = item.getHasSubtypes() ? iblockstate.getBlock().getMetaFromState(iblockstate) : 0;
            this.entityDropItem(new ItemStack(item, 1, i2), 0.0f);
        }
    }
    ...
    extends EntityAIBase {
        ...
        public void updateTask() {
            ...
            int i2 = MathHelper.floor(this.enderman.posX - 1.0 + random.nextDouble() * 2.0);
            int j2 = MathHelper.floor(this.enderman.posY + random.nextDouble() * 2.0);
            int k2 = MathHelper.floor(this.enderman.posZ - 1.0 + random.nextDouble() * 2.0);
    ...
    extends EntityAIBase {
        ...
        public void updateTask() {
            ...
            Random random = this.enderman.getRNG();
            World world = this.enderman.world;
            int i2 = MathHelper.floor(this.enderman.posX - 2.0 + random.nextDouble() * 4.0);
            int j2 = MathHelper.floor(this.enderman.posY + random.nextDouble() * 3.0);
            int k2 = MathHelper.floor(this.enderman.posZ - 2.0 + random.nextDouble() * 4.0);
            BlockPos blockpos = new BlockPos(i2, j2, k2);
            IBlockState iblockstate = world.getBlockState(blockpos);
            Block block = iblockstate.getBlock();
            RayTraceResult raytraceresult = world.rayTraceBlocks(new Vec3d((float)MathHelper.floor(this.enderman.posX) + 0.5f, (float)j2 + 0.5f, (float)MathHelper.floor(this.enderman.posZ) + 0.5f), new Vec3d((float)i2 + 0.5f, (float)j2 + 0.5f, (float)k2 + 0.5f), false, true, false);
            boolean bl2 = flag = raytraceresult != null && raytraceresult.getBlockPos().equals(blockpos);
