...
extends Entity {
    ...
    public boolean isInRangeToRender3d(double x2, double y2, double z2) {
        return super.isInRangeToRender3d(x2, y2, z2) && !this.func_191511_j();
    }
        ...
        if (f2 > 0.0f) {
            ...
            for (EntityLivingBase entitylivingbase : this.world.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().expandXyz(5.0))) {
                ...
                for (int i2 = 0; i2 < 2; ++i2) {
                    RayTraceResult raytraceresult = this.world.rayTraceBlocks(vec3d, new Vec3d(entitylivingbase.posX, entitylivingbase.posY + (double)entitylivingbase.height * 0.5 * (double)i2, entitylivingbase.posZ), false, true, false);
                    if (raytraceresult != null && raytraceresult.typeOfHit != RayTraceResult.Type.MISS) continue;
                    flag = true;
                    break;
                }
                if (!flag) continue;
                float f1 = f2 * (float)Math.sqrt((5.0 - (double)this.getDistanceToEntity(entitylivingbase)) / 5.0);
