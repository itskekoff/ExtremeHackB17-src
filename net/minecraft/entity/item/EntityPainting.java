...
extends EntityHanging {
    ...
    public EntityPainting(World worldIn, BlockPos pos, EnumFacing facing) {
        ...
        int n2 = arrenumArt.length;
        for (int i3 = 0; i3 < n2; ++i3) {
            EnumArt entitypainting$enumart;
            this.art = entitypainting$enumart = arrenumArt[i3];
        ...
        if (!list.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if (entitypainting$enumart1.sizeX * entitypainting$enumart1.sizeY >= i2) continue;
                iterator.remove();
            }
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        String s2 = compound.getString("Motive");
        for (EnumArt entitypainting$enumart : EnumArt.values()) {
            if (!entitypainting$enumart.title.equals(s2)) continue;
            this.art = entitypainting$enumart;
        }
        if (this.art == null) {
    ...
    public void setPositionAndRotationDirect(double x2, double y2, double z2, float yaw, float pitch, int posRotationIncrements, boolean teleport) {
        BlockPos blockpos = this.hangingPosition.add(x2 - this.posX, y2 - this.posY, z2 - this.posZ);
