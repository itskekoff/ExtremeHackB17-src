...
implements IRangedAttackMob {
    ...
    public void onLivingUpdate() {
        ...
        for (int i2 = 0; i2 < 2; ++i2) {
            this.yRotOHeads[i2] = this.yRotationHeads[i2];
            this.xRotOHeads[i2] = this.xRotationHeads[i2];
        }
        for (int j2 = 0; j2 < 2; ++j2) {
            int k2 = this.getWatchedTargetId(j2 + 1);
            Entity entity1 = null;
            if (k2 > 0) {
                entity1 = this.world.getEntityByID(k2);
            }
            if (entity1 != null) {
                double d11 = this.getHeadX(j2 + 1);
                double d12 = this.getHeadY(j2 + 1);
                double d13 = this.getHeadZ(j2 + 1);
                ...
                float f2 = (float)(MathHelper.atan2(d8, d6) * 57.29577951308232) - 90.0f;
                float f1 = (float)(-(MathHelper.atan2(d7, d9) * 57.29577951308232));
                this.xRotationHeads[j2] = this.rotlerp(this.xRotationHeads[j2], f1, 40.0f);
                this.yRotationHeads[j2] = this.rotlerp(this.yRotationHeads[j2], f2, 10.0f);
                continue;
            }
            this.yRotationHeads[j2] = this.rotlerp(this.yRotationHeads[j2], this.renderYawOffset, 10.0f);
        }
        boolean flag = this.isArmored();
        for (int l2 = 0; l2 < 3; ++l2) {
            double d10 = this.getHeadX(l2);
            double d2 = this.getHeadY(l2);
            double d4 = this.getHeadZ(l2);
    ...
    protected void updateAITasks() {
        if (this.getInvulTime() > 0) {
            ...
            super.updateAITasks();
            block0: for (int i2 = 1; i2 < 3; ++i2) {
                int k1;
                if (this.ticksExisted < this.nextHeadUpdate[i2 - 1]) continue;
                this.nextHeadUpdate[i2 - 1] = this.ticksExisted + 10 + this.rand.nextInt(10);
                if (this.world.getDifficulty() == EnumDifficulty.NORMAL || this.world.getDifficulty() == EnumDifficulty.HARD) {
                    int j3 = i2 - 1;
                    int k3 = this.idleHeadUpdates[i2 - 1];
                    this.idleHeadUpdates[j3] = this.idleHeadUpdates[i2 - 1] + 1;
                    if (k3 > 15) {
                        float f2 = 10.0f;
                        float f1 = 5.0f;
                        ...
                        this.launchWitherSkullToCoords(i2 + 1, d0, d1, d2, true);
                        this.idleHeadUpdates[i2 - 1] = 0;
                    }
                }
                if ((k1 = this.getWatchedTargetId(i2)) > 0) {
                    ...
                    if (entity != null && entity.isEntityAlive() && this.getDistanceSqToEntity(entity) <= 900.0 && this.canEntityBeSeen(entity)) {
                        ...
                        this.launchWitherSkullToEntity(i2 + 1, (EntityLivingBase)entity);
                        this.nextHeadUpdate[i2 - 1] = this.ticksExisted + 40 + this.rand.nextInt(20);
                        this.idleHeadUpdates[i2 - 1] = 0;
                        continue;
                    }
                    this.updateWatchedTargetId(i2, 0);
                ...
                for (int j2 = 0; j2 < 10 && !list.isEmpty(); ++j2) {
                    ...
                    if (entitylivingbase != this && entitylivingbase.isEntityAlive() && this.canEntityBeSeen(entitylivingbase)) {
                        if (entitylivingbase instanceof EntityPlayer) {
                            ...
                            this.updateWatchedTargetId(i2, entitylivingbase.getEntityId());
                            continue block0;
                        }
                        this.updateWatchedTargetId(i2, entitylivingbase.getEntityId());
                        continue block0;
                    ...
                    for (int k2 = -1; k2 <= 1; ++k2) {
                        for (int l2 = -1; l2 <= 1; ++l2) {
                            for (int j2 = 0; j2 <= 3; ++j2) {
                                ...
                                int k3 = i1 + j2;
                                int l3 = i2 + l2;
                                BlockPos blockpos = new BlockPos(i3, k3, l3);
    ...
    private double getHeadX(int p_82214_1_) {
        ...
        float f2 = (this.renderYawOffset + (float)(180 * (p_82214_1_ - 1))) * ((float)Math.PI / 180);
    ...
    private double getHeadZ(int p_82213_1_) {
        ...
        float f2 = (this.renderYawOffset + (float)(180 * (p_82213_1_ - 1))) * ((float)Math.PI / 180);
    ...
    private float rotlerp(float p_82204_1_, float p_82204_2_, float p_82204_3_) {
        float f2 = MathHelper.wrapDegrees(p_82204_2_ - p_82204_1_);
        if (f2 > p_82204_3_) {
            f2 = p_82204_3_;
        }
        if (f2 < -p_82204_3_) {
    ...
    private void launchWitherSkullToCoords(int p_82209_1_, double x2, double y2, double z2, boolean invulnerable) {
