    ...
    public void doLocalUpdate() {
        ...
        if (entitylivingbase != null) {
            if (this.scanningTime > 25) {
                ...
                float f2 = (float)vec3d1.dotProduct(vec3d);
                float f1 = (float)(Math.acos(f2) * 57.29577951308232) + 0.5f;
                if (f1 < 0.0f || f1 > 10.0f) {
                    float f22;
                    ...
                    float f3 = f22 = MathHelper.sqrt(d0 * d0 + d1 * d1) + 1.0f;
                    if (f22 > 40.0f) {
                        f22 = 40.0f;
                    }
                    this.dragon.randomYawVelocity = (float)((double)this.dragon.randomYawVelocity + d2 * (double)(0.7f / f22 / f3));
