...
IMob {
    ...
    public double[] getMovementOffsets(int p_70974_1_, float p_70974_2_) {
        ...
        int i2 = this.ringBufferIndex - p_70974_1_ & 0x3F;
        int j2 = this.ringBufferIndex - p_70974_1_ - 1 & 0x3F;
        double[] adouble = new double[3];
        double d0 = this.ringBuffer[i2][0];
        double d1 = MathHelper.wrapDegrees(this.ringBuffer[j2][0] - d0);
        adouble[0] = d0 + d1 * (double)p_70974_2_;
        d0 = this.ringBuffer[i2][1];
        d1 = this.ringBuffer[j2][1] - d0;
        adouble[1] = d0 + d1 * (double)p_70974_2_;
        adouble[2] = this.ringBuffer[i2][2] + (this.ringBuffer[j2][2] - this.ringBuffer[i2][2]) * (double)p_70974_2_;
        return adouble;
    }
    ...
    public void onLivingUpdate() {
        if (this.world.isRemote) {
            ...
            if (!this.isSilent()) {
                float f2 = MathHelper.cos(this.animTime * ((float)Math.PI * 2));
            ...
            if (this.isAIDisabled()) {
                ...
                if (this.ringBufferIndex < 0) {
                    for (int i2 = 0; i2 < this.ringBuffer.length; ++i2) {
                        ...
                        this.ringBuffer[i2][1] = this.posY;
                    }
                }
                ...
                for (int j2 = 0; j2 < this.dragonPartArray.length; ++j2) {
                    avec3d[j2] = new Vec3d(this.dragonPartArray[j2].posX, this.dragonPartArray[j2].posY, this.dragonPartArray[j2].posZ);
                }
                ...
                for (int k2 = 0; k2 < 3; ++k2) {
                    ...
                    double[] adouble1 = this.getMovementOffsets(12 + k2 * 2, 1.0f);
                ...
                for (int l2 = 0; l2 < this.dragonPartArray.length; ++l2) {
                    this.dragonPartArray[l2].prevPosX = avec3d[l2].xCoord;
                    this.dragonPartArray[l2].prevPosY = avec3d[l2].yCoord;
                    this.dragonPartArray[l2].prevPosZ = avec3d[l2].zCoord;
                }
            }
        }
    }
    ...
    private void attackEntitiesInList(List<Entity> p_70971_1_) {
        for (int i2 = 0; i2 < p_70971_1_.size(); ++i2) {
            Entity entity = p_70971_1_.get(i2);
            if (!(entity instanceof EntityLivingBase)) continue;
    ...
    private boolean destroyBlocksInAABB(AxisAlignedBB p_70972_1_) {
        int i2 = MathHelper.floor(p_70972_1_.minX);
        int j2 = MathHelper.floor(p_70972_1_.minY);
        int k2 = MathHelper.floor(p_70972_1_.minZ);
        int l2 = MathHelper.floor(p_70972_1_.maxX);
        ...
        for (int k1 = i2; k1 <= l2; ++k1) {
            for (int l1 = j2; l1 <= i1; ++l1) {
                for (int i22 = k2; i22 <= j1; ++i22) {
                    BlockPos blockpos = new BlockPos(k1, l1, i22);
    ...
    protected void onDeathUpdate() {
        ...
        if (this.deathTicks >= 180 && this.deathTicks <= 200) {
            ...
            this.world.spawnParticle(EnumParticleTypes.EXPLOSION_HUGE, this.posX + (double)f2, this.posY + 2.0 + (double)f1, this.posZ + (double)f22, 0.0, 0.0, 0.0, new int[0]);
        }
        ...
        if (!this.world.isRemote) {
            if (this.deathTicks > 150 && this.deathTicks % 5 == 0 && flag) {
                this.dropExperience(MathHelper.floor((float)i2 * 0.08f));
            }
        ...
        if (this.deathTicks == 200 && !this.world.isRemote) {
            if (flag) {
                this.dropExperience(MathHelper.floor((float)i2 * 0.2f));
            }
            if (this.fightManager != null) {
                this.fightManager.processDragonDeath(this);
            }
    ...
    private void dropExperience(int p_184668_1_) {
        while (p_184668_1_ > 0) {
            int i2 = EntityXPOrb.getXPSplit(p_184668_1_);
            p_184668_1_ -= i2;
            this.world.spawnEntityInWorld(new EntityXPOrb(this.world, this.posX, this.posY, this.posZ, i2));
        }
    }
    ...
    public int initPathPoints() {
        if (this.pathPoints[0] == null) {
            for (int i2 = 0; i2 < 24; ++i2) {
                ...
                int j2 = 5;
                if (i2 < 12) {
                    l2 = (int)(60.0f * MathHelper.cos(2.0f * ((float)(-Math.PI) + 0.2617994f * (float)i2)));
                    i1 = (int)(60.0f * MathHelper.sin(2.0f * ((float)(-Math.PI) + 0.2617994f * (float)i2)));
                } else if (i2 < 20) {
                    int lvt_3_1_ = i2 - 12;
                    l2 = (int)(40.0f * MathHelper.cos(2.0f * ((float)(-Math.PI) + 0.3926991f * (float)lvt_3_1_)));
                    i1 = (int)(40.0f * MathHelper.sin(2.0f * ((float)(-Math.PI) + 0.3926991f * (float)lvt_3_1_)));
                    j2 += 10;
                } else {
                    int k1 = i2 - 20;
                    l2 = (int)(20.0f * MathHelper.cos(2.0f * ((float)(-Math.PI) + 0.7853982f * (float)k1)));
                    i1 = (int)(20.0f * MathHelper.sin(2.0f * ((float)(-Math.PI) + 0.7853982f * (float)k1)));
                }
                int j1 = Math.max(this.world.getSeaLevel() + 10, this.world.getTopSolidOrLiquidBlock(new BlockPos(l2, 0, i1)).getY() + j2);
                this.pathPoints[i2] = new PathPoint(l2, j1, i1);
            }
    ...
    public int getNearestPpIdx(double x2, double y2, double z2) {
        float f2 = 10000.0f;
        int i2 = 0;
        PathPoint pathpoint = new PathPoint(MathHelper.floor(x2), MathHelper.floor(y2), MathHelper.floor(z2));
        int j2 = 0;
        ...
        for (int k2 = j2; k2 < 24; ++k2) {
            float f1;
            if (this.pathPoints[k2] == null || !((f1 = this.pathPoints[k2].distanceToSquared(pathpoint)) < f2)) continue;
            f2 = f1;
            i2 = k2;
        }
        return i2;
    ...
    public Path findPath(int startIdx, int finishIdx, @Nullable PathPoint andThen) {
        ...
        while (!this.pathFindQueue.isPathEmpty()) {
            ...
            for (int l2 = 0; l2 < 24; ++l2) {
                if (this.pathPoints[l2] != pathpoint2) continue;
                k2 = l2;
                break;
            }
            for (int i1 = j2; i1 < 24; ++i1) {
                if ((this.neighbors[k2] & 1 << i1) <= 0) continue;
                PathPoint pathpoint3 = this.pathPoints[i1];
                if (pathpoint3.visited) continue;
                float f2 = pathpoint2.totalPathDistance + pathpoint2.distanceTo(pathpoint3);
                if (pathpoint3.isAssigned() && !(f2 < pathpoint3.totalPathDistance)) continue;
    ...
    public float getHeadPartYOffset(int p_184667_1_, double[] p_184667_2_, double[] p_184667_3_) {
        ...
        if (phaselist != PhaseList.LANDING && phaselist != PhaseList.TAKEOFF) {
            ...
            float f2 = Math.max(MathHelper.sqrt(this.getDistanceSqToCenter(blockpos)) / 4.0f, 1.0f);
            d0 = (float)p_184667_1_ / f2;
        }
        return (float)d0;
    }
