...
implements IContainerListener {
    ...
    public EntityPlayerMP(MinecraftServer server, WorldServer worldIn, GameProfile profile, PlayerInteractionManager interactionManagerIn) {
        ...
        if (worldIn.provider.func_191066_m() && worldIn.getWorldInfo().getGameType() != GameType.ADVENTURE) {
            int i2 = Math.max(0, server.getSpawnRadius(worldIn));
            int j2 = MathHelper.floor(worldIn.getWorldBorder().getClosestDistance(blockpos.getX(), blockpos.getZ()));
            if (j2 < i2) {
                i2 = j2;
            }
            if (j2 <= 1) {
            ...
            blockpos = worldIn.getTopSolidOrLiquidBlock(blockpos.add(this.rand.nextInt(i2 * 2 + 1) - i2, 0, this.rand.nextInt(i2 * 2 + 1) - i2));
        }
    ...
    public void onUpdate() {
        ...
        while (!this.entityRemoveQueue.isEmpty()) {
            int i2 = Math.min(this.entityRemoveQueue.size(), Integer.MAX_VALUE);
            int[] aint = new int[i2];
    ...
    private Collection<ScoreObjective> func_192038_E(Entity p_192038_1_) {
        ...
        int i2;
        String s2 = p_192038_1_ instanceof EntityPlayer ? p_192038_1_.getName() : p_192038_1_.getCachedUniqueIdString();
        ScorePlayerTeam scoreplayerteam = this.getWorldScoreboard().getPlayersTeam(this.getName());
        if (scoreplayerteam != null && (i2 = scoreplayerteam.getChatFormat().getColorIndex()) >= 0 && i2 < IScoreCriteria.KILLED_BY_TEAM.length) {
            for (ScoreObjective scoreobjective : this.getWorldScoreboard().getObjectivesFromCriteria(IScoreCriteria.KILLED_BY_TEAM[i2])) {
                Score score = this.getWorldScoreboard().getOrCreateScore(s2, scoreobjective);
                score.incrementScore();
            }
        }
        if ((scoreplayerteam1 = this.getWorldScoreboard().getPlayersTeam(s2)) != null && (j2 = scoreplayerteam1.getChatFormat().getColorIndex()) >= 0 && j2 < IScoreCriteria.TEAM_KILL.length) {
            return this.getWorldScoreboard().getObjectivesFromCriteria(IScoreCriteria.TEAM_KILL[j2]);
        }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        ...
        boolean bl2 = flag = this.mcServer.isDedicatedServer() && this.canPlayersAttack() && "fall".equals(source.damageType);
    ...
    public void handleFalling(double y2, boolean onGroundIn) {
        BlockPos blockpos1;
        ...
        super.updateFallState(y2, onGroundIn, iblockstate, blockpos);
    }
    ...
    public void sendAllWindowProperties(Container containerIn, IInventory inventory) {
        for (int i2 = 0; i2 < inventory.getFieldCount(); ++i2) {
            this.connection.sendPacket(new SPacketWindowProperty(containerIn.windowId, i2, inventory.getField(i2)));
        }
    }
    ...
    public void func_193102_a(ResourceLocation[] p_193102_1_) {
        ...
        ResourceLocation[] arrresourceLocation = p_193102_1_;
        int n2 = p_193102_1_.length;
        for (int i2 = 0; i2 < n2; ++i2) {
            ResourceLocation resourcelocation = arrresourceLocation[i2];
    ...
    protected void onNewPotionEffect(PotionEffect id2) {
        super.onNewPotionEffect(id2);
        this.connection.sendPacket(new SPacketEntityEffect(this.getEntityId(), id2));
        if (id2.getPotion() == MobEffects.LEVITATION) {
    ...
    protected void onChangedPotionEffect(PotionEffect id2, boolean p_70695_2_) {
        super.onChangedPotionEffect(id2, p_70695_2_);
        this.connection.sendPacket(new SPacketEntityEffect(this.getEntityId(), id2));
        CriteriaTriggers.field_193139_z.func_193153_a(this);
    }
    ...
    public void setPositionAndUpdate(double x2, double y2, double z2) {
        this.connection.setPlayerLocation(x2, y2, z2, this.rotationYaw, this.rotationPitch);
    }
    ...
    public String getPlayerIP() {
        String s2 = this.connection.netManager.getRemoteAddress().toString();
