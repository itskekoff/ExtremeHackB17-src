...
extends EntityLivingBase {
    ...
    protected void updateSize() {
        ...
        if (f2 != this.width || f1 != this.height) {
            AxisAlignedBB axisalignedbb = this.getEntityBoundingBox();
            axisalignedbb = new AxisAlignedBB(axisalignedbb.minX, axisalignedbb.minY, axisalignedbb.minZ, axisalignedbb.minX + (double)f2, axisalignedbb.minY + (double)f1, axisalignedbb.minZ + (double)f2);
    ...
    private void func_192028_j(@Nullable NBTTagCompound p_192028_1_) {
        String s2;
        if ((p_192028_1_ != null && !p_192028_1_.hasKey("Silent") || !p_192028_1_.getBoolean("Silent")) && (s2 = p_192028_1_.getString("id")).equals(EntityList.func_191306_a(EntityParrot.class).toString())) {
    ...
    public void addScore(int scoreIn) {
        ...
        this.dataManager.set(PLAYER_SCORE, i2 + scoreIn);
    }
    ...
    protected void func_190776_cN() {
        for (int i2 = 0; i2 < this.inventory.getSizeInventory(); ++i2) {
            ...
            this.inventory.removeStackFromSlot(i2);
        }
    }
    ...
    public float getDigSpeed(IBlockState state) {
        float f2 = this.inventory.getStrVsBlock(state);
        if (f2 > 1.0f) {
        ...
        if (this.isPotionActive(MobEffects.HASTE)) {
            f2 *= 1.0f + (float)(this.getActivePotionEffect(MobEffects.HASTE).getAmplifier() + 1) * 0.2f;
        }
    ...
    protected void damageShield(float damage) {
        if (damage >= 3.0f && this.activeItemStack.getItem() == Items.SHIELD) {
            int i2 = 1 + MathHelper.floor(damage);
            this.activeItemStack.damageItem(i2, this);
    ...
    public float getArmorVisibility() {
        ...
        return (float)i2 / (float)this.inventory.armorInventory.size();
    }
    ...
    public void attackTargetEntityWithCurrentItem(Entity targetEntity) {
        if (targetEntity.canBeAttackedWithItem() && !targetEntity.hitByEntity(this)) {
            float f2 = (float)this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).getAttributeValue();
            ...
            float f22 = this.getCooledAttackStrength(0.5f);
            f1 *= f22;
            this.resetCooldown();
            if ((f2 *= 0.2f + f22 * f22 * 0.8f) > 0.0f || f1 > 0.0f) {
                ...
                boolean bl2 = flag2 = flag2 && !this.isSprinting();
                if (flag2) {
                    f2 *= 1.5f;
                }
                f2 += f1;
                ...
                boolean flag5 = targetEntity.attackEntityFrom(DamageSource.causePlayerDamage(this), f2);
                if (flag5) {
                    IEntityMultiPart ientitymultipart;
                    if (i2 > 0) {
                        if (targetEntity instanceof EntityLivingBase) {
                            ((EntityLivingBase)targetEntity).knockBack(this, (float)i2 * 0.5f, MathHelper.sin(this.rotationYaw * ((float)Math.PI / 180)), -MathHelper.cos(this.rotationYaw * ((float)Math.PI / 180)));
                        } else {
                            targetEntity.addVelocity(-MathHelper.sin(this.rotationYaw * ((float)Math.PI / 180)) * (float)i2 * 0.5f, 0.1, MathHelper.cos(this.rotationYaw * ((float)Math.PI / 180)) * (float)i2 * 0.5f);
                        }
                        this.motionX *= 0.6;
                        this.motionZ *= 0.6;
                    ...
                    if (flag3) {
                        float f3 = 1.0f + EnchantmentHelper.func_191527_a(this) * f2;
                    ...
                    if (targetEntity instanceof EntityLivingBase) {
                        ...
                        if (this.world instanceof WorldServer && f5 > 2.0f) {
                            ...
                            ((WorldServer)this.world).spawnParticle(EnumParticleTypes.DAMAGE_INDICATOR, targetEntity.posX, targetEntity.posY + (double)(targetEntity.height * 0.5f), targetEntity.posZ, k2, 0.1, 0.0, 0.1, 0.2, new int[0]);
                        }
                    }
    ...
    public SleepResult trySleep(BlockPos bedLocation) {
        ...
        if (this.world.isBlockLoaded(bedLocation)) {
            ...
            float f2 = 0.5f + (float)enumfacing.getFrontOffsetZ() * 0.4f;
            this.setRenderOffsetForSleep(enumfacing);
            this.setPosition((float)bedLocation.getX() + f1, (float)bedLocation.getY() + 0.6875f, (float)bedLocation.getZ() + f2);
        } else {
    ...
    public void wakeUpPlayer(boolean immediately, boolean updateWorldFlag, boolean setSpawn) {
        ...
        int n2 = this.sleepTimer = immediately ? 0 : 100;
        if (setSpawn) {
    ...
    public void addMovementStat(double p_71000_1_, double p_71000_3_, double p_71000_5_) {
        if (!this.isRiding()) {
            if (this.isInsideOfMaterial(Material.WATER)) {
                int i2 = Math.round(MathHelper.sqrt(p_71000_1_ * p_71000_1_ + p_71000_3_ * p_71000_3_ + p_71000_5_ * p_71000_5_) * 100.0f);
                if (i2 > 0) {
                    this.addStat(StatList.DIVE_ONE_CM, i2);
                    this.addExhaustion(0.01f * (float)i2 * 0.01f);
                }
            } else if (this.isInWater()) {
                int j2 = Math.round(MathHelper.sqrt(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0f);
                if (j2 > 0) {
                    this.addStat(StatList.SWIM_ONE_CM, j2);
                    this.addExhaustion(0.01f * (float)j2 * 0.01f);
                }
                ...
                int k2 = Math.round(MathHelper.sqrt(p_71000_1_ * p_71000_1_ + p_71000_5_ * p_71000_5_) * 100.0f);
                if (k2 > 0) {
                    if (this.isSprinting()) {
                        this.addStat(StatList.SPRINT_ONE_CM, k2);
                        this.addExhaustion(0.1f * (float)k2 * 0.01f);
                    } else if (this.isSneaking()) {
                        this.addStat(StatList.CROUCH_ONE_CM, k2);
                        this.addExhaustion(0.0f * (float)k2 * 0.01f);
                    } else {
                        this.addStat(StatList.WALK_ONE_CM, k2);
                        this.addExhaustion(0.0f * (float)k2 * 0.01f);
                    }
                }
            } else if (this.isElytraFlying()) {
                int l2 = Math.round(MathHelper.sqrt(p_71000_1_ * p_71000_1_ + p_71000_3_ * p_71000_3_ + p_71000_5_ * p_71000_5_) * 100.0f);
                this.addStat(StatList.AVIATE_ONE_CM, l2);
            } else {
    ...
    private void addMountedMovementStat(double p_71015_1_, double p_71015_3_, double p_71015_5_) {
        int i2;
        if (this.isRiding() && (i2 = Math.round(MathHelper.sqrt(p_71015_1_ * p_71015_1_ + p_71015_3_ * p_71015_3_ + p_71015_5_ * p_71015_5_) * 100.0f)) > 0) {
            if (this.getRidingEntity() instanceof EntityMinecart) {
                this.addStat(StatList.MINECART_ONE_CM, i2);
            } else if (this.getRidingEntity() instanceof EntityBoat) {
                this.addStat(StatList.BOAT_ONE_CM, i2);
            } else if (this.getRidingEntity() instanceof EntityPig) {
                this.addStat(StatList.PIG_ONE_CM, i2);
            } else if (this.getRidingEntity() instanceof AbstractHorse) {
                this.addStat(StatList.HORSE_ONE_CM, i2);
            }
        }
    }
    ...
    public void addExperienceLevel(int levels) {
        ...
        if (levels > 0 && this.experienceLevel % 5 == 0 && (float)this.lastXPSound < (float)this.ticksExisted - 100.0f) {
            ...
            this.world.playSound(null, this.posX, this.posY, this.posZ, SoundEvents.ENTITY_PLAYER_LEVELUP, this.getSoundCategory(), f2 * 0.75f, 1.0f);
    ...
    public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
        ...
        if (entityequipmentslot == null) {
            ...
            if (i2 >= 0 && i2 < this.theInventoryEnderChest.getSizeInventory()) {
                this.theInventoryEnderChest.setInventorySlotContents(i2, itemStackIn);
                return true;
            }
            return false;
        }
    ...
    public static enum EnumChatVisibility {
        ...
        static {
            ...
            int n2 = arrenumChatVisibility.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                EnumChatVisibility entityplayer$enumchatvisibility;
                EnumChatVisibility.ID_LOOKUP[entityplayer$enumchatvisibility.chatVisibility] = entityplayer$enumchatvisibility = arrenumChatVisibility[i2];
            }
        }

        private EnumChatVisibility(int id2, String resourceKey) {
            this.chatVisibility = id2;
        ...
        public static EnumChatVisibility getEnumChatVisibility(int id2) {
            return ID_LOOKUP[id2 % ID_LOOKUP.length];
        }
