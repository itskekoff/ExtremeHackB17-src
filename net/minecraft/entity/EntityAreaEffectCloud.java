...
extends Entity {
    ...
    public EntityAreaEffectCloud(World worldIn, double x2, double y2, double z2) {
        this(worldIn);
    ...
    public void onUpdate() {
        block25: {
            ...
            block23: {
                ...
                block24: {
                    ...
                    for (int i2 = 0; i2 < 2; ++i2) {
                        ...
                        float f22 = MathHelper.sqrt(this.rand.nextFloat()) * 0.2f;
                        float f3 = MathHelper.cos(f1) * f22;
                        float f4 = MathHelper.sin(f1) * f22;
                        if (enumparticletypes == EnumParticleTypes.SPELL_MOB) {
                            int j2 = this.rand.nextBoolean() ? 0xFFFFFF : this.getColor();
                            int k2 = j2 >> 16 & 0xFF;
                            int l2 = j2 >> 8 & 0xFF;
                            int i1 = j2 & 0xFF;
                            this.world.func_190523_a(EnumParticleTypes.SPELL_MOB.getParticleID(), this.posX + (double)f3, this.posY, this.posZ + (double)f4, (float)k2 / 255.0f, (float)l2 / 255.0f, (float)i1 / 255.0f, new int[0]);
                            continue;
                        }
                ...
                float f5 = (float)Math.PI * f2 * f2;
                int k1 = 0;
                while ((float)k1 < f5) {
                    float f6 = this.rand.nextFloat() * ((float)Math.PI * 2);
                    float f7 = MathHelper.sqrt(this.rand.nextFloat()) * f2;
            ...
            boolean bl2 = flag1 = this.ticksExisted < this.waitTime;
            if (flag != flag1) {
                ...
                if (lstPotions.isEmpty()) {
                    ...
                    if (!list.isEmpty()) {
                        for (EntityLivingBase entitylivingbase : list) {
                            ...
                            double d0;
                            double d2;
                            if (this.reapplicationDelayMap.containsKey(entitylivingbase) || !entitylivingbase.canBeHitWithPotion() || !((d2 = (d0 = entitylivingbase.posX - this.posX) * d0 + (d1 = entitylivingbase.posZ - this.posZ) * d1) <= (double)(f2 * f2))) continue;
    ...
    protected void readEntityFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("Effects", 9)) {
            ...
            for (int i2 = 0; i2 < nbttaglist.tagCount(); ++i2) {
                PotionEffect potioneffect = PotionEffect.readCustomPotionEffectFromNBT(nbttaglist.getCompoundTagAt(i2));
