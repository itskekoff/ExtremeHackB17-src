...
extends EntityThrowable {
    ...
    public EntityPotion(World worldIn, double x2, double y2, double z2, ItemStack potionDamageIn) {
    ...
    protected void onImpact(RayTraceResult result) {
        if (!this.world.isRemote) {
            ...
            boolean bl2 = flag = potiontype == PotionTypes.WATER && list.isEmpty();
            ...
            int i2 = potiontype.hasInstantEffect() ? 2007 : 2002;
            this.world.playEvent(i2, new BlockPos(this), PotionUtils.func_190932_c(itemstack));
            this.setDead();
        }
    }
        ...
        if (!list.isEmpty()) {
            for (EntityLivingBase entitylivingbase : list) {
                ...
                for (PotionEffect potioneffect : p_190543_2_) {
                    ...
                    int i2 = (int)(d1 * (double)potioneffect.getDuration() + 0.5);
                    if (i2 <= 20) continue;
                    entitylivingbase.addPotionEffect(new PotionEffect(potion, i2, potioneffect.getAmplifier(), potioneffect.getIsAmbient(), potioneffect.doesShowParticles()));
                }
            }
        }
    }
