...
implements IProjectile {
    ...
    public void setAim(Entity shooter, float pitch, float yaw, float p_184547_4_, float velocity, float inaccuracy) {
        float f2 = -MathHelper.sin(yaw * ((float)Math.PI / 180)) * MathHelper.cos(pitch * ((float)Math.PI / 180));
        float f1 = -MathHelper.sin(pitch * ((float)Math.PI / 180));
        float f22 = MathHelper.cos(yaw * ((float)Math.PI / 180)) * MathHelper.cos(pitch * ((float)Math.PI / 180));
    ...
    public void onUpdate() {
        ...
        if (this.inGround) {
            ...
            if (!(block == this.inTile && j2 == this.inData || this.world.collidesWithAnyBlock(this.getEntityBoundingBox().expandXyz(0.05)))) {
                this.inGround = false;
            ...
            if (this.getIsCritical()) {
                for (int k2 = 0; k2 < 4; ++k2) {
                    this.world.spawnParticle(EnumParticleTypes.CRIT, this.posX + this.motionX * (double)k2 / 4.0, this.posY + this.motionY * (double)k2 / 4.0, this.posZ + this.motionZ * (double)k2 / 4.0, -this.motionX, -this.motionY + 0.2, -this.motionZ, new int[0]);
                }
            }
    ...
    protected void onHit(RayTraceResult raytraceResultIn) {
        ...
        if (entity != null) {
            ...
            int i2 = MathHelper.ceil((double)f2 * this.damage);
            if (this.getIsCritical()) {
            ...
            if (entity.attackEntityFrom(damagesource, i2)) {
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        boolean bl2 = this.inGround = compound.getByte("inGround") == 1;
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn) {
        if (!this.world.isRemote && this.inGround && this.arrowShake <= 0) {
            boolean flag;
            boolean bl2 = flag = this.pickupStatus == PickupStatus.ALLOWED || this.pickupStatus == PickupStatus.CREATIVE_ONLY && entityIn.capabilities.isCreativeMode;
    ...
    public void func_190547_a(EntityLivingBase p_190547_1_, float p_190547_2_) {
        int i2 = EnchantmentHelper.getMaxEnchantmentLevel(Enchantments.POWER, p_190547_1_);
        int j2 = EnchantmentHelper.getMaxEnchantmentLevel(Enchantments.PUNCH, p_190547_1_);
        ...
        if (i2 > 0) {
            this.setDamage(this.getDamage() + (double)i2 * 0.5 + 0.5);
        }
        if (j2 > 0) {
            this.setKnockbackStrength(j2);
        }
