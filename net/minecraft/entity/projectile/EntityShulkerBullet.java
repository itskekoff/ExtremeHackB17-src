...
extends Entity {
    ...
    public EntityShulkerBullet(World worldIn, double x2, double y2, double z2, double motionXIn, double motionYIn, double motionZIn) {
        this(worldIn);
    ...
    private void selectNextMoveDirection(@Nullable EnumFacing.Axis p_184569_1_) {
        ...
        if (blockpos.distanceSqToCenter(this.posX, this.posY, this.posZ) >= 4.0) {
            ...
            if (list.isEmpty()) {
                for (int i2 = 5; !this.world.isAirBlock(blockpos1.offset(enumfacing)) && i2 > 0; --i2) {
                    enumfacing = EnumFacing.random(this.rand);
                }
            } else {
