...
extends EntityLivingBase {
    ...
    public EntityLiving(World worldIn) {
        ...
        this.randomMobsId = (int)(i2 & Integer.MAX_VALUE);
    }
    ...
    public float getPathPriority(PathNodeType nodeType) {
        Float f2 = this.mapPathPriority.get((Object)nodeType);
        return f2 == null ? nodeType.getPriority() : f2.floatValue();
    }
    ...
    protected int getExperiencePoints(EntityPlayer player) {
        if (this.experienceValue > 0) {
            ...
            for (int j2 = 0; j2 < this.inventoryArmor.size(); ++j2) {
                if (this.inventoryArmor.get(j2).func_190926_b() || !(this.inventoryArmorDropChances[j2] <= 1.0f)) continue;
                i2 += 1 + this.rand.nextInt(3);
            }
            for (int k2 = 0; k2 < this.inventoryHands.size(); ++k2) {
                if (this.inventoryHands.get(k2).func_190926_b() || !(this.inventoryHandsDropChances[k2] <= 1.0f)) continue;
                i2 += 1 + this.rand.nextInt(3);
            }
            return i2;
        }
    ...
    protected void dropFewItems(boolean wasRecentlyHit, int lootingModifier) {
        ...
        if (item != null) {
            ...
            if (lootingModifier > 0) {
                i2 += this.rand.nextInt(lootingModifier + 1);
            }
    ...
    public void writeEntityToNBT(NBTTagCompound compound) {
        ...
        float[] arrf = this.inventoryArmorDropChances;
        int n2 = this.inventoryArmorDropChances.length;
        for (int nbttagcompound1 = 0; nbttagcompound1 < n2; ++nbttagcompound1) {
            float f2 = arrf[nbttagcompound1];
            nbttaglist2.appendTag(new NBTTagFloat(f2));
        }
        compound.setTag("ArmorDropChances", nbttaglist2);
        NBTTagList nbttaglist3 = new NBTTagList();
        float[] arrf2 = this.inventoryHandsDropChances;
        int n3 = this.inventoryHandsDropChances.length;
        for (n2 = 0; n2 < n3; ++n2) {
            float f1 = arrf2[n2];
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("ArmorItems", 9)) {
            ...
            for (int i2 = 0; i2 < this.inventoryArmor.size(); ++i2) {
                this.inventoryArmor.set(i2, new ItemStack(nbttaglist.getCompoundTagAt(i2)));
            }
        }
        if (compound.hasKey("HandItems", 9)) {
            NBTTagList nbttaglist1 = compound.getTagList("HandItems", 10);
            for (int j2 = 0; j2 < this.inventoryHands.size(); ++j2) {
                this.inventoryHands.set(j2, new ItemStack(nbttaglist1.getCompoundTagAt(j2)));
            }
        }
        if (compound.hasKey("ArmorDropChances", 9)) {
            NBTTagList nbttaglist2 = compound.getTagList("ArmorDropChances", 5);
            for (int k2 = 0; k2 < nbttaglist2.tagCount(); ++k2) {
                this.inventoryArmorDropChances[k2] = nbttaglist2.getFloatAt(k2);
            }
        }
        if (compound.hasKey("HandDropChances", 9)) {
            NBTTagList nbttaglist3 = compound.getTagList("HandDropChances", 5);
            for (int l2 = 0; l2 < nbttaglist3.tagCount(); ++l2) {
                this.inventoryHandsDropChances[l2] = nbttaglist3.getFloatAt(l2);
            }
        }
    ...
    public void faceEntity(Entity entityIn, float maxYawIncrease, float maxPitchIncrease) {
        ...
        float f2 = (float)(MathHelper.atan2(d1, d0) * 57.29577951308232) - 90.0f;
        ...
        this.rotationYaw = this.updateRotation(this.rotationYaw, f2, maxYawIncrease);
    }

    private float updateRotation(float angle, float targetAngle, float maxIncrease) {
        float f2 = MathHelper.wrapDegrees(targetAngle - angle);
        if (f2 > maxIncrease) {
            f2 = maxIncrease;
        }
        if (f2 < -maxIncrease) {
    ...
    public int getMaxFallHeight() {
        ...
        if ((i2 -= (3 - this.world.getDifficulty().getDifficultyId()) * 4) < 0) {
            i2 = 0;
        }
        return i2 + 3;
    }
    ...
    protected void setEquipmentBasedOnDifficulty(DifficultyInstance difficulty) {
        if (this.rand.nextFloat() < 0.15f * difficulty.getClampedAdditionalDifficulty()) {
            ...
            float f3 = f2 = this.world.getDifficulty() == EnumDifficulty.HARD ? 0.1f : 0.25f;
            ...
            for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
                ...
                ItemStack itemstack = this.getItemStackFromSlot(entityequipmentslot);
                if (!flag && this.rand.nextFloat() < f2) break;
                flag = false;
                if (!itemstack.func_190926_b() || (item = EntityLiving.getArmorByChance(entityequipmentslot, i2)) == null) continue;
    ...
    public static EntityEquipmentSlot getSlotForItemStack(ItemStack stack) {
        if (stack.getItem() != Item.getItemFromBlock(Blocks.PUMPKIN) && stack.getItem() != Items.SKULL) {
            ...
            boolean bl2 = flag = stack.getItem() == Items.SHIELD;
    ...
    protected void setEnchantmentBasedOnDifficulty(DifficultyInstance difficulty) {
        ...
        if (!this.getHeldItemMainhand().func_190926_b() && this.rand.nextFloat() < 0.25f * f2) {
            this.setItemStackToSlot(EntityEquipmentSlot.MAINHAND, EnchantmentHelper.addRandomEnchantment(this.rand, this.getHeldItemMainhand(), (int)(5.0f + f2 * (float)this.rand.nextInt(18)), false));
        }
        for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values()) {
            ItemStack itemstack;
            if (entityequipmentslot.getSlotType() != EntityEquipmentSlot.Type.ARMOR || (itemstack = this.getItemStackFromSlot(entityequipmentslot)).func_190926_b() || !(this.rand.nextFloat() < 0.5f * f2)) continue;
            this.setItemStackToSlot(entityequipmentslot, EnchantmentHelper.addRandomEnchantment(this.rand, itemstack, (int)(5.0f + f2 * (float)this.rand.nextInt(18)), false));
        }
    }
    ...
    private void onUpdateMinimal() {
        float f2;
        ++this.entityAge;
        if (this instanceof EntityMob && (f2 = this.getBrightness()) > 0.5f) {
            this.entityAge += 2;
        }
        this.despawnEntity();
    }
