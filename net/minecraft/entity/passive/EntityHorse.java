...
extends AbstractHorse {
    ...
    static {
        ...
        HORSE_MARKING_TEXTURES = arrstring;
    ...
    private void setHorseTexturePaths() {
        int i2 = this.getHorseVariant();
        int j2 = (i2 & 0xFF) % 7;
        int k2 = ((i2 & 0xFF00) >> 8) % 5;
        HorseArmorType horsearmortype = this.getHorseArmorType();
        this.horseTexturesArray[0] = HORSE_TEXTURES[j2];
        this.horseTexturesArray[1] = HORSE_MARKING_TEXTURES[k2];
        this.horseTexturesArray[2] = horsearmortype.getTextureName();
        this.texturePrefix = "horse/" + HORSE_TEXTURES_ABBR[j2] + HORSE_MARKING_TEXTURES_ABBR[k2] + horsearmortype.getHash();
    }
    ...
    public void setHorseArmorStack(ItemStack itemStackIn) {
        ...
        if (!this.world.isRemote) {
            ...
            int i2 = horsearmortype.getProtection();
            if (i2 != 0) {
                this.getEntityAttribute(SharedMonsterAttributes.ARMOR).applyModifier(new AttributeModifier(ARMOR_MODIFIER_UUID, "Horse armor bonus", i2, 0).setSaved(false));
            }
        }
    }
    ...
    public boolean processInteract(EntityPlayer player, EnumHand hand) {
        ...
        boolean bl2 = flag = !itemstack.func_190926_b();
        ...
        if (flag) {
            ...
            boolean bl3 = flag2 = !this.isChild() && !this.isHorseSaddled() && itemstack.getItem() == Items.SADDLE;
            if (flag1 || flag2) {
                this.openGUI(player);
                return true;
            }
    ...
    public EntityAgeable createChild(EntityAgeable ageable) {
        ...
        if (ageable instanceof EntityDonkey) {
            ...
            int j2 = this.rand.nextInt(9);
            int i2 = j2 < 4 ? this.getHorseVariant() & 0xFF : (j2 < 8 ? entityhorse.getHorseVariant() & 0xFF : this.rand.nextInt(7));
            int k2 = this.rand.nextInt(5);
            i2 = k2 < 2 ? (i2 |= this.getHorseVariant() & 0xFF00) : (k2 < 4 ? (i2 |= entityhorse.getHorseVariant() & 0xFF00) : (i2 |= this.rand.nextInt(5) << 8 & 0xFF00));
            ((EntityHorse)abstracthorse).setHorseVariant(i2);
        }
    ...
    public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
        ...
        if ((livingdata = super.onInitialSpawn(difficulty, livingdata)) instanceof GroupData) {
            i2 = ((GroupData)livingdata).field_190885_a;
        } else {
            i2 = this.rand.nextInt(7);
            livingdata = new GroupData(i2);
        }
        this.setHorseVariant(i2 | this.rand.nextInt(5) << 8);
        return livingdata;
    }
