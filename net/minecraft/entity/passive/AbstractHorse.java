...
IJumpingMount {
    ...
    public int increaseTemper(int p_110198_1_) {
        int i2 = MathHelper.clamp(this.getTemper() + p_110198_1_, 0, this.func_190676_dC());
        this.setTemper(i2);
        return i2;
    }
    ...
    public void fall(float distance, float damageMultiplier) {
        ...
        if ((i2 = MathHelper.ceil((distance * 0.5f - 3.0f) * damageMultiplier)) > 0) {
            this.attackEntityFrom(DamageSource.fall, i2);
            if (this.isBeingRidden()) {
                for (Entity entity : this.getRecursivePassengers()) {
                    entity.attackEntityFrom(DamageSource.fall, i2);
                }
            }
    ...
    protected void initHorseChest() {
        ...
        if (containerhorsechest != null) {
            ...
            int i2 = Math.min(containerhorsechest.getSizeInventory(), this.horseChest.getSizeInventory());
            for (int j2 = 0; j2 < i2; ++j2) {
                ItemStack itemstack = containerhorsechest.getStackInSlot(j2);
    ...
    protected boolean func_190678_b(EntityPlayer p_190678_1_, ItemStack p_190678_2_) {
        ...
        if (j2 > 0 && (flag || !this.isTame()) && this.getTemper() < this.func_190676_dC()) {
            flag = true;
            if (!this.world.isRemote) {
                this.increaseTemper(j2);
            }
        }
        if (flag) {
            this.eatingHorse();
    ...
    public void onDeath(DamageSource cause) {
        ...
        if (!this.world.isRemote && this.horseChest != null) {
            for (int i2 = 0; i2 < this.horseChest.getSizeInventory(); ++i2) {
    ...
    public void readEntityFromNBT(NBTTagCompound compound) {
        ...
        if (compound.hasKey("OwnerUUID", 8)) {
            s2 = compound.getString("OwnerUUID");
        } else {
            String s1 = compound.getString("Owner");
            s2 = PreYggdrasilConverter.convertMobOwnerIfNeeded(this.getServer(), s1);
        }
        if (!s2.isEmpty()) {
            this.setOwnerUniqueId(UUID.fromString(s2));
        }
    ...
    public void updatePassenger(Entity passenger) {
        ...
        if (this.prevRearingAmount > 0.0f) {
            ...
            float f22 = 0.15f * this.prevRearingAmount;
            passenger.setPosition(this.posX + (double)(f1 * f3), this.posY + this.getMountedYOffset() + passenger.getYOffset() + (double)f22, this.posZ - (double)(f1 * f2));
    ...
    public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn) {
        int i2 = inventorySlot - 400;
        if (i2 >= 0 && i2 < 2 && i2 < this.horseChest.getSizeInventory()) {
            if (i2 == 0 && itemStackIn.getItem() != Items.SADDLE) {
                return false;
            }
            if (i2 != 1 || this.func_190677_dK() && this.func_190682_f(itemStackIn)) {
