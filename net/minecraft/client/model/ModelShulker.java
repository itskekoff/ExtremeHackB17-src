...
extends ModelBase {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        float f2 = ageInTicks - (float)entityshulker.ticksExisted;
        float f1 = (0.5f + entityshulker.getClientPeekAmount(f2)) * (float)Math.PI;
        float f22 = -1.0f + MathHelper.sin(f1);
        float f3 = 0.0f;
        if (f1 > (float)Math.PI) {
            f3 = MathHelper.sin(ageInTicks * 0.1f) * 0.7f;
        }
        this.lid.setRotationPoint(0.0f, 16.0f + MathHelper.sin(f1) * 8.0f + f3, 0.0f);
        this.lid.rotateAngleY = entityshulker.getClientPeekAmount(f2) > 0.3f ? f22 * f22 * f22 * f22 * (float)Math.PI * 0.125f : 0.0f;
