...
extends ModelVillager {
    ...
    public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn) {
        ...
        float f2 = 0.01f * (float)(entityIn.getEntityId() % 10);
        this.villagerNose.rotateAngleX = MathHelper.sin((float)entityIn.ticksExisted * f2) * 4.5f * ((float)Math.PI / 180);
        this.villagerNose.rotateAngleY = 0.0f;
        this.villagerNose.rotateAngleZ = MathHelper.cos((float)entityIn.ticksExisted * f2) * 2.5f * ((float)Math.PI / 180);
        if (this.holdingItem) {
