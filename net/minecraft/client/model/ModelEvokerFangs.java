...
extends ModelBase {
    ...
    public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        float f2 = limbSwing * 2.0f;
        if (f2 > 1.0f) {
            f2 = 1.0f;
        }
        f2 = 1.0f - f2 * f2 * f2;
        this.field_191214_b.rotateAngleZ = (float)Math.PI - f2 * 0.35f * (float)Math.PI;
        this.field_191215_c.rotateAngleZ = (float)Math.PI + f2 * 0.35f * (float)Math.PI;
