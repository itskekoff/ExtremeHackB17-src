...
extends ModelBase {
    ...
    public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
        ...
        float f2 = entitydragon.prevAnimTime + (entitydragon.animTime - entitydragon.prevAnimTime) * this.partialTicks;
        this.jaw.rotateAngleX = (float)(Math.sin(f2 * ((float)Math.PI * 2)) + 1.0) * 0.2f;
        float f1 = (float)(Math.sin(f2 * ((float)Math.PI * 2) - 1.0f) + 1.0);
        ...
        float f8 = f2 * ((float)Math.PI * 2);
        f22 = 20.0f;
        float f3 = -12.0f;
        for (int i2 = 0; i2 < 5; ++i2) {
            double[] adouble1 = entitydragon.getMovementOffsets(5 - i2, this.partialTicks);
            float f9 = (float)Math.cos((float)i2 * 0.45f + f8) * 0.15f;
            this.spine.rotateAngleY = this.updateRotations(adouble1[0] - adouble[0]) * ((float)Math.PI / 180) * 1.5f;
            this.spine.rotateAngleX = f9 + entitydragon.getHeadPartYOffset(i2, adouble, adouble1) * ((float)Math.PI / 180) * 1.5f * 5.0f;
            this.spine.rotateAngleZ = -this.updateRotations(adouble1[0] - (double)f7) * ((float)Math.PI / 180) * 1.5f;
            this.spine.rotationPointY = f22;
            this.spine.rotationPointZ = f3;
            this.spine.rotationPointX = f4;
            f22 = (float)((double)f22 + Math.sin(this.spine.rotateAngleX) * 10.0);
        ...
        float f10 = -((float)Math.sin(f2 * ((float)Math.PI * 2))) * 0.0f;
        f8 = f2 * ((float)Math.PI * 2);
        f22 = 10.0f;
        f3 = 60.0f;
        f4 = 0.0f;
        adouble = entitydragon.getMovementOffsets(11, this.partialTicks);
        for (int k2 = 0; k2 < 12; ++k2) {
            adouble2 = entitydragon.getMovementOffsets(12 + k2, this.partialTicks);
            f10 = (float)((double)f10 + Math.sin((float)k2 * 0.45f + f8) * (double)0.05f);
