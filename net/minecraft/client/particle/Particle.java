...
public class Particle {
    ...
    public void renderParticle(BufferBuilder worldRendererIn, Entity entityIn, float partialTicks, float rotationX, float rotationZ, float rotationYZ, float rotationXY, float rotationXZ) {
        float f2 = (float)this.particleTextureIndexX / 16.0f;
        float f1 = f2 + 0.0624375f;
        float f22 = (float)this.particleTextureIndexY / 16.0f;
        float f3 = f22 + 0.0624375f;
        float f4 = 0.1f * this.particleScale;
        if (this.particleTexture != null) {
            f2 = this.particleTexture.getMinU();
            f1 = this.particleTexture.getMaxU();
            f22 = this.particleTexture.getMinV();
        ...
        if (this.particleAngle != 0.0f) {
            ...
            for (int l2 = 0; l2 < 4; ++l2) {
                avec3d[l2] = vec3d.scale(2.0 * avec3d[l2].dotProduct(vec3d)).add(avec3d[l2].scale((double)(f9 * f9) - vec3d.dotProduct(vec3d))).add(vec3d.crossProduct(avec3d[l2]).scale(2.0f * f9));
            }
        }
        worldRendererIn.pos((double)f5 + avec3d[0].xCoord, (double)f6 + avec3d[0].yCoord, (double)f7 + avec3d[0].zCoord).tex(f1, f3).color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha).lightmap(j2, k2).endVertex();
        worldRendererIn.pos((double)f5 + avec3d[1].xCoord, (double)f6 + avec3d[1].yCoord, (double)f7 + avec3d[1].zCoord).tex(f1, f22).color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha).lightmap(j2, k2).endVertex();
        worldRendererIn.pos((double)f5 + avec3d[2].xCoord, (double)f6 + avec3d[2].yCoord, (double)f7 + avec3d[2].zCoord).tex(f2, f22).color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha).lightmap(j2, k2).endVertex();
        worldRendererIn.pos((double)f5 + avec3d[3].xCoord, (double)f6 + avec3d[3].yCoord, (double)f7 + avec3d[3].zCoord).tex(f2, f3).color(this.particleRed, this.particleGreen, this.particleBlue, this.particleAlpha).lightmap(j2, k2).endVertex();
    }
    ...
    public void setPosition(double p_187109_1_, double p_187109_3_, double p_187109_5_) {
        ...
        this.setEntityBoundingBox(new AxisAlignedBB(p_187109_1_ - (double)f2, p_187109_3_, p_187109_5_ - (double)f2, p_187109_1_ + (double)f2, p_187109_3_ + (double)f1, p_187109_5_ + (double)f2));
    }

    public void moveEntity(double x2, double y2, double z2) {
        double d0 = y2;
        if (this.canCollide) {
            List<AxisAlignedBB> list = this.worldObj.getCollisionBoxes(null, this.getEntityBoundingBox().addCoord(x2, y2, z2));
            ...
            for (AxisAlignedBB axisalignedbb1 : list) {
                x2 = axisalignedbb1.calculateXOffset(this.getEntityBoundingBox(), x2);
            }
            this.setEntityBoundingBox(this.getEntityBoundingBox().offset(x2, 0.0, 0.0));
            for (AxisAlignedBB axisalignedbb2 : list) {
                z2 = axisalignedbb2.calculateZOffset(this.getEntityBoundingBox(), z2);
            }
        ...
        boolean bl2 = this.isCollided = y2 != y2 && d0 < 0.0;
        if (x2 != x2) {
            this.motionX = 0.0;
        }
        if (z2 != z2) {
