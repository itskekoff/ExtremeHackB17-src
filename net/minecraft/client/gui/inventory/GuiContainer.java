...
extends GuiScreen {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (!itemstack.func_190926_b()) {
            int j22 = 8;
            int k22 = this.draggedStack.func_190926_b() ? 8 : 16;
            String s2 = null;
            if (!this.draggedStack.func_190926_b() && this.isRightMouseClick) {
                ...
                if (itemstack.func_190926_b()) {
                    s2 = (Object)((Object)TextFormatting.YELLOW) + "0";
                }
            }
            this.drawItemStack(itemstack, mouseX - i2 - 8, mouseY - j2 - k22, s2);
        }
    ...
    private void drawItemStack(ItemStack stack, int x2, int y2, String altText) {
        ...
        this.itemRender.renderItemOverlayIntoGUI(this.fontRendererObj, stack, x2, y2 - (this.draggedStack.func_190926_b() ? 0 : 8), altText);
        zLevel = 0.0f;
    ...
    private void drawSlot(Slot slotIn) {
        ...
        if (slotIn == this.clickedSlot && !this.draggedStack.func_190926_b() && this.isRightMouseClick && !itemstack.func_190926_b()) {
            ...
            if (Container.canAddItemToSlot(slotIn, itemstack1, true) && this.inventorySlots.canDragIntoSlot(slotIn)) {
                ...
                int k2 = Math.min(itemstack.getMaxStackSize(), slotIn.getItemStackLimit(itemstack));
                if (itemstack.func_190916_E() > k2) {
                    s2 = String.valueOf(TextFormatting.YELLOW.toString()) + k2;
                    itemstack.func_190920_e(k2);
                }
            } else {
        ...
        if (!flag1) {
            ...
            GlStateManager.enableDepth();
            this.itemRender.renderItemAndEffectIntoGUI(this.mc.player, itemstack, i2, j2);
        ...
        if (!itemstack.func_190926_b() && this.dragSplitting) {
            if (this.dragSplittingLimit == 2) {
                ...
                for (Slot slot : this.dragSplittingSlots) {
                    ...
                    int i2 = itemstack2.func_190926_b() ? 0 : itemstack2.func_190916_E();
                    Container.computeStackSize(this.dragSplittingSlots, this.dragSplittingLimit, itemstack1, i2);
                    int j2 = Math.min(itemstack1.getMaxStackSize(), slot.getItemStackLimit(itemstack1));
                    if (itemstack1.func_190916_E() > j2) {
                        itemstack1.func_190920_e(j2);
                    }
                    this.dragSplittingRemnant -= itemstack1.func_190916_E() - i2;
                }
            }
        }
    }
    ...
    private Slot getSlotAtPosition(int x2, int y2) {
        for (int i2 = 0; i2 < this.inventorySlots.inventorySlots.size(); ++i2) {
            ...
            if (!this.isMouseOverSlot(slot, x2, y2) || !slot.canBeHovered()) continue;
            return slot;
        }
        return null;
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (mouseButton == 0 || mouseButton == 1 || flag) {
            int j2 = this.guiLeft;
            int k2 = this.guiTop;
            boolean flag1 = this.func_193983_c(mouseX, mouseY, j2, k2);
            int l2 = -1;
            if (slot != null) {
                l2 = slot.slotNumber;
            }
                ...
                if (this.mc.gameSettings.touchscreen) {
                    ...
                    if (this.mc.player.inventory.getItemStack().func_190926_b()) {
                        if (mouseButton == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
                            this.handleMouseClick(slot, l2, mouseButton, ClickType.CLONE);
                        } else {
                            ...
                            this.handleMouseClick(slot, l2, mouseButton, clicktype);
                        }
                        this.ignoreMouseUp = true;
                    } else {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state) {
        ...
        int i2 = this.guiLeft;
        int j2 = this.guiTop;
        boolean flag = this.func_193983_c(mouseX, mouseY, i2, j2);
        int k2 = -1;
        if (slot != null) {
            k2 = slot.slotNumber;
        }
        ...
        if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot(ItemStack.field_190927_a, slot)) {
            if (GuiContainer.isShiftKeyDown()) {
                ...
                this.handleMouseClick(slot, k2, state, ClickType.PICKUP_ALL);
            }
            ...
            if (this.clickedSlot != null && this.mc.gameSettings.touchscreen) {
                if (state == 0 || state == 1) {
                    ...
                    if (k2 != -1 && !this.draggedStack.func_190926_b() && flag2) {
                        this.handleMouseClick(this.clickedSlot, this.clickedSlot.slotNumber, state, ClickType.PICKUP);
                        this.handleMouseClick(slot, k2, 0, ClickType.PICKUP);
                        if (this.mc.player.inventory.getItemStack().func_190926_b()) {
                            this.returningStack = ItemStack.field_190927_a;
                        } else {
                            this.handleMouseClick(this.clickedSlot, this.clickedSlot.slotNumber, state, ClickType.PICKUP);
                ...
                if (state == this.mc.gameSettings.keyBindPickBlock.getKeyCode() + 100) {
                    this.handleMouseClick(slot, k2, state, ClickType.CLONE);
                } else {
                    boolean flag1;
                    boolean bl2 = flag1 = k2 != -999 && (Keyboard.isKeyDown(42) || Keyboard.isKeyDown(54));
                    if (flag1) {
                        this.shiftClickedSlot = slot != null && slot.getHasStack() ? slot.getStack().copy() : ItemStack.field_190927_a;
                    }
                    this.handleMouseClick(slot, k2, state, flag1 ? ClickType.QUICK_MOVE : ClickType.PICKUP);
                }
            }
        }
        if (this.mc.player.inventory.getItemStack().func_190926_b()) {
    ...
    protected boolean checkHotbarKeys(int keyCode) {
        if (this.mc.player.inventory.getItemStack().func_190926_b() && this.theSlot != null) {
            for (int i2 = 0; i2 < 9; ++i2) {
                ...
                this.handleMouseClick(this.theSlot, this.theSlot.slotNumber, i2, ClickType.SWAP);
                return true;
            }
        }
        return false;
