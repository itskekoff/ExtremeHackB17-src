...
extends GuiScreen {
    ...
    private static boolean isValidCharacterForName(char p_190301_0_, int p_190301_1_) {
        int n2;
        boolean flag = true;
        Object[] arrobject = LEGAL_KEY_CODES;
        int n3 = LEGAL_KEY_CODES.length;
        for (n2 = 0; n2 < n3; ++n2) {
            int i2 = arrobject[n2];
            if (i2 != p_190301_1_) continue;
            return true;
        }
        arrobject = ChatAllowedCharacters.ILLEGAL_STRUCTURE_CHARACTERS;
        n3 = ChatAllowedCharacters.ILLEGAL_STRUCTURE_CHARACTERS.length;
        for (n2 = 0; n2 < n3; ++n2) {
            int c0 = arrobject[n2];
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        if (tileentitystructure$mode == TileEntityStructure.Mode.LOAD || tileentitystructure$mode == TileEntityStructure.Mode.SAVE) {
            ...
            String s2 = I18n.format("structure_block.include_entities", new Object[0]);
            int i2 = this.fontRendererObj.getStringWidth(s2);
            this.drawString(this.fontRendererObj, s2, width / 2 + 154 - i2, 150, 0xA0A0A0);
        }
        if (tileentitystructure$mode == TileEntityStructure.Mode.SAVE) {
            ...
            int k2 = this.fontRendererObj.getStringWidth(s2);
            this.drawString(this.fontRendererObj, s2, width / 2 + 154 - k2, 110, 0xA0A0A0);
            String s1 = I18n.format("structure_block.show_air", new Object[0]);
            int j2 = this.fontRendererObj.getStringWidth(s1);
            this.drawString(this.fontRendererObj, s1, width / 2 + 154 - j2, 70, 0xA0A0A0);
        }
        if (tileentitystructure$mode == TileEntityStructure.Mode.LOAD) {
            ...
            int l2 = this.fontRendererObj.getStringWidth(s3);
            this.drawString(this.fontRendererObj, s3, width / 2 + 154 - l2, 70, 0xA0A0A0);
        }
        if (tileentitystructure$mode == TileEntityStructure.Mode.DATA) {
