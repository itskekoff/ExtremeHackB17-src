...
extends GuiScreen {
    ...
    public void initGui() {
        ...
        int i2 = 0;
        this.buttonList.add(new Button(SoundCategory.MASTER.ordinal(), width / 2 - 155 + i2 % 2 * 160, height / 6 - 12 + 24 * (i2 >> 1), SoundCategory.MASTER, true));
        i2 += 2;
        for (SoundCategory soundcategory : SoundCategory.values()) {
            if (soundcategory == SoundCategory.MASTER) continue;
            this.buttonList.add(new Button(soundcategory.ordinal(), width / 2 - 155 + i2 % 2 * 160, height / 6 - 12 + 24 * (i2 >> 1), soundcategory, false));
            ++i2;
        }
        int j2 = width / 2 - 75;
        int k2 = height / 6 - 12;
        this.buttonList.add(new GuiOptionButton(201, j2, k2 + 24 * (++i2 >> 1), GameSettings.Options.SHOW_SUBTITLES, this.game_settings_4.getKeyBinding(GameSettings.Options.SHOW_SUBTITLES)));
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        GuiScreenOptionsSounds.drawDefaultBackground();
    ...
    protected String getDisplayString(SoundCategory category) {
        float f2 = this.game_settings_4.getSoundLevel(category);
        return f2 == 0.0f ? this.offDisplayString : String.valueOf((int)(f2 * 100.0f)) + "%";
    }
    ...
    extends GuiButton {
        ...
        public Button(int p_i46744_2_, int x2, int y2, SoundCategory categoryIn, boolean master) {
            super(p_i46744_2_, x2, y2, master ? 310 : 150, 20, "");
            this.volume = 1.0f;
