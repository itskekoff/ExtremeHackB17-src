...
public abstract class GuiSlot {
    ...
    public int getSlotIndexFromScreenCoords(int posX, int posY) {
        ...
        int k2 = posY - this.top - this.headerPadding + (int)this.amountScrolled - 4;
        int l2 = k2 / this.slotHeight;
        return posX < this.getScrollBarX() && posX >= i2 && posX <= j2 && l2 >= 0 && k2 >= 0 && l2 < this.getSize() ? l2 : -1;
    }
    ...
    public void drawScreen(int mouseXIn, int mouseYIn, float partialTicks) {
        if (this.visible) {
            ...
            GuiRenderUtils.drawBorderedRect2(this.width / 2 - 170, 0.0f, this.width / 2 + 170, this.height, 2.0f, HackConfigs.ThemeColor, new Color(10, 50, 100).getRGB() * 10000);
            if (this.hasListHeader) {
                this.drawListHeader(k2, l2, tessellator);
            }
            this.func_192638_a(k2, l2, mouseXIn, mouseYIn, partialTicks);
            ...
            if (j1 > 0) {
                ...
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                bufferbuilder.pos(i2, this.bottom, 0.0).tex(0.0, 1.0).color(0, 0, 0, 255).endVertex();
                bufferbuilder.pos(j2, this.bottom, 0.0).tex(1.0, 1.0).color(0, 0, 0, 255).endVertex();
                bufferbuilder.pos(j2, this.top, 0.0).tex(1.0, 0.0).color(0, 0, 0, 255).endVertex();
                bufferbuilder.pos(i2, this.top, 0.0).tex(0.0, 0.0).color(0, 0, 0, 255).endVertex();
                tessellator.draw();
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                bufferbuilder.pos(i2, l1 + k1, 0.0).tex(0.0, 1.0).color(128, 128, 128, 255).endVertex();
                bufferbuilder.pos(j2, l1 + k1, 0.0).tex(1.0, 1.0).color(128, 128, 128, 255).endVertex();
                bufferbuilder.pos(j2, l1, 0.0).tex(1.0, 0.0).color(128, 128, 128, 255).endVertex();
                bufferbuilder.pos(i2, l1, 0.0).tex(0.0, 0.0).color(128, 128, 128, 255).endVertex();
                tessellator.draw();
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                bufferbuilder.pos(i2, l1 + k1 - 1, 0.0).tex(0.0, 1.0).color(192, 192, 192, 255).endVertex();
                bufferbuilder.pos(j2 - 1, l1 + k1 - 1, 0.0).tex(1.0, 1.0).color(192, 192, 192, 255).endVertex();
                bufferbuilder.pos(j2 - 1, l1, 0.0).tex(1.0, 0.0).color(192, 192, 192, 255).endVertex();
                bufferbuilder.pos(i2, l1, 0.0).tex(0.0, 0.0).color(192, 192, 192, 255).endVertex();
                tessellator.draw();
            }
        ...
        if (this.isMouseYWithinSlotBounds(this.mouseY)) {
            ...
            if (Mouse.getEventButton() == 0 && Mouse.getEventButtonState() && this.mouseY >= this.top && this.mouseY <= this.bottom) {
                ...
                if (l2 < this.getSize() && this.mouseX >= i3 && this.mouseX <= j2 && l2 >= 0 && k2 >= 0) {
                    ...
                    this.clickedHeader(this.mouseX - i3, this.mouseY - this.top + (int)this.amountScrolled - 4);
                }
            }
    ...
    protected void func_192638_a(int p_192638_1_, int p_192638_2_, int p_192638_3_, int p_192638_4_, float p_192638_5_) {
        ...
        for (int j2 = 0; j2 < i2; ++j2) {
            int k2 = p_192638_2_ + j2 * this.slotHeight + this.headerPadding;
            int l2 = this.slotHeight - 4;
            if (k2 > this.bottom || k2 + l2 < this.top) {
                this.func_192639_a(j2, p_192638_1_, k2, p_192638_5_);
            }
            if (this.showSelectionBox && this.isSelected(j2)) {
                ...
                GlStateManager.disableTexture2D();
                bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
                bufferbuilder.pos(i1, k2 + l2 + 2, 0.0).tex(0.0, 1.0).color(128, 128, 128, 255).endVertex();
                bufferbuilder.pos(j1, k2 + l2 + 2, 0.0).tex(1.0, 1.0).color(128, 128, 128, 255).endVertex();
                bufferbuilder.pos(j1, k2 - 2, 0.0).tex(1.0, 0.0).color(128, 128, 128, 255).endVertex();
                bufferbuilder.pos(i1, k2 - 2, 0.0).tex(0.0, 0.0).color(128, 128, 128, 255).endVertex();
                bufferbuilder.pos(i1 + 1, k2 + l2 + 1, 0.0).tex(0.0, 1.0).color(0, 0, 0, 255).endVertex();
                bufferbuilder.pos(j1 - 1, k2 + l2 + 1, 0.0).tex(1.0, 1.0).color(0, 0, 0, 255).endVertex();
                bufferbuilder.pos(j1 - 1, k2 - 1, 0.0).tex(1.0, 0.0).color(0, 0, 0, 255).endVertex();
                bufferbuilder.pos(i1 + 1, k2 - 1, 0.0).tex(0.0, 0.0).color(0, 0, 0, 255).endVertex();
                tessellator.draw();
                GlStateManager.enableTexture2D();
            }
            if (k2 < this.top - this.slotHeight || k2 > this.bottom) continue;
            this.func_192637_a(j2, p_192638_1_, k2, l2, p_192638_3_, p_192638_4_, p_192638_5_);
        }
    }
