...
public class SpectatorMenu {
    ...
    public ISpectatorMenuObject getItem(int p_178643_1_) {
        int i2 = p_178643_1_ + this.page * 6;
        if (this.page > 0 && p_178643_1_ == 0) {
            return SCROLL_LEFT;
        }
        if (p_178643_1_ == 7) {
            return i2 < this.category.getItems().size() ? SCROLL_RIGHT_ENABLED : SCROLL_RIGHT_DISABLED;
        }
        if (p_178643_1_ == 8) {
            return CLOSE_ITEM;
        }
        return i2 >= 0 && i2 < this.category.getItems().size() ? MoreObjects.firstNonNull(this.category.getItems().get(i2), EMPTY_SLOT) : EMPTY_SLOT;
    }

    public List<ISpectatorMenuObject> getItems() {
        ArrayList<ISpectatorMenuObject> list = Lists.newArrayList();
        for (int i2 = 0; i2 <= 8; ++i2) {
            list.add(this.getItem(i2));
        }
        return list;
    }
