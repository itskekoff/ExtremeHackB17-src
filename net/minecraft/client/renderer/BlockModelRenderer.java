...
public class BlockModelRenderer {
    ...
    public boolean renderModel(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
        ...
        try {
            ...
            boolean bl2 = flag1 = flag ? this.renderModelSmooth(worldIn, modelIn, stateIn, posIn, buffer, checkSides, rand) : this.renderModelFlat(worldIn, modelIn, stateIn, posIn, buffer, checkSides, rand);
            if (flag1) {
    ...
    public boolean renderModelSmooth(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
        ...
        for (int i2 = 0; i2 < n2; ++i2) {
            EnumFacing enumfacing = arrenumFacing[i2];
    ...
    public boolean renderModelFlat(IBlockAccess worldIn, IBakedModel modelIn, IBlockState stateIn, BlockPos posIn, BufferBuilder buffer, boolean checkSides, long rand) {
        ...
        for (int i2 = 0; i2 < n2; ++i2) {
            ...
            List<BakedQuad> list = modelIn.getQuads(stateIn, enumfacing, rand);
            if (list.isEmpty() || checkSides && !stateIn.shouldSideBeRendered(worldIn, posIn, enumfacing)) continue;
            int i3 = stateIn.getPackedLightmapCoords(worldIn, posIn.offset(enumfacing));
            list = BlockModelCustomizer.getRenderQuads(list, worldIn, stateIn, posIn, enumfacing, rand, renderenv);
            this.renderQuadsFlat(worldIn, stateIn, posIn, i3, false, buffer, list, renderenv);
            flag = true;
        }
        List<BakedQuad> list1 = modelIn.getQuads(stateIn, null, rand);
        if (!list1.isEmpty()) {
    ...
    private void renderQuadsSmooth(IBlockAccess p_renderQuadsSmooth_1_, IBlockState p_renderQuadsSmooth_2_, BlockPos p_renderQuadsSmooth_3_, BufferBuilder p_renderQuadsSmooth_4_, List<BakedQuad> p_renderQuadsSmooth_5_, RenderEnv p_renderQuadsSmooth_6_) {
        ...
        int j2 = p_renderQuadsSmooth_5_.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            BakedQuad bakedquad = p_renderQuadsSmooth_5_.get(i2);
            ...
            if (bakedquad.shouldApplyDiffuseLighting()) {
                float f2 = FaceBakery.getFaceBrightness(bakedquad.getFace());
                float[] afloat1 = blockmodelrenderer$ambientocclusionface.vertexColorMultiplier;
                afloat1[0] = afloat1[0] * f2;
                afloat1 = blockmodelrenderer$ambientocclusionface.vertexColorMultiplier;
                afloat1[1] = afloat1[1] * f2;
                afloat1 = blockmodelrenderer$ambientocclusionface.vertexColorMultiplier;
                afloat1[2] = afloat1[2] * f2;
                afloat1 = blockmodelrenderer$ambientocclusionface.vertexColorMultiplier;
                afloat1[3] = afloat1[3] * f2;
            }
            int l2 = CustomColors.getColorMultiplier(bakedquad, p_renderQuadsSmooth_2_, p_renderQuadsSmooth_1_, p_renderQuadsSmooth_3_, p_renderQuadsSmooth_6_);
            if (!bakedquad.hasTintIndex() && l2 == -1) {
                ...
                int k2 = l2;
                if (l2 == -1) {
                    k2 = this.blockColors.colorMultiplier(p_renderQuadsSmooth_2_, p_renderQuadsSmooth_1_, p_renderQuadsSmooth_3_, bakedquad.getTintIndex());
                }
                if (EntityRenderer.anaglyphEnable) {
                    k2 = TextureUtil.anaglyphColor(k2);
                }
                float f1 = (float)(k2 >> 16 & 0xFF) / 255.0f;
                float f2 = (float)(k2 >> 8 & 0xFF) / 255.0f;
                float f3 = (float)(k2 & 0xFF) / 255.0f;
    ...
    private void fillQuadBounds(IBlockState stateIn, int[] vertexData, EnumFacing face, @Nullable float[] quadBounds, BitSet boundsFlags) {
        ...
        switch (face) {
            ...
            case NORTH: {
                ...
                boundsFlags.set(0, (f22 < 1.0E-4f || stateIn.isFullCube()) && f22 == f5);
                break;
            }
            case SOUTH: {
                boundsFlags.set(1, f2 >= 1.0E-4f || f1 >= 1.0E-4f || f3 <= 0.9999f || f4 <= 0.9999f);
                boundsFlags.set(0, (f5 > 0.9999f || stateIn.isFullCube()) && f22 == f5);
                break;
            }
            case WEST: {
                boundsFlags.set(1, f1 >= 1.0E-4f || f22 >= 1.0E-4f || f4 <= 0.9999f || f5 <= 0.9999f);
                boundsFlags.set(0, (f2 < 1.0E-4f || stateIn.isFullCube()) && f2 == f3);
                break;
            }
            case EAST: {
                boundsFlags.set(1, f1 >= 1.0E-4f || f22 >= 1.0E-4f || f4 <= 0.9999f || f5 <= 0.9999f);
                boundsFlags.set(0, (f3 > 0.9999f || stateIn.isFullCube()) && f2 == f3);
            }
        }
    }
    ...
    private void renderQuadsFlat(IBlockAccess p_renderQuadsFlat_1_, IBlockState p_renderQuadsFlat_2_, BlockPos p_renderQuadsFlat_3_, int p_renderQuadsFlat_4_, boolean p_renderQuadsFlat_5_, BufferBuilder p_renderQuadsFlat_6_, List<BakedQuad> p_renderQuadsFlat_7_, RenderEnv p_renderQuadsFlat_8_) {
        ...
        int j2 = p_renderQuadsFlat_7_.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            BakedQuad bakedquad = p_renderQuadsFlat_7_.get(i2);
            ...
            int l2 = CustomColors.getColorMultiplier(bakedquad, p_renderQuadsFlat_2_, p_renderQuadsFlat_1_, p_renderQuadsFlat_3_, p_renderQuadsFlat_8_);
            if (!bakedquad.hasTintIndex() && l2 == -1) {
                if (bakedquad.shouldApplyDiffuseLighting()) {
                ...
                int k2 = l2;
                if (l2 == -1) {
                    k2 = this.blockColors.colorMultiplier(p_renderQuadsFlat_2_, p_renderQuadsFlat_1_, p_renderQuadsFlat_3_, bakedquad.getTintIndex());
                }
                if (EntityRenderer.anaglyphEnable) {
                    k2 = TextureUtil.anaglyphColor(k2);
                }
                ...
                p_renderQuadsFlat_6_.putColorMultiplier(f2, f1, f22, 4);
                p_renderQuadsFlat_6_.putColorMultiplier(f2, f1, f22, 3);
                p_renderQuadsFlat_6_.putColorMultiplier(f2, f1, f22, 2);
                p_renderQuadsFlat_6_.putColorMultiplier(f2, f1, f22, 1);
            }
    ...
    public void renderModelBrightness(IBakedModel model, IBlockState state, float brightness, boolean p_178266_4_) {
        ...
        int i2 = this.blockColors.colorMultiplier(state, null, null, 0);
        if (EntityRenderer.anaglyphEnable) {
        ...
        this.renderModelBrightnessColor(state, model, brightness, f2, f1, f22);
    }
    ...
    private void renderModelBrightnessColorQuads(float brightness, float red, float green, float blue, List<BakedQuad> listQuads) {
        ...
        int j2 = listQuads.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            BakedQuad bakedquad = listQuads.get(i2);
    ...
    private void renderOverlayModels(IBlockAccess p_renderOverlayModels_1_, IBakedModel p_renderOverlayModels_2_, IBlockState p_renderOverlayModels_3_, BlockPos p_renderOverlayModels_4_, BufferBuilder p_renderOverlayModels_5_, boolean p_renderOverlayModels_6_, long p_renderOverlayModels_7_, RenderEnv p_renderOverlayModels_9_, boolean p_renderOverlayModels_10_) {
        if (p_renderOverlayModels_9_.isOverlaysRendered()) {
            for (int i2 = 0; i2 < OVERLAY_LAYERS.length; ++i2) {
                BlockRenderLayer blockrenderlayer = OVERLAY_LAYERS[i2];
                ...
                if (regionrendercachebuilder != null) {
                    ...
                    for (int j2 = 0; j2 < listquadsoverlay.size(); ++j2) {
                        ...
                        int k2 = iblockstate.getPackedLightmapCoords(p_renderOverlayModels_1_, p_renderOverlayModels_4_.offset(bakedquad.getFace()));
                        this.renderQuadsFlat(p_renderOverlayModels_1_, iblockstate, p_renderOverlayModels_4_, k2, false, bufferbuilder, list, p_renderOverlayModels_9_);
                    }
                }
    ...
    public static class AmbientOcclusionFace {
        ...
        public void updateVertexBrightness(IBlockAccess worldIn, IBlockState state, BlockPos centerPos, EnumFacing direction, float[] faceShape, BitSet shapeState) {
            ...
            int i2 = state.getPackedLightmapCoords(worldIn, blockpos$pooledmutableblockpos1);
            int j2 = state.getPackedLightmapCoords(worldIn, blockpos$pooledmutableblockpos2);
            int k2 = state.getPackedLightmapCoords(worldIn, blockpos$pooledmutableblockpos3);
            int l2 = state.getPackedLightmapCoords(worldIn, blockpos$pooledmutableblockpos4);
            float f2 = worldIn.getBlockState(blockpos$pooledmutableblockpos1).getAmbientOcclusionLightValue();
            float f1 = worldIn.getBlockState(blockpos$pooledmutableblockpos2).getAmbientOcclusionLightValue();
            float f22 = worldIn.getBlockState(blockpos$pooledmutableblockpos3).getAmbientOcclusionLightValue();
            ...
            f22 = BlockModelRenderer.fixAoLightValue(f22);
            ...
            if (shapeState.get(1) && blockmodelrenderer$enumneighborinfo.doNonCubicWeight) {
                float f29 = (f3 + f2 + f26 + f4) * 0.25f;
                float f30 = (f22 + f2 + f25 + f4) * 0.25f;
                float f31 = (f22 + f1 + f27 + f4) * 0.25f;
                ...
                float f222 = faceShape[blockmodelrenderer$enumneighborinfo.vert3Weights[2].shape] * faceShape[blockmodelrenderer$enumneighborinfo.vert3Weights[3].shape];
                ...
                this.vertexColorMultiplier[((VertexTranslations)blockmodelrenderer$vertextranslations).vert3] = f29 * f21 + f30 * f222 + f31 * f23 + f32 * f24;
                ...
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert0] = this.getVertexBrightness(i22, j22, k22, l22, f9, f10, f11, f12);
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert1] = this.getVertexBrightness(i22, j22, k22, l22, f13, f14, f15, f16);
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert2] = this.getVertexBrightness(i22, j22, k22, l22, f17, f18, f19, f20);
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert3] = this.getVertexBrightness(i22, j22, k22, l22, f21, f222, f23, f24);
            } else {
                float f5 = (f3 + f2 + f26 + f4) * 0.25f;
                float f6 = (f22 + f2 + f25 + f4) * 0.25f;
                float f7 = (f22 + f1 + f27 + f4) * 0.25f;
                float f8 = (f3 + f1 + f28 + f4) * 0.25f;
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert0] = this.getAoBrightness(l2, i2, j1, i3);
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert1] = this.getAoBrightness(k2, i2, i1, i3);
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert2] = this.getAoBrightness(k2, j2, k1, i3);
                this.vertexBrightness[((VertexTranslations)blockmodelrenderer$vertextranslations).vert3] = this.getAoBrightness(l2, j2, l1, i3);
