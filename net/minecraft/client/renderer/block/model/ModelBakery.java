...
public class ModelBakery {
    ...
    private ModelBlock loadModel(ResourceLocation location) throws IOException {
        ...
        block9: {
            ...
            block8: {
                ...
                reader = null;
                iresource = null;
                s2 = location.getResourcePath();
                if (!"builtin/generated".equals(s2)) break block8;
                ModelBlock modelBlock = modelblock4 = MODEL_GENERATED;
                IOUtils.closeQuietly(reader);
                IOUtils.closeQuietly(iresource);
                return modelBlock;
            }
            if ("builtin/entity".equals(s2)) break block9;
            if (s2.startsWith("builtin/")) {
                String s22 = s2.substring("builtin/".length());
                String s1 = BUILT_IN_MODELS.get(s22);
                if (s1 == null) {
    ...
    private ResourceLocation getModelLocation(ResourceLocation location) {
        ...
        if (!s2.startsWith("mcpatcher") && !s2.startsWith("optifine")) {
            return new ResourceLocation(location.getResourceDomain(), "models/" + location.getResourcePath() + ".json");
        }
        if (!s2.endsWith(".json")) {
            location = new ResourceLocation(location.getResourceDomain(), String.valueOf(s2) + ".json");
        }
        return location;
    }
    ...
    private void loadItemModels() {
        ...
        for (Item item : Item.REGISTRY) {
            for (String s2 : this.getVariantNames(item)) {
                ModelBlock modelblock;
                ResourceLocation resourcelocation = this.getItemLocation(s2);
                ResourceLocation resourcelocation1 = Item.REGISTRY.getNameForObject(item);
                this.loadItemModel(s2, resourcelocation, resourcelocation1);
    ...
    private void bakeBlockModels() {
        ...
        for (Map.Entry entry : this.multipartVariantMap.entrySet()) {
            ...
            String s2 = Block.REGISTRY.getNameForObject(multipart.getStateContainer().getBlock()).toString();
            ...
            for (Selector selector : multipart.getSelectors()) {
                IBakedModel ibakedmodel1 = this.createRandomModelForVariantList(selector.getVariantList(), "selector of " + s2);
    ...
    private IBakedModel createRandomModelForVariantList(VariantList variantsIn, String modelLocation) {
        ...
        if (i2 == 0) {
            ...
            ibakedmodel1 = i2 == 1 ? weightedbakedmodel$builder.first() : weightedbakedmodel$builder.build();
        }
        return ibakedmodel1;
    }
    ...
    private Set<ResourceLocation> getItemsTextureLocations() {
        ...
        for (ResourceLocation resourcelocation : this.itemLocations.values()) {
            ...
            if (this.hasItemModel(modelblock)) {
                for (String s2 : ItemModelGenerator.LAYERS) {
                    set.add(new ResourceLocation(modelblock.resolveTextureName(s2)));
                }
                continue;
            }
    ...
    public static void fixModelLocations(ModelBlock p_fixModelLocations_0_, String p_fixModelLocations_1_) {
        ...
        if (p_fixModelLocations_0_.textures != null) {
            for (Map.Entry<String, String> entry : p_fixModelLocations_0_.textures.entrySet()) {
                ...
                String s1 = ModelBakery.fixResourcePath(s2, p_fixModelLocations_1_);
                if (s1 == s2) continue;
                entry.setValue(s1);
            }
        }
    ...
    public static ResourceLocation fixModelLocation(ResourceLocation p_fixModelLocation_0_, String p_fixModelLocation_1_) {
        if (p_fixModelLocation_0_ != null && p_fixModelLocation_1_ != null) {
            ...
            String s2 = p_fixModelLocation_0_.getResourcePath();
            String s1 = ModelBakery.fixResourcePath(s2, p_fixModelLocation_1_);
            if (s1 != s2) {
    ...
    public static void registerItemVariants(Item p_registerItemVariants_0_, ResourceLocation ... p_registerItemVariants_1_) {
        ...
        ResourceLocation[] arrresourceLocation = p_registerItemVariants_1_;
        int n2 = p_registerItemVariants_1_.length;
