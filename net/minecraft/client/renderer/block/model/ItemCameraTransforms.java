...
public class ItemCameraTransforms {
    ...
    public static void applyTransformSide(ItemTransformVec3f vec, boolean leftHand) {
        if (vec != ItemTransformVec3f.DEFAULT) {
            int i2 = leftHand ? -1 : 1;
            GlStateManager.translate((float)i2 * (offsetTranslateX + vec.translation.x), offsetTranslateY + vec.translation.y, offsetTranslateZ + vec.translation.z);
            float f2 = offsetRotationX + vec.rotation.x;
            float f1 = offsetRotationY + vec.rotation.y;
            float f22 = offsetRotationZ + vec.rotation.z;
            if (leftHand) {
                f1 = -f1;
                f22 = -f22;
            }
            GlStateManager.rotate(ItemCameraTransforms.makeQuaternion(f2, f1, f22));
    ...
    private static Quaternion makeQuaternion(float p_188035_0_, float p_188035_1_, float p_188035_2_) {
        float f2 = p_188035_0_ * ((float)Math.PI / 180);
        float f1 = p_188035_1_ * ((float)Math.PI / 180);
        float f22 = p_188035_2_ * ((float)Math.PI / 180);
        float f3 = MathHelper.sin(0.5f * f2);
        float f4 = MathHelper.cos(0.5f * f2);
        float f5 = MathHelper.sin(0.5f * f1);
        float f6 = MathHelper.cos(0.5f * f1);
        float f7 = MathHelper.sin(0.5f * f22);
        float f8 = MathHelper.cos(0.5f * f22);
