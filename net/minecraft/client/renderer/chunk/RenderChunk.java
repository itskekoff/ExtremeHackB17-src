...
public class RenderChunk {
    ...
    public RenderChunk(World p_i47120_1_, RenderGlobal p_i47120_2_, int p_i47120_3_) {
        for (int i2 = 0; i2 < this.mapEnumFacing.length; ++i2) {
            this.mapEnumFacing[i2] = new BlockPos.MutableBlockPos();
        }
        this.world = p_i47120_1_;
        this.renderGlobal = p_i47120_2_;
        this.index = p_i47120_3_;
        if (OpenGlHelper.useVbo()) {
            for (int j2 = 0; j2 < BlockRenderLayer.values().length; ++j2) {
                this.vertexBuffers[j2] = new VertexBuffer(DefaultVertexFormats.BLOCK);
            }
        }
    }
    ...
    public void resortTransparency(float x2, float y2, float z2, ChunkCompileTaskGenerator generator) {
        ...
        if (compiledchunk.getState() != null && !compiledchunk.isLayerEmpty(BlockRenderLayer.TRANSLUCENT)) {
            ...
            this.postRenderBlocks(BlockRenderLayer.TRANSLUCENT, x2, y2, z2, bufferbuilder, compiledchunk);
        }
    }
        ...
        if (this.world != null) {
            ...
            if (!chunkcacheof.isEmpty()) {
                ...
                for (Object blockposm0 : BlockPosM.getAllInBoxMutable(blockpos, blockpos1)) {
                    ...
                    for (int j2 = 0; j2 < ablockrenderlayer.length; ++j2) {
                        BlockRenderLayer blockrenderlayer = ablockrenderlayer[j2];
                        if (flag) {
                        ...
                        int k2 = blockrenderlayer.ordinal();
                        if (block.getDefaultState().getRenderType() == EnumBlockRenderType.INVISIBLE) continue;
                        BufferBuilder bufferbuilder = generator.getRegionRenderCacheBuilder().getWorldRendererByLayerId(k2);
                        ...
                        int n2 = k2;
                        aboolean[n2] = aboolean[n2] | blockrendererdispatcher.renderBlock(iblockstate, blockposm, chunkcacheof, bufferbuilder);
                ...
                BlockRenderLayer[] arrblockRenderLayer = ENUM_WORLD_BLOCK_LAYERS;
                int n3 = ENUM_WORLD_BLOCK_LAYERS.length;
                for (int i3 = 0; i3 < n3; ++i3) {
                    BlockRenderLayer blockrenderlayer1 = arrblockRenderLayer[i3];
                    ...
                    this.postRenderBlocks(blockrenderlayer1, x2, y2, z2, generator.getRegionRenderCacheBuilder().getWorldRendererByLayer(blockrenderlayer1), compiledchunk);
                }
    ...
    private void postRenderBlocks(BlockRenderLayer layer, float x2, float y2, float z2, BufferBuilder worldRendererIn, CompiledChunk compiledChunkIn) {
    ...
    public void deleteGlResources() {
        this.stopCompileTask();
        this.world = null;
        for (int i2 = 0; i2 < BlockRenderLayer.values().length; ++i2) {
