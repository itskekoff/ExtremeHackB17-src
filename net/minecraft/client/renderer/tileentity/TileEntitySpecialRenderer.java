...
implements IEntityRenderer {
    ...
    public boolean isGlobalRenderer(T te2) {
        return false;
    }

    protected void drawNameplate(T te2, String str, double x2, double y2, double z2, int maxDistance) {
        Entity entity = this.rendererDispatcher.entity;
        double d0 = ((TileEntity)te2).getDistanceSq(entity.posX, entity.posY, entity.posZ);
        if (d0 <= (double)(maxDistance * maxDistance)) {
            float f2 = this.rendererDispatcher.entityYaw;
            float f1 = this.rendererDispatcher.entityPitch;
            boolean flag = false;
            EntityRenderer.drawNameplate(this.getFontRenderer(), str, (float)x2 + 0.5f, (float)y2 + 1.5f, (float)z2 + 0.5f, 0, f2, f1, false, false);
        }
    }
