...
extends Render<EntityItem> {
    ...
    private int transformModelCount(EntityItem itemIn, double p_177077_2_, double p_177077_4_, double p_177077_6_, float p_177077_8_, IBakedModel p_177077_9_) {
        ...
        int i2 = this.getModelCount(itemstack);
        float f2 = 0.25f;
    ...
    public void doRender(EntityItem entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        ...
        int i2 = itemstack.func_190926_b() ? 187 : Item.getIdFromItem(itemstack.getItem()) + itemstack.getMetadata();
        ...
        int j2 = this.transformModelCount(entity, x2, y2, z2, partialTicks, ibakedmodel);
        float f2 = ibakedmodel.getItemCameraTransforms().ground.scale.x;
        float f1 = ibakedmodel.getItemCameraTransforms().ground.scale.y;
        float f22 = ibakedmodel.getItemCameraTransforms().ground.scale.z;
        boolean flag1 = ibakedmodel.isGui3d();
        if (!flag1) {
            float f3 = -0.0f * (float)(j2 - 1) * 0.5f * f2;
            float f4 = -0.0f * (float)(j2 - 1) * 0.5f * f1;
            float f5 = -0.09375f * (float)(j2 - 1) * 0.5f * f22;
        ...
        for (int k2 = 0; k2 < j2; ++k2) {
            ...
            this.itemRenderer.renderItem(itemstack, ibakedmodel);
            GlStateManager.popMatrix();
            GlStateManager.translate(0.0f * f2, 0.0f * f1, 0.09375f * f22);
        }
        if (this.renderOutlines) {
