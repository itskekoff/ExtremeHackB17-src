...
extends RenderLivingBase<T> {
    ...
    public void setLightmap(T entityLivingIn) {
        int i2 = ((Entity)entityLivingIn).getBrightnessForRender();
        int j2 = i2 % 65536;
        int k2 = i2 / 65536;
    ...
    protected void renderLeash(T entityLivingIn, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        ...
        if (!(Config.isShaders() && Shaders.isShadowPass || (entity = ((EntityLiving)entityLivingIn).getLeashedToEntity()) == null)) {
            ...
            for (int j2 = 0; j2 <= 24; ++j2) {
                ...
                bufferbuilder.pos(x2 + d13 * (double)f3 + 0.0, y2 + d14 * (double)(f3 * f3 + f3) * 0.5 + (double)((24.0f - (float)j2) / 18.0f + 0.125f), z2 + d15 * (double)f3).color(f2, f1, f22, 1.0f).endVertex();
                bufferbuilder.pos(x2 + d13 * (double)f3 + 0.025, y2 + d14 * (double)(f3 * f3 + f3) * 0.5 + (double)((24.0f - (float)j2) / 18.0f + 0.125f) + 0.025, z2 + d15 * (double)f3).color(f2, f1, f22, 1.0f).endVertex();
            }
            tessellator.draw();
            bufferbuilder.begin(5, DefaultVertexFormats.POSITION_COLOR);
            for (int k2 = 0; k2 <= 24; ++k2) {
                ...
                bufferbuilder.pos(x2 + d13 * (double)f7 + 0.0, y2 + d14 * (double)(f7 * f7 + f7) * 0.5 + (double)((24.0f - (float)k2) / 18.0f + 0.125f) + 0.025, z2 + d15 * (double)f7).color(f4, f5, f6, 1.0f).endVertex();
                bufferbuilder.pos(x2 + d13 * (double)f7 + 0.025, y2 + d14 * (double)(f7 * f7 + f7) * 0.5 + (double)((24.0f - (float)k2) / 18.0f + 0.125f), z2 + d15 * (double)f7 + 0.025).color(f4, f5, f6, 1.0f).endVertex();
            }
            tessellator.draw();
            if (Config.isShaders()) {
                Shaders.endLeash();
