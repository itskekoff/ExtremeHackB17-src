...
extends RenderLiving<EntityShulker> {
    ...
    @Override
    public void doRender(EntityShulker entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        int i2 = entity.getClientTeleportInterp();
        if (i2 > 0 && entity.isAttachedToBlock()) {
            BlockPos blockpos = entity.getAttachmentPos();
            BlockPos blockpos1 = entity.getOldAttachPos();
            double d0 = (double)((float)i2 - partialTicks) / 6.0;
            d0 *= d0;
            ...
            super.doRender(entity, x2 - d1, y2 - d2, z2 - d3, entityYaw, partialTicks);
        } else {
