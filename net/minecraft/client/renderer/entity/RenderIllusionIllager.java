...
extends RenderLiving<EntityMob> {
    ...
    @Override
    public void doRender(EntityMob entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        if (entity.isInvisible()) {
            Vec3d[] avec3d = ((EntityIllusionIllager)entity).func_193098_a(partialTicks);
            float f2 = this.handleRotationFloat(entity, partialTicks);
            for (int i2 = 0; i2 < avec3d.length; ++i2) {
                super.doRender(entity, x2 + avec3d[i2].xCoord + (double)MathHelper.cos((float)i2 + f2 * 0.5f) * 0.025, y2 + avec3d[i2].yCoord + (double)MathHelper.cos((float)i2 + f2 * 0.75f) * 0.0125, z2 + avec3d[i2].zCoord + (double)MathHelper.cos((float)i2 + f2 * 0.7f) * 0.025, entityYaw, partialTicks);
            }
        } else {
    ...
    @Override
    public void renderName(EntityMob entity, double x2, double y2, double z2) {
        super.renderName(entity, x2, y2, z2);
    }
