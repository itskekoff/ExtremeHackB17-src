...
public class RenderManager {
    ...
    public void renderEntityStatic(Entity entityIn, float partialTicks, boolean p_188388_3_) {
        ...
        float f2 = entityIn.prevRotationYaw + (entityIn.rotationYaw - entityIn.prevRotationYaw) * partialTicks;
        int i2 = entityIn.getBrightnessForRender();
        ...
        this.doRenderEntity(entityIn, d0 - renderPosX, d1 - renderPosY, d2 - renderPosZ, f2, partialTicks, p_188388_3_);
    }

    public void doRenderEntity(Entity entityIn, double x2, double y2, double z2, float yaw, float partialTicks, boolean p_188391_10_) {
        block10: {
            ...
            try {
                ...
                try {
                    ...
                    render.doRender(entityIn, x2, y2, z2, yaw, partialTicks);
                }
                ...
                try {
                    if (!this.renderOutlines) {
                        render.doRenderShadowAndFire(entityIn, x2, y2, z2, yaw, partialTicks);
                    }
                }
                ...
                try {
                    this.renderDebugBoundingBox(entityIn, x2, y2, z2, yaw, partialTicks);
                }
    ...
    public void renderMultipass(Entity p_188389_1_, float p_188389_2_) {
        ...
        float f2 = p_188389_1_.prevRotationYaw + (p_188389_1_.rotationYaw - p_188389_1_.prevRotationYaw) * p_188389_2_;
        int i2 = p_188389_1_.getBrightnessForRender();
        ...
        if (render != null && this.renderEngine != null) {
            render.renderMultipass(p_188389_1_, d0 - renderPosX, d1 - renderPosY, d2 - renderPosZ, f2, p_188389_2_);
        }
    }
    ...
    private void renderDebugBoundingBox(Entity entityIn, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        ...
        RenderGlobal.drawBoundingBox(axisalignedbb.minX - entityIn.posX + x2, axisalignedbb.minY - entityIn.posY + y2, axisalignedbb.minZ - entityIn.posZ + z2, axisalignedbb.maxX - entityIn.posX + x2, axisalignedbb.maxY - entityIn.posY + y2, axisalignedbb.maxZ - entityIn.posZ + z2, 1.0f, 1.0f, 1.0f, 1.0f);
        ...
        if (entityIn instanceof EntityLivingBase) {
            float f1 = 0.01f;
            RenderGlobal.drawBoundingBox(x2 - (double)f2, y2 + (double)entityIn.getEyeHeight() - (double)0.01f, z2 - (double)f2, x2 + (double)f2, y2 + (double)entityIn.getEyeHeight() + (double)0.01f, z2 + (double)f2, 1.0f, 0.0f, 0.0f, 1.0f);
        }
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder bufferbuilder = tessellator.getBuffer();
        Vec3d vec3d = entityIn.getLook(partialTicks);
        bufferbuilder.begin(3, DefaultVertexFormats.POSITION_COLOR);
        bufferbuilder.pos(x2, y2 + (double)entityIn.getEyeHeight(), z2).color(0, 0, 255, 255).endVertex();
        bufferbuilder.pos(x2 + vec3d.xCoord * 2.0, y2 + (double)entityIn.getEyeHeight() + vec3d.yCoord * 2.0, z2 + vec3d.zCoord * 2.0).color(0, 0, 255, 255).endVertex();
        tessellator.draw();
