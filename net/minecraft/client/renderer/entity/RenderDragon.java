...
extends RenderLiving<EntityDragon> {
    ...
    protected void rotateCorpse(EntityDragon entityLiving, float p_77043_2_, float p_77043_3_, float partialTicks) {
        float f2 = (float)entityLiving.getMovementOffsets(7, partialTicks)[0];
        ...
        if (entityLiving.deathTime > 0) {
            float f22 = ((float)entityLiving.deathTime + partialTicks - 1.0f) / 20.0f * 1.6f;
            if ((f22 = MathHelper.sqrt(f22)) > 1.0f) {
                f22 = 1.0f;
            }
            GlStateManager.rotate(f22 * this.getDeathMaxRotation(entityLiving), 0.0f, 0.0f, 1.0f);
        }
    }
    ...
    public void doRender(EntityDragon entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        ...
        if (entity.healingEnderCrystal != null) {
            ...
            float f2 = MathHelper.sin(((float)entity.healingEnderCrystal.ticksExisted + partialTicks) * 0.2f) / 2.0f + 0.5f;
            f2 = (f2 * f2 + f2) * 0.2f;
            RenderDragon.renderCrystalBeams(x2, y2, z2, partialTicks, entity.posX + (entity.prevPosX - entity.posX) * (double)(1.0f - partialTicks), entity.posY + (entity.prevPosY - entity.posY) * (double)(1.0f - partialTicks), entity.posZ + (entity.prevPosZ - entity.posZ) * (double)(1.0f - partialTicks), entity.ticksExisted, entity.healingEnderCrystal.posX, (double)f2 + entity.healingEnderCrystal.posY, entity.healingEnderCrystal.posZ);
        }
    }
    ...
    public static void renderCrystalBeams(double p_188325_0_, double p_188325_2_, double p_188325_4_, float p_188325_6_, double p_188325_7_, double p_188325_9_, double p_188325_11_, int p_188325_13_, double p_188325_14_, double p_188325_16_, double p_188325_18_) {
        float f2 = (float)(p_188325_14_ - p_188325_7_);
        float f1 = (float)(p_188325_16_ - 1.0 - p_188325_9_);
        float f22 = (float)(p_188325_18_ - p_188325_11_);
        float f3 = MathHelper.sqrt(f2 * f2 + f22 * f22);
        float f4 = MathHelper.sqrt(f2 * f2 + f1 * f1 + f22 * f22);
        GlStateManager.pushMatrix();
        GlStateManager.translate((float)p_188325_0_, (float)p_188325_2_ + 2.0f, (float)p_188325_4_);
        GlStateManager.rotate((float)(-Math.atan2(f22, f2)) * 57.295776f - 90.0f, 0.0f, 1.0f, 0.0f);
        ...
        float f6 = MathHelper.sqrt(f2 * f2 + f1 * f1 + f22 * f22) / 32.0f - ((float)p_188325_13_ + p_188325_6_) * 0.01f;
        bufferbuilder.begin(5, DefaultVertexFormats.POSITION_TEX_COLOR);
        int i2 = 8;
        for (int j2 = 0; j2 <= 8; ++j2) {
            float f7 = MathHelper.sin((float)(j2 % 8) * ((float)Math.PI * 2) / 8.0f) * 0.75f;
            float f8 = MathHelper.cos((float)(j2 % 8) * ((float)Math.PI * 2) / 8.0f) * 0.75f;
            float f9 = (float)(j2 % 8) / 8.0f;
