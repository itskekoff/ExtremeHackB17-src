...
extends Render<EntityBoat> {
    ...
    public void doRender(EntityBoat entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        ...
        this.setupTranslation(x2, y2, z2);
    ...
    public void setupRotation(EntityBoat p_188311_1_, float p_188311_2_, float p_188311_3_) {
        ...
        float f2 = (float)p_188311_1_.getTimeSinceHit() - p_188311_3_;
        float f1 = p_188311_1_.getDamageTaken() - p_188311_3_;
        if (f1 < 0.0f) {
            f1 = 0.0f;
        }
        if (f2 > 0.0f) {
            GlStateManager.rotate(MathHelper.sin(f2) * f2 * f1 / 10.0f * (float)p_188311_1_.getForwardDirection(), 1.0f, 0.0f, 0.0f);
        }
