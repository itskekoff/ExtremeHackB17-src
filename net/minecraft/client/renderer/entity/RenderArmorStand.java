...
extends RenderLivingBase<EntityArmorStand> {
    ...
    protected void rotateCorpse(EntityArmorStand entityLiving, float p_77043_2_, float p_77043_3_, float partialTicks) {
        ...
        float f2 = (float)(entityLiving.world.getTotalWorldTime() - entityLiving.punchCooldown) + partialTicks;
        if (f2 < 5.0f) {
            GlStateManager.rotate(MathHelper.sin(f2 / 1.5f * (float)Math.PI) * 3.0f, 0.0f, 1.0f, 0.0f);
        }
    }
    ...
    @Override
    public void doRender(EntityArmorStand entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        if (entity.hasMarker()) {
