...
extends Render<EntityFishHook> {
    ...
    @Override
    public void doRender(EntityFishHook entity, double x2, double y2, double z2, float entityYaw, float partialTicks) {
        ...
        if (entityplayer != null && !this.renderOutlines) {
            ...
            int k2 = entityplayer.getPrimaryHand() == EnumHandSide.RIGHT ? 1 : -1;
            ...
            if ((this.renderManager.options == null || this.renderManager.options.thirdPersonView <= 0) && entityplayer == Minecraft.getMinecraft().player) {
                ...
                Vec3d vec3d = new Vec3d((double)k2 * -0.36 * (double)(f10 /= 100.0f), -0.045 * (double)f10, 0.4);
            ...
            for (int i1 = 0; i1 <= 16; ++i1) {
                ...
                bufferbuilder.pos(x2 + d10 * (double)f11, y2 + d11 * (double)(f11 * f11 + f11) * 0.5 + 0.25, z2 + d12 * (double)f11).color(0, 0, 0, 255).endVertex();
            }
            tessellator.draw();
