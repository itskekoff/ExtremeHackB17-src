...
implements IResourceManagerReloadListener {
    ...
    protected void registerBlock(Block blk2, int subType, String identifier) {
        this.registerItem(Item.getItemFromBlock(blk2), subType, identifier);
    }

    private void registerBlock(Block blk2, String identifier) {
        this.registerBlock(blk2, 0, identifier);
    }
    ...
    private void func_191967_a(IBakedModel p_191967_1_, int p_191967_2_, ItemStack p_191967_3_) {
        ...
        boolean bl2 = flag1 = Config.isMultiTexture() && flag;
        if (flag1) {
    ...
    private void func_191970_a(BufferBuilder p_191970_1_, List<BakedQuad> p_191970_2_, int p_191970_3_, ItemStack p_191970_4_) {
        ...
        int j2 = p_191970_2_.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            BakedQuad bakedquad = p_191970_2_.get(i2);
            int k2 = p_191970_3_;
            if (flag && bakedquad.hasTintIndex()) {
                k2 = this.itemColors.getColorFromItemstack(p_191970_4_, bakedquad.getTintIndex());
                if (Config.isCustomColors()) {
                    k2 = CustomColors.getColorFromItemStack(p_191970_4_, bakedquad.getTintIndex(), k2);
                }
            ...
            this.func_191969_a(p_191970_1_, bakedquad, k2);
        }
    }
    ...
    public void renderItemIntoGUI(ItemStack stack, int x2, int y2) {
        this.func_191962_a(stack, x2, y2, this.getItemModelWithOverrides(stack, null, null));
    }
        ...
        if (!stack.func_190926_b()) {
            ...
            if (ReflectorForge.isItemDamaged(stack)) {
                ...
                if (Reflector.ForgeItem_getDurabilityForDisplay.exists() && Reflector.ForgeItem_getRGBDurabilityForDisplay.exists()) {
                    ...
                    int k2 = Reflector.callInt(stack.getItem(), Reflector.ForgeItem_getRGBDurabilityForDisplay, stack);
                    i2 = Math.round(13.0f - (float)d0 * 13.0f);
                    j2 = k2;
                }
                if (Config.isCustomColors()) {
                ...
                if (Reflector.ForgeItem_getDurabilityForDisplay.exists() && Reflector.ForgeItem_getRGBDurabilityForDisplay.exists()) {
                    ...
                    int l2 = Reflector.callInt(stack.getItem(), Reflector.ForgeItem_getRGBDurabilityForDisplay, stack);
                    i2 = Math.round(13.0f - (float)d1 * 13.0f);
                    j2 = l2;
                }
                if (Config.isCustomColors()) {
                    j2 = CustomColors.getDurabilityColor(f22, j2);
                ...
                this.draw(bufferbuilder, xPosition + 2, yPosition + 13, i2, 1, j2 >> 16 & 0xFF, j2 >> 8 & 0xFF, j2 & 0xFF, 255);
                GlStateManager.enableBlend();
                GlStateManager.enableAlpha();
            ...
            float f4 = f3 = (entityplayersp = Minecraft.getMinecraft().player) == null ? 0.0f : entityplayersp.getCooldownTracker().getCooldown(stack.getItem(), Minecraft.getMinecraft().getRenderPartialTicks());
            if (f3 > 0.0f) {
                GlStateManager.disableLighting();
                GlStateManager.disableDepth();
                GlStateManager.disableTexture2D();
    ...
    private void draw(BufferBuilder renderer, int x2, int y2, int width, int height, int red, int green, int blue, int alpha) {
        renderer.begin(7, DefaultVertexFormats.POSITION_COLOR);
        renderer.pos(x2 + 0, y2 + 0, 0.0).color(red, green, blue, alpha).endVertex();
        renderer.pos(x2 + 0, y2 + height, 0.0).color(red, green, blue, alpha).endVertex();
        renderer.pos(x2 + width, y2 + height, 0.0).color(red, green, blue, alpha).endVertex();
        renderer.pos(x2 + width, y2 + 0, 0.0).color(red, green, blue, alpha).endVertex();
