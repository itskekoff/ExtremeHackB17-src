...
public class ActiveRenderInfo {
    ...
    public static void updateRenderInfo(EntityPlayer entityplayerIn, boolean p_74583_1_) {
        ...
        float f2 = (VIEWPORT.get(0) + VIEWPORT.get(2)) / 2;
        float f1 = (VIEWPORT.get(1) + VIEWPORT.get(3)) / 2;
        GLU.gluUnProject(f2, f1, 0.0f, MODELVIEW, PROJECTION, VIEWPORT, OBJECTCOORDS);
        position = new Vec3d(OBJECTCOORDS.get(0), OBJECTCOORDS.get(1), OBJECTCOORDS.get(2));
        int i2 = p_74583_1_ ? 1 : 0;
        float f22 = entityplayerIn.rotationPitch;
        float f3 = entityplayerIn.rotationYaw;
        rotationX = MathHelper.cos(f3 * ((float)Math.PI / 180)) * (float)(1 - i2 * 2);
        rotationZ = MathHelper.sin(f3 * ((float)Math.PI / 180)) * (float)(1 - i2 * 2);
        rotationYZ = -rotationZ * MathHelper.sin(f22 * ((float)Math.PI / 180)) * (float)(1 - i2 * 2);
        rotationXY = rotationX * MathHelper.sin(f22 * ((float)Math.PI / 180)) * (float)(1 - i2 * 2);
        rotationXZ = MathHelper.cos(f22 * ((float)Math.PI / 180));
    }
    ...
    public static IBlockState getBlockStateAtEntityViewpoint(World worldIn, Entity entityIn, float p_186703_2_) {
        ...
        if (iblockstate.getMaterial().isLiquid()) {
            ...
            if (iblockstate.getBlock() instanceof BlockLiquid) {
                f2 = BlockLiquid.getLiquidHeightPercent(iblockstate.getValue(BlockLiquid.LEVEL)) - 0.11111111f;
            }
            if (vec3d.yCoord >= (double)(f1 = (float)(blockpos.getY() + 1) - f2)) {
