...
implements DebugRenderer.IDebugRenderer {
    ...
    public void render(float p_190060_1_, long p_190060_2_) {
        if (!this.pathMap.isEmpty()) {
            ...
            for (Integer integer : this.pathMap.keySet()) {
                ...
                float f2 = this.pathMaxDistance.get(integer).floatValue();
                ...
                for (int j2 = 0; j2 < path.getCurrentPathLength(); ++j2) {
                    PathPoint pathpoint1 = path.getPathPointFromIndex(j2);
                    if (!(this.addDistanceToPlayer(pathpoint1) <= 40.0f)) continue;
                    float f1 = j2 == path.getCurrentPathIndex() ? 1.0f : 0.0f;
                    float f22 = j2 == path.getCurrentPathIndex() ? 0.0f : 1.0f;
                    RenderGlobal.renderFilledBox(new AxisAlignedBB((float)pathpoint1.xCoord + 0.5f - f2, (float)pathpoint1.yCoord + 0.01f * (float)j2, (float)pathpoint1.zCoord + 0.5f - f2, (float)pathpoint1.xCoord + 0.5f + f2, (float)pathpoint1.yCoord + 0.25f + 0.01f * (float)j2, (float)pathpoint1.zCoord + 0.5f + f2).offset(-this.xo, -this.yo, -this.zo), f1, 0.0f, f22, 0.5f);
                }
            }
            for (Integer integer1 : this.pathMap.keySet()) {
                ...
                for (int k2 = 0; k2 < path1.getCurrentPathLength(); ++k2) {
                    PathPoint pathpoint2 = path1.getPathPointFromIndex(k2);
            ...
            for (Integer integer2 : this.creationMap.keySet().toArray(new Integer[0])) {
                if (i2 - this.creationMap.get(integer2) <= 20000L) continue;
    ...
    public void renderPathLine(float p_190067_1_, Path p_190067_2_) {
        ...
        for (int i2 = 0; i2 < p_190067_2_.getCurrentPathLength(); ++i2) {
            PathPoint pathpoint = p_190067_2_.getPathPointFromIndex(i2);
            if (!(this.addDistanceToPlayer(pathpoint) <= 40.0f)) continue;
            float f2 = (float)i2 / (float)p_190067_2_.getCurrentPathLength() * 0.33f;
            int j2 = i2 == 0 ? 0 : MathHelper.hsvToRGB(f2, 0.9f, 0.9f);
            int k2 = j2 >> 16 & 0xFF;
            int l2 = j2 >> 8 & 0xFF;
            int i1 = j2 & 0xFF;
            bufferbuilder.pos((double)pathpoint.xCoord - this.xo + 0.5, (double)pathpoint.yCoord - this.yo + 0.5, (double)pathpoint.zCoord - this.zo + 0.5).color(k2, l2, i1, 255).endVertex();
        }
        tessellator.draw();
    }
