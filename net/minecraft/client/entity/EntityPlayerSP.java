...
import ShwepSS.B17.modules.Cmds;
...
extends AbstractClientPlayer {
    ...
    public static CopyOnWriteArrayList<EntityPlayerSP> bots = new CopyOnWriteArrayList();
    ...
    public void sendPacket(Packet packet) {
        if (OneBotsMother.get.isEnabled()) {
            for (EntityPlayerSP p2 : bots) {
                p2.connection.sendPacket(packet);
            }
        } else {
    ...
    public void onUpdate() {
        if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0, this.posZ))) {
            ...
            Hook.onTickIngame();
            if (this.isRiding()) {
                this.sendPacket(new CPacketPlayer.Rotation(this.rotationYaw, this.rotationPitch, this.onGround));
                this.sendPacket(new CPacketInput(this.moveStrafing, this.field_191988_bg, this.movementInput.jump, this.movementInput.sneak));
    ...
    private void onUpdateWalkingPlayer() {
        ...
        if (flag != this.serverSprintState) {
            if (flag) {
                this.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.START_SPRINTING));
            } else {
                this.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.STOP_SPRINTING));
            }
            this.serverSprintState = flag;
        }
        if ((flag1 = this.isSneaking()) != this.serverSneakState) {
            if (flag1) {
                this.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.START_SNEAKING));
            } else {
                this.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.STOP_SNEAKING));
            }
        ...
        if (this.isCurrentViewEntity()) {
            ...
            if (this.isRiding()) {
                this.sendPacket(new CPacketPlayer.PositionRotation(this.motionX, -999.0, this.motionZ, this.rotationYaw, this.rotationPitch, this.onGround));
                flag2 = false;
            } else if (flag2 && flag3) {
                this.sendPacket(new CPacketPlayer.PositionRotation(this.posX, axisalignedbb.minY, this.posZ, this.rotationYaw, this.rotationPitch, this.onGround));
            } else if (flag2) {
                this.sendPacket(new CPacketPlayer.Position(this.posX, axisalignedbb.minY, this.posZ, this.onGround));
            } else if (flag3) {
                this.sendPacket(new CPacketPlayer.Rotation(this.rotationYaw, this.rotationPitch, this.onGround));
            } else if (this.prevOnGround != this.onGround) {
                this.sendPacket(new CPacketPlayer(this.onGround));
            }
            if (flag2) {
    ...
    public EntityItem dropItem(boolean dropAll) {
        ...
        this.sendPacket(new CPacketPlayerDigging(cpacketplayerdigging$action, BlockPos.ORIGIN, EnumFacing.DOWN));
        return null;
    }
    ...
    public void sendChatMessage(String message) {
        try {
            EventChatSend chat = new EventChatSend(message);
            EventManager.call(chat);
            Cmds cmd = new Cmds();
            ExtremeHack.getManager();
            if (ModuleManager.getModuleByName("\u041a\u043c\u0434 \u0447\u0438\u0442\u0430").isEnabled()) {
                cmd.onCommand(message);
                if (message.startsWith(".")) {
                    return;
                }
            }
            this.mc.player.connection.sendPacket(new CPacketChatMessage(chat.streng));
        }
    ...
    public void swingArm(EnumHand hand) {
        super.swingArm(hand);
        this.sendPacket(new CPacketAnimation(hand));
    }
    ...
    public void closeScreen() {
        this.sendPacket(new CPacketCloseWindow(this.openContainer.windowId));
    ...
    public void sendPlayerAbilities() {
        this.sendPacket(new CPacketPlayerAbilities(this.capabilities));
    }
    ...
    protected void sendHorseJump() {
        this.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.START_RIDING_JUMP, MathHelper.floor(this.getHorseJumpPower() * 100.0f)));
    }

    public void sendHorseInventory() {
        this.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.OPEN_INVENTORY));
    }
    ...
    public void func_193103_a(IRecipe p_193103_1_) {
        if (this.field_192036_cb.func_194076_e(p_193103_1_)) {
            ...
            this.sendPacket(new CPacketRecipeInfo(p_193103_1_));
        }
    }
    ...
    public void handleStatusUpdate(byte id2) {
        if (id2 >= 24 && id2 <= 28) {
            this.setPermissionLevel(id2 - 24);
        } else {
    ...
    public boolean isSneaking() {
        boolean flag;
        boolean bl2 = flag = this.movementInput != null && this.movementInput.sneak;
    ...
    public void onLivingUpdate() {
        ...
        boolean flag4;
        ExtremeHack.getManager();
        ModuleManager.getModuleByName("RepeatPackets").isEnabled();
        ...
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35, axisalignedbb.minY + 0.5, this.posZ - (double)this.width * 0.35);
        this.pushOutOfBlocks(this.posX + (double)this.width * 0.35, axisalignedbb.minY + 0.5, this.posZ + (double)this.width * 0.35);
        boolean bl2 = flag4 = (float)this.getFoodStats().getFoodLevel() > 6.0f || this.capabilities.allowFlying;
        ...
        if (this.movementInput.jump && !flag && !this.onGround && this.motionY < 0.0 && !this.isElytraFlying() && !this.capabilities.isFlying && (itemstack = this.getItemStackFromSlot(EntityEquipmentSlot.CHEST)).getItem() == Items.ELYTRA && ItemElytra.isBroken(itemstack)) {
            this.sendPacket(new CPacketEntityAction(this, CPacketEntityAction.Action.START_FALL_FLYING));
        }
    ...
    public void moveEntity(MoverType x2, double p_70091_2_, double p_70091_4_, double p_70091_6_) {
        double d0 = this.posX;
        double d1 = this.posZ;
        super.moveEntity(x2, p_70091_2_, p_70091_4_, p_70091_6_);
                ...
                if (f13 >= -0.15f && (iblockstate = this.world.getBlockState(blockpos = new BlockPos(this.posX, this.getEntityBoundingBox().maxY, this.posZ))).getCollisionBoundingBox(this.world, blockpos) == null && (iblockstate1 = this.world.getBlockState(blockpos = blockpos.up())).getCollisionBoundingBox(this.world, blockpos) == null) {
                    ...
                    for (AxisAlignedBB axisalignedbb2 : list) {
                        ...
                        while (!((float)i2 >= f7)) {
                            ...
                            BlockPos blockpos2 = blockpos1.up(i2);
                            ...
                            if (i2 > 1 && (iblockstate3 = this.world.getBlockState(blockpos = blockpos.up())).getCollisionBoundingBox(this.world, blockpos) != null) {
                                return;
                            }
                            ++i2;
                        }
