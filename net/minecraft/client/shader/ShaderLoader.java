    ...
    public static ShaderLoader loadShader(IResourceManager resourceManager, ShaderType type, String filename) throws IOException {
        ...
        if (shaderloader == null) {
            ...
            try {
                ...
                int i2 = OpenGlHelper.glCreateShader(type.getShaderMode());
                OpenGlHelper.glShaderSource(i2, bytebuffer);
                OpenGlHelper.glCompileShader(i2);
                if (OpenGlHelper.glGetShaderi(i2, OpenGlHelper.GL_COMPILE_STATUS) == 0) {
                    String s2 = StringUtils.trim(OpenGlHelper.glGetShaderInfoLog(i2, 32768));
                    JsonException jsonexception = new JsonException("Couldn't compile " + type.getShaderName() + " program: " + s2);
                    jsonexception.setFilenameAndFlush(resourcelocation.getResourcePath());
                    throw jsonexception;
                }
                shaderloader = new ShaderLoader(type, i2, filename);
