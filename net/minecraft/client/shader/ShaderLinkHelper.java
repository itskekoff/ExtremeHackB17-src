...
public class ShaderLinkHelper {
    ...
    public int createProgram() throws JsonException {
        int i2 = OpenGlHelper.glCreateProgram();
        if (i2 <= 0) {
            throw new JsonException("Could not create shader program (returned program ID " + i2 + ")");
        }
        return i2;
    }
    ...
    public void linkProgram(ShaderManager manager) throws IOException {
        ...
        int i2 = OpenGlHelper.glGetProgrami(manager.getProgram(), OpenGlHelper.GL_LINK_STATUS);
        if (i2 == 0) {
