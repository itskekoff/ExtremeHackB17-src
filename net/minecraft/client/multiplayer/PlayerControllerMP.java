...
public class PlayerControllerMP {
    ...
    public PlayerControllerMP(Minecraft mcIn, NetHandlerPlayClient netHandler, String string) {
        this.mc = mcIn;
    ...
    public boolean clickBlock(BlockPos loc, EnumFacing face) {
        ...
        if (this.currentGameType.isCreative()) {
            ...
            this.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, loc, face));
            PlayerControllerMP.clickBlockCreative(this.mc, this, loc, face);
            this.blockHitDelay = 5;
        } else if (!this.isHittingBlock || !this.isHittingPosition(loc)) {
            boolean flag;
            if (this.isHittingBlock) {
                this.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.ABORT_DESTROY_BLOCK, this.currentBlock, face));
            }
            ...
            this.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, loc, face));
            boolean bl2 = flag = iblockstate.getMaterial() != Material.AIR;
    ...
    public void resetBlockRemoving() {
        if (this.isHittingBlock) {
            ...
            this.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.ABORT_DESTROY_BLOCK, this.currentBlock, EnumFacing.DOWN));
    ...
    public boolean onPlayerDamageBlock(BlockPos posBlock, EnumFacing directionFacing) {
        ...
        if (this.currentGameType.isCreative() && this.mc.world.getWorldBorder().contains(posBlock)) {
            ...
            this.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, posBlock, directionFacing));
        ...
        if (this.isHittingPosition(posBlock)) {
            ...
            if (this.curBlockDamageMP >= 1.0f) {
                this.isHittingBlock = false;
                this.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, posBlock, directionFacing));
    ...
    public void updateController() {
        try {
            this.syncCurrentPlayItem();
            if (NetHandlerPlayClient.bots.size() >= 2) {
                if (this.connection.getBotManager().isChannelOpen()) {
                    this.connection.getBotManager().processReceivedPackets();
                } else {
                    this.connection.getBotManager().checkDisconnected();
                }
            }
            if (NetHandlerPlayClient.bots.size() < 2) {
    ...
    private boolean isHittingPosition(BlockPos pos) {
        boolean flag;
        ItemStack itemstack = this.mc.player.getHeldItemMainhand();
        boolean bl2 = flag = this.currentItemHittingBlock.func_190926_b() && itemstack.func_190926_b();
    ...
    private void syncCurrentPlayItem() {
        int i2 = this.mc.player.inventory.currentItem;
        if (i2 != this.currentPlayerItem) {
            this.currentPlayerItem = i2;
            this.sendPacket(new CPacketHeldItemChange(this.currentPlayerItem));
        }
    }
    ...
    public EnumActionResult processRightClickBlock(EntityPlayerSP player, WorldClient worldIn, BlockPos stack, EnumFacing pos, Vec3d facing, EnumHand vec) {
        ...
        float f2 = (float)(facing.xCoord - (double)stack.getX());
        float f1 = (float)(facing.yCoord - (double)stack.getY());
        float f22 = (float)(facing.zCoord - (double)stack.getZ());
        boolean flag = false;
        ...
        if (this.currentGameType != GameType.SPECTATOR) {
            ...
            if ((!player.isSneaking() || player.getHeldItemMainhand().func_190926_b() && player.getHeldItemOffhand().func_190926_b()) && iblockstate.getBlock().onBlockActivated(worldIn, stack, iblockstate, player, vec, pos, f2, f1, f22)) {
                flag = true;
            }
        ...
        this.sendPacket(new CPacketPlayerTryUseItemOnBlock(stack, pos, vec, f2, f1, f22));
        if (!flag && this.currentGameType != GameType.SPECTATOR) {
            ...
            if (this.currentGameType.isCreative()) {
                ...
                int j2 = itemstack.func_190916_E();
                EnumActionResult enumactionresult = itemstack.onItemUse(player, worldIn, stack, vec, pos, f2, f1, f22);
                itemstack.setItemDamage(i2);
                itemstack.func_190920_e(j2);
                return enumactionresult;
            }
            return itemstack.onItemUse(player, worldIn, stack, vec, pos, f2, f1, f22);
        }
    ...
    public EnumActionResult processRightClick(EntityPlayer player, World worldIn, EnumHand stack) {
        ...
        this.syncCurrentPlayItem();
        this.sendPacket(new CPacketPlayerTryUseItem(stack));
        ...
        if (itemstack1 != itemstack || itemstack1.func_190916_E() != i2) {
    ...
    public void attackEntity(EntityPlayer playerIn, Entity targetEntity) {
        this.syncCurrentPlayItem();
        this.sendPacket(new CPacketUseEntity(targetEntity));
        if (this.currentGameType != GameType.SPECTATOR) {
    ...
    public EnumActionResult interactWithEntity(EntityPlayer player, Entity target, EnumHand heldItem) {
        this.syncCurrentPlayItem();
        this.sendPacket(new CPacketUseEntity(target, heldItem));
    ...
    public EnumActionResult interactWithEntity(EntityPlayer player, Entity target, RayTraceResult raytrace, EnumHand heldItem) {
        ...
        this.sendPacket(new CPacketUseEntity(target, heldItem, vec3d));
    ...
    public ItemStack windowClick(int windowId, int slotId, int mouseButton, ClickType type, EntityPlayer player) {
        ...
        this.sendPacket(new CPacketClickWindow(windowId, slotId, mouseButton, type, itemstack, short1));
        return itemstack;
    }

    public void func_194338_a(int p_194338_1_, IRecipe p_194338_2_, boolean p_194338_3_, EntityPlayer p_194338_4_) {
        this.sendPacket(new CPacketPlaceRecipe(p_194338_1_, p_194338_2_, p_194338_3_));
    }

    public void sendEnchantPacket(int windowID, int button) {
        this.sendPacket(new CPacketEnchantItem(windowID, button));
    }

    public void sendSlotPacket(ItemStack itemStackIn, int slotId) {
        if (this.currentGameType.isCreative()) {
            this.sendPacket(new CPacketCreativeInventoryAction(slotId, itemStackIn));
        }
    }

    public void sendPacketDropItem(ItemStack itemStackIn) {
        if (this.currentGameType.isCreative() && !itemStackIn.func_190926_b()) {
            this.sendPacket(new CPacketCreativeInventoryAction(-1, itemStackIn));
        }
    }

    public void onStoppedUsingItem(EntityPlayer playerIn) {
        this.syncCurrentPlayItem();
        this.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.RELEASE_USE_ITEM, BlockPos.ORIGIN, EnumFacing.DOWN));
    ...
    public void pickItem(int index) {
        this.sendPacket(new CPacketCustomPayload("MC|PickItem", new PacketBuffer(Unpooled.buffer()).writeVarIntToBuffer(index)));
    }
}
