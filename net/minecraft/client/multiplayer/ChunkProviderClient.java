...
implements IChunkProvider {
    ...
    public void unloadChunk(int x2, int z2) {
        ...
        this.chunkMapping.remove(ChunkPos.asLong(x2, z2));
    }

    @Override
    @Nullable
    public Chunk getLoadedChunk(int x2, int z2) {
        return (Chunk)this.chunkMapping.get(ChunkPos.asLong(x2, z2));
    }
    ...
    public Chunk provideChunk(int x2, int z2) {
        return MoreObjects.firstNonNull(this.getLoadedChunk(x2, z2), this.blankChunk);
    }
    ...
    public boolean unloadQueuedChunks() {
        ...
        for (Chunk chunk : this.chunkMapping.values()) {
            chunk.onTick(System.currentTimeMillis() - i2 > 5L);
        }
        if (System.currentTimeMillis() - i2 > 100L) {
            LOGGER.info("Warning: Clientside chunk ticking took {} ms", (Object)(System.currentTimeMillis() - i2));
        }
        return false;
    }
