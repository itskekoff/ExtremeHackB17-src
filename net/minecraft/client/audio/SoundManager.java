...
public class SoundManager {
        ...
        if (this.loaded) {
            if (category == SoundCategory.MASTER) {
                ...
                for (String s2 : this.categorySounds.get(category)) {
                    ...
                    float f2 = this.getClampedVolume(isound);
                    if (f2 <= 0.0f) {
    ...
    public void updateAllSounds() {
        ...
        for (ITickableSound itickablesound : this.tickableSounds) {
            ...
            this.sndSystem.setVolume(s2, this.getClampedVolume(itickablesound));
            this.sndSystem.setPitch(s2, this.getClampedPitch(itickablesound));
    ...
    public boolean isSoundPlaying(ISound sound) {
        ...
        return this.sndSystem.playing(s2) || this.playingSoundsStopTime.containsKey(s2) && this.playingSoundsStopTime.get(s2) <= this.playTime;
    }
            ...
            if (soundeventaccessor == null) {
                ...
                if (this.sndSystem.getMasterVolume() <= 0.0f) {
                    ...
                    if (sound == SoundHandler.MISSING_SOUND) {
                        ...
                        float f22 = this.getClampedPitch(p_sound);
                        if (f1 == 0.0f) {
                            ...
                            if (sound.isStreaming()) {
                                this.sndSystem.newStreamingSource(false, s2, SoundManager.getURLForSoundResource(resourcelocation1), resourcelocation1.toString(), flag, p_sound.getXPosF(), p_sound.getYPosF(), p_sound.getZPosF(), p_sound.getAttenuationType().getTypeInt(), f2);
                            } else {
                                this.sndSystem.newSource(false, s2, SoundManager.getURLForSoundResource(resourcelocation1), resourcelocation1.toString(), flag, p_sound.getXPosF(), p_sound.getYPosF(), p_sound.getZPosF(), p_sound.getAttenuationType().getTypeInt(), f2);
                            }
                            LOGGER.debug(LOG_MARKER, "Playing sound {} for event {} as channel {}", (Object)sound.getSoundLocation(), (Object)resourcelocation, (Object)s2);
                            this.sndSystem.setPitch(s2, f22);
                            this.sndSystem.setVolume(s2, f1);
                            this.sndSystem.play(s2);
                            this.playingSoundsStopTime.put(s2, this.playTime + 20);
                            this.playingSounds.put(s2, p_sound);
    ...
    public void pauseAllSounds() {
        for (Map.Entry<String, ISound> entry : this.playingSounds.entrySet()) {
            ...
            if (!flag) continue;
            LOGGER.debug(LOG_MARKER, "Pausing channel {}", (Object)s2);
            this.sndSystem.pause(s2);
            this.pausedChannels.add(s2);
        }
    }

    public void resumeAllSounds() {
        for (String s2 : this.pausedChannels) {
            LOGGER.debug(LOG_MARKER, "Resuming channel {}", (Object)s2);
            this.sndSystem.play(s2);
        }
    ...
    public void stop(String p_189567_1_, SoundCategory p_189567_2_) {
        if (p_189567_2_ != null) {
            for (String s2 : this.categorySounds.get(p_189567_2_)) {
                ISound isound = this.playingSounds.get(s2);
    ...
    extends SoundSystem {
        ...
        public boolean playing(String p_playing_1_) {
            ...
            synchronized (object) {
                ...
                block6: {
                    block5: {
                        if (this.soundLibrary != null) break block5;
                        return false;
                    }
                    source = this.soundLibrary.getSources().get(p_playing_1_);
