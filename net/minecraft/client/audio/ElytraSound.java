...
extends MovingSound {
    ...
    public void update() {
        ...
        if (!this.player.isDead && (this.time <= 20 || this.player.isElytraFlying())) {
            ...
            float f2 = MathHelper.sqrt(this.player.motionX * this.player.motionX + this.player.motionZ * this.player.motionZ + this.player.motionY * this.player.motionY);
            float f1 = f2 / 2.0f;
            this.volume = (double)f2 >= 0.01 ? MathHelper.clamp(f1 * f1, 0.0f, 1.0f) : 0.0f;
            if (this.time < 20) {
                this.volume = 0.0f;
