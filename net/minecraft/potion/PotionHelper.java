...
public class PotionHelper {
    ...
    protected static boolean isItemConversionReagent(ItemStack stack) {
        int j2 = POTION_ITEM_CONVERSIONS.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            if (!PotionHelper.POTION_ITEM_CONVERSIONS.get((int)i2).reagent.apply(stack)) continue;
            return true;
        }
        return false;
    }
    ...
    protected static boolean isTypeConversionReagent(ItemStack stack) {
        int j2 = POTION_TYPE_CONVERSIONS.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            if (!PotionHelper.POTION_TYPE_CONVERSIONS.get((int)i2).reagent.apply(stack)) continue;
            return true;
        }
        return false;
    }
    ...
    protected static boolean hasItemConversions(ItemStack p_185206_0_, ItemStack p_185206_1_) {
        ...
        int j2 = POTION_ITEM_CONVERSIONS.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            MixPredicate<Item> mixpredicate = POTION_ITEM_CONVERSIONS.get(i2);
    ...
    protected static boolean hasTypeConversions(ItemStack p_185209_0_, ItemStack p_185209_1_) {
        ...
        int j2 = POTION_TYPE_CONVERSIONS.size();
        for (int i2 = 0; i2 < j2; ++i2) {
            MixPredicate<PotionType> mixpredicate = POTION_TYPE_CONVERSIONS.get(i2);
    ...
    public static ItemStack doReaction(ItemStack reagent, ItemStack potionIn) {
        if (!potionIn.func_190926_b()) {
            ...
            int k2 = POTION_TYPE_CONVERSIONS.size();
            for (i2 = 0; i2 < k2; ++i2) {
                MixPredicate<PotionType> mixpredicate1 = POTION_TYPE_CONVERSIONS.get(i2);
