...
public class Potion {
    ...
    public void affectEntity(@Nullable Entity source, @Nullable Entity indirectSource, EntityLivingBase entityLivingBaseIn, int amplifier, double health) {
        if (!(this == MobEffects.INSTANT_HEALTH && !entityLivingBaseIn.isEntityUndead() || this == MobEffects.INSTANT_DAMAGE && entityLivingBaseIn.isEntityUndead())) {
            if (this == MobEffects.INSTANT_DAMAGE && !entityLivingBaseIn.isEntityUndead() || this == MobEffects.INSTANT_HEALTH && entityLivingBaseIn.isEntityUndead()) {
                int j2 = (int)(health * (double)(6 << amplifier) + 0.5);
                if (source == null) {
                    entityLivingBaseIn.attackEntityFrom(DamageSource.magic, j2);
                } else {
                    entityLivingBaseIn.attackEntityFrom(DamageSource.causeIndirectMagicDamage(source, indirectSource), j2);
                }
            }
        } else {
            int i2 = (int)(health * (double)(4 << amplifier) + 0.5);
            entityLivingBaseIn.heal(i2);
        }
    }
    ...
    public static String getPotionDurationString(PotionEffect p_188410_0_, float p_188410_1_) {
        ...
        int i2 = MathHelper.floor((float)p_188410_0_.getDuration() * p_188410_1_);
        return StringUtils.ticksToElapsedTime(i2);
    }
