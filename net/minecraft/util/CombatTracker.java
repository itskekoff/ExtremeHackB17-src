...
public class CombatTracker {
    ...
    public EntityLivingBase getBestAttacker() {
        ...
        for (CombatEntry combatentry : this.combatEntries) {
            ...
            if (!(combatentry.getDamageSrc().getEntity() instanceof EntityLivingBase) || entitylivingbase != null && !(combatentry.getDamage() > f2)) continue;
            f2 = combatentry.getDamage();
            entitylivingbase = (EntityLivingBase)combatentry.getDamageSrc().getEntity();
        }
        if (entityplayer != null && f1 >= f2 / 3.0f) {
            return entityplayer;
        }
    ...
    private CombatEntry getBestCombatEntry() {
        ...
        float f2 = 0.0f;
        float f1 = 0.0f;
        for (int i2 = 0; i2 < this.combatEntries.size(); ++i2) {
            CombatEntry combatentry3;
            CombatEntry combatentry2 = this.combatEntries.get(i2);
            CombatEntry combatEntry = combatentry3 = i2 > 0 ? this.combatEntries.get(i2 - 1) : null;
            ...
            if (combatentry2.getFallSuffix() == null || combatentry1 != null && !(combatentry2.getDamage() > f2)) continue;
            combatentry1 = combatentry2;
            f2 = combatentry2.getDamage();
        }
        if (f1 > 5.0f && combatentry != null) {
            return combatentry;
        }
        if (f2 > 5.0f && combatentry1 != null) {
            return combatentry1;
        }
        return null;
    }
    ...
    public void reset() {
        int i2;
        int n2 = i2 = this.inCombat ? 300 : 100;
        if (this.takingDamage && (!this.fighter.isEntityAlive() || this.fighter.ticksExisted - this.lastDamageTime > i2)) {
