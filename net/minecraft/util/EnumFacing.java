...
public enum EnumFacing implements IStringSerializable
{
    ...
    static {
        ...
        for (int i2 = 0; i2 < n2; ++i2) {
            ...
            EnumFacing.VALUES[enumfacing.index] = enumfacing = arrenumFacing[i2];
    ...
    public static EnumFacing getFacingFromVector(float x2, float y2, float z2) {
        EnumFacing enumfacing = NORTH;
        float f2 = Float.MIN_VALUE;
        for (EnumFacing enumfacing1 : EnumFacing.values()) {
            float f1 = x2 * (float)enumfacing1.directionVec.getX() + y2 * (float)enumfacing1.directionVec.getY() + z2 * (float)enumfacing1.directionVec.getZ();
            if (!(f1 > f2)) continue;
            f2 = f1;
