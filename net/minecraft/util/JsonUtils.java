...
public class JsonUtils {
    ...
    public static Item getItem(JsonElement json, String memberName) {
        if (json.isJsonPrimitive()) {
            ...
            Item item = Item.getByNameOrId(s2);
            if (item == null) {
                throw new JsonSyntaxException("Expected " + memberName + " to be an item, was unknown string '" + s2 + "'");
            }
            return item;
        }
    ...
    public static String toString(JsonElement json) {
        String s2 = StringUtils.abbreviateMiddle(String.valueOf(json), "...", 10);
        if (json == null) {
            return "null (missing)";
        }
        if (json.isJsonNull()) {
