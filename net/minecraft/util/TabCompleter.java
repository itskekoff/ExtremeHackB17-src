...
public abstract class TabCompleter {
    ...
    public void complete() {
        if (this.didComplete) {
            ...
            int i2 = this.textField.getNthWordFromPosWS(-1, this.textField.getCursorPosition(), false);
            this.completions.clear();
            this.completionIdx = 0;
            String s2 = this.textField.getText().substring(0, this.textField.getCursorPosition());
            this.requestCompletions(s2);
            if (this.completions.isEmpty()) {
                return;
            }
            this.didComplete = true;
            this.textField.deleteFromCursor(i2 - this.textField.getCursorPosition());
        }
    ...
    public void setCompletions(String ... newCompl) {
        if (this.requestedCompletions) {
            ...
            this.completions.clear();
            String[] arrstring = newCompl;
            int n2 = newCompl.length;
            for (int i2 = 0; i2 < n2; ++i2) {
                String s2 = arrstring[i2];
                if (s2.isEmpty()) continue;
                this.completions.add(s2);
            }
