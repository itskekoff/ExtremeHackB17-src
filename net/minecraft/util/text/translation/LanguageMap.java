    ...
    public LanguageMap() {
        try {
            ...
            for (String s2 : IOUtils.readLines(inputstream, StandardCharsets.UTF_8)) {
                String[] astring;
                if (s2.isEmpty() || s2.charAt(0) == '#' || (astring = Iterables.toArray(EQUAL_SIGN_SPLITTER.split(s2), String.class)) == null || astring.length != 2) continue;
                String s1 = astring[0];
                String s22 = NUMERIC_VARIABLE_PATTERN.matcher(astring[1]).replaceAll("%$1s");
