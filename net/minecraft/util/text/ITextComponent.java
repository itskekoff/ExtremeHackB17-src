    ...
    JsonSerializer<ITextComponent> {
        ...
        public ITextComponent deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_, JsonDeserializationContext p_deserialize_3_) throws JsonParseException {
            ...
            if (jsonobject.has("text")) {
                ...
                String s2 = jsonobject.get("translate").getAsString();
                if (jsonobject.has("with")) {
                    JsonArray jsonarray = jsonobject.getAsJsonArray("with");
                    Object[] aobject = new Object[jsonarray.size()];
                    for (int i2 = 0; i2 < aobject.length; ++i2) {
                        TextComponentString textcomponentstring;
                        aobject[i2] = this.deserialize(jsonarray.get(i2), p_deserialize_2_, p_deserialize_3_);
                        if (!(aobject[i2] instanceof TextComponentString) || !(textcomponentstring = (TextComponentString)aobject[i2]).getStyle().isEmpty() || !textcomponentstring.getSiblings().isEmpty()) continue;
                        aobject[i2] = textcomponentstring.getText();
                    }
                    itextcomponent = new TextComponentTranslation(s2, aobject);
                } else {
                    itextcomponent = new TextComponentTranslation(s2, new Object[0]);
                }
            ...
            if (jsonobject.has("extra")) {
                ...
                for (int j2 = 0; j2 < jsonarray2.size(); ++j2) {
                    itextcomponent.appendSibling(this.deserialize(jsonarray2.get(j2), p_deserialize_2_, p_deserialize_3_));
                }
            }
