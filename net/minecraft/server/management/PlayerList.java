...
public abstract class PlayerList {
    ...
    public void initializeConnectionToPlayer(NetworkManager netManager, EntityPlayerMP playerIn) {
        ...
        TextComponentTranslation textcomponenttranslation = playerIn.getName().equalsIgnoreCase(s2) ? new TextComponentTranslation("multiplayer.player.joined", playerIn.getDisplayName()) : new TextComponentTranslation("multiplayer.player.joined.renamed", playerIn.getDisplayName(), s2);
    ...
    public void playerLoggedIn(EntityPlayerMP playerIn) {
        ...
        for (int i2 = 0; i2 < this.playerEntityList.size(); ++i2) {
            playerIn.connection.sendPacket(new SPacketPlayerListItem(SPacketPlayerListItem.Action.ADD_PLAYER, this.playerEntityList.get(i2)));
        }
    ...
    public EntityPlayerMP recreatePlayerEntity(EntityPlayerMP playerIn, int dimension, boolean conqueredEnd) {
        ...
        for (String s2 : playerIn.getTags()) {
            entityplayermp.addTag(s2);
        }
        WorldServer worldserver = this.mcServer.worldServerForDimension(playerIn.dimension);
    ...
    public void updatePermissionLevel(EntityPlayerMP player) {
        ...
        int i2 = this.canSendCommands(gameprofile) ? this.ops.getPermissionLevel(gameprofile) : 0;
        i2 = this.mcServer.isSinglePlayer() && this.mcServer.worldServers[0].getWorldInfo().areCommandsAllowed() ? 4 : i2;
        i2 = this.commandsAllowedForAll ? 4 : i2;
        this.sendPlayerPermissionLevel(player, i2);
    }

    public void changePlayerDimension(EntityPlayerMP player, int dimensionIn) {
        int i2 = player.dimension;
        ...
        player.isDead = false;
        this.transferEntityToWorld(player, i2, worldserver, worldserver1);
    ...
    public void sendPacketToAllPlayers(Packet<?> packetIn) {
        for (int i2 = 0; i2 < this.playerEntityList.size(); ++i2) {
            this.playerEntityList.get((int)i2).connection.sendPacket(packetIn);
        }
    }
    ...
    public String getFormattedListOfPlayers(boolean includeUUIDs) {
        ...
        for (int i2 = 0; i2 < list.size(); ++i2) {
            ...
            s2 = String.valueOf(s2) + " (" + ((EntityPlayerMP)list.get(i2)).getCachedUniqueIdString() + ")";
        }
        return s2;
    }
    ...
    public void sendToAllNearExcept(@Nullable EntityPlayer except, double x2, double y2, double z2, double radius, int dimension, Packet<?> packetIn) {
        for (int i2 = 0; i2 < this.playerEntityList.size(); ++i2) {
            double d2;
            double d1;
            double d0;
            EntityPlayerMP entityplayermp = this.playerEntityList.get(i2);
            if (entityplayermp == except || entityplayermp.dimension != dimension || !((d0 = x2 - entityplayermp.posX) * d0 + (d1 = y2 - entityplayermp.posY) * d1 + (d2 = z2 - entityplayermp.posZ) * d2 < radius * radius)) continue;
            entityplayermp.connection.sendPacket(packetIn);
        }
    }
    ...
    public void removeAllPlayers() {
        for (int i2 = 0; i2 < this.playerEntityList.size(); ++i2) {
            this.playerEntityList.get((int)i2).connection.func_194028_b(new TextComponentTranslation("multiplayer.disconnect.server_shutdown", new Object[0]));
        }
    }
