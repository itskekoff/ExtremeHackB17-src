...
public class PlayerInteractionManager {
    ...
    public void updateBlockRemoving() {
        ...
        if (this.receivedFinishDiggingPacket) {
            ...
            if (iblockstate.getMaterial() == Material.AIR) {
                ...
                float f2 = iblockstate.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.world, this.delayedDestroyPos) * (float)(i2 + 1);
                int j2 = (int)(f2 * 10.0f);
                if (j2 != this.durabilityRemainingOnBlock) {
                    this.theWorld.sendBlockBreakProgress(this.thisPlayerMP.getEntityId(), this.delayedDestroyPos, j2);
            ...
            if (iblockstate1.getMaterial() == Material.AIR) {
                ...
                float f1 = iblockstate1.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.world, this.delayedDestroyPos) * (float)(k2 + 1);
                int l2 = (int)(f1 * 10.0f);
                if (l2 != this.durabilityRemainingOnBlock) {
                    this.theWorld.sendBlockBreakProgress(this.thisPlayerMP.getEntityId(), this.destroyPos, l2);
    ...
    public void onBlockClicked(BlockPos pos, EnumFacing side) {
        if (this.isCreative()) {
            ...
            if (iblockstate.getMaterial() != Material.AIR) {
                ...
                f2 = iblockstate.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.world, pos);
            }
            if (iblockstate.getMaterial() != Material.AIR && f2 >= 1.0f) {
                this.tryHarvestBlock(pos);
            } else {
    ...
    public void blockRemoving(BlockPos pos) {
        if (pos.equals(this.destroyPos)) {
            ...
            IBlockState iblockstate = this.theWorld.getBlockState(pos);
            if (iblockstate.getMaterial() != Material.AIR) {
                float f2 = iblockstate.getPlayerRelativeBlockHardness(this.thisPlayerMP, this.thisPlayerMP.world, pos) * (float)(i2 + 1);
                if (f2 >= 0.7f) {
                    this.isDestroyingBlock = false;
    ...
    public EnumActionResult processRightClick(EntityPlayer player, World worldIn, ItemStack stack, EnumHand hand) {
        ...
        if (itemstack == stack && itemstack.func_190916_E() == i2 && itemstack.getMaxItemUseDuration() <= 0 && itemstack.getMetadata() == j2) {
