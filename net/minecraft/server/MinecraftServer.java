...
ISnooperInfo {
    ...
    public void loadAllWorlds(String saveName, String worldNameIn, long seed, WorldType type, String generatorOptions) {
        ...
        for (int i2 = 0; i2 < this.worldServers.length; ++i2) {
            ...
            if (i2 == 0) {
                this.worldServers[i2] = this.isDemo() ? (WorldServer)new WorldServerDemo(this, isavehandler, worldinfo, j2, this.theProfiler).init() : (WorldServer)new WorldServer(this, isavehandler, worldinfo, j2, this.theProfiler).init();
            ...
            this.worldServers[i2].addEventListener(new ServerWorldEventHandler(this, this.worldServers[i2]));
    ...
    public void initialWorldChunkLoad() {
        ...
        for (int l1 = -192; l1 <= 192 && this.isServerRunning(); l1 += 16) {
            for (int i22 = -192; i22 <= 192 && this.isServerRunning(); i22 += 16) {
                ...
                ++i1;
                worldserver.getChunkProvider().provideChunk(blockpos.getX() + l1 >> 4, blockpos.getZ() + i22 >> 4);
            }
        }
    ...
    public void saveAllWorlds(boolean isSilent) {
        WorldServer[] arrworldServer = this.worldServers;
    ...
    public void stopServer() {
        ...
        if (this.worldServers != null) {
            ...
            WorldServer[] arrworldServer = this.worldServers;
            int n3 = this.worldServers.length;
            for (n2 = 0; n2 < n3; ++n2) {
                WorldServer worldserver = arrworldServer[n2];
                if (worldserver == null) continue;
                worldserver.disableLevelSaving = false;
            }
            this.saveAllWorlds(false);
            arrworldServer = this.worldServers;
            n3 = this.worldServers.length;
            for (n2 = 0; n2 < n3; ++n2) {
                WorldServer worldserver1 = arrworldServer[n2];
...
lbl73:
    ...
    public void tick() {
        ...
        if (i2 - this.nanoTimeSinceStatusRefresh >= 5000000000L) {
            ...
            int j2 = MathHelper.getInt(this.random, 0, this.getCurrentPlayerCount() - agameprofile.length);
            for (int k2 = 0; k2 < agameprofile.length; ++k2) {
                agameprofile[k2] = this.playerList.getPlayerList().get(j2 + k2).getGameProfile();
            }
    ...
    public void updateTimeLightAndEntities() {
        ...
        for (int j2 = 0; j2 < this.worldServers.length; ++j2) {
            long i2 = System.nanoTime();
        ...
        for (int k2 = 0; k2 < this.tickables.size(); ++k2) {
            this.tickables.get(k2).update();
        }
        this.theProfiler.endSection();
    }
    ...
    public void setDifficultyForAllWorlds(EnumDifficulty difficulty) {
        ...
        int n2 = this.worldServers.length;
        for (int i2 = 0; i2 < n2; ++i2) {
            WorldServer worldserver1 = arrworldServer[i2];
            if (worldserver1 == null) continue;
    ...
    public void addServerStatsToSnooper(Snooper playerSnooper) {
        ...
        if (this.worldServers != null) {
            ...
            for (int i2 = 0; i2 < n2; ++i2) {
                ...
                playerSnooper.addClientStat("world[" + l2 + "][dimension]", worldserver1.provider.getDimensionType().getId());
                playerSnooper.addClientStat("world[" + l2 + "][mode]", (Object)worldinfo.getGameType());
                playerSnooper.addClientStat("world[" + l2 + "][difficulty]", (Object)worldserver1.getDifficulty());
                playerSnooper.addClientStat("world[" + l2 + "][hardcore]", worldinfo.isHardcoreModeEnabled());
                playerSnooper.addClientStat("world[" + l2 + "][generator_name]", worldinfo.getTerrainType().getWorldTypeName());
                playerSnooper.addClientStat("world[" + l2 + "][generator_version]", worldinfo.getTerrainType().getGeneratorVersion());
                playerSnooper.addClientStat("world[" + l2 + "][height]", this.buildLimit);
                playerSnooper.addClientStat("world[" + l2 + "][chunks_loaded]", worldserver1.getChunkProvider().getLoadedChunkCount());
                ++l2;
            }
        }
        playerSnooper.addClientStat("worlds", l2);
    }
