...
ITickable {
    ...
    public void tryAcceptPlayer() {
        ...
        if ((s2 = this.server.getPlayerList().allowUserToConnect(this.networkManager.getRemoteAddress(), this.loginGameProfile)) != null) {
            this.func_194026_b(new TextComponentTranslation(s2, new Object[0]));
        } else {
    ...
    public void processEncryptionResponse(CPacketEncryptionResponse packetIn) {
        ...
        new Thread("User Authenticator #" + AUTHENTICATOR_THREAD_ID.incrementAndGet()){
            ...
            public void run() {
                ...
                try {
                    String s2 = new BigInteger(CryptManager.getServerIdHash("", NetHandlerLoginServer.this.server.getKeyPair().getPublic(), NetHandlerLoginServer.this.secretKey)).toString(16);
                    NetHandlerLoginServer.this.loginGameProfile = NetHandlerLoginServer.this.server.getMinecraftSessionService().hasJoinedServer(new GameProfile(null, gameprofile.getName()), s2, this.func_191235_a());
