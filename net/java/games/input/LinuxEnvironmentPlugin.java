...
implements Plugin {
        ...
        AccessController.doPrivileged(new PrivilegedAction(){
            ...
            public final Object run() {
                ...
                try {
                    ...
                    ControllerEnvironment.logln("Failed to load library: " + e2.getMessage());
                    e2.printStackTrace();
                    supported = false;
                }
                return null;
    ...
    private static final Component[] createComponents(List event_components, LinuxEventDevice device) {
        ...
        for (i2 = 0; i2 < event_components.size(); ++i2) {
            LinuxEventComponent event_component = (LinuxEventComponent)event_components.get(i2);
    ...
    private final void enumerateJoystickControllers(List controllers) {
        ...
        for (int i2 = 0; i2 < joystick_device_files.length; ++i2) {
            ...
            try {
                ...
                LinuxEnvironmentPlugin.logln("Failed to open device (" + event_file + "): " + e2.getMessage());
            }
        }
    }
        ...
        for (int i2 = 0; i2 < event_device_files.length; ++i2) {
            ...
            try {
                ...
                try {
                    ...
                    LinuxEnvironmentPlugin.logln("Failed to create Controller: " + e2.getMessage());
                    device.close();
                }
                continue;
            }
            catch (IOException e3) {
                LinuxEnvironmentPlugin.logln("Failed to open device (" + event_file + "): " + e3.getMessage());
            }
        }
    }
        ...
        public final void run() {
            for (int i2 = 0; i2 < LinuxEnvironmentPlugin.this.devices.size(); ++i2) {
                try {
                    ...
                    ControllerEnvironment.logln("Failed to close device: " + e2.getMessage());
                }
            }
        }
    }
