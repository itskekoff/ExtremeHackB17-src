...
implements Plugin {
    ...
    public DirectInputEnvironmentPlugin() {
        ...
        if (this.isSupported()) {
            try {
                ...
                DirectInputEnvironmentPlugin.logln("Failed to enumerate devices: " + e3.getMessage());
            }
            this.window = window;
    ...
    private final Component[] createComponents(IDirectInputDevice device, boolean map_mouse_buttons) {
        ...
        for (int i2 = 0; i2 < device_objects.size(); ++i2) {
            DIDeviceObject device_object = (DIDeviceObject)device_objects.get(i2);
    ...
    private final Controller[] enumControllers(DummyWindow window) throws IOException {
        ...
        try {
            ...
            for (int i2 = 0; i2 < devices.size(); ++i2) {
                IDirectInputDevice device = (IDirectInputDevice)devices.get(i2);
