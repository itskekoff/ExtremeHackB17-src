...
final class RawDevice {
    ...
    private final boolean processMouseEvent(RawMouseEvent event) {
        ...
        boolean bl2 = has_update = this.updateButtonState(4, button_flags, 256, 512) || has_update;
        if ((event.getFlags() & 1) != 0) {
            dx2 = event.getLastX() - this.last_x;
            dy2 = event.getLastY() - this.last_y;
        ...
        this.relative_y += dy2;
        this.wheel += dwheel;
        has_update = dx2 != 0 || dy2 != 0 || dwheel != 0 || has_update;
        return has_update;
    }
