...
implements Plugin {
    ...
    private static final boolean isMacOSXEqualsOrBetterThan(int major_required, int minor_required) {
        ...
        try {
            ...
            OSXEnvironmentPlugin.logln("Exception occurred while trying to determine OS version: " + e2);
            return false;
        }
    ...
    private static final void addElements(OSXHIDQueue queue, List elements, List components, boolean map_mouse_buttons) throws IOException {
        ...
        while (it2.hasNext()) {
            OSXHIDElement element = (OSXHIDElement)it2.next();
            Component.Identifier id2 = element.getIdentifier();
            if (id2 == null) continue;
            if (map_mouse_buttons) {
                if (id2 == Component.Identifier.Button._0) {
                    id2 = Component.Identifier.Button.LEFT;
                } else if (id2 == Component.Identifier.Button._1) {
                    id2 = Component.Identifier.Button.RIGHT;
                } else if (id2 == Component.Identifier.Button._2) {
                    id2 = Component.Identifier.Button.MIDDLE;
                }
            }
            OSXComponent component = new OSXComponent(id2, element);
    ...
    private static final Controller[] enumerateControllers() {
        ...
        block12: {
            ...
            try {
                OSXHIDDeviceIterator it2 = new OSXHIDDeviceIterator();
                block9: while (true) {
                    while (true) {
                        try {
                            OSXHIDDevice device = it2.next();
                            if (device == null) {
                                break block12;
                            }
                            ...
                            OSXEnvironmentPlugin.logln("Failed to enumerate device: " + e3.getMessage());
                            continue;
                        }
                        break;
                    }
