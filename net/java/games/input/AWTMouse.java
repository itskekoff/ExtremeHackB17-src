...
implements AWTEventListener {
    ...
    private final void processEvent(AWTEvent event) throws IOException {
        if (event instanceof MouseWheelEvent) {
            ...
            Axis x2 = (Axis)this.getX();
            Axis y2 = (Axis)this.getY();
            x2.setValue(me2.getX());
            y2.setValue(me2.getY());
            switch (me2.getID()) {
                case 501: {
                    this.processButtons(me2.getButton(), 1.0f);
                    break;
                }
                case 502: {
                    this.processButtons(me2.getButton(), 0.0f);
                    break;
                }
            }
        }
    ...
    public final synchronized void pollDevice() throws IOException {
        ...
        for (int i2 = 0; i2 < this.awt_events.size(); ++i2) {
            AWTEvent event = (AWTEvent)this.awt_events.get(i2);
