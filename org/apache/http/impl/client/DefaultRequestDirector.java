    ...
    private void tryConnect(RoutedRequest req, HttpContext context) throws HttpException, IOException {
        ...
        while (true) {
            ...
            try {
                ...
                if (this.retryHandler.retryRequest(ex2, connectCount, context)) {
                    if (!this.log.isInfoEnabled()) continue;
                    this.log.info("I/O exception (" + ex2.getClass().getName() + ") caught when connecting to " + route + ": " + ex2.getMessage());
                    if (this.log.isDebugEnabled()) {
    ...
    private HttpResponse tryExecute(RoutedRequest req, HttpContext context) throws HttpException, IOException {
        ...
        while (true) {
            ...
            try {
                ...
                if (this.retryHandler.retryRequest(ex2, wrapper.getExecCount(), context)) {
                    if (this.log.isInfoEnabled()) {
                        this.log.info("I/O exception (" + ex2.getClass().getName() + ") caught when processing request to " + route + ": " + ex2.getMessage());
                    }
                    if (this.log.isDebugEnabled()) {
                        this.log.debug(ex2.getMessage(), ex2);
                    }
