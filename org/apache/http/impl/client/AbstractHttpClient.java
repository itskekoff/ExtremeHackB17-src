...
extends CloseableHttpClient {
    ...
    protected ClientConnectionManager createClientConnectionManager() {
        ...
        if (className != null) {
            try {
                ...
                throw new InstantiationError(ex4.getMessage());
            }
        }
    ...
    private synchronized HttpProcessor getProtocolProcessor() {
        if (this.protocolProcessor == null) {
            ...
            for (int i2 = 0; i2 < reqc; ++i2) {
                reqinterceptors[i2] = proc.getRequestInterceptor(i2);
            }
            int resc = proc.getResponseInterceptorCount();
            HttpResponseInterceptor[] resinterceptors = new HttpResponseInterceptor[resc];
            for (int i3 = 0; i3 < resc; ++i3) {
                resinterceptors[i3] = proc.getResponseInterceptor(i3);
            }
        ...
        try {
            if (connectionBackoffStrategy != null && backoffManager != null) {
                ...
                try {
                    ...
                    if (connectionBackoffStrategy.shouldBackoff(re2)) {
