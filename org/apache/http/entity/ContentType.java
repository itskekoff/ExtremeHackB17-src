...
implements Serializable {
    ...
    ContentType(String mimeType, NameValuePair[] params) throws UnsupportedCharsetException {
        this.mimeType = mimeType;
        this.params = params;
        String s2 = this.getParameter("charset");
        this.charset = !TextUtils.isBlank(s2) ? Charset.forName(s2) : null;
    }
    ...
    public String toString() {
        CharArrayBuffer buf2 = new CharArrayBuffer(64);
        buf2.append(this.mimeType);
        if (this.params != null) {
            buf2.append("; ");
            BasicHeaderValueFormatter.INSTANCE.formatParameters(buf2, this.params, false);
        } else if (this.charset != null) {
            buf2.append("; charset=");
            buf2.append(this.charset.name());
        }
    ...
    public static ContentType parse(String s2) throws ParseException, UnsupportedCharsetException {
        ...
        CharArrayBuffer buf2 = new CharArrayBuffer(s2.length());
        buf2.append(s2);
        ParserCursor cursor = new ParserCursor(0, s2.length());
        HeaderElement[] elements = BasicHeaderValueParser.INSTANCE.parseElements(buf2, cursor);
