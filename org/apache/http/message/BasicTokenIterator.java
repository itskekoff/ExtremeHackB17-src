...
implements TokenIterator {
    ...
    protected int findTokenStart(int pos) {
        ...
        while (!found && this.currentHeader != null) {
            ...
            while (!found && from < to2) {
                char ch2 = this.currentHeader.charAt(from);
                if (this.isTokenSeparator(ch2) || this.isWhitespace(ch2)) {
                    ++from;
                    continue;
                }
    ...
    protected int findTokenEnd(int from) {
        ...
        int to2 = this.currentHeader.length();
        for (end = from + 1; end < to2 && this.isTokenChar(this.currentHeader.charAt(end)); ++end) {
        }
        return end;
    }
    ...
    protected boolean isTokenChar(char ch2) {
        ...
        return !this.isHttpSeparator(ch2);
    }
