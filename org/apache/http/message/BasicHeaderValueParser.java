...
implements HeaderValueParser {
    ...
    public HeaderElement parseHeaderElement(CharArrayBuffer buffer, ParserCursor cursor) {
        ...
        if (!cursor.atEnd() && (ch2 = buffer.charAt(cursor.getPos() - 1)) != ',') {
    ...
    public NameValuePair[] parseParameters(CharArrayBuffer buffer, ParserCursor cursor) {
        ...
        for (pos = cursor.getPos(); pos < indexTo && HTTP.isWhitespace(ch2 = buffer.charAt(pos)); ++pos) {
        }
        ...
        while (!cursor.atEnd()) {
            ...
            params.add(param);
            char ch3 = buffer.charAt(cursor.getPos() - 1);
            if (ch3 != ',') continue;
            break;
        }
    ...
    private static boolean isOneOf(char ch2, char[] chs) {
        if (chs != null) {
            for (char ch22 : chs) {
                if (ch2 != ch22) continue;
                return true;
            }
        }
        return false;
    ...
    public NameValuePair parseNameValuePair(CharArrayBuffer buffer, ParserCursor cursor, char[] delimiters) {
        ...
        int indexTo = cursor.getUpperBound();
        for (pos = cursor.getPos(); pos < indexTo && (ch2 = buffer.charAt(pos)) != '='; ++pos) {
            if (!BasicHeaderValueParser.isOneOf(ch2, delimiters)) continue;
            terminated = true;
            break;
        }
        if (pos == indexTo) {
        ...
        while (pos < indexTo) {
            char ch3 = buffer.charAt(pos);
            if (ch3 == '\"' && !escaped) {
                boolean bl2 = qouted = !qouted;
            }
            if (!qouted && !escaped && BasicHeaderValueParser.isOneOf(ch3, delimiters)) {
                terminated = true;
                break;
            }
            escaped = escaped ? false : qouted && ch3 == '\\';
            ++pos;
        }
        int i2 = pos;
