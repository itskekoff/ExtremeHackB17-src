...
public class HttpService {
    ...
    public void handleRequest(HttpServerConnection conn, HttpContext context) throws IOException, HttpException {
        ...
        try {
            ...
            response = this.responseFactory.newHttpResponse(HttpVersion.HTTP_1_0, 500, context);
            this.handleException(ex3, response);
        }
    ...
    protected void handleException(HttpException ex2, HttpResponse response) {
        if (ex2 instanceof MethodNotSupportedException) {
            response.setStatusCode(501);
        } else if (ex2 instanceof UnsupportedHttpVersionException) {
            response.setStatusCode(505);
        } else if (ex2 instanceof ProtocolException) {
