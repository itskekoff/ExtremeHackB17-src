...
extends ArchiveInputStream {
    ...
    public int read(byte[] buf2, int off, int len) throws IOException {
        ...
        while (len > 0) {
            int sz2;
            int n2 = sz2 = len > this.readBuf.length - this.recordOffset ? this.readBuf.length - this.recordOffset : len;
            if (this.recordOffset + sz2 <= this.readBuf.length) {
                System.arraycopy(this.readBuf, this.recordOffset, buf2, off, sz2);
                totalRead += sz2;
                this.recordOffset += sz2;
                len -= sz2;
                off += sz2;
            }
            if (len <= 0) continue;
