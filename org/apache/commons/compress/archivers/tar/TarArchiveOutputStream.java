...
extends ArchiveOutputStream {
    ...
    private void writeRecord(byte[] buf2, int offset) throws IOException {
        if (offset + this.recordSize > buf2.length) {
            throw new IOException("record has length '" + buf2.length + "' with offset '" + offset + "' which is less than the record size of '" + this.recordSize + "'");
        }
        this.out.write(buf2, offset, this.recordSize);
        ++this.recordsWritten;
    }
