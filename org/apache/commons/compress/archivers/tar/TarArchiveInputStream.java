...
extends ArchiveInputStream {
    ...
    public TarArchiveInputStream(InputStream is2, String encoding) {
        this(is2, 10240, 512, encoding);
    }

    public TarArchiveInputStream(InputStream is2, int blockSize) {
        this(is2, blockSize, 512);
    }

    public TarArchiveInputStream(InputStream is2, int blockSize, String encoding) {
        this(is2, blockSize, 512, encoding);
    }

    public TarArchiveInputStream(InputStream is2, int blockSize, int recordSize) {
        this(is2, blockSize, recordSize, null);
    }

    public TarArchiveInputStream(InputStream is2, int blockSize, int recordSize, String encoding) {
        this.is = is2;
    ...
    public long skip(long n2) throws IOException {
        ...
        long skipped = this.is.skip(Math.min(n2, available));
        this.count(skipped);
    ...
    protected byte[] getLongNameData() throws IOException {
        ...
        if (length != longNameData.length) {
            byte[] l2 = new byte[length];
            System.arraycopy(longNameData, 0, l2, 0, length);
            longNameData = l2;
        }
        return longNameData;
    }
    ...
    Map<String, String> parsePaxHeaders(InputStream i2) throws IOException {
        int ch2;
                ...
                if (ch2 == 32) {
                    ...
                    while ((ch2 = i2.read()) != -1) {
                        ...
                        if (ch2 == 61) {
                            ...
                            int got = IOUtils.readFully(i2, rest);
                            if (got != len - read) {
    ...
    @Override
    public int read(byte[] buf2, int offset, int numToRead) throws IOException {
        int totalRead = 0;
        ...
        totalRead = this.is.read(buf2, offset, numToRead = Math.min(numToRead, this.available()));
        if (totalRead == -1) {
            if (numToRead > 0) {
    ...
    public boolean canReadEntryData(ArchiveEntry ae2) {
        if (ae2 instanceof TarArchiveEntry) {
            TarArchiveEntry te2 = (TarArchiveEntry)ae2;
            return !te2.isGNUSparse();
        }
        return false;
    }
    ...
    protected final void setCurrentEntry(TarArchiveEntry e2) {
        this.currEntry = e2;
    }
    ...
    protected final void setAtEOF(boolean b2) {
        this.hasHitEOF = b2;
    }
