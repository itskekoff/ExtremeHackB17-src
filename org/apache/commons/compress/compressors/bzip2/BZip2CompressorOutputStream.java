...
implements BZip2Constants {
    ...
    public void write(byte[] buf2, int offs, int len) throws IOException {
        if (offs < 0) {
        ...
        if (offs + len > buf2.length) {
            throw new IndexOutOfBoundsException("offs(" + offs + ") + len(" + len + ") > buf.length(" + buf2.length + ").");
        }
        if (this.out == null) {
