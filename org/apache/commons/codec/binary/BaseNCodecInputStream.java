    ...
    public int read(byte[] b2, int offset, int len) throws IOException {
        ...
        while (readLen == 0) {
            if (!this.baseNCodec.hasData(this.context)) {
                byte[] buf2 = new byte[this.doEncode ? 4096 : 8192];
                int c2 = this.in.read(buf2);
                if (this.doEncode) {
                    this.baseNCodec.encode(buf2, 0, c2, this.context);
                } else {
                    this.baseNCodec.decode(buf2, 0, c2, this.context);
                }
            }
