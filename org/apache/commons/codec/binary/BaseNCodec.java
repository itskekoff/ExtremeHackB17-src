...
BinaryDecoder {
    ...
    public byte[] encode(byte[] pArray) {
        ...
        byte[] buf2 = new byte[context.pos - context.readPos];
        this.readResults(buf2, 0, buf2.length, context);
        return buf2;
    }
    ...
    public boolean isInAlphabet(byte[] arrayOctet, boolean allowWSPad) {
        for (int i2 = 0; i2 < arrayOctet.length; ++i2) {
            if (this.isInAlphabet(arrayOctet[i2]) || allowWSPad && (arrayOctet[i2] == this.pad || BaseNCodec.isWhiteSpace(arrayOctet[i2]))) continue;
            return false;
        }
        return true;
    }
