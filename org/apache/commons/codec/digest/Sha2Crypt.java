...
public class Sha2Crypt {
    ...
    private static String sha2Crypt(byte[] keyBytes, String salt, String saltPrefix, int blocksize, String algorithm) {
        ...
        for (cp2 = 0; cp2 < keyLen - blocksize; cp2 += blocksize) {
            System.arraycopy(tempResult, 0, pBytes, cp2, blocksize);
        }
        System.arraycopy(tempResult, 0, pBytes, cp2, keyLen - cp2);
        altCtx = DigestUtils.getDigest(algorithm);
        ...
        for (cp2 = 0; cp2 < saltLen - blocksize; cp2 += blocksize) {
            System.arraycopy(tempResult, 0, sBytes, cp2, blocksize);
        }
        System.arraycopy(tempResult, 0, sBytes, cp2, saltLen - cp2);
