...
Builder<String> {
    ...
    public int readFrom(Readable readable) throws IOException {
        ...
        if (readable instanceof Reader) {
            ...
            while (true) {
                this.ensureCapacity(this.size + 1);
                CharBuffer buf2 = CharBuffer.wrap(this.buffer, this.size, this.buffer.length - this.size);
                int read = readable.read(buf2);
                if (read == -1) break;
                this.size += read;
            }
        }
    ...
    public StrBuilder append(CharBuffer buf2) {
        if (buf2 == null) {
            return this.appendNull();
        }
        if (buf2.hasArray()) {
            int length = buf2.remaining();
            int len = this.length();
            this.ensureCapacity(len + length);
            System.arraycopy(buf2.array(), buf2.arrayOffset() + buf2.position(), this.buffer, len, length);
            this.size += length;
        } else {
            this.append(buf2.toString());
        }
        return this;
    }

    public StrBuilder append(CharBuffer buf2, int startIndex, int length) {
        if (buf2 == null) {
            return this.appendNull();
        }
        if (buf2.hasArray()) {
            int totalLength = buf2.remaining();
            ...
            int len = this.length();
            this.ensureCapacity(len + length);
            System.arraycopy(buf2.array(), buf2.arrayOffset() + buf2.position() + startIndex, this.buffer, len, length);
            this.size += length;
        } else {
            this.append(buf2.toString(), startIndex, length);
        }
        return this;
    }
    ...
    private StrBuilder replaceImpl(StrMatcher matcher, String replaceStr, int from, int to2, int replaceCount) {
        ...
        int replaceLen = replaceStr == null ? 0 : replaceStr.length();
        char[] buf2 = this.buffer;
        for (int i2 = from; i2 < to2 && replaceCount != 0; ++i2) {
            int removeLen = matcher.isMatch(buf2, i2, from, to2);
    ...
    public StrBuilder reverse() {
        ...
        while (leftIdx < half) {
            char swap = buf2[leftIdx];
            buf2[leftIdx] = buf2[rightIdx];
            buf2[rightIdx] = swap;
            ++leftIdx;
            --rightIdx;
        }
    ...
    public StrBuilder trim() {
        ...
        int len = this.size;
        char[] buf2 = this.buffer;
        for (pos = 0; pos < len && buf2[pos] <= ' '; ++pos) {
        }
        while (pos < len && buf2[len - 1] <= ' ') {
            --len;
        }
        if (len < this.size) {
    ...
    public int indexOf(StrMatcher matcher, int startIndex) {
        ...
        for (int i2 = startIndex; i2 < len; ++i2) {
            if (matcher.isMatch(buf2, i2, startIndex, len) <= 0) continue;
            return i2;
        }
        return -1;
    }
    ...
    public int lastIndexOf(StrMatcher matcher, int startIndex) {
        ...
        for (int i2 = startIndex; i2 >= 0; --i2) {
            if (matcher.isMatch(buf2, i2, 0, endIndex) <= 0) continue;
            return i2;
        }
        return -1;
    }
