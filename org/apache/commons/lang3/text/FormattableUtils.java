...
public class FormattableUtils {
    ...
    public static Formatter append(CharSequence seq, Formatter formatter, int flags, int width, int precision, char padChar, CharSequence ellipsis) {
        ...
        StringBuilder buf2 = new StringBuilder(seq);
        if (precision >= 0 && precision < seq.length()) {
            CharSequence _ellipsis = ObjectUtils.defaultIfNull(ellipsis, "");
            buf2.replace(precision - _ellipsis.length(), seq.length(), _ellipsis.toString());
        }
        boolean leftJustify = (flags & 1) == 1;
        for (int i2 = buf2.length(); i2 < width; ++i2) {
            buf2.insert(leftJustify ? i2 : 0, padChar);
        }
        formatter.format(buf2.toString(), new Object[0]);
        return formatter;
    }
}
