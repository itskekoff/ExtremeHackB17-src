...
Serializable {
    ...
    @Override
    public StringBuffer format(long millis, StringBuffer buf2) {
        Calendar c2 = this.newCalendar();
        c2.setTimeInMillis(millis);
        return this.applyRules(c2, (Appendable)buf2);
    }

    @Override
    public StringBuffer format(Date date, StringBuffer buf2) {
        Calendar c2 = this.newCalendar();
        c2.setTime(date);
        return this.applyRules(c2, (Appendable)buf2);
    }
    ...
    @Override
    public StringBuffer format(Calendar calendar, StringBuffer buf2) {
        return this.format(calendar.getTime(), buf2);
    }

    @Override
    public <B extends Appendable> B format(long millis, B buf2) {
        Calendar c2 = this.newCalendar();
        c2.setTimeInMillis(millis);
        return this.applyRules(c2, buf2);
    }

    @Override
    public <B extends Appendable> B format(Date date, B buf2) {
        Calendar c2 = this.newCalendar();
        c2.setTime(date);
        return this.applyRules(c2, buf2);
    }
    ...
    @Override
    public <B extends Appendable> B format(Calendar calendar, B buf2) {
        if (!calendar.getTimeZone().equals(this.mTimeZone)) {
            calendar = (Calendar)calendar.clone();
            calendar.setTimeZone(this.mTimeZone);
        }
        return this.applyRules(calendar, buf2);
    }

    @Deprecated
    protected StringBuffer applyRules(Calendar calendar, StringBuffer buf2) {
        return this.applyRules(calendar, (Appendable)buf2);
    }

    private <B extends Appendable> B applyRules(Calendar calendar, B buf2) {
        try {
            for (Rule rule : this.mRules) {
                rule.appendTo(buf2, calendar);
            }
        }
    ...
    static String getTimeZoneDisplay(TimeZone tz2, boolean daylight, int style, Locale locale) {
        String prior;
        TimeZoneDisplayKey key = new TimeZoneDisplayKey(tz2, daylight, style, locale);
