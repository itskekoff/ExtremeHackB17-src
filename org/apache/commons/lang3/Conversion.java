...
public class Conversion {
    ...
    public static String longToHex(long src, int srcPos, String dstInit, int dstPos, int nHexs) {
        ...
        StringBuilder sb2 = new StringBuilder(dstInit);
        int append = sb2.length();
        for (int i2 = 0; i2 < nHexs; ++i2) {
            int shift = i2 * 4 + srcPos;
            int bits = (int)(0xFL & src >> shift);
            if (dstPos + i2 == append) {
                ++append;
                sb2.append(Conversion.intToHexDigit(bits));
                continue;
            }
            sb2.setCharAt(dstPos + i2, Conversion.intToHexDigit(bits));
        }
        return sb2.toString();
    }
