...
public class ArrayUtils {
    ...
    public static Object toPrimitive(Object array) {
        ...
        Class<?> ct2 = array.getClass().getComponentType();
        Class<?> pt2 = ClassUtils.wrapperToPrimitive(ct2);
        if (Integer.TYPE.equals(pt2)) {
            return ArrayUtils.toPrimitive((Integer[])array);
        }
        if (Long.TYPE.equals(pt2)) {
            return ArrayUtils.toPrimitive((Long[])array);
        }
        if (Short.TYPE.equals(pt2)) {
            return ArrayUtils.toPrimitive((Short[])array);
        }
        if (Double.TYPE.equals(pt2)) {
            return ArrayUtils.toPrimitive((Double[])array);
        }
        if (Float.TYPE.equals(pt2)) {
    ...
    public static <T> T[] addAll(T[] array1, T ... array2) {
        ...
        catch (ArrayStoreException ase2) {
            Class<?> type2 = array2.getClass().getComponentType();
            if (!type1.isAssignableFrom(type2)) {
                throw new IllegalArgumentException("Cannot store " + type2.getName() + " in an array of " + type1.getName(), ase2);
            }
            throw ase2;
        }
        return joinedArray;
    ...
    static Object removeAll(Object array, int ... indices) {
        ...
        if (diff < length) {
            ...
            for (int i3 = clonedIndices.length - 1; i3 >= 0; --i3) {
                ...
                if (end - index > 1) {
                    int cp2 = end - index - 1;
                    System.arraycopy(array, index + 1, result, dest -= cp2, cp2);
                }
                end = index;
            }
            if (end > 0) {
