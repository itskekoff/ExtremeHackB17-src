...
Serializable {
    ...
    protected void log(int type, Object message, Throwable t2) {
        ...
        if (showShortName) {
            ...
            buf2.append(String.valueOf(this.shortLogName)).append(" - ");
        } else if (showLogName) {
            buf2.append(String.valueOf(this.logName)).append(" - ");
        }
        buf2.append(String.valueOf(message));
        if (t2 != null) {
            buf2.append(" <");
            buf2.append(t2.toString());
            buf2.append(">");
            StringWriter sw2 = new StringWriter(1024);
            PrintWriter pw = new PrintWriter(sw2);
            t2.printStackTrace(pw);
            pw.close();
            buf2.append(sw2.toString());
        }
        this.write(buf2);
    }
