...
extends OutputStream {
    ...
    public synchronized int write(InputStream in2) throws IOException {
        int readCount = 0;
        int inBufferPos = this.count - this.filledBufferSum;
        int n2 = in2.read(this.currentBuffer, inBufferPos, this.currentBuffer.length - inBufferPos);
        while (n2 != -1) {
            readCount += n2;
            this.count += n2;
            ...
            n2 = in2.read(this.currentBuffer, inBufferPos, this.currentBuffer.length - inBufferPos);
        }
        return readCount;
    }
    ...
    public synchronized void writeTo(OutputStream out) throws IOException {
        ...
        for (byte[] buf2 : this.buffers) {
            int c2 = Math.min(buf2.length, remaining);
            out.write(buf2, 0, c2);
    ...
    public synchronized InputStream toInputStream() {
        ...
        for (byte[] buf2 : this.buffers) {
            int c2 = Math.min(buf2.length, remaining);
            list.add(new ByteArrayInputStream(buf2, 0, c2));
            if ((remaining -= c2) != 0) continue;
            break;
        }
    ...
    public synchronized byte[] toByteArray() {
        ...
        for (byte[] buf2 : this.buffers) {
            int c2 = Math.min(buf2.length, remaining);
            System.arraycopy(buf2, 0, newbuf, pos, c2);
            pos += c2;
            if ((remaining -= c2) != 0) continue;
            break;
        }
