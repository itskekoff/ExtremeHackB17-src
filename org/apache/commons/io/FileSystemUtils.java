...
public class FileSystemUtils {
    ...
    long parseDir(String line, String path) throws IOException {
        ...
        StringBuilder buf2 = new StringBuilder(line.substring(bytesStart, bytesEnd));
        for (int k2 = 0; k2 < buf2.length(); ++k2) {
            if (buf2.charAt(k2) != ',' && buf2.charAt(k2) != '.') continue;
            buf2.deleteCharAt(k2--);
        }
        return this.parseBytes(buf2.toString(), path);
    }
    ...
    long parseBytes(String freeSpace, String path) throws IOException {
        try {
            ...
            throw new IOException("Command line '" + DF + "' did not return numeric data as expected " + "for path '" + path + "'- check path is valid", ex2);
        }
    }
    ...
    List<String> performCommand(String[] cmdAttribs, int max, long timeout) throws IOException {
        ...
        try {
            ...
            inr = new BufferedReader(new InputStreamReader(in2, Charset.defaultCharset()));
            ...
            try {
                throw new IOException("Command line threw an InterruptedException for command " + Arrays.asList(cmdAttribs) + " timeout=" + timeout, ex2);
            }
    ...
    static {
        ...
        try {
            ...
            if ((osName = osName.toLowerCase(Locale.ENGLISH)).contains("windows")) {
                ...
                os2 = osName.contains("hp-ux") || osName.contains("aix") ? 3 : 0;
            }
        }
        catch (Exception ex2) {
            os2 = -1;
