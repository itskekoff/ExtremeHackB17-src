...
extends FilterReader {
    ...
    @Override
    public int read(char[] chr2) throws IOException {
        try {
            this.beforeRead(chr2 != null ? chr2.length : 0);
            int n2 = this.in.read(chr2);
            this.afterRead(n2);
            return n2;
        }
        catch (IOException e2) {
    ...
    @Override
    public int read(char[] chr2, int st2, int len) throws IOException {
        try {
            this.beforeRead(len);
            int n2 = this.in.read(chr2, st2, len);
            this.afterRead(n2);
            return n2;
        }
        catch (IOException e2) {
