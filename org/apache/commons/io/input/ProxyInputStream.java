...
extends FilterInputStream {
    ...
    public int read(byte[] bts2) throws IOException {
        try {
            this.beforeRead(bts2 != null ? bts2.length : 0);
            int n2 = this.in.read(bts2);
            this.afterRead(n2);
            return n2;
        }
        catch (IOException e2) {
    ...
    @Override
    public int read(byte[] bts2, int off, int len) throws IOException {
        try {
            this.beforeRead(len);
            int n2 = this.in.read(bts2, off, len);
            this.afterRead(n2);
            return n2;
        }
        catch (IOException e2) {
