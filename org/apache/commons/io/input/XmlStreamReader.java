...
extends Reader {
    ...
    public XmlStreamReader(InputStream is2, boolean lenient) throws IOException {
        this(is2, lenient, null);
    }

    public XmlStreamReader(InputStream is2, boolean lenient, String defaultEncoding) throws IOException {
        this.defaultEncoding = defaultEncoding;
        BOMInputStream bom2 = new BOMInputStream((InputStream)new BufferedInputStream(is2, 4096), false, BOMS);
        BOMInputStream pis = new BOMInputStream((InputStream)bom2, true, XML_GUESS_BYTES);
        this.encoding = this.doRawStream(bom2, pis, lenient);
    ...
    public XmlStreamReader(URLConnection conn, String defaultEncoding) throws IOException {
        ...
        BOMInputStream bom2 = new BOMInputStream((InputStream)new BufferedInputStream(is2, 4096), false, BOMS);
        BOMInputStream pis = new BOMInputStream((InputStream)bom2, true, XML_GUESS_BYTES);
        this.encoding = conn instanceof HttpURLConnection || contentType != null ? this.doHttpStream(bom2, pis, contentType, true) : this.doRawStream(bom2, pis, true);
        this.reader = new InputStreamReader((InputStream)pis, this.encoding);
    }
    ...
    public XmlStreamReader(InputStream is2, String httpContentType, boolean lenient, String defaultEncoding) throws IOException {
        this.defaultEncoding = defaultEncoding;
        BOMInputStream bom2 = new BOMInputStream((InputStream)new BufferedInputStream(is2, 4096), false, BOMS);
        BOMInputStream pis = new BOMInputStream((InputStream)bom2, true, XML_GUESS_BYTES);
        this.encoding = this.doHttpStream(bom2, pis, httpContentType, lenient);
        this.reader = new InputStreamReader((InputStream)pis, this.encoding);
    }

    public XmlStreamReader(InputStream is2, String httpContentType, boolean lenient) throws IOException {
    ...
    @Override
    public int read(char[] buf2, int offset, int len) throws IOException {
        return this.reader.read(buf2, offset, len);
    }
    ...
    private String doRawStream(BOMInputStream bom2, BOMInputStream pis, boolean lenient) throws IOException {
        String bomEnc = bom2.getBOMCharsetName();
        ...
        try {
            ...
            if (lenient) {
                return this.doLenientDetection(null, ex2);
            }
            throw ex2;
        }
    }

    private String doHttpStream(BOMInputStream bom2, BOMInputStream pis, String httpContentType, boolean lenient) throws IOException {
        String bomEnc = bom2.getBOMCharsetName();
        String xmlGuessEnc = pis.getBOMCharsetName();
        String xmlEnc = XmlStreamReader.getXmlProlog(pis, xmlGuessEnc);
        try {
            ...
            if (lenient) {
                return this.doLenientDetection(httpContentType, ex2);
            }
            throw ex2;
        }
    }

    private String doLenientDetection(String httpContentType, XmlStreamReaderException ex2) throws IOException {
        String encoding;
        if (httpContentType != null && httpContentType.startsWith("text/html")) {
            httpContentType = httpContentType.substring("text/html".length());
            httpContentType = "text/xml" + httpContentType;
            try {
                return this.calculateHttpEncoding(httpContentType, ex2.getBomEncoding(), ex2.getXmlGuessEncoding(), ex2.getXmlEncoding(), true);
            }
            catch (XmlStreamReaderException ex22) {
                ex2 = ex22;
            }
        }
        if ((encoding = ex2.getXmlEncoding()) == null) {
            encoding = ex2.getContentTypeEncoding();
        }
        if (encoding == null) {
    ...
    private static String getXmlProlog(InputStream is2, String guessedEnc) throws IOException {
        String encoding = null;
        if (guessedEnc != null) {
            ...
            int offset = 0;
            int max = 4096;
            int c2 = is2.read(bytes, offset, max);
            int firstGT = -1;
            String xmlProlog = "";
            while (c2 != -1 && firstGT == -1 && offset < 4096) {
                c2 = is2.read(bytes, offset += c2, max -= c2);
