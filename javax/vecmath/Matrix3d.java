...
Cloneable {
        ...
        if ((ga2 = Math.abs(gt2 = g2)) == 0.0) {
            ...
            if (gasmal) {
                ...
                if (gasmal) {
                    ...
                    ssmin = ha2 / a2;
                    ssmax = fa2 * a2;
                    t2 = mm2 == 0.0 ? (l2 == 0.0 ? Matrix3d.d_sign(c_b3, ft2) * Matrix3d.d_sign(c_b4, gt2) : gt2 / Matrix3d.d_sign(d2, ft2) + m2 / t2) : (m2 / (s2 + t2) + m2 / (r2 + l2)) * (a2 + 1.0);
    ...
    static void print_mat(double[] mat) {
        for (int i2 = 0; i2 < 3; ++i2) {
            System.out.println(String.valueOf(mat[i2 * 3 + 0]) + " " + mat[i2 * 3 + 1] + " " + mat[i2 * 3 + 2] + "\n");
        }
    }
