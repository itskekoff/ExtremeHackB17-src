...
implements MetaEventListener {
    ...
    public void cleanup() {
        ...
        this.sequence = null;
        Object e2 = this.sequenceQueueLock;
        synchronized (e2) {
    ...
    public void dequeueSound(String filename) {
        ...
        synchronized (object) {
            if (this.sequenceQueue != null) {
                ListIterator i2 = this.sequenceQueue.listIterator();
                while (i2.hasNext()) {
    ...
    public void play() {
        if (!this.loading()) {
            ...
            try {
                ...
                SoundSystemException sse = new SoundSystemException(e2.getMessage());
    ...
    public void resetGain() {
        ...
        if (this.synthesizer != null) {
            ...
            for (int c2 = 0; channels != null && c2 < channels.length; ++c2) {
                channels[c2].controlChange(7, midiVolume);
            }
        } else if (this.synthDevice != null) {
            try {
                ShortMessage volumeMessage = new ShortMessage();
                for (int i2 = 0; i2 < 16; ++i2) {
                    volumeMessage.setMessage(176, i2, 7, midiVolume);
            ...
            javax.sound.midi.MidiChannel[] channels = this.synthesizer.getChannels();
            for (int c3 = 0; channels != null && c3 < channels.length; ++c3) {
                channels[c3].controlChange(7, midiVolume);
            }
        } else {
            try {
                ...
                for (int c4 = 0; c4 < 16; ++c4) {
                    volumeMessage.setMessage(176, c4, 7, midiVolume);
    ...
    private Sequencer openSequencer(String containsString) {
        Sequencer s2 = null;
        s2 = (Sequencer)this.openMidiDevice(containsString);
        if (s2 == null) {
            return null;
        }
        try {
    ...
    private MidiDevice openMidiDevice(String containsString) {
        ...
        for (int i2 = 0; i2 < midiDevices.length; ++i2) {
            device = null;
            try {
                device = MidiSystem.getMidiDevice(midiDevices[i2]);
            }
            catch (MidiUnavailableException e2) {
                this.message("    Problem in method 'getMidiDevice':  MIDIUnavailableException was thrown");
                device = null;
            }
            if (device == null || !midiDevices[i2].getName().contains(containsString)) continue;
            this.message("    Found MIDI device named '" + midiDevices[i2].getName() + "'");
