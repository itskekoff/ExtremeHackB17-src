...
extends Channel {
    ...
    public boolean attachBuffer(IntBuffer buf2) {
        if (this.errorCheck(this.channelType != 0, "Sound buffers may only be attached to normal sources.")) {
            return false;
        }
        AL10.alSourcei(this.ALSource.get(0), 4105, buf2.get(0));
    ...
    public boolean preLoadBuffers(LinkedList<byte[]> bufferList) {
        ...
        for (int i2 = 0; i2 < bufferList.size(); ++i2) {
            byteBuffer = (ByteBuffer)BufferUtils.createByteBuffer(bufferList.get(i2).length).put(bufferList.get(i2)).flip();
            try {
                AL10.alBufferData(streamBuffers.get(i2), this.ALformat, byteBuffer, this.sampleRate);
            }
            catch (Exception e2) {
