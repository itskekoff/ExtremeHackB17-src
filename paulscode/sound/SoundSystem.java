...
public class SoundSystem {
    ...
    public SoundSystem() {
        ...
        if (libraries != null) {
            ListIterator i2 = libraries.listIterator();
            while (i2.hasNext()) {
    ...
    public void fadeOut(String sourcename, String filename, long milis) {
        FilenameURL fu2 = null;
        if (filename != null) {
            fu2 = new FilenameURL(filename);
        }
        this.CommandQueue(new CommandObject(7, sourcename, fu2, milis));
        this.commandThread.interrupt();
    }
    ...
    public void fadeOut(String sourcename, URL url, String identifier, long milis) {
        ...
        if (url != null && identifier != null) {
            fu2 = new FilenameURL(url, identifier);
        }
        this.CommandQueue(new CommandObject(7, sourcename, fu2, milis));
        this.commandThread.interrupt();
    }
    ...
    public void newSource(boolean priority, String sourcename, String filename, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        this.CommandQueue(new CommandObject(10, priority, false, toLoop, sourcename, new FilenameURL(filename), x2, y2, z2, attmodel, distOrRoll));
        this.commandThread.interrupt();
    }

    public void newSource(boolean priority, String sourcename, URL url, String identifier, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        this.CommandQueue(new CommandObject(10, priority, false, toLoop, sourcename, new FilenameURL(url, identifier), x2, y2, z2, attmodel, distOrRoll));
        this.commandThread.interrupt();
    }

    public void newStreamingSource(boolean priority, String sourcename, String filename, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        this.CommandQueue(new CommandObject(10, priority, true, toLoop, sourcename, new FilenameURL(filename), x2, y2, z2, attmodel, distOrRoll));
        this.commandThread.interrupt();
    }

    public void newStreamingSource(boolean priority, String sourcename, URL url, String identifier, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        this.CommandQueue(new CommandObject(10, priority, true, toLoop, sourcename, new FilenameURL(url, identifier), x2, y2, z2, attmodel, distOrRoll));
        this.commandThread.interrupt();
    }
    ...
    public void rawDataStream(AudioFormat audioFormat, boolean priority, String sourcename, float x2, float y2, float z2, int attModel, float distOrRoll) {
        this.CommandQueue(new CommandObject(11, audioFormat, priority, sourcename, x2, y2, z2, attModel, distOrRoll));
        this.commandThread.interrupt();
    }

    public String quickPlay(boolean priority, String filename, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        String sourcename = "Source_" + this.randomNumberGenerator.nextInt() + "_" + this.randomNumberGenerator.nextInt();
        this.CommandQueue(new CommandObject(12, priority, false, toLoop, sourcename, new FilenameURL(filename), x2, y2, z2, attmodel, distOrRoll, true));
        this.CommandQueue(new CommandObject(24, sourcename));
        this.commandThread.interrupt();
        return sourcename;
    }

    public String quickPlay(boolean priority, URL url, String identifier, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        String sourcename = "Source_" + this.randomNumberGenerator.nextInt() + "_" + this.randomNumberGenerator.nextInt();
        this.CommandQueue(new CommandObject(12, priority, false, toLoop, sourcename, new FilenameURL(url, identifier), x2, y2, z2, attmodel, distOrRoll, true));
        this.CommandQueue(new CommandObject(24, sourcename));
        this.commandThread.interrupt();
        return sourcename;
    }

    public String quickStream(boolean priority, String filename, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        String sourcename = "Source_" + this.randomNumberGenerator.nextInt() + "_" + this.randomNumberGenerator.nextInt();
        this.CommandQueue(new CommandObject(12, priority, true, toLoop, sourcename, new FilenameURL(filename), x2, y2, z2, attmodel, distOrRoll, true));
        this.CommandQueue(new CommandObject(24, sourcename));
        this.commandThread.interrupt();
        return sourcename;
    }

    public String quickStream(boolean priority, URL url, String identifier, boolean toLoop, float x2, float y2, float z2, int attmodel, float distOrRoll) {
        String sourcename = "Source_" + this.randomNumberGenerator.nextInt() + "_" + this.randomNumberGenerator.nextInt();
        this.CommandQueue(new CommandObject(12, priority, true, toLoop, sourcename, new FilenameURL(url, identifier), x2, y2, z2, attmodel, distOrRoll, true));
        this.CommandQueue(new CommandObject(24, sourcename));
        this.commandThread.interrupt();
        return sourcename;
    }

    public void setPosition(String sourcename, float x2, float y2, float z2) {
        this.CommandQueue(new CommandObject(13, sourcename, x2, y2, z2));
        this.commandThread.interrupt();
    }
    ...
    public void setLooping(String sourcename, boolean lp2) {
        this.CommandQueue(new CommandObject(17, sourcename, lp2));
        this.commandThread.interrupt();
    }
    ...
    public void setDistOrRoll(String sourcename, float dr2) {
        this.CommandQueue(new CommandObject(19, sourcename, dr2));
        this.commandThread.interrupt();
    }
    ...
    public void setVelocity(String sourcename, float x2, float y2, float z2) {
        this.CommandQueue(new CommandObject(22, sourcename, x2, y2, z2));
        this.commandThread.interrupt();
    }

    public void setListenerVelocity(float x2, float y2, float z2) {
        this.CommandQueue(new CommandObject(23, x2, y2, z2));
        this.commandThread.interrupt();
    }
    ...
    public void moveListener(float x2, float y2, float z2) {
        this.CommandQueue(new CommandObject(34, x2, y2, z2));
        this.commandThread.interrupt();
    }

    public void setListenerPosition(float x2, float y2, float z2) {
        this.CommandQueue(new CommandObject(35, x2, y2, z2));
        this.commandThread.interrupt();
    }
    ...
    public boolean newLibrary(Class libraryClass) throws SoundSystemException {
        ...
        this.commandThread.interrupt();
        for (int x2 = 0; !SoundSystem.initialized(false, false) && x2 < 100; ++x2) {
    ...
    private void CommandNewSource(boolean priority, boolean toStream, boolean toLoop, String sourcename, FilenameURL filenameURL, float x2, float y2, float z2, int attModel, float distORroll) {
        if (this.soundLibrary != null) {
            if (filenameURL.getFilename().matches(SoundSystemConfig.EXTENSION_MIDI) && !SoundSystemConfig.midiCodec()) {
                ...
                this.soundLibrary.newSource(priority, toStream, toLoop, sourcename, filenameURL, x2, y2, z2, attModel, distORroll);
            }
        } else {
    ...
    private void CommandRawDataStream(AudioFormat audioFormat, boolean priority, String sourcename, float x2, float y2, float z2, int attModel, float distOrRoll) {
        if (this.soundLibrary != null) {
            this.soundLibrary.rawDataStream(audioFormat, priority, sourcename, x2, y2, z2, attModel, distOrRoll);
        } else {
            this.errorMessage("Variable 'soundLibrary' null in method 'CommandRawDataStream'", 0);
        }
    }

    private void CommandQuickPlay(boolean priority, boolean toStream, boolean toLoop, String sourcename, FilenameURL filenameURL, float x2, float y2, float z2, int attModel, float distORroll, boolean temporary) {
        if (this.soundLibrary != null) {
            if (filenameURL.getFilename().matches(SoundSystemConfig.EXTENSION_MIDI) && !SoundSystemConfig.midiCodec()) {
                this.soundLibrary.loadMidi(toLoop, sourcename, filenameURL);
            } else {
                this.soundLibrary.quickPlay(priority, toStream, toLoop, sourcename, filenameURL, x2, y2, z2, attModel, distORroll, temporary);
            }
        } else {
    ...
    private void CommandSetPosition(String sourcename, float x2, float y2, float z2) {
        if (this.soundLibrary != null) {
            this.soundLibrary.setPosition(sourcename, x2, y2, z2);
        } else {
    ...
    private void CommandSetLooping(String sourcename, boolean lp2) {
        if (this.soundLibrary != null) {
            this.soundLibrary.setLooping(sourcename, lp2);
        } else {
    ...
    private void CommandSetDistOrRoll(String sourcename, float dr2) {
        if (this.soundLibrary != null) {
            this.soundLibrary.setDistOrRoll(sourcename, dr2);
        } else {
    ...
    private void CommandSetVelocity(String sourcename, float x2, float y2, float z2) {
        if (this.soundLibrary != null) {
            this.soundLibrary.setVelocity(sourcename, x2, y2, z2);
        } else {
            this.errorMessage("Variable 'soundLibrary' null in method 'CommandVelocity'", 0);
        }
    }

    private void CommandSetListenerVelocity(float x2, float y2, float z2) {
        if (this.soundLibrary != null) {
            this.soundLibrary.setListenerVelocity(x2, y2, z2);
        } else {
    ...
    private void CommandMoveListener(float x2, float y2, float z2) {
        if (this.soundLibrary != null) {
            this.soundLibrary.moveListener(x2, y2, z2);
        } else {
            this.errorMessage("Variable 'soundLibrary' null in method 'CommandMoveListener'", 0);
        }
    }

    private void CommandSetListenerPosition(float x2, float y2, float z2) {
        if (this.soundLibrary != null) {
            this.soundLibrary.setListenerPosition(x2, y2, z2);
        } else {
    ...
    public static void setException(SoundSystemException e2) {
        SoundSystem.lastException(true, e2);
    }
     ...
     * WARNING - Removed try catching itself - possible behaviour change.
     */
    private static SoundSystemException lastException(boolean action, SoundSystemException e2) {
        Object object = SoundSystemConfig.THREAD_SYNC;
        synchronized (object) {
            if (action) {
                lastException = e2;
