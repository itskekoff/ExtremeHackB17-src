...
public class Library {
    ...
    public void init() throws SoundSystemException {
        int x2;
        Channel channel = null;
        for (x2 = 0; x2 < SoundSystemConfig.getNumberStreamingChannels() && (channel = this.createChannel(1)) != null; ++x2) {
            this.streamingChannels.add(channel);
        }
        for (x2 = 0; x2 < SoundSystemConfig.getNumberNormalChannels() && (channel = this.createChannel(0)) != null; ++x2) {
    ...
    public void setVelocity(String sourcename, float x2, float y2, float z2) {
        Source mySource = this.sourceMap.get(sourcename);
        if (mySource != null) {
            mySource.setVelocity(x2, y2, z2);
        }
    }

    public void setListenerVelocity(float x2, float y2, float z2) {
        this.listener.setVelocity(x2, y2, z2);
    }
    ...
    public void moveListener(float x2, float y2, float z2) {
        this.setListenerPosition(this.listener.position.x + x2, this.listener.position.y + y2, this.listener.position.z + z2);
    }

    public void setListenerPosition(float x2, float y2, float z2) {
        this.listener.setPosition(x2, y2, z2);
        Set<String> keys = this.sourceMap.keySet();
        for (String sourcename : keys) {
            Source source = this.sourceMap.get(sourcename);
            if (source == null) continue;
    ...
    private Channel getNextChannel(Source source) {
        ...
        for (x2 = 0; x2 < channels; ++x2) {
            if (!sourcename.equals(sourceNames[x2])) continue;
            return channelList.get(x2);
        }
        int n2 = nextChannel;
        for (x2 = 0; x2 < channels; ++x2) {
            name = sourceNames[n2];
            ...
            if (src == null || !src.playing()) {
                ...
                sourceNames[n2] = sourcename;
                return channelList.get(n2);
            }
            if (++n2 < channels) continue;
            n2 = 0;
        }
        n2 = nextChannel;
        for (x2 = 0; x2 < channels; ++x2) {
    ...
    public void checkFadeVolumes() {
        ...
        for (int x2 = 0; x2 < this.streamingChannels.size(); ++x2) {
            c2 = this.streamingChannels.get(x2);
            if (c2 == null || (s2 = c2.attachedSource) == null) continue;
            s2.checkFadeOut();
        }
        c2 = null;
        s2 = null;
    ...
    public void setMidiChannel(MidiChannel c2) {
        if (this.midiChannel != null && this.midiChannel != c2) {
            this.midiChannel.cleanup();
        }
        this.midiChannel = c2;
    }
    ...
    protected void printStackTrace(Exception e2) {
